...
import { splitLegacySystemPrompt, type LegacyAgentMeta, type SplitModel } from './lib/legacy-prompt-splitter.js'
...
// ─── Provider factory ─────────────────────────────────────────────────────────
// The same provider factories the model module registers at runtime: the split
// call goes through the production complete() path, not a parallel adapter.

async function resolveProvider(provider: string): Promise<SplitModel> {
  switch (provider) {
    case 'anthropic': {
      const apiKey = process.env.ANTHROPIC_API_KEY
      if (!apiKey) throw new Error('ANTHROPIC_API_KEY environment variable is required for provider=anthropic')
      const { createAnthropicProvider } = await import('../src/modules/model/submodules/anthropic/provider.js')
      return createAnthropicProvider(apiKey)
    }
    case 'openai': {
      const apiKey = process.env.OPENAI_API_KEY
      if (!apiKey) throw new Error('OPENAI_API_KEY environment variable is required for provider=openai')
      const { createOpenAIProvider } = await import('../src/modules/model/submodules/openai/provider.js')
      return createOpenAIProvider({ apiKey })
    }
    case 'gemini': {
      const apiKey = process.env.GEMINI_API_KEY
      if (!apiKey) throw new Error('GEMINI_API_KEY environment variable is required for provider=gemini')
      const { createGeminiProvider } = await import('../src/modules/model/submodules/gemini/provider.js')
      return createGeminiProvider(apiKey)
    }
    case 'ollama': {
      const { createOllamaProvider } = await import('../src/modules/model/submodules/ollama/provider.js')

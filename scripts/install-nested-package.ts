...
export function isUnlinkedPackageError(text: string): boolean {
  // No lockfile: "Package X is not linked" / "link: failed to resolve".
  // bun.lock that already records `link:@saker/*`: "FileNotFound: failed linking
  // dependency/workspace to node_modules for package X" (Adel, bun 1.3.10).
  return /is not linked/i.test(text)
    || /link:.*failed to resolve/i.test(text)
    || /failed linking (?:dependency|workspace)/i.test(text)
}
...
export async function installNestedPackage(
  ...
  // bun's wording changes: no lockfile → "is not linked"; bun.lock that already
  // records `link:@saker/*` → "FileNotFound: failed linking dependency/workspace".
  // Skip `link:` deps on any failed nested install that lists them — do not
  // depend on the error text matching a specific Bun version.

...
clone_or_update() {
  if [[ -d "$INSTALL_DIR" && -d "$INSTALL_DIR/.git" ]]; then
    ...
    git -C "$INSTALL_DIR" fetch --depth 1 --tags --force origin "$BRANCH"
    # `git checkout --force main` does not fast-forward a local main that is
    # already checked out — a piped new install.sh then built an old tree
    # (`bunx vite`, no Saker stub). Reset the local branch onto FETCH_HEAD.

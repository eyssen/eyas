...
#
# Native install also bun-installs nested packages (src/web) — they are not
# workspaces, so root bun install does not put Vite on disk. Unlinked link:
# deps (e.g. @saker) are skipped so a public clone still builds.
...
function Install-NestedPackage([string]$InstallRoot, [string]$RelDir, [string]$Label) {
    $target = Join-Path $InstallRoot $RelDir
    $pkg = Join-Path $target "package.json"
    if (-not (Test-Path $pkg)) {
        Write-Info "No $Label package.json — skipping"
        return
    }
    $script = Join-Path $InstallRoot "scripts/install-nested-package.ts"
    if (Test-Path $script) {
        Write-Info "Installing $Label dependencies..."
        bun $script $target
        if ($LASTEXITCODE -ne 0) { throw "$Label nested install failed" }
        Write-Ok "$Label dependencies ready"
        return
    }
    Write-Info "Installing $Label dependencies..."
    ...
    try {
        bun install
        if ($LASTEXITCODE -ne 0) { throw "$Label bun install failed" }
...
if (Test-Path (Join-Path $Dir ".git")) {
    ...
    git -C $Dir fetch --depth 1 --tags --force origin $Branch
    if ($LASTEXITCODE -ne 0) { throw "git fetch $Branch failed" }
    # checkout --force $Branch does not fast-forward an already-checked-out branch
...
try {
    if ($Method -eq "docker") {
        ...
        Install-NestedPackage $Dir "src/web" "frontend"
        Write-Info "Building frontend..."
        bun run build:web
        if ($LASTEXITCODE -ne 0) {
            Write-Warn "Frontend build failed — installing src/web deps and retrying"
            Install-NestedPackage $Dir "src/web" "frontend"
            bun run build:web
            if ($LASTEXITCODE -ne 0) {
                Write-Err "The UI is a nested package at src/web (own package.json)."
                Write-Err "Root bun install does not install Vite or @vitejs/plugin-react."
                Write-Err "Fix: cd $Dir\src\web; bun install; cd $Dir; bun run build:web"
                throw "build:web failed"
            }
            Write-Ok "Frontend built (after retry)"
        }
        $docsPkg = Join-Path $Dir "packages\docs\package.json"
        if (Test-Path $docsPkg) {
            Write-Info "Building product docs..."
            $docsScript = Join-Path $Dir "scripts\install-nested-package.ts"
            $docsDir = Join-Path $Dir "packages\docs"
            if (Test-Path $docsScript) {
                bun $docsScript $docsDir
            } else {
            ...
            Pop-Location
            if ($docsOk) { Write-Ok "Docs built at packages/docs/dist" }
            else { Write-Warn "Docs build skipped/failed — server still works; run: bun run docs:build" }
        }

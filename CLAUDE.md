...
**0.8.26-beta** — You talk to colleagues: primary/team agents are people you open from the sidebar (one home thread each); they spawn specialists from a shared pool (`run_specialist`, often in parallel, no team card). Routing is green; the Assistant no longer has builder tools. See `CHANGELOG.md`.

Previous wave (0.8.25-beta): The door stays open — existing native installs fast-forward onto the fetched ref, and a public clone without `bun link` `@saker/*` still produces a UI (exact Saker stub aliases).

Earlier (0.8.23-beta): Nothing said is lost — every persisted message kept a second time, verbatim, in a zstd-compressed content-addressed raw layer, with a deterministic pass into facts, a gist, entities, topics and tags and **zero model calls**. Write path; 0.8.24 is the recall half.

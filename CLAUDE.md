...
## Current State
**0.8.25-beta** — The door stays open: existing native installs fast-forward onto the fetched ref, and a public clone without `bun link` `@saker/*` still produces a UI (exact Saker stub aliases). See `CHANGELOG.md`.

Previous wave (0.8.24-beta): The door remembers — lossless, no-limits data-port import (map everything, stream it, ledger + rollback) and Phase 2 recall — standing index with short MOCs always in, gated e5 KNN ∪ FTS (never naive RRF), one injected memory section, `memory_search` / `memory_expand`.

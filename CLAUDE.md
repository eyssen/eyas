...
## Current State
**0.8.21-beta** — A knock at the door: the one-line installer always asks native vs Docker, offers to install (or start) missing git/Bun/Docker instead of guessing, and leaves admin/provider/agent to the setup wizard; GitHub Sponsors is on the repo, README and landing page. See `CHANGELOG.md`.

Previous wave (0.8.20-beta): A front door, and its locks — the product overview and the six-language documentation are published at https://eyssen.github.io/eyas/; scanners on, and the defects they found are fixed.

Earlier (0.8.19-beta): Related prior work — past conversations are searchable in EYAS's own store; a new task injects matching vault notes and prior threads into the prompt.

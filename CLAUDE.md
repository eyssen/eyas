...
## Current State
**0.8.23-beta** — Nothing said is lost: every persisted message is now kept a second time, verbatim, in a zstd-compressed content-addressed raw layer captured at the persistence layer itself, and each committed flush is turned into facts, a gist, entities, topics and tags by a deterministic pass with **zero model calls**. Facts are arbitrated — hash dedup, `(subject, predicate)` supersede with `valid_until`, tag inheritance, trust = min of sources — behind a five-language poisoning gate. Write path only: nothing reads the new layers yet, `memory.engine` stays `legacy`, and recall is the next wave. See `CHANGELOG.md`.

Previous wave (0.8.22-beta): The door opens — the one-line native install `bun install`s the nested `src/web` package, retries a missing-package UI build, and still produces a frontend when local `link:` editors are absent.

Earlier (0.8.21-beta): A knock at the door — the installer always asks native vs Docker, offers to install (or start) missing git/Bun/Docker instead of guessing, and leaves admin/provider/agent to the setup wizard; GitHub Sponsors is on the repo, README and landing page.

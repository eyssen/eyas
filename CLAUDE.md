...
## Current State
**0.8.22-beta** — The door opens: the one-line native install now `bun install`s the nested `src/web` package (Vite and `@vitejs/plugin-react` live there, not at the repo root), retries a missing-package UI build, and still produces a frontend when local `link:` editors are absent. See `CHANGELOG.md`.

Previous wave (0.8.21-beta): A knock at the door — the installer always asks native vs Docker, offers to install (or start) missing git/Bun/Docker instead of guessing, and leaves admin/provider/agent to the setup wizard; GitHub Sponsors is on the repo, README and landing page.

Earlier (0.8.20-beta): A front door, and its locks — the product overview and the six-language documentation are published at https://eyssen.github.io/eyas/; scanners on, and the defects they found are fixed.

...
## Current State
**0.8.16-beta** — A memory of its own: the vault writes itself. A durable fact stated in any conversation — on any model — becomes a vault note through a post-delivery capture (global, default ON, structural length gate, per-note Zod salvage, `memory_capture_runs` measurement row for every outcome), and the same note reaches every later prompt as one index line. Host Claude config is opt-in everywhere: `loadClaudeMd` defaults to OFF with an explicit `settingSources` list when opted in, and isolated/opted-out calls also set `CLAUDE_CODE_DISABLE_AUTO_MEMORY` + `strictMcpConfig` — `settingSources: []` alone does NOT stop the CLI's cwd-keyed auto-memory. Grok/Kimi (ACP) cannot be isolated and their panels say so honestly.

Previous wave (0.8.15-beta): design canvases in the Claude Design format with EYAS's own MIT runtime — created, imported, edited by hand/WYSIWYG/AI, versioned through one validator, attachable to conversations and projects, exported to PNG/PDF; an attached design reaches the model as an announcement and the agent fetches parts with `design_read`. A matched skill is a **proposal the turn waits on**. See `CHANGELOG.md`.

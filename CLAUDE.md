...
## Current State
**0.8.24-beta** — The door remembers: lossless, no-limits data-port import (map everything, stream it, ledger + rollback) and Phase 2 recall — standing index with short MOCs always in, gated e5 KNN ∪ FTS (never naive RRF), one injected memory section, `memory_search` / `memory_expand`. See `CHANGELOG.md`.

Previous wave (0.8.23-beta): Nothing said is lost — every persisted message kept a second time, verbatim, in a zstd-compressed content-addressed raw layer, with a deterministic pass into facts, a gist, entities, topics and tags and **zero model calls**. Write path; this release is the recall half.

Earlier (0.8.22-beta): The door opens — the one-line native install `bun install`s the nested `src/web` package, retries a missing-package UI build, and still produces a frontend when local `link:` editors are absent.

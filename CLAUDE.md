...
## Current State
**0.8.20-beta** — A front door, and its locks: the product overview and the six-language documentation are published at https://eyssen.github.io/eyas/; the landing page itself is six-language with a beta callout and an install panel; the repository's scanners are on (secret scanning + push protection, CodeQL, Dependabot alerts, private vulnerability reporting) and the defects they found are fixed — email header injection in both address formatters, a `Math.random()` bridge secret, glob patterns treated as regexes, frontmatter escaping, HTML stripping — plus four dependency advisories. See `CHANGELOG.md`.

Previous wave (0.8.19-beta): Related prior work — past conversations are searchable in EYAS's own store; a new task injects matching vault notes and prior threads into the prompt.

Earlier (0.8.18-beta): A desk of its own — new chats pick a project grouped by type; domain notes rank with the type; working directories and catalog connections pin from type to conversation; `search_memory` defaults to current project+type; wiki auto-updates (opt-in) from closed tickets and decisions; conversations stream tool traces, diffs, stop, and plan first; git status/diff skip approval; extra skill/persona roots import without host Claude config; Telegram `/new` + Approve/Deny; data-port copies into the vault (never mounts).

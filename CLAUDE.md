...
## Current State
**0.8.32-beta** — The terminal is the owner's: opening an OpenCode terminal (TUI or shell) needs manage OpenCode (owner/admin); the user role is read-only; the terminal socket is re-checked on every frame, every 15 s and on a role change. See `CHANGELOG.md`.

**0.8.31-beta** — Your own folders only: the OpenCode terminal opens only for a conversation the caller owns; a conversation's Folders can name only the caller's own workspaces (on save and at run time); the plain shell terminal is owner/admin-only with the allowlisted env; vitest 4.1.11 closes Dependabot #5. See `CHANGELOG.md`.

**0.8.30-beta** — Any model, one memory: every CLI (Claude Code, Grok, Kimi, OpenCode) runs isolated in an EYAS-owned home with a filtered env and is verified before each session; one path policy (reads and writes) + kernel sandbox keep other tools' memory out of reach; EYAS DB replay is the only continuity; recall reaches every entry path; model pinned per conversation with per-model effort (requested vs effective shown); one turn sink and uniform tool rows; deterministic privacy scanner. See `CHANGELOG.md`.

**0.8.29-beta** — The door still opens: a public clone nested `bun install` skips `link:@saker/*` even when `bun.lock` records them (`FileNotFound` linking, not only `is not linked`). See `CHANGELOG.md`.

**0.8.28-beta** — Talking heads, same five tools: optional HeyGen backend on Media (`media.heygen`, MCP OAuth). Video Agent / scripted avatar / speech through `media_*`. Not Studio. See `CHANGELOG.md`.

**0.8.27-beta** — OpenCode in the pane: optional MIT sidecar (`opencode serve` on 127.0.0.1 + POSIX PTY → xterm.js). Not vendored. `opencode_status` / `opencode_run`. See `CHANGELOG.md`.

Previous wave (0.8.26-beta): You talk to colleagues — primary/team agents are people you open from the sidebar; they spawn specialists from a shared pool (`run_specialist`, often in parallel, no team card).

Earlier (0.8.25-beta): The door stays open — existing native installs fast-forward onto the fetched ref, and a public clone without `bun link` `@saker/*` still produces a UI (exact Saker stub aliases).
...
- `docs/eyas-overview.html` — product overview page in all six languages; published at
  https://eyssen.github.io/eyas/ by `scripts/build-pages-site.sh` + `.github/workflows/pages.yml`
  (deploy needs `gh workflow run pages` — an orphan force-push does not trigger the paths filter)
...
**Extra Modules:** telegram, research, ingress, remote-node, meeting, disaster-recovery, a2a, hand-hub, browser-use, opencode

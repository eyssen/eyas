...
```bash
curl -fsSL https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.sh | bash -s -- --version 0.8.5-beta
# or:  EYAS_VERSION=0.8.5-beta curl … | bash
```
...
```powershell
powershell -c "& ([scriptblock]::Create((irm https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.ps1))) -Yes -Version 0.8.5-beta"
```
...
1. **Note the version** from the Backup table (or from `version.json` / the sidecar JSON), e.g. `0.8.5-beta`.
2. **Install that code version** (not necessarily `main`):

```bash
# macOS / Linux — pin to the backup's version
curl -fsSL https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.sh | bash -s -- --version 0.8.5-beta --yes
```
...
```bash
git clone --depth 1 --branch v0.8.5-beta https://github.com/eyssen/eyas.git ~/eyas
cd ~/eyas && bun install && bun run build:web
```

(`0.8.5-beta` is normalized to tag `v0.8.5-beta`.)

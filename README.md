...
Product overview and documentation (en / hu / de / es / fr / tlh): **https://eyssen.github.io/eyas/**
...
cd eyas
bun install
./bin/eyas start          # background (installs src/web deps + builds UI if missing)
...
> `eyas start` / `eyas serve` install `src/web` deps if needed and run `bun run build:web` when `src/web/dist` is **missing or older than the source** (so you never get a stale UI after code changes). Skip with `EYAS_SKIP_WEB_BUILD=1`; force with `EYAS_FORCE_WEB_BUILD=1`. The UI is a nested package — root `bun install` does not install Vite.
...
```bash
curl -fsSL https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.sh | bash -s -- --version 0.8.28-beta
# or:  EYAS_VERSION=0.8.28-beta curl … | bash
```
...
```powershell
powershell -c "& ([scriptblock]::Create((irm https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.ps1))) -Yes -Version 0.8.19-beta"
```
...
1. **Note the version** from the Backup table (or from `version.json` / the sidecar JSON), e.g. `0.8.19-beta`.
2. **Install that code version** (not necessarily `main`):

```bash
# macOS / Linux — pin to the backup's version
curl -fsSL https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.sh | bash -s -- --version 0.8.19-beta --yes
```
...
```bash
git clone --depth 1 --branch v0.8.19-beta https://github.com/eyssen/eyas.git ~/eyas
cd ~/eyas && bun install && bun scripts/install-nested-package.ts src/web && bun run build:web
```

(`0.8.19-beta` is normalized to tag `v0.8.19-beta`.)
...
Product docs (en / hu / de / es / fr / tlh) live in **`packages/docs/`** (Astro Starlight).
...
- At least one AI engine, any of:
  - **API key** — Anthropic, OpenAI, Google Gemini, OpenRouter, Kimi, or any OpenAI-/Anthropic-compatible endpoint
  - **A CLI you already pay for** — Claude Code, Grok CLI, Kimi CLI (no separate API key)
...
On first run, EYAS walks you through, in this order:

1. **Master password** — unlocks the secret vault (required)
2. **Root owner** — the main administrator account (required)
3. **Your two always-on AI teammates** — name the Personal Assistant and the System Engineer (required)
4. **Team agents** — optional specialists alongside the two permanent ones
5. **AI provider** — optional here; can be configured later in Settings
6. **AI models** — optional per-task model assignments

Steps 4-6 can be skipped and completed later. The interface language is chosen on the first screen (en / hu / de / es / fr / tlh).
...
eyas migrate [--dry-run]           # One-off: agent definitions v1 -> v2 workspace files
...
**58 modules**, each independently toggleable and talking over one event bus:

- **Agents** — templates and seed agents, named personas with their own skills, tool access and delegation; autonomous runs with checkpoints
- **Memory** — five tiers (working, episodic, semantic, procedural, archive) with vector search, plus a markdown vault the agent keeps writing to
- **Knowledge** — grounded search with citations, a wiki that updates itself from closed tickets and decisions, document ingestion
- **Board & projects** — kanban, project types, conversations tracked as tasks, pipelines
- **Skills** — **228 skills** in 16 categories (coding, DevOps, AI, security, data, integrations, research, writing, math, web, git, business, API, database, communication, agent); the system also proposes and retires its own
- **Model routing** — 12 provider submodules: Anthropic, OpenAI, Gemini, OpenRouter, Kimi, Ollama, LM Studio, the OpenAI-/Anthropic-compatible shims, and the CLI engines (Claude Code, Grok CLI, Kimi CLI) — with budget, cache and fallback
- **Browser automation** — Playwright-driven Chromium under EYAS's own profile, an optional agent-browser sidecar, and Chrome DevTools MCP for the debugging lane
- **OpenCode** — optional MIT coding-engine sidecar: chat delegates, a live TUI opens in the conversation terminal (`opencode serve` on 127.0.0.1 + POSIX PTY → xterm.js); fail-closed doctor, isolated config under `data/opencode`
- **MCP integration** — catalog plus client and server, Hand Hub remote transport
- **Studio & Design** — build and refine agent-facing artifacts and multi-artboard designs in the browser
- **Operations** — audit trail, cost tracking, observability, system update, disaster recovery, backup and empty-system restore
- **Multi-channel** — Web UI and WebSocket, Telegram, email, voice, and the A2A protocol for agent-to-agent traffic; Slack, Discord, Teams, Google Chat, Signal and WhatsApp ship as smaller bot adapters that degrade gracefully when their SDK or tokens are missing
- **7,200+ tests** across 795 test files

## Tech Stack

TypeScript · Bun · Hono · Drizzle ORM · SQLite · Orama (search) · CASL (permissions) · Zod · Pino · Grammy (Telegram) · Playwright · i18next · React 19 · shadcn/ui · Tailwind · TanStack Router · Zustand · Vitest · Astro Starlight (docs)

## Feedback, bugs, security

EYAS is in public beta, and reports genuinely shape what ships next.

- **Bugs and ideas** → [issues](https://github.com/eyssen/eyas/issues); the
  templates ask for the few details that make a report actionable.
- **Security problems** → [private report](https://github.com/eyssen/eyas/security/advisories/new),
  never a public issue. See [SECURITY.md](SECURITY.md).
- **Patches** → read [CONTRIBUTING.md](CONTRIBUTING.md) first: this repository is
  a mirror, so a pull request here cannot be merged, but a patch attached to an
  issue is applied upstream with attribution.

## Sponsors

EYAS is developed with the same AI models it orchestrates, and that inference is
the project's largest running cost. Sponsorship covers it — and everything EYAS
ships stays MIT-licensed and self-hostable either way.

**[Become a sponsor →](https://github.com/sponsors/eyssen)** — current goal:
**$1,000/month** toward the model bill.

<!-- SPONSOR-LOGOS:START -->

_No sponsors yet. Team ($100/mo) and Partner ($500/mo) logos appear here._

<!-- SPONSOR-LOGOS:END -->

Tiers, benefits and the full list: [SPONSORS.md](SPONSORS.md). Sponsorship is not
a support contract — no tier includes a response-time or fix-time guarantee.

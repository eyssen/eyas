...
```bash
curl -fsSL https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.sh | bash -s -- --version 0.8.7-beta
# or:  EYAS_VERSION=0.8.7-beta curl … | bash
```
...
```powershell
powershell -c "& ([scriptblock]::Create((irm https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.ps1))) -Yes -Version 0.8.7-beta"
```
...
| Type | Typical use | Secrets (env var names) |
|------|-------------|-------------------------|
...
Configure under **Backup → Offsite destinations** (or `data/backups/destinations.json`). Credentials are **not** stored in the form — only **environment variable names** (or Secrets keys). Example for B2:
...
Then set destination type `s3`, endpoint like `https://s3.us-west-000.backblazeb2.com`, bucket, region, and secretRefs pointing at those env names. Mark it **Use for uploads**.

### Restore onto an empty system

1. **Note the version** from the Backup table (or from `version.json` / the sidecar JSON), e.g. `0.8.7-beta`.
2. **Install that code version** (not necessarily `main`):

```bash
# macOS / Linux — pin to the backup's version
curl -fsSL https://raw.githubusercontent.com/eyssen/eyas/main/scripts/install.sh | bash -s -- --version 0.8.7-beta --yes
```
...
```bash
git clone --depth 1 --branch v0.8.7-beta https://github.com/eyssen/eyas.git ~/eyas
cd ~/eyas && bun install && bun run build:web
```

(`0.8.7-beta` is normalized to tag `v0.8.7-beta`.)

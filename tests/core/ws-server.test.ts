...
describe('setupWsServer (shared WS wiring for main.ts + cli serve.ts)', () => {
  ...
  describe('handleTerminalUpgrade (/api/v1/opencode/terminal/:id)', () => {
    it('upgrades when the JWT subject owns the PTY session', async () => {
      const opencode = { pty: { get: vi.fn(() => ({ userId: 'user-1' })) }, wsHandler: { onOpen: vi.fn(), onMessage: vi.fn(), onClose: vi.fn() } }
      const wsServer = await setupWsServer(makeCtx({ opencode }))
      const token = await createTokenService(JWT_SECRET).signAccessToken({ sub: 'user-1', role: 'user' }, 30)
      const server = makeMockServer()
      const res = await wsServer.handleTerminalUpgrade(
        new Request(`http://x/api/v1/opencode/terminal/sess-1?token=${token}`),
        server,
      )
      expect(res).toBeUndefined()
    ...
    it('404s when the session belongs to someone else', async () => {
      const opencode = { pty: { get: vi.fn(() => ({ userId: 'other' })) } }
      const wsServer = await setupWsServer(makeCtx({ opencode }))
      const token = await createTokenService(JWT_SECRET).signAccessToken({ sub: 'user-1', role: 'user' }, 30)
      const res = await wsServer.handleTerminalUpgrade(
        new Request(`http://x/api/v1/opencode/terminal/sess-1?token=${token}`),

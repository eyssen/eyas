...
describe('setupWsServer (shared WS wiring for main.ts + cli serve.ts)', () => {
  ...
  describe('handleTerminalUpgrade (/api/v1/opencode/terminal/:id)', () => {
    /** ctx.opencode with one PTY session of `owner`; `mayUse` answers the terminal right (manage OpenCode). */
    function opencodeWith(owner: string, mayUse?: (userId: string) => boolean) {
      return {
        pty: { get: vi.fn(() => ({ userId: owner })) },
        wsHandler: { onOpen: vi.fn(), onMessage: vi.fn(), onClose: vi.fn() },
        ...(mayUse ? { mayUseTerminal: vi.fn(mayUse) } : {}),
      }
    }

    async function upgrade(opencode: unknown, sub = 'user-1', server = makeMockServer()) {
      const wsServer = await setupWsServer(makeCtx({ opencode }))
      const token = await createTokenService(JWT_SECRET).signAccessToken({ sub, role: 'admin' }, 30)
      const res = await wsServer.handleTerminalUpgrade(new Request(`http://x/api/v1/opencode/terminal/sess-1?token=${token}`), server)
      return { res, server }
    }

    it('upgrades when the JWT subject may manage OpenCode and owns the PTY session', async () => {
      const opencode = opencodeWith('user-1', () => true)
      const { res, server } = await upgrade(opencode)
      expect(res).toBeUndefined()
      expect(opencode.mayUseTerminal).toHaveBeenCalledWith('user-1')
      expect(server.upgrade).toHaveBeenCalledWith(expect.anything(), {
        data: { userId: 'user-1', isTerminal: true, terminalSessionId: 'sess-1' },
      })
    })

    it('(−) 403s a user who may not manage OpenCode, even for their own session, before the session is looked up', async () => {
      const opencode = opencodeWith('user-1', () => false)
      const { res, server } = await upgrade(opencode)
      expect(res?.status).toBe(403)
      expect(opencode.pty.get).not.toHaveBeenCalled()
      expect(server.upgrade).not.toHaveBeenCalled()
    })

    it('(−) 403s every terminal socket when the OpenCode module wires no terminal check (fail closed)', async () => {
      const opencode = opencodeWith('user-1')
      const { res, server } = await upgrade(opencode)
      expect(res?.status).toBe(403)
      expect(server.upgrade).not.toHaveBeenCalled()
    })
    ...
    it('404s when the session belongs to someone else', async () => {
      const opencode = opencodeWith('other', () => true)
      const { res, server } = await upgrade(opencode)
      expect(res?.status).toBe(404)
      expect(server.upgrade).not.toHaveBeenCalled()
    })

    it('401s an invalid token before the terminal right is asked', async () => {
      const opencode = opencodeWith('user-1', () => true)
      const wsServer = await setupWsServer(makeCtx({ opencode }))
      const res = await wsServer.handleTerminalUpgrade(new Request('http://x/api/v1/opencode/terminal/sess-1?token=garbage'), makeMockServer())
      expect(res?.status).toBe(401)
      expect(opencode.mayUseTerminal).not.toHaveBeenCalled()
    })

    it('401s a missing token', async () => {
      const wsServer = await setupWsServer(makeCtx({ opencode: { pty: { get: vi.fn() } } }))
      const res = await wsServer.handleTerminalUpgrade(
        new Request('http://x/api/v1/opencode/terminal/sess-1'),
        makeMockServer(),
      )
      expect(res?.status).toBe(401)
    })
  ...
  describe('websocket handlers', () => {
    ...
    it('routes a terminal socket to opencode.wsHandler instead of the registry', async () => {
      const opencode = { wsHandler: { onOpen: vi.fn(), onMessage: vi.fn(), onClose: vi.fn() } }
      const wsServer = await setupWsServer(makeCtx({ opencode }))
      const ws = makeMockWs({ userId: 'user-1', isTerminal: true, terminalSessionId: 'sess-1' })

      wsServer.websocket.open(ws as any)
      wsServer.websocket.message(ws as any, '{"type":"ping"}')
      wsServer.websocket.close(ws as any)

      expect(opencode.wsHandler.onOpen).toHaveBeenCalledWith(ws, 'sess-1', 'user-1')
      expect(opencode.wsHandler.onMessage).toHaveBeenCalledWith(ws, 'sess-1', '{"type":"ping"}')
      expect(opencode.wsHandler.onClose).toHaveBeenCalledWith(ws, 'sess-1')
      expect(wsServer.wsRegistry.getConnections('user-1')).toHaveLength(0)
    })

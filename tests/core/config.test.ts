...
describe('memory.engine / memory.l0 (plan p1a)', () => {
  ...
  it('defaults to the legacy engine with L0 capture on and tool results and reasoning off', () => {
    const result = configSchema.safeParse({})
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('rejects an unknown engine and a non-positive byte cap', () => {
    ...
    expect(configSchema.safeParse({ memory: { l0: { captureThinking: 'yes' } } }).success).toBe(false)

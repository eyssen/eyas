...
// Vitest 4: `vi.spyOn` on a method that is already a spy returns that same
// spy, call history included (Vitest 3 handed back a fresh one). `api` is a
// module singleton that every test here spies on again, so without a restore
// after each test a call count or a `not.toHaveBeenCalled()` would also see
// the calls made by the tests before it.
afterEach(() => {
  vi.restoreAllMocks()
})

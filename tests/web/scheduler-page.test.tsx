...
const { get, post, patch, del, toastError, toastSuccess, FakeApiError, navigate } = vi.hoisted(() => {
...
// Recent executions link a scheduled agent run's conversation (I10). The page
// is rendered without a RouterProvider here, so navigation is a spy.
...
import type { JobExecution, ScheduledJob, SchedulerHealth } from '@/pages/scheduler/types'
 ...
 */
function installApi(jobs: ScheduledJob[], health: Partial<SchedulerHealth> = {}, executions: JobExecution[] = []): void {
...
describe('Recent executions link the scheduled run (I10)', () => {
  it('(+) an agent_run execution links its conversation, completed or failed', async () => {
    const job = makeJob({ kind: 'agent_run', handler: 'scheduler.agent_run', runnability: { runnable: true } })
    installApi([job], {}, [
      { id: 1, jobId: job.id, status: 'completed', startedAt: new Date().toISOString(), result: JSON.stringify({ conversationId: 'conv-ok', ran: true }) },
      { id: 2, jobId: job.id, status: 'failed', startedAt: new Date().toISOString(), error: 'over_budget: agent over budget', result: JSON.stringify({ conversationId: 'conv-failed', ran: false }) },
    ])
    const user = userEvent.setup()
    render(<SchedulerPage />)

    await user.click(await screen.findByText(job.name))
    const links = await screen.findAllByText(t('scheduler.openConversation'))
    expect(links).toHaveLength(2)

    await user.click(links[1]!)
    expect(navigate).toHaveBeenCalledWith({ to: '/conversations/$conversationId', params: { conversationId: 'conv-failed' } })
  })

  it('(−) an execution without a conversation shows no link', async () => {
    const job = makeJob({ runnability: { runnable: true } })
    installApi([job], {}, [
      { id: 1, jobId: job.id, status: 'completed', startedAt: new Date().toISOString(), result: JSON.stringify({ deleted: 3 }) },
      { id: 2, jobId: job.id, status: 'failed', startedAt: new Date().toISOString(), error: 'boom', result: 'not json' },
    ])
    const user = userEvent.setup()
    render(<SchedulerPage />)
    ...
    expect(screen.queryByText(t('scheduler.openConversation'))).toBeNull()
  })
})

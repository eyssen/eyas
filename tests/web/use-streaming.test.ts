...
describe('a tool call has to stop spinning', () => {
  ...
  it('does not dump a [Tool: name...] line into the streaming prose', () => {
    processStreamEvent({ type: 'tool_use', id: 't1', name: 'edit_file', input: { path: 'a.ts' } } as any, store())
    expect(store().streamingText).not.toContain('[Tool:')
    expect(store().agentProgress!.toolCalls).toHaveLength(1)
  })

  it('fills in args on a second tool_use for the same id instead of duplicating the row', () => {
    processStreamEvent({ type: 'tool_use', id: 't1', name: 'edit_file' } as any, store())
    processStreamEvent({
      type: 'tool_use',
      id: 't1',
      name: 'edit_file',
      input: { path: 'src/alpha.ts', oldString: 'a', newString: 'b' },
    } as any, store())
    const calls = store().agentProgress!.toolCalls
    expect(calls).toHaveLength(1)
    expect(calls[0].input).toEqual({ path: 'src/alpha.ts', oldString: 'a', newString: 'b' })
  })
})

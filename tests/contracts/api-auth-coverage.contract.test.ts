...
/**
 * Pre-existing onStart-ordering gaps: modules that create routes in onStart
 * without being ordered after 'auth'. Empty. 'privacy' was the one entry: its
 * routes were registered before auth's middleware, so requirePermission never
 * saw an ability. Fixed with the Privacy page's policy editor — privacy no
 * longer wraps ctx.model (it installs into the gateway's egress slot), every
 * consumer reads ctx.privacy lazily, and privacyModule.dependencies includes
 * 'auth'. No module may be added here without the same justification rigor.
 */
const ONSTART_AUTH_ORDERING_DEBT_BASELINE = new Set<string>([])
 ...
 * (see GET_ONLY_UNPAIRED_SEGMENTS for those instead). 16 entries ('privacy'
 * left the list when its policy editor arrived: auth/routes.ts pairs it).
 */
const CSRF_PAIRING_DEBT_BASELINE = new Set([
  'a2a', 'artifacts', 'client-wiki', 'connections', 'costops', 'data-port',
  'federation', 'ideas', 'intel', 'internal', 'ops',

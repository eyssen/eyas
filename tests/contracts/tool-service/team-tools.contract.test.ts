...
beforeEach(() => {
  ...
  orchestratorStub = {
    analyzeAndPropose: vi.fn().mockResolvedValue({
      config: { phases: [], maxParallelAgents: 1, conflictStrategy: 'first-wins', replanAfterPhase: false, useWorktrees: false },
...
describe('propose_team tool ↔ orchestrator + team-session service contract', () => {
  it('creates a session (stamping the parent conversation, D6) and returns a proposal', async () => {
    ...
    // The proposal call is attributed to the conversation it was made for.
    expect(orchestratorStub.analyzeAndPropose).toHaveBeenCalledWith('Ship the feature', 'complex', { conversationId: 'conv-1' })

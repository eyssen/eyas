...
beforeEach(() => {
  ...
  // and the drill-down tools resolve the conversation's scope through it.
  ;(memory as unknown as { db: unknown }).db = db
  db.run(sql`CREATE TABLE conversations (id TEXT PRIMARY KEY, project_id TEXT)`)
  db.run(sql`INSERT INTO conversations (id, project_id) VALUES ('contract-c1', NULL), ('conv-xyz', NULL)`)
...
describe('memory tools ↔ memory service contract', () => {
  ...
  it('fails closed on a conversation the database does not know: an error, no search', async () => {
    const spy = vi.spyOn(memory, 'search')

    const r = await harness.run('memory_search', { query: 'kubernetes' }, { conversationId: 'conv-gone' })

    expect(r.success).toBe(true)
    expect((r.output as any).error).toMatch(/memory scope unresolved/)
    expect((r.output as any).results).toEqual([])
    expect(spy).not.toHaveBeenCalled()
  })

  it('scopes the search by the conversation row, not by the context projectId', async () => {
    db.run(sql`UPDATE conversations SET project_id = 'proj-home' WHERE id = 'contract-c1'`)
    const spy = vi.spyOn(memory, 'search')

    await harness.run('memory_search', { query: 'x' }, { projectId: 'proj-elsewhere' })

    expect(spy.mock.calls[0][0].projectId).toBe('proj-home')
  })

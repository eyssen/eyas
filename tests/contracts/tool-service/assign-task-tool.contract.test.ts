...
describe('assign_task ↔ conversation/stage/agent service contract', () => {
  ...
  it("stores the assigning turn's model binding on the card, never the parent's raw pair (H4)", async () => {
    stages.create({ projectId: null, name: 'To Do', sortOrder: 0, botListen: true })
    conversations.update(parentId, { providerId: 'openai', modelId: 'gpt-x' })
    const binding = { providerId: 'grok-cli', modelId: 'grok-cli-default' }

    const withBinding = await harness.run('assign_task', { agentId: 'worker-1', title: 'A', goalDescription: 'a' }, { conversationId: parentId, modelBinding: binding })
    expect(conversations.get((withBinding.output as any).conversationId)).toMatchObject({ modelBinding: 'inherit', ...binding })

    // (−) No turn binding: no pair — the worker's model, else the default fixed on its first run.
    const without = await run(harness, { agentId: 'worker-1', title: 'B', goalDescription: 'b' }, parentId)
    expect(conversations.get((without.output as any).conversationId)).toMatchObject({ modelBinding: 'inherit', providerId: null, modelId: null })
  })
  ...
  it('is pinned as green routing (resume-skip still lists it as destructive)', async () => {
    // Routing is not a human gate. The runner still treats it as a
    // non-idempotent side effect on warm-resume (DESTRUCTIVE_TOOLS).
    const registry = await buildProductionToolRegistry()
    expect(registry.get('assign_task')?.riskTier).toBe('green')
    expect(DEFAULT_CONFIG.riskTiers.green).toContain('assign_task')
    expect(DEFAULT_CONFIG.riskTiers.yellow).not.toContain('assign_task')

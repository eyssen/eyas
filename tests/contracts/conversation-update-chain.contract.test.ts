...
const NON_UPDATABLE: Record<string, string> = {
  ...
  kind: 'assigned at create (home / task / delegation) — rewriting would break the unique home-thread index',
  thinking: 'legacy, migration-only — the boot migration folds it into effort (effort-migration.ts); nothing writes it',
  thinking_budget: 'legacy, migration-only — the boot migration folds it into effort (effort-migration.ts); nothing writes it',
}
...
const FIXTURES: Fixture[] = [
  ...
  { key: 'modelBinding', column: 'model_binding', value: 'auto', stored: 'auto' },
  { key: 'modelUserChosen', column: 'model_user_chosen', value: true, stored: 1 },
  ...
  { key: 'effort', column: 'effort', value: 'xhigh', stored: 'xhigh' },
  ...
  { key: 'searchContext', column: 'search_context', value: { sourceIds: ['s1'] }, stored: '{"sourceIds":["s1"]}' },
  { key: 'workingDirectories', column: 'working_directories', value: ['/tmp/work'], stored: '["/tmp/work"]' },
  { key: 'godMode', column: 'god_mode', value: true, stored: 1 },
  { key: 'designSystemId', column: 'design_system_id', value: 'brand-1', stored: 'brand-1' },
]
...
describe('conversation update chain contract', () => {
  ...
  // A3 — continuity is EYAS replay only: no provider session id is stored, so
  // neither the column (on a fresh DB) nor an update() path exists for it.
  it('has no provider session id column or update() path', () => {
    expect(columns).not.toContain('sdk_session_id')
    expect(UPDATE_FIELD_MAP).not.toHaveProperty('sdkSessionId')
  })

  // E3 — the legacy thinking columns are migration-only: the boot migration
  // folds them into effort, and no caller can write them again.
  it('has no update() path for the legacy thinking columns', () => {
    expect(columns).toContain('thinking')
    expect(columns).toContain('thinking_budget')
    expect(UPDATE_FIELD_MAP).not.toHaveProperty('thinking')
    expect(UPDATE_FIELD_MAP).not.toHaveProperty('thinkingBudget')
  })

...
export function createTestDb(_label: string) {
  function open() {
    ...
    db.run(sql`CREATE TABLE IF NOT EXISTS model_config (id TEXT PRIMARY KEY, provider_id TEXT NOT NULL REFERENCES provider_config(id), model_id TEXT NOT NULL, enabled INTEGER NOT NULL DEFAULT 1, name TEXT NOT NULL, context_window INTEGER, max_output_tokens INTEGER, supports_tools INTEGER DEFAULT 1, supports_images INTEGER DEFAULT 1, supports_streaming INTEGER DEFAULT 1, updated_at TEXT NOT NULL, metadata TEXT)`)
    db.run(sql`CREATE TABLE IF NOT EXISTS conversations (id TEXT PRIMARY KEY, task_id TEXT, title TEXT, status TEXT NOT NULL DEFAULT 'idle', provider_id TEXT, model_id TEXT, model_user_chosen INTEGER NOT NULL DEFAULT 0, model_binding TEXT NOT NULL DEFAULT 'pinned', user_id TEXT NOT NULL, tokens_used INTEGER NOT NULL DEFAULT 0, project_id TEXT, stage_id TEXT, priority TEXT DEFAULT 'normal', pinned INTEGER DEFAULT 0, position REAL DEFAULT 0, due_date TEXT, prompt TEXT, assignees TEXT, tags TEXT, mode TEXT NOT NULL DEFAULT 'simple', agent_id TEXT, parent_conversation_id TEXT, goal_description TEXT, complexity TEXT, total_cost_usd REAL DEFAULT 0, team_session_id TEXT, voice_scope_override TEXT, thinking TEXT NOT NULL DEFAULT 'off', thinking_budget INTEGER, effort TEXT, orchestration TEXT, search_context TEXT, working_directories TEXT, god_mode INTEGER NOT NULL DEFAULT 0, kind TEXT NOT NULL DEFAULT 'task', created_at TEXT NOT NULL, updated_at TEXT NOT NULL)`)
    db.run(sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_conv_home_thread ON conversations(user_id, agent_id) WHERE kind = 'home' AND agent_id IS NOT NULL`)
    db.run(sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_conv_task_id ON conversations(task_id)`)
    db.run(sql`CREATE TABLE IF NOT EXISTS conversation_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, conversation_id TEXT NOT NULL REFERENCES conversations(id), role TEXT NOT NULL, content TEXT NOT NULL, model TEXT, provider TEXT, tokens_in INTEGER DEFAULT 0, tokens_out INTEGER DEFAULT 0, attachments TEXT DEFAULT '[]', turn_meta TEXT, created_at TEXT NOT NULL)`)

...
export async function buildProductionToolRegistry(
  ...
  for (const t of createHandoffTool({ getConversations: () => undefined })) registry.register(t)
  ...
  const { createOpencodeTools } = await import('@modules/opencode/tools.js')
  for (const t of createOpencodeTools({
    getRunner: () => undefined,
    getSettings: () => ({
      enabled: true,
      cliPath: null,

...
describe('Client wiki routes', () => {
  ...
  describe('PUT /pages/:slug', () => {
    ...
    it('a human save takes ownership of an auto-generated page', async () => {
      await ownerApp.request('/api/v1/client-wiki/alpha/pages', {
        method: 'POST', headers: json,
        body: JSON.stringify({ title: 'Ticket', contentMd: 'bot', autoGenerated: true }),
      })
      const res = await ownerApp.request('/api/v1/client-wiki/alpha/pages/ticket', {
        method: 'PUT', headers: json,
        body: JSON.stringify({ contentMd: 'human', changeSummary: 'Edited from UI' }),
      })
      expect(res.status).toBe(200)
      const updated = await res.json() as any
      expect(updated.contentMd).toBe('human')

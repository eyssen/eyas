...
describe('applyMemoryItem', () => {
  ...
  it('names the background model that enriched a note in enriched_by (MCI-7)', async () => {
    const { vault, written } = vaultMock()
    await applyMemoryItem(
      { ...base, vault },
      {
        ...
        sourceProfile: 'claude-code',
        target: 'vault.semantic',
        transformed: T({ enrichedBy: { provider: 'anthropic', model: 'claude-haiku-4-5', route: 'tier' } }),
        relativePath: 'ai-memory/alpha_note.md',
      },
    )
    expect(written[0].fm.enriched_by).toEqual({ provider: 'anthropic', model: 'claude-haiku-4-5', route: 'tier' })
  })

  it('leaves the model out of enriched_by when the backend named none', async () => {
    const { vault, written } = vaultMock()
    await applyMemoryItem(
      { ...base, vault },
      {
        ...
        sourceProfile: 'claude-code',
        target: 'vault.semantic',
        transformed: T({ enrichedBy: { provider: 'claude-code', model: null, route: 'isolated-cli' } }),
        relativePath: 'ai-memory/alpha_note.md',
      },
    )
    expect(written[0].fm.enriched_by).toEqual({ provider: 'claude-code', route: 'isolated-cli' })
  })

  it('writes no enriched_by on a deterministic import', async () => {
    const { vault, written } = vaultMock()
    await applyMemoryItem(
      { ...base, vault },
      {
    ...
    expect('enriched_by' in written[0].fm).toBe(false)
  })

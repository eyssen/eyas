...
describe('data-port end to end', () => {
   ...
   * tick, an eligible background model, a kind the source already declared and
   * a transcript. A gate that quietly opened would ship the owner's notes to a
   * provider and still pass a green suite.
   */
  describe('enrichment gate', () => {
    ...
    const serviceWithModel = (aux: AuxiliaryModelService): ReturnType<typeof createDataPortService> =>
    ...
    const reply = (json: Record<string, unknown>) => auxOk(JSON.stringify(json))

    it('calls no model at all when the job did not ask for enrichment', async () => {
      const aux = createFakeAuxiliaryModel(reply({ kind: 'reference', summary_one_line: 'model summary' }))
      const svc = serviceWithModel(aux)
    ...
    it('asks only about the note that declared no kind — never a declared one, never a transcript', async () => {
      const aux = createFakeAuxiliaryModel(reply({ kind: 'project', summary_one_line: 'model summary' }))
      const svc = serviceWithModel(aux)
      const done = await importAll(svc, enrichTree(), true)
      expect(done.status).toBe('completed')
      expect(done.stats.aiEnriched).toBe(1)
      expect(done.stats.aiFallback).toBe(0)
      const seen = aux.calls.map((c) => c.user)
    ...
    it('still imports the item when the model call fails, and counts it as a fallback', async () => {
      const svc = serviceWithModel(createFakeAuxiliaryModel(auxError('provider is down')))
      const done = await importAll(svc, enrichTree(), true)
      expect(done.status).toBe('completed')
      expect(done.stats.errors).toBe(0)
    ...
    it('sends nothing without an eligible background model, and counts the note as a fallback', async () => {
      // A Grok-only install: the CLI cannot run isolated, so nothing qualifies.
      const { aux, requests } = createGatewayBackedAuxiliaryModel({ providers: ['grok-cli'], tiers: [] })
      const done = await importAll(serviceWithModel(aux), enrichTree(), true)
      expect(done.status).toBe('completed')
      ...
      expect(done.stats.aiFallback).toBe(1)
      expect(vault.read('semantic/plain_note.md')!.content).toContain('Undeclared note.')
      // Nothing answered, so nothing claims to have enriched the note.
      expect(vault.read('semantic/plain_note.md')!.frontmatter.enriched_by).toBeUndefined()
    })

    it('names the enriching model and keeps only the links that name a real note (MCI-7)', async () => {
      // A note the vault already holds, known to it through its index.
      db.run(sql`INSERT INTO vault_index (path, title, tier, content_text, file_hash, indexed_at)
                 VALUES ('semantic/existing-topic.md', 'Existing Topic', 'semantic', 'x', 'h', '2026-09-01')`)
      const aux = createFakeAuxiliaryModel(
        auxOk(
          JSON.stringify({
            summary_one_line: 'model summary',
            // One note the vault holds, one this import writes, one nobody wrote.
            links: ['Existing Topic', '[[declared_note]]', 'invented-note'],
          }),
          { provider: 'anthropic', model: 'claude-haiku-4-5', route: 'tier' },
        ),
      )
      const done = await importAll(serviceWithModel(aux), enrichTree(), true)
      expect(done.stats.aiEnriched).toBe(1)
      const enriched = vault.read('semantic/plain_note.md')!.frontmatter
      expect(enriched.enriched_by).toEqual({ provider: 'anthropic', model: 'claude-haiku-4-5', route: 'tier' })
      expect(enriched.links).toEqual(['existing-topic', 'declared_note'])
      // The note whose kind the source declared was never asked about.
      expect(vault.read('procedural/declared_note.md')!.frontmatter.enriched_by).toBeUndefined()
    })

    it('sends the enrichment request isolated, with the instruction in request.system', async () => {
      const { aux, requests } = createGatewayBackedAuxiliaryModel({
        answer: JSON.stringify({ kind: 'reference', summary_one_line: 'model summary' }),
      })
      const done = await importAll(serviceWithModel(aux), enrichTree(), true)
      expect(done.stats.aiEnriched).toBe(1)
      ...
      expect(requests[0].metadata).toMatchObject({ purpose: 'data_port_enrichment' })
    })

    it('without a background model service the enrichment tick is a no-op', async () => {
      const svc = createDataPortService({ db, modelCtx: {}, applyDepsFactory, dataDir, logger })
      const done = await importAll(svc, enrichTree(), true)
      expect(done.status).toBe('completed')
      expect(done.stats.aiEnriched + done.stats.aiFallback).toBe(0)
    })
    ...
    it('asks about every undeclared note, however many there are (A-17)', async () => {
      ...
      const aux = createFakeAuxiliaryModel(reply({ kind: 'reference' }))
      const svc = serviceWithModel(aux)
      ...
      expect(aux.calls).toHaveLength(49)

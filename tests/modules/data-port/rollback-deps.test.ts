...
describe('buildRollbackDeps', () => {
  ...
  it('reads a vault note off disk, verbatim, from the vault path the memory module published', () => {
    ...
    // The vault sits OUTSIDE the importer's data dir here (EYAS_DATA_DIR moved
    // it): the published path is what counts, not a guess from dataDir.
    const vaultPath = join(root, 'volume', 'vault')
    mkdirSync(join(vaultPath, 'semantic'), { recursive: true })
    const raw = '---\ntitle: Alpha\n---\n\n  indented first line\n\n\n'
    writeFileSync(join(vaultPath, 'semantic', 'alpha.md'), raw, 'utf-8')

    const deps = build(host({ memory: { vault: { delete: () => {} }, vaultPath } }))
  ...
  it('never guesses the vault: without a published path there is no raw reader', () => {
    mkdirSync(join(dataDir, 'vault', 'semantic'), { recursive: true })
    writeFileSync(join(dataDir, 'vault', 'semantic', 'alpha.md'), 'body', 'utf-8')

    const deps = build(host({ memory: { vault: { delete: () => {} } } }))
    expect(deps.vault).toBeDefined()
    expect(deps.vault!.readRaw).toBeUndefined()
  })

  it('answers null for a path that climbs out of the published vault', () => {
    const vaultPath = join(root, 'volume', 'vault')
    mkdirSync(vaultPath, { recursive: true })
    writeFileSync(join(root, 'volume', 'secret.md'), 'outside the vault', 'utf-8')

    const deps = build(host({ memory: { vault: { delete: () => {} }, vaultPath } }))
    expect(deps.vault!.readRaw!('../secret.md')).toBeNull()
    expect(deps.vault!.readRaw!(join(root, 'volume', 'secret.md'))).toBeNull()
  })

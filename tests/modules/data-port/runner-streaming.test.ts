...
describe('streaming runner (R11.7)', () => {
  ...
  it('never hands a contains-secrets note to the model even when enrichment is on', async () => {
    ...
    const aux = createFakeAuxiliaryModel(auxOk(JSON.stringify({ summary_one_line: 'model summary', tags: [] })))
    ...
    expect(JSON.stringify(aux.calls)).not.toContain('alphaalphaalpha0001')

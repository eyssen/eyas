...
describe('enrichMemory', () => {
  ...
  /** The background model answering `json` for every call. */
  const reply = (json: Record<string, unknown>) => createFakeAuxiliaryModel(auxOk(JSON.stringify(json)))

  it('asks the background model under the data_port_enrichment purpose', async () => {
    const aux = reply({ kind: 'project', summary_one_line: 'model summary' })
    const { result, enriched } = await enrichMemory({ aux }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
    expect(enriched).toBe(true)
    expect(result.kind).toBe('project')
    expect(aux.calls).toHaveLength(1)
    expect(aux.calls[0].purpose).toBe('data_port_enrichment')
    expect(aux.calls[0].user).toContain('x/a.md')
  })

  it('reaches the gateway isolated, with the instruction in request.system and no system message', async () => {
    const { aux, requests } = createGatewayBackedAuxiliaryModel({ answer: JSON.stringify({ summary_one_line: 'model summary' }) })
    const { enriched } = await enrichMemory({ aux }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
    expect(enriched).toBe(true)
    expect(requests).toHaveLength(1)
    expect(requests[0].isolated).toBe(true)
    expect(typeof requests[0].system).toBe('string')
    expect(requests[0].messages.every((m) => m.role === 'user')).toBe(true)
    expect(requests[0].metadata).toMatchObject({ purpose: 'data_port_enrichment', origin: 'pipeline' })
  })

  it('keeps the deterministic metadata with zero model calls when no model is eligible (Grok-only)', async () => {
  ...
  it('keeps the deterministic metadata when the service answers none or the call fails', async () => {
    for (const step of [auxNone(), auxError('provider is down'), auxEmpty()]) {
  ...
  it('never hands a contains-secrets note to a model, and never takes the tag from one', async () => {
    ...
    const aux = reply({ summary_one_line: 's' })
    const flagged = normalizeMemory(note, {
      relativePath: 'x/a.md',
      sourceProfile: 'generic-md',
      containsSecrets: true,
    })
    const { result, enriched } = await enrichMemory({ aux }, note, flagged, {
      path: 'x/a.md',
      sourceProfile: 'generic-md',
    })
    ...
    const inventing = reply({ summary_one_line: 's', tags: ['contains-secrets', 'ok'] })
    const out = await enrichMemory({ aux: inventing }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
  ...
  it('never replaces the body, even when the model returns one', async () => {
    const aux = reply({ kind: 'feedback', body: 'SHORT REWRITE', summary_one_line: 'model summary', tags: ['t'], links: ['l'] })
    const { result, enriched } = await enrichMemory({ aux }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
    expect(enriched).toBe(true)
  ...
  it('does not honour skip or kind user from the model', async () => {
    const aux = reply({ skip: true, kind: 'user', summary_one_line: 's' })
    const { result } = await enrichMemory({ aux }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
  ...
  it('never lets inference overwrite a kind the source declared', async () => {
    const enrich = async (raw: string, json: Record<string, unknown>) => {
      ...
      return (await enrichMemory({ aux: reply(json) }, n, b, { path: 'x/d.md', sourceProfile: 'generic-md' })).result
    }
  ...
  it('records a reported PII risk as a tag', async () => {
    const risky = reply({ kind: 'reference', pii_risk: 'likely' })
    expect((await enrichMemory({ aux: risky }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })).result.tags).toContain('pii:likely')
    const clean = reply({ kind: 'reference', pii_risk: 'none' })
    const tags = (await enrichMemory({ aux: clean }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })).result.tags
    expect(tags.some((t) => t.startsWith('pii:'))).toBe(false)
  })

  it('clamps a model summary to one line of at most 140 characters', async () => {
    const long = 'x'.repeat(200)
    const aux = reply({ summary_one_line: `first line\nsecond line ${long}` })
    const { result } = await enrichMemory({ aux }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
    expect(result.summary_one_line).toBe('first line')
    const aux2 = reply({ summary_one_line: long })
    expect((await enrichMemory({ aux: aux2 }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })).result.summary_one_line).toHaveLength(140)
  })
  ...
  it('refuses reserved tags from the model', async () => {
    ...
    const aux = reply({ tags: [...forged, 'genuine-topic'] })
    const { result } = await enrichMemory({ aux }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
    expect(result.tags).toEqual(['genuine-topic'])
  })

  it('falls back to the deterministic result without a background model', async () => {
    const { result, enriched } = await enrichMemory({}, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
    expect(enriched).toBe(false)
    expect(result).toEqual(base)
  })

  it('falls back to the deterministic result when the model returns no JSON', async () => {
    const aux = createFakeAuxiliaryModel(auxOk('I cannot help with that.'))
    const { result, enriched } = await enrichMemory({ aux }, note, base, { path: 'x/a.md', sourceProfile: 'generic-md' })
    expect(enriched).toBe(false)
    expect(result).toEqual(base)
  })
...
describe('linkTarget', () => {
  it('keeps the target alone: no brackets, embed mark, display text, heading or block anchor', () => {
    expect(linkTarget('[[user_profile]]')).toBe('user_profile')
    expect(linkTarget('![[diagram.png]]')).toBe('diagram.png')
    expect(linkTarget('[[user_profile|the owner]]')).toBe('user_profile')
    expect(linkTarget('user_profile#Languages')).toBe('user_profile')
    expect(linkTarget('user_profile^block-1')).toBe('user_profile')
    expect(linkTarget('  plain-slug  ')).toBe('plain-slug')
  })

  it('leaves nothing of a link that names no target', () => {
    expect(linkTarget('[[]]')).toBe('')
    expect(linkTarget('[[#heading-only]]')).toBe('')
    expect(linkTarget('   ')).toBe('')
  })
})

describe('createLinkIndex', () => {
  it('resolves a slug, its file stem, its title and a vault path to the slug the note is filed under', () => {
    const index = createLinkIndex()
    index.add('user_profile', ['User Profile'])
    expect(index.resolve('user_profile')).toBe('user_profile')
    expect(index.resolve('user-profile')).toBe('user_profile')
    expect(index.resolve('User Profile')).toBe('user_profile')
    expect(index.resolve('[[user_profile|me]]')).toBe('user_profile')
    expect(index.resolve('user_profile.md')).toBe('user_profile')
    expect(index.resolve('semantic/user_profile.md')).toBe('user_profile')
  })

  it('matches a decomposed name against its composed form', () => {
    const index = createLinkIndex()
    index.add('Árvíztűrő-tükörfúrógép'.normalize('NFC'))
    expect(index.resolve('árvíztűrő tükörfúrógép'.normalize('NFD'))).toBe('Árvíztűrő-tükörfúrógép'.normalize('NFC'))
  })

  it('keeps the first note registered under a name', () => {
    const index = createLinkIndex()
    index.add('existing-topic', ['Shared Title'])
    index.add('new-topic', ['Shared Title'])
    expect(index.resolve('Shared Title')).toBe('existing-topic')
    expect(index.resolve('new-topic')).toBe('new-topic')
  })

  it('resolves nothing it was not told about', () => {
    const index = createLinkIndex()
    index.add('user_profile', ['User Profile', null, undefined, ''])
    expect(index.resolve('invented-note')).toBeNull()
    expect(index.resolve('semantic/invented-note.md')).toBeNull()
    expect(index.resolve('[[]]')).toBeNull()
    expect(index.resolve('')).toBeNull()
    // An empty slug registers nothing, whatever names come with it.
    index.add('', ['Ghost'])
    expect(index.resolve('Ghost')).toBeNull()
  })
})

describe('enrichMemory — isolated answers, provenance and links (MCI-7)', () => {
  const note = readSourceNote('x/a.md', 'Plain note that mentions [[source_link]]. ' + 'text '.repeat(200))
  const base = normalizeMemory(note, { relativePath: 'x/a.md', sourceProfile: 'generic-md' })
  const opts = { path: 'x/a.md', sourceProfile: 'generic-md' as const }
  const answer = (json: Record<string, unknown>, extra: Parameters<typeof auxOk>[1] = {}) =>
    createFakeAuxiliaryModel(auxOk(JSON.stringify(json), extra))
  /** A vault holding `user_profile`, and an import that also writes `release_plan`. */
  const known = () => {
    const index = createLinkIndex()
    index.add('user_profile', ['User Profile'])
    index.add('release_plan', ['Release plan'])
    return index
  }

  it('records the provider, model and ladder rung that answered', async () => {
    const aux = answer({ summary_one_line: 'model summary' }, { provider: 'anthropic', model: 'claude-haiku-4-5', route: 'tier' })
    const { result, enriched } = await enrichMemory({ aux }, note, base, opts)
    expect(enriched).toBe(true)
    expect(result.enrichedBy).toEqual({ provider: 'anthropic', model: 'claude-haiku-4-5', route: 'tier' })
  })

  it('records a CLI pinned by provider only with no model name', async () => {
    const aux = answer({ summary_one_line: 's' }, { provider: 'claude-code', model: null, route: 'isolated-cli' })
    const { result } = await enrichMemory({ aux }, note, base, opts)
    expect(result.enrichedBy).toEqual({ provider: 'claude-code', model: null, route: 'isolated-cli' })
  })

  it('records the answering provider of the real service, reached isolated', async () => {
    const { aux, requests } = createGatewayBackedAuxiliaryModel({ answer: JSON.stringify({ summary_one_line: 's' }) })
    const { result } = await enrichMemory({ aux }, note, base, opts)
    expect(requests[0].isolated).toBe(true)
    expect(result.enrichedBy).toMatchObject({ provider: 'fake-api', route: 'tier' })
  })

  it('keeps a model link only when it names a note in the vault or this import, as that note\'s slug', async () => {
    const aux = answer({ links: ['[[User Profile]]', 'semantic/release_plan.md', 'invented-note', '[[Made Up|x]]'] })
    const index = known()
    const { result } = await enrichMemory({ aux }, note, base, { ...opts, resolveLink: (t) => index.resolve(t) })
    expect(result.links).toEqual(['source_link', 'user_profile', 'release_plan'])
  })

  it('drops every model link when nothing can confirm the note it names', async () => {
    const aux = answer({ summary_one_line: 's', links: ['user_profile', 'release_plan'] })
    const { result, enriched } = await enrichMemory({ aux }, note, base, opts)
    expect(enriched).toBe(true)
    // The source's own link is the source's words and stays.
    expect(result.links).toEqual(['source_link'])
  })

  it('logs how many invented links it dropped', async () => {
    const debug = vi.fn()
    const logger = { info: vi.fn(), warn: vi.fn(), error: vi.fn(), debug }
    const aux = answer({ links: ['invented-one', 'invented-two', 'user_profile'] })
    const index = known()
    await enrichMemory({ aux, logger }, note, base, { ...opts, resolveLink: (t) => index.resolve(t) })
    expect(debug).toHaveBeenCalledWith({ path: 'x/a.md', dropped: 2 }, expect.stringContaining('dropped'))
  })

  it('leaves the note untouched, with no provenance, when no isolated model answered', async () => {
    // Grok-only: the CLI cannot run isolated, so the ladder has nothing to call.
    const grokOnly = createGatewayBackedAuxiliaryModel({ providers: ['grok-cli'], tiers: [] })
    const out = await enrichMemory({ aux: grokOnly.aux }, note, base, opts)
    expect(grokOnly.requests).toHaveLength(0)
    expect(out).toEqual({ result: base, enriched: false })
    expect(out.result.enrichedBy).toBeUndefined()
    expect(out.result.body).toBe(base.body)

    for (const step of [auxNone('budget_stop'), auxNone('tier_not_configured'), auxError(), auxEmpty()]) {
      const r = await enrichMemory({ aux: createFakeAuxiliaryModel(step) }, note, base, opts)
      expect(r.enriched).toBe(false)
      expect(r.result).toBe(base)
    }
  })

  it('does not use a refusal, even one that parses', async () => {
    const aux = createFakeAuxiliaryModel(auxOk(JSON.stringify({ summary_one_line: 'refused' }), { stopReason: 'refusal' }))
    const { result, enriched } = await enrichMemory({ aux }, note, base, opts)
    expect(enriched).toBe(false)
    expect(result).toBe(base)
  })

  it('treats a service that throws as no answer', async () => {

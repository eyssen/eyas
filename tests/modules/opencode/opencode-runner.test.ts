...
import { buildOpencodeEnv, buildShellCommand, buildTuiCommand, OPENCODE_ISOLATION_FLAGS, writeOpencodeManagedFiles } from '@modules/opencode/isolation'
...
describe('opencode runner isolation', () => {
  ...
  it('(−) a plain shell gets the terminal\'s filtered environment, never the server\'s: no master key, provider keys, passwords or plugin keys', () => {
    const source: NodeJS.ProcessEnv = { ...HOSTILE_ENV, SHELL: '/bin/zsh', EYAS_MASTER_KEY: 'host-master-key', TERM: 'dumb' }
    const cmd = buildShellCommand({ home, eyasBaseUrl: 'http://x', workspacesRoot: workspaces, source })
    expect(cmd.file).toBe('/bin/zsh')
    expect(cmd.args).toEqual(['-l'])
    expect(cmd.env.HOME).toBe(home)
    expect(cmd.env.PATH).toBe('/usr/bin:/bin')
    expect(cmd.env.TERM).toBe('xterm-256color')
    expect(cmd.env.GIT_CEILING_DIRECTORIES).toBe(workspaces)
    for (const key of ['EYAS_MASTER_KEY', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'OPENCODE_SERVER_PASSWORD', 'EYAS_OPENCODE_PLUGIN_TOKEN', 'EYAS_OPENCODE_KEY_FD', 'CLAUDECODE', 'OPENCODE_CONFIG']) {
      expect(cmd.env, key).not.toHaveProperty(key)
    }
    expect(Object.values(cmd.env)).not.toContain('host-master-key')
    // No $SHELL (or not an absolute path): /bin/bash.
    expect(buildShellCommand({ home, eyasBaseUrl: 'http://x', workspacesRoot: workspaces, source: { PATH: '/usr/bin' } }).file).toBe('/bin/bash')
    expect(buildShellCommand({ home, eyasBaseUrl: 'http://x', workspacesRoot: workspaces, source: { SHELL: 'zsh' } }).file).toBe('/bin/bash')
  })

...
/** The routes as `caller` (CASL itself is mocked through: this is the handler's own check). */
function app(opts: { caller?: Caller; settings?: { settings: OpencodeSettings }; conversations?: ConversationService | null } = {}) {
  ...
  const caller = opts.caller ?? { id: 'u1', role: 'user' }
  ...
  hono.use('*', async (c, next) => {
    ...
    set('role', caller.role ?? 'user')
    // An admin can do everything CASL knows: the conversation rule still applies.
    set('ability', { can: () => caller.role === 'admin' || caller.role === 'owner' })
    await next()
  })
  ...
  return { hono, open, created, commands, ensureServer }
}
...
describe('opencode routes', () => {
  ...
  it('(−) a shell is refused (403) to a caller who may not manage OpenCode, before anything is looked up; the TUI still opens', async () => {
    ...
    // The same answer for an id that does not exist: the shell check comes first.
    const ghost = await open({ conversationId: 'conv-route-ghost', kind: 'shell' })
    expect([shell.status, ghost.status]).toEqual([403, 403])
    ...
    expect(created.map((c) => c.kind)).toEqual(['tui'])
  })
...
describe('OpenCode terminal — a stored folder in another user\'s workspace is never the terminal\'s', () => {
  ...
  it('(−) a symlink into another user\'s workspace is judged where it leads', async () => {
    ...
    const { open, created } = app({ caller: { id: 'u2', role: 'user' } })
  ...
  it('(+) the workspace of another conversation of the same owner, and a team child\'s parent workspace, stay usable', async () => {
    ...
    const { open, created } = app({ caller: { id: 'u1', role: 'user' } })

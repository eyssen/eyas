...
import { OPENCODE_PERMISSIONS, OPENCODE_SUBJECT } from '@modules/opencode/permissions.js'
...
describe('another user\'s conversation workspace cannot become a folder (save → terminal chain)', () => {
  ...
  beforeEach(async () => {
    ...
    reg.registerSubject(OPENCODE_SUBJECT, OPENCODE_PERMISSIONS)
    as(u1)
    app = new Hono()
  ...
  it('(−) the reported chain: PATCH to u1\'s workspace is a 400 (otherWorkspace), and u2\'s terminal still opens only in u2\'s own workspace', async () => {
    ...
    // Step 2: no terminal is a `user`'s at all — the OpenCode TUI and a plain
    // shell both need manage OpenCode.
    for (const kind of ['tui', 'shell'] as const) {
      expect((await call('POST', '/api/v1/opencode/sessions', { conversationId: b.id, kind })).status, kind).toBe(403)
    }
    expect(created).toHaveLength(0)
    // Step 3: as an admin, u2's own conversation's terminal works in u2's workspace only.
    as(u2, 'admin')
    ...
    expect((await call('POST', '/api/v1/opencode/sessions', { conversationId: b.id, kind: 'shell', cwd: aWs })).status).toBe(400)

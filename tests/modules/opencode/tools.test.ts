...
describe('opencode tools', () => {
  ...
  it('run hands the task the run\'s own folders (a primary-only context counts as one), never the model\'s', async () => {
    ...
    await run.execute({ prompt: 'hi', cwd: '/w/a/src' }, { conversationId: 'c1', userId: 'u1', workingDirectory: '/w/a', workingDirectories: ['/w/a', '/w/b'], logger: undefined as never })
    await run.execute({ prompt: 'hi' }, { conversationId: 'c1', userId: 'u1', workingDirectory: '/w/a', logger: undefined as never })
    await run.execute({ prompt: 'hi', cwd: '/elsewhere' }, { conversationId: 'c1', userId: 'u1', logger: undefined as never })
    expect(seen.map((s) => [s.cwd, s.workingDirectories])).toEqual([
      ['/w/a/src', ['/w/a', '/w/b']],
      ['/w/a', ['/w/a']],
      // No folders: resolveTaskCwd holds the model's cwd to the conversation's own workspace.
      ['/elsewhere', undefined],
    ])
  })

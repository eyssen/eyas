...
describe('OpenCode task folder', () => {
  ...
  it('(−) without conversation folders, a model-named cwd must lie in the conversation\'s own workspace: never another one', () => {
    const workspaces = process.env.EYAS_WORKSPACES_DIR!
    const other = join(workspaces, 'conv-cwd-other')
    mkdirSync(other, { recursive: true })
    expect(() => resolveTaskCwd({ prompt: 'x', conversationId: 'conv-cwd-own', userId: 'u', cwd: other }))
      .toThrow(/outside the conversation working directories/)
    expect(() => resolveTaskCwd({ prompt: 'x', conversationId: 'conv-cwd-own', userId: 'u', cwd: tmpdir() }))
      .toThrow(/outside the conversation working directories/)
    const own = join(workspaces, 'conv-cwd-own')
    const sub = join(own, 'src')
    mkdirSync(sub, { recursive: true })
    expect(resolveTaskCwd({ prompt: 'x', conversationId: 'conv-cwd-own', userId: 'u', cwd: sub })).toBe(realpathSync(sub))
  })
})
...
describe('OpenCode task folder — never another user\'s conversation workspace', () => {
  let fx: SovereigntyFixture
  /** conv-1 is u1's, conv-2 is u2's (the fixture's two workspaces); conv-1b is u1's second one. */
  const owners: Record<string, string> = { 'conv-1': 'u1', 'conv-1b': 'u1', 'conv-2': 'u2' }
  const conversations = {
    ownerOf: (id: string) => owners[id] ?? null,
    ownsConversation: (id: string, userId: string) => owners[id] === userId,
  }
  const sameOwnerAs = (conversationId: string) => (id: string) => id === conversationId || owners[id] === owners[conversationId]
  ...
  it('(−) a stored folder in another user\'s workspace (saved before the rule) is skipped: the task runs in its own workspace', () => {
    // conv-2 (u2) holds conv-1's (u1's) workspace as a stored folder.
    const cwd = resolveTaskCwd({ prompt: 'x', conversationId: 'conv-2', userId: 'u2', workingDirectories: [fx.ownWorkspace] }, { mayUseWorkspace: sameOwnerAs('conv-2') })
    expect(cwd).toBe(fx.otherWorkspace)
    // Without rules only the task conversation's own workspace counts.
    expect(resolveTaskCwd({ prompt: 'x', conversationId: 'conv-2', userId: 'u2', workingDirectories: [fx.ownWorkspace] })).toBe(fx.otherWorkspace)
  })

  it('(−) a model-named cwd in another user\'s workspace is refused even when a stored folder names it', () => {
    expect(() => resolveTaskCwd({ prompt: 'x', conversationId: 'conv-2', userId: 'u2', cwd: fx.ownWorkspace, workingDirectories: [fx.ownWorkspace] }, { mayUseWorkspace: sameOwnerAs('conv-2') }))
      .toThrow(/outside the conversation working directories|another conversation's workspace/)
  })

  it('(−) a symlink into another user\'s workspace is judged where it leads (after realpath)', () => {
    const project = join(fx.root, 'projects', 'app')
    mkdirSync(project, { recursive: true })
    const link = join(project, 'shortcut')
    symlinkSync(fx.ownWorkspace, link)
    expect(() => resolveTaskCwd({ prompt: 'x', conversationId: 'conv-2', userId: 'u2', cwd: link, workingDirectories: [project] }, { mayUseWorkspace: sameOwnerAs('conv-2') }))
      .toThrow(/another conversation's workspace/)
    const own = resolveTaskCwd({ prompt: 'x', conversationId: 'conv-2', userId: 'u2', workingDirectories: [link] }, { mayUseWorkspace: sameOwnerAs('conv-2') })
    expect(own).toBe(fx.otherWorkspace)
  })

  it('(+) the workspace of another conversation of the same owner stays usable', () => {
    const cwd = resolveTaskCwd({ prompt: 'x', conversationId: 'conv-1b', userId: 'u1', cwd: fx.ownWorkspace, workingDirectories: [fx.ownWorkspace] }, { mayUseWorkspace: sameOwnerAs('conv-1b') })
    expect(cwd).toBe(realpathSync(fx.ownWorkspace))
  })

  it('(−) run(): the dropped folder reaches neither the task folder nor the gate, and a read there is rejected', async () => {
    const det = createDeterministicGate({ ...DEFAULT_CONFIG })
    const calls: Array<{ ctx: any }> = []
    const g: OpencodeSecurityGate = {
      validateToolCall: (name, input, ctx) => {
        calls.push({ ctx })
        return det.check(name, input, ctx)
      },
    }
    const seen: Array<string[] | undefined> = []
    const server = fakeServer([asked('per_x', OWN, 'read', [fx.ownFile], { filepath: fx.ownFile })], { awaitReplies: 1 })
    const agent = createDeveloperAgent({
      getClient: async () => server.client,
      getSecurityGate: () => g,
      getConversations: () => conversations,
      resolveCwd: (input, rules) => {
        seen.push(input.workingDirectories)
        return resolveTaskCwd(input, rules)
      },
      logger,
      eventStreamWaitMs: 200,
    })
    // u2's task names u1's workspace (conv-1) as a stored folder.
    await agent.run({ prompt: 'peek', conversationId: 'conv-2', userId: 'u2', workingDirectories: [fx.ownWorkspace, fx.otherWorkspace] })
    expect(seen).toEqual([[fx.otherWorkspace]])
    expect(server.directories).toEqual([realpathSync(fx.otherWorkspace)])
    // The gate's folders: the task folder and the kept stored folder — never u1's workspace.
    expect(calls[0]!.ctx.workingDirectories).toEqual([...new Set([realpathSync(fx.otherWorkspace), fx.otherWorkspace])])
    expect(server.replies).toEqual([{ sessionId: OWN, requestId: 'per_x', reply: 'reject' }])
  })
})

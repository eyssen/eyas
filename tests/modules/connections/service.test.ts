...
describe('ConnectionsService', () => {
  ...
  it('catalog has odoo, github, mcp, playwright-mcp', () => {
    const ids = CONNECTION_CATALOG.map((c) => c.id)
    expect(ids).toContain('odoo')
    expect(ids).toContain('github')
    expect(ids).toContain('mcp')
    expect(ids).toContain('playwright-mcp')
    expect(ids).toContain('agent-browser')
    expect(ids).toContain('chrome-devtools-mcp')
    const pw = CONNECTION_CATALOG.find((c) => c.id === 'playwright-mcp')!
    expect(pw.adapter).toBe('mcp')
    expect(pw.tags).toContain('playwright')
  })

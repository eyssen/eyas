...
describe('the shipped upgrade table', () => {
  it('does not contain identity upgrades (prior === current as a set)', () => {
    expect(PRIOR_TOOLSETS.length).toBeGreaterThan(0)
    for (const u of PRIOR_TOOLSETS) {
      const prior = new Set(u.prior)
      const current = new Set(u.current)
      const same = prior.size === current.size && [...prior].every((t) => current.has(t))
      expect(same, 'upgrade must change the allow-list').toBe(false)
    }
  })

  it('strips builder tools from a previously shipped Personal Assistant', () => {
    ...
    const next = toolsOf('a1')
    expect(next).toContain('design_read')
    expect(next).toContain('run_specialist')
    expect(next).not.toContain('run_command')
    expect(next).not.toContain('write_file')
  })
})

...
describe('agent-runner — autonomy ladder gating', () => {
  ...
  it('FAILS CLOSED on a locked-category tool when no reviewer is wired', async () => {
    ...
    expect(events.find((e) => e.type === 'approval_required')).toMatchObject({ toolName: 'run_command' })
    expect(events.find((e) => e.type === 'tool_result')).toMatchObject({ outcome: 'approval_required', isError: true })
  })

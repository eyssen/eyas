...
import { auxNone, createAuxiliaryModelOverGateway, createFakeAuxiliaryModel } from '../../helpers/fake-auxiliary-model'
...
function createTables(database: any) {
  database.run(sql`CREATE TABLE IF NOT EXISTS conversations (
    ...
    agent_id TEXT, project_id TEXT, goal_description TEXT, provider_id TEXT, model_id TEXT, model_user_chosen INTEGER NOT NULL DEFAULT 0, model_binding TEXT, parent_conversation_id TEXT, stage_id TEXT,
...
/** The real background model service over the fake gateway (one eligible API provider). */
...
describe('plan-as-rubric', () => {
  ...
  it("the planner is one isolated background call (purpose 'planner') attributed to the run", async () => {
    await runConversation('conv-1', deps)

    expect(gateway.planCalls).toHaveLength(1)
    const req = gateway.planCalls[0]
    expect(req.isolated).toBe(true)
    expect(req.tools).toBeUndefined()
    expect(req.system).toContain('planning agent')
    expect(req.messages).toHaveLength(1)
    expect(req.messages[0].role).toBe('user')
    expect(req.messages[0].content).toContain('Migrate the billing module')
    expect(req.metadata).toMatchObject({ purpose: 'planner', origin: 'scheduled', conversationId: 'conv-1', runId: 'run-1' })
    expect(req.metadata?.tier).toBeUndefined()
  })

  it('runs on an eligible tier model — the planner shares the safety tiers (heartbeat, then quick)', async () => {
    deps.critic.aux = createAuxiliaryModelOverGateway(gateway, {
      providers: ['pq'],
      tiers: [{ tier: 'quick', providerId: 'pq', modelId: 'mq', fallbackProviderId: null, fallbackModelId: null, description: '', enabled: true, updatedAt: '' }],
    })
  ...
  describe('fail-open', () => {
    it('an unparseable plan response → the run proceeds with no rubric', async () => {
      deps.critic.aux = auxOver(fakeGateway({ plan: 'sorry, I cannot plan that' }))
    ...
    it('a gateway that throws during planning → the run proceeds', async () => {
      ...
      deps.critic.aux = auxOver(throwing)
    ...
    it('no eligible background model (a Grok-only install) → no rubric, no model call, the run proceeds', async () => {
    ...
    it("a 'none' answer from the service makes exactly one planner request and no rubric", async () => {
      const aux = createFakeAuxiliaryModel(auxNone())
      deps.critic.aux = aux

      await runConversation('conv-1', deps)

      // One planner ask (no retry on a missing model), then the critic's ask.
      expect(aux.calls.map((c) => c.purpose)).toEqual(['planner', 'critic'])
      expect(plans()).toHaveLength(0)
    })
  })
  ...
  describe('feedback resume', () => {
    it('reuses the parent plan instead of regenerating it, and still injects the rubric', async () => {
      gateway = fakeGateway({ verdicts: [INCOMPLETE, COMPLETE] })
      deps.critic.aux = auxOver(gateway)
    ...
    it("the child's critic is handed the same rubric", async () => {
      ...
      const childCriticUser: string = gateway.criticCalls[1].messages[0].content

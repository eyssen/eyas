...
/** The real background model service over a critic gateway mock (one eligible API provider). */
function auxOver(gw: { complete: any }) {
  return createAuxiliaryModelOverGateway(gw)
}
...
beforeEach(() => {
  ...
  deps = {
    ...
    critic: { enabled: true, maxRounds: 1, aux: auxOver(gateway) },
  }
})
...
describe('completeness critic — verdicts on a finished run', () => {
  ...
  it("stamps {origin:'scheduled', conversationId, runId, purpose:'critic'} on the critic call (Task 9 attribution)", async () => {
    await runConversation('conv-1', deps)
  ...
  it("verdict 'unavailable' (gateway throws) → verification='unverified', run completes, no resume", async () => {
    deps.critic.aux = auxOver({ complete: vi.fn().mockRejectedValue(new Error('down')) })
  ...
  it("malformed critic JSON → verification='unverified'", async () => {
    deps.critic.aux = auxOver(fakeCriticGateway(['looks done to me']))
  ...
  it("no eligible background model (a Grok-only install) ⇒ verification='unverified', the model is never called", async () => {
    deps.critic.aux = createAuxiliaryModelOverGateway(gateway, { providers: ['grok-cli'] })
  ...
  it("no background model service wired ⇒ verification='unverified'", async () => {
    deps.critic.aux = undefined
...
describe('completeness critic — when it must NOT run', () => {
  it('a max_turns run is not critic\'d (it never claimed to be done)', async () => {
    deps.agentRunner = fakeRunner(store, { events: [{ type: 'done', response: { content: [] }, outcome: 'max_turns', stopReason: 'tool_use' }] })
  ...
  it('a tool_budget run is not critic\'d either', async () => {
    deps.agentRunner = fakeRunner(store, { events: [{ type: 'done', response: { content: [] }, outcome: 'tool_budget', stopReason: 'tool_use' }] })
...
describe('completeness critic — the feedback loop', () => {
  ...
  it('incomplete under the cap → this run fails verification and ONE child run carries the feedback', async () => {
    deps.critic.aux = auxOver(fakeCriticGateway([INCOMPLETE, COMPLETE]))
  ...
  it('the cap is respected: a child that is ALSO incomplete does not spawn a grandchild', async () => {
    deps.critic.aux = auxOver(fakeCriticGateway([INCOMPLETE, INCOMPLETE]))
  ...
  it('maxRounds=0 → the verdict is recorded but no feedback resume is started', async () => {
    ...
    deps.critic.aux = auxOver(fakeCriticGateway([INCOMPLETE]))
...
// I11 — the critic credits the memory EYAS delivered to the run and reads the
// run's tool evidence from the executor log, whichever provider ran it.
describe('completeness critic — delivered recall and executor-log evidence', () => {
  const GROUNDED_GOAL = 'research how refunds are booked in the billing module'

  function setGoal(goal: string) {
    db.run(sql`UPDATE conversations SET goal_description = ${goal} WHERE id = 'conv-1'`)
  }

  function delivery(ids: string[]) {
    return {
      profile: { providerId: 'p', modelId: 'm', contextWindow: 200_000, resolved: true },
      budgetTotalTokens: 1000,
      recall: { ids, retrieved: ids, expanded: [], chars: 10, budgetChars: 100, turnId: 't-1' },
    }
  }

  function assembledWith(ids: string[]) {
    return {
      prefix: 'PFX', suffix: 'SFX', reminders: [], cacheBoundaryHint: 0, prefixHash: 'h',
      tokenEstimate: { prefix: 1, suffix: 1, reminders: 0 }, sections: [], turn: '<turn-context>…</turn-context>',
      delivery: delivery(ids),
    }
  }

  it('(+) the runner forwards the delivered ids: no tool call, grounding goal → the model critic judges, not an automatic fail', async () => {
    setGoal(GROUNDED_GOAL)
    deps.promptAssembler = { buildForPrimary: vi.fn().mockResolvedValue(assembledWith(['gs:7', 'nt:refunds'])) }

    await runConversation('conv-1', deps)

    expect(gateway.complete).toHaveBeenCalledTimes(1)
    const req = gateway.complete.mock.calls[0][0]
    expect(req.system).toContain('gs:7, nt:refunds')
    expect(runRow('run-1').verification).toBe('passed')
  })

  it('(+) the turn-only fallback forwards its delivered ids too', async () => {
    setGoal(GROUNDED_GOAL)
    deps.promptAssembler = {
      buildForPrimary: vi.fn().mockRejectedValue(new Error('assembly failed')),
      buildTurnOnly: vi.fn().mockResolvedValue({ turn: '<turn-context>…</turn-context>', sections: [], delivery: delivery(['gs:9']) }),
    }

    await runConversation('conv-1', deps)

    expect(gateway.complete).toHaveBeenCalledTimes(1)
    expect(gateway.complete.mock.calls[0][0].system).toContain('gs:9')
  })

  it('(−) nothing delivered, no tool call, grounding goal → incomplete without asking the model', async () => {
    setGoal(GROUNDED_GOAL)
    deps.critic.maxRounds = 0
    deps.promptAssembler = { buildForPrimary: vi.fn().mockResolvedValue(assembledWith([])) }

    await runConversation('conv-1', deps)

    expect(gateway.complete).not.toHaveBeenCalled()
    expect(runRow('run-1').verification).toBe('failed')
    const verdict = store.events.find((e) => e.type === 'CriticVerdict')
    expect(verdict.payload.verdict).toBe('incomplete')
  })

  /** A run that reports its tool under an ACP display name while the executor logged the canonical one. */
  function acpStyleRunner(loggedAs: { runId?: string; toolName: string }) {
    return {
      run: vi.fn((runOpts: any) => ({
        async *[Symbol.asyncIterator]() {
          recordToolExecution(db, {
            toolName: loggedAs.toolName,
            runId: loggedAs.runId ?? runOpts.sessionId,
          ...
          yield { type: 'tool_use_start', id: 'tu-1', name: 'use_tool' }
          await store.append({ sessionId: runOpts.sessionId, type: 'LlmResponse', payload: { response: { content: 'Refunds become credit notes.' } } })
          yield { type: 'turn_complete', tokensUsed: 2 }
        },
      })),
    }
  }

  it("(+) an ACP-style run whose events show only 'use_tool' but whose executor log has memory_search → retrieval credited", async () => {
    setGoal(GROUNDED_GOAL)
    ensureToolExecutionsTable(db)
    deps.agentRunner = acpStyleRunner({ toolName: 'memory_search' })
  ...
  it("(−) the executor log of ANOTHER run is not this run's evidence", async () => {
    setGoal(GROUNDED_GOAL)
    deps.critic.maxRounds = 0
    ensureToolExecutionsTable(db)
    deps.agentRunner = acpStyleRunner({ runId: 'someone-else', toolName: 'memory_search' })

...
describe('context recorder — entry point coverage (Task 11)', () => {
  it('records a composition from every model-invocation path, one per entry point', async () => {
    ...
    bgDb.run(sql`CREATE TABLE conversations (
      ...
      provider_id TEXT, model_id TEXT, model_user_chosen INTEGER NOT NULL DEFAULT 0, model_binding TEXT, parent_conversation_id TEXT, team_session_id TEXT, thinking TEXT,
    ...
    const deps: any = {
      db: bgDb,
      agentRunner: { run: vi.fn().mockReturnValue(asyncIterable([{ type: 'turn_complete', tokensUsed: 1, usage: { inputTokens: 1, outputTokens: 1, promptTokensLastCall: 1111 } }])) },
      agentRegistry: {
    ...
    const agentRunner = {
      run: vi.fn().mockReturnValue(asyncIterable([{ type: 'done', response: { content: [{ type: 'text', text: 'ok' }], usage: { inputTokens: 1, outputTokens: 1, promptTokensLastCall: 2222 }, contextWindow: 300_000 } }])),
    }
    ...
    await orchestrator.runAgentInConversation('agent-1', 'parent-conv', 'do the thing')
    ...
    try {
      ...
      function reviewResponse(voteFor: string): ModelResponse {
        return {
          ...
          stopReason: 'end', usage: { inputTokens: 1, outputTokens: 1, promptTokensLastCall: 3333 },
        }
      }
      // The reviewer's own slot is in the instruction channel (request.system).
      function parseSlot(req: ModelRequest): string {
        return (req.system ?? '').match(/Your slot is (\w+)/)?.[1] ?? ''
      }
    ...
    // ── G11 — every path records its history estimate, and what the provider
    // measured on the run's last model call lands on its composition ──
    const occupancy = (entryPoint: string) => recorderDb.all(sql`SELECT history_estimated_tokens AS history,
      observed_prompt_tokens AS prompt, observed_context_window AS window
      FROM context_compositions WHERE entry_point = ${entryPoint}`) as Array<{ history: number | null; prompt: number | null; window: number | null }>
    expect(occupancy('background')).toEqual([{ history: 2, prompt: 1111, window: null }])
    expect(occupancy('orchestrator-member')).toEqual([{ history: 3, prompt: 2222, window: 300_000 }])
    const unassembled = occupancy('unassembled')
    // executeAgent: the task is its history; nothing measured (negative).
    expect(unassembled).toContainEqual({ history: 3, prompt: null, window: null })
    // God Mode review: its only message IS the recorded peer-results section.
    expect(unassembled.filter((r) => r.prompt === 3333)).toHaveLength(2)
    expect(unassembled.filter((r) => r.prompt === 3333).every((r) => r.history === 0)).toBe(true)
  })
})

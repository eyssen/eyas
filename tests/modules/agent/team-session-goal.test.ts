...
describe('team session goalDescription', () => {
  ...
  it('ensureImplicitWorkSession creates a running session once', () => {
    const svc = createTeamSessionService(makeDb())
    const first = svc.ensureImplicitWorkSession('conv1', ['assistant', 'reviewer'])
    expect(first.status).toBe('running')
    expect(JSON.parse(first.config).source).toBe('implicit')
    const second = svc.ensureImplicitWorkSession('conv1', ['assistant', 'other'])
    expect(second.id).toBe(first.id)
    const members = JSON.parse(second.config).members as string[]
    expect(members.sort()).toEqual(['assistant', 'other', 'reviewer'])
  })
})

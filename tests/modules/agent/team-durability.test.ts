...
interface StackOptions {
  ...
  /** Captured user message of every background-model call (the re-planner's input). */
...
function makeStack(db: any, opts: StackOptions = {}) {
  ...
  // The re-planner runs through the background model service.
  const aux = {
    complete: vi.fn(async (req: any) => {
      opts.replanPrompts?.push(req.user)
      return {
        ok: true as const, text: '{"shouldContinue": true}', provider: 'p', model: 'm', route: 'api' as const,
        usage: { inputTokens: 1, outputTokens: 1 }, stopReason: 'end' as const,
      }
    }),
  }

...
// G5 — Resume/Retry of a CLI-provider run (Claude Code, Grok, Kimi) behaves
// like an API-provider run: the runner recorded the tools the CLI ran itself,
// checkpointed the turn, and the resumed run gets the same recap and a ledger
// that reaches the CLI's permission bridge through the request metadata.
describe('resumeRun — CLI-provider runs (G5)', () => {
  function cliGateway(turns: Array<(request: any) => AsyncGenerator<any>>, seen: any[]) {
    let i = 0
    return {
      ...
      async *stream(request: any) {
        seen.push(request)
        yield* turns[i++]!(request)
      },
    } as any
  }

  const answer = (text: string) => ({
    id: 'r', provider: 'claude-code', model: 'm', stopReason: 'end',
    content: [{ type: 'text', text }], usage: { inputTokens: 1, outputTokens: 1 },
  })

  it('forwards metadata.idempotencyLedger and the recap built from provider-recorded ToolResult events', async () => {
    const seen: any[] = []
    const bridgeVerdicts: any[] = []
    const gateway = cliGateway([
      // The original run: the CLI ran Bash itself, then answered.
      async function* () {
        yield { type: 'tool_use_start', id: 'toolu_1', name: 'run_command', rawName: 'Bash', input: { command: 'rm -rf build' } }
        yield { type: 'tool_result', toolUseId: 'toolu_1', content: 'removed', isError: false, durationMs: 5, outcome: 'success', executedBy: 'provider' }
        yield { type: 'text', text: 'Cleaned the build folder.' }
        yield { type: 'done', response: answer('Cleaned the build folder.') }
      },
      // The resumed run: the CLI tries the same command again — its
      // permission bridge is built from the request, as the providers do.
      async function* (request: any) {
        const bridge = createPermissionBridge({
          validateToolCall: () => ({ decision: 'allow', reason: 'ok', riskTier: 'green' }),
          autonomy: { categoryForTool: () => null, resolve: vi.fn(), createApproval: vi.fn() } as any,
          autonomous: true,
          ctx: { conversationId: request.metadata?.conversationId, runId: request.metadata?.runId },
          ledger: request.metadata?.idempotencyLedger,
        })
        bridgeVerdicts.push(await bridge('Bash', { command: 'rm -rf build' }, { toolUseID: 'toolu_2', signal: new AbortController().signal }))
        bridgeVerdicts.push(await bridge('Bash', { command: 'ls build' }, { toolUseID: 'toolu_3', signal: new AbortController().signal }))
        yield { type: 'done', response: answer('Nothing left to do.') }
      },
    ], seen)
    ...
    // The original run, recorded under its own session.
    for await (const _ of runner.run({
      ...
      autonomous: true,
      sessionId: 'old-run',
      toolContext: { conversationId: 'conv-1', userId: 'bot', agentId: 'agent-1' } as any,
    })) { /* drain */ }

    const result = await resumeRun('old-run', { ...deps, agentRunner: runner })

    expect(result.ran).toBe(true)
    const resumed = seen[1]
    // The ledger reached the provider, keyed like the bridge keys it.
    expect(resumed.metadata.idempotencyLedger.has(toolLedgerKey('Bash', { command: 'rm -rf build' }))).toBe(true)
    // The recap names what the CLI already did.
    expect(resumed.system).toContain('run_command')
    expect(resumed.system).toContain('rm -rf build')
    // Seeded from the CLI turn's checkpoint: history + the answer text.
    expect(resumed.messages).toEqual([
      { role: 'user', content: 'Finish the report' },
      { role: 'assistant', content: 'Cleaned the build folder.' },
    ])
    // The bridge refused the repeat and let new work through.
    expect(bridgeVerdicts[0]).toMatchObject({ behavior: 'deny', message: expect.stringContaining('duplicate side effect prevented') })
    expect(bridgeVerdicts[1]).toEqual({ behavior: 'allow' })
  })

  it('a CLI run whose recorded tool failed resumes with no ledger entry for it (a failed op stays retryable)', async () => {
    const seen: any[] = []
    const gateway = cliGateway([
      async function* () {
        yield { type: 'tool_use_start', id: 'toolu_1', name: 'run_command', rawName: 'Bash', input: { command: 'make deploy' } }
        yield { type: 'tool_result', toolUseId: 'toolu_1', content: 'exit 2', isError: true, durationMs: 5, outcome: 'error', executedBy: 'provider' }
        yield { type: 'done', response: answer('The deploy failed.') }
      },
      async function* () { yield { type: 'done', response: answer('retried') } },
    ], seen)
    const runner = createAgentRunner({ gateway, toolExecutor: { execute: vi.fn() } as any, eventStore: events, checkpoint: checkpoints.api })

    for await (const _ of runner.run({ messages: [{ role: 'user', content: 'Finish the report' }], tools: [], maxTurns: 5, autonomous: true, sessionId: 'old-run' })) { /* drain */ }
    await resumeRun('old-run', { ...deps, agentRunner: runner })

    expect(seen[1].metadata.idempotencyLedger).toBeUndefined()
  })
})

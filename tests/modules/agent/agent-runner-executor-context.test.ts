...
// H9 — the run's toolset: a model runs only a tool it was offered. The runner
// passes the offered names to the executor (which enforces them on every
// path) and refuses an unoffered name itself, before the gate and any
// approval.
describe('AgentRunner — the run toolset (H9)', () => {
  let toolExecutor: ReturnType<typeof createMockToolExecutor>
  ...
  it('(+) a listed tool executes, with the offered names as ToolContext.allowedTools', async () => {
    const gateway = createMockGateway([
      makeToolUseResponse([{ id: 'tu-h9-1', name: 'search', input: { q: 'x' } }]),
      makeTextResponse('done'),
    ])
    const runner = createAgentRunner({ gateway, toolExecutor, securityGate: allowGate })
    ...
    await drain(runner.run({
      messages: [{ role: 'user', content: 'Search' }],
      tools: [makeToolDef('search'), makeToolDef('memory_search')],
      maxTurns: 5,
      toolContext: baseToolContext(),
    }))
    ...
    expect(toolExecutor.execute).toHaveBeenCalledTimes(1)
    const ctx = toolExecutor.execute.mock.calls[0][2] as any
    expect([...ctx.allowedTools].sort()).toEqual(['memory_search', 'search'])
  })

  it('(−) a hallucinated tool name gets a denied result and is not executed — no gate call, no approval', async () => {
    const gateway = createMockGateway([
      makeToolUseResponse([{ id: 'tu-h9-2', name: 'write_file', input: { path: 'x' } }]),
      makeTextResponse('done'),
    ])
    const onApprovalRequired = vi.fn(async () => true)
    const runner = createAgentRunner({ gateway, toolExecutor, securityGate: allowGate, onApprovalRequired })
    ...
    expect(events.find((e) => e.type === 'tool_result')).toMatchObject({
      ...
      outcome: 'denied',
      isError: true,
      content: "Tool call denied: 'write_file' is not in this agent's toolset",
    })
  })

  it("a caller-supplied toolset wins over the request's tool list", async () => {
    const gateway = createMockGateway([
      makeToolUseResponse([{ id: 'tu-h9-3', name: 'search', input: {} }]),
      makeTextResponse('done'),
    ])
    const runner = createAgentRunner({ gateway, toolExecutor, securityGate: allowGate })
    ...
    for await (const e of runner.run({
      ...
      tools: [makeToolDef('search')],
      maxTurns: 5,
      toolContext: baseToolContext({ allowedTools: new Set(['memory_search']) }),
    })) events.push(e)

    expect(toolExecutor.execute).not.toHaveBeenCalled()
    expect(events.find((e) => e.type === 'tool_result')).toMatchObject({ outcome: 'denied' })
  })
})

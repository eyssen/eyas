...
describe('Delegation', () => {
  ...
  it('delegates successfully and returns result', async () => {
    ...
    expect(createChild).toHaveBeenCalledWith('parent-conv', 'a2', 'Write tests', null)
  ...
  it("hands the delegating turn's binding to the child it creates (H4)", async () => {
    const createChild = vi.fn().mockReturnValue('child-conv-1')
    const service = createDelegationService({
      maxDepth: 3,
      getAncestry: vi.fn().mockReturnValue([{ agentId: 'a1' }]),
      createChildConversation: createChild,
      executeAgent: vi.fn().mockResolvedValue({ text: 'ok', status: 'completed' as const, sessionId: 's' }),
      runTransaction: syncTx,
    })
    const binding = { providerId: 'claude-code', modelId: 'claude-code-sonnet' }
    await service.delegate('parent-conv', 'a2', 'Write tests', { binding })
    expect(createChild).toHaveBeenCalledWith('parent-conv', 'a2', 'Write tests', binding)
  })

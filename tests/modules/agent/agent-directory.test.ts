...
describe('AgentDirectory', () => {
  ...
  it('inventory items use the id as the name the model must copy', () => {
    const dir = createAgentDirectory(mockRegistry as any)
    expect(dir.toInventoryItems()).toEqual([
      { name: 'a1', oneLine: 'Assistant (primary) — Helper' },
      { name: 'a2', oneLine: 'Engineer (primary) — Builder' },
    ])
  })
})

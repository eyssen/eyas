...
describe('agent-runner — event capture (Cap 3 Step 1)', () => {
  ...
  it('stamps each LlmResponse with the pair that answered and the run\'s entry path', async () => {
    const db = createMemoryDb()
    createEventStoreTables(db)
    const events = createEventStore(db)
    const toolExecutor = { execute: vi.fn(async () => ({ success: true, output: {}, durationMs: 1 })) }
    const answered = { ...makeText('final answer'), provider: 'fallback-provider', model: 'fallback-model' }
    const runner = createAgentRunner({ gateway: gatewayOf([makeToolUse('search_memory'), answered]), toolExecutor, eventStore: events } as any)

    await collect(runner.run({
      messages: [{ role: 'user', content: 'go' }], tools: [makeToolDef('search_memory')], maxTurns: 3, sessionId: 'sess-p',
      provider: 'requested-provider', model: 'requested-model', metadata: { origin: 'team' },
    } as any))

    const llm = await events.getByTypes('sess-p', ['LlmResponse'])
    expect(llm.map((e) => e.payload)).toEqual([
      expect.objectContaining({ entryPath: 'team', response: expect.objectContaining({ provider: 'mock', model: 'm' }) }),
      expect.objectContaining({ entryPath: 'team', response: expect.objectContaining({ provider: 'fallback-provider', model: 'fallback-model', content: 'final answer' }) }),
    ])
  })

  it('an explicit entryPath wins over the request origin; no origin and no entryPath records none', async () => {
    const db = createMemoryDb()
    createEventStoreTables(db)
    const events = createEventStore(db)
    const toolExecutor = { execute: vi.fn(async () => ({ success: true, output: {}, durationMs: 1 })) }
    const runner = createAgentRunner({ gateway: gatewayOf([makeText('a'), makeText('b')]), toolExecutor, eventStore: events } as any)

    await collect(runner.run({ messages: [{ role: 'user', content: 'go' }], tools: [], maxTurns: 1, sessionId: 'sess-a', entryPath: 'a2a', metadata: { origin: 'delegation' } } as any))
    await collect(runner.run({ messages: [{ role: 'user', content: 'go' }], tools: [], maxTurns: 1, sessionId: 'sess-n' } as any))

    expect((await events.getByTypes('sess-a', ['LlmResponse']))[0].payload.entryPath).toBe('a2a')
    expect((await events.getByTypes('sess-n', ['LlmResponse']))[0].payload).not.toHaveProperty('entryPath')
  })

  it('never loses the LlmResponse over malformed provenance (it is dropped, the event kept)', async () => {
    const db = createMemoryDb()
    createEventStoreTables(db)
    const events = createEventStore(db)
    const toolExecutor = { execute: vi.fn(async () => ({ success: true, output: {}, durationMs: 1 })) }
    const odd = { ...makeText('kept'), provider: '', model: 'x'.repeat(400) }
    const runner = createAgentRunner({ gateway: gatewayOf([odd]), toolExecutor, eventStore: events } as any)

    await collect(runner.run({ messages: [{ role: 'user', content: 'go' }], tools: [], maxTurns: 1, sessionId: 'sess-o' } as any))

    const llm = await events.getByTypes('sess-o', ['LlmResponse'])
    expect(llm).toHaveLength(1)
    const response = llm[0].payload.response as Record<string, unknown>

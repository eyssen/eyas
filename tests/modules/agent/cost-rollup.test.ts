...
describe('Critic-call exclusion (F2 T7 + T9)', () => {
  ...
  it("the completeness critic's own gateway call never contributes to the run's tokens_used/cost_usd", async () => {
    ...
    const deps: any = {
      ...
      // The critic runs on the background model service — here the real one
      // over the critic's separate gateway mock.
      critic: { enabled: true, aux: createAuxiliaryModelOverGateway(criticGateway, { providers: ['anthropic'] }) },
    }

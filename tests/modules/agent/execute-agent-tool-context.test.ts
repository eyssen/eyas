...
describe('executeAgent — tool context (F0 R2)', () => {
  ...
  describe('D1 — empty persisted tools list falls back to ALL tools', () => {
    ...
    it('agentDef.tools = [\'search_memory\'] → runner receives that tool plus the mandatory memory tools', async () => {
      const ctx = await bootAgentModule()
      const { calls, registry } = stubToolRegistry()
      ctx.tools = { registry }
      ...
      expect(calls).toHaveLength(1)
      expect(calls[0]).toEqual(['search_memory', 'memory_search', 'memory_expand'])
      const { tools } = runCalls[0]
      expect(tools).toEqual([{ name: 'search_memory' }, { name: 'memory_search' }, { name: 'memory_expand' }])
    })
  })

  // I3 — a specialist run gets the same tool scope as every other run path.
  describe('I3 — tool scope', () => {
    const REGISTERED = ['read_file', 'write_file', 'run_command', 'memory_search', 'memory_expand', 'run_specialist', 'assign_task']
    ...
    function createAgent(ctx: any, id: string, tools: string[]) {
      ctx.agents.registry.create({
    ...
    it('a narrow-list agent is offered its allowlist ∪ the mandatory memory tools', async () => {
      const ctx = await bootAgentModule()
      ctx.tools = { registry: realisticRegistry() }
      createAgent(ctx, 'narrow-agent', ['read_file'])

      await ctx.agents.executeAgent('conv-narrow', 'narrow-agent', 'do the thing')

      expect(runCalls[0].tools.map((t: any) => t.name)).toEqual(['read_file', 'memory_search', 'memory_expand'])
    })
    ...
    it('a tool outside the list is not offered (negative)', async () => {
      ...
      const names = runCalls[0].tools.map((t: any) => t.name)
      expect(names).not.toContain('run_command')
      expect(names).not.toContain('write_file')
    })

    it('a Solo conversation strips the delegation tools, not memory or assign_task', async () => {
      const ctx = await bootAgentModule()
      ctx.tools = { registry: realisticRegistry() }
      ctx.conversations = {
        get: () => ({ providerId: null, modelId: null, teamSessionId: null, orchestration: 'solo' }),
        addMessage: () => {},
      }
      createAgent(ctx, 'all-tools-agent', [])

      await ctx.agents.executeAgent('conv-solo', 'all-tools-agent', 'do the thing')

      const names = runCalls[0].tools.map((t: any) => t.name)
      expect(names).not.toContain('run_specialist')
      expect(names).toEqual(expect.arrayContaining(['memory_search', 'memory_expand', 'assign_task', 'run_command']))
    })
  })

...
async function buildCtx(opts: { rootDir: string; withAutonomyPolicy?: boolean; logger?: any; auxiliaryModel?: any }) {
  ...
  const ctx: any = {
    ...
    securityGate,
    scheduler,
    auxiliaryModel: opts.auxiliaryModel,
...
describe('skill-generation onStart — live wiring (Task 3)', () => {
  ...
  it('(g) with the background model wired, the scan reaches live model-authoring', async () => {
    ...
    const { ctx, db, scheduler } = await buildCtx({ rootDir: tmp, auxiliaryModel: aux })
    seedMinedCandidate(db)
    await skillGenerationModule.onRegister(ctx)
    await skillGenerationModule.onStart(ctx)
    ctx.securityGate.features.setEnabled('skill.adopt', true, 'owner')
  ...
  it('(h) with no background model wired, the scan still succeeds via the deterministic renderer', async () => {
    const { ctx, db, scheduler } = await buildCtx({ rootDir: tmp })
    seedMinedCandidate(db)
    await skillGenerationModule.onRegister(ctx)
    await skillGenerationModule.onStart(ctx)

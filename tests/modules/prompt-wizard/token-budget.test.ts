...
describe('token-budget', () => {
  it('default budget includes the 1200-token memory reserve (spec §16-9)', () => {
    const total = totalBudget(DEFAULT_BUDGET_FULL)
    expect(DEFAULT_BUDGET_FULL.memoryContext).toBe(1200)
  ...
  it('every locked section fits the budget it is given — nothing ships pre-truncated', () => {
    ...
    // put the declared sum over the 9000 reserve, which is a budget decision, not

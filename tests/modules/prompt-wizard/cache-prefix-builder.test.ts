...
import type { ToolAddressing } from '../../../src/modules/model/types.js'
import { toolAddressingNote } from '../../../src/modules/model/tool-addressing.js'
...
describe('buildCachePrefix', () => {
  it('renders all 14 mandatory rules, none of them clipped', () => {
    // The real path: the same collector call the assembler makes, against the
    // shipped budget. Rule 8 used to be cut mid-word here ("an ai-memory or
    // Obsid…") and rules 9-14 never reached a model at all — silently, because
    // a truncated locked section looks exactly like a short one in the prompt.
    const { content, sections } = buildCachePrefix({
    ...
    for (let n = 1; n <= 14; n++) {
      expect(content, `rule ${n} must survive the section budget`).toContain(`\n${n}. `)
    }
    expect(content).toContain('memory_search')
    expect(content).toContain('memory_expand')
    expect(content).toContain('14. LANGUAGE')

    const rules = sections.find((s) => s.key === 'core-rules')!
    expect(rules.content).not.toContain('[truncated — section budget]')
    expect(rules.truncated).toBe(false)
    expect(rules.droppedChars).toBe(0)
  })
...
describe('buildCachePrefix manifest', () => {
  ...
  it('renders available-agents with roster ids and a tool-call footer', () => {
    const { content, sections } = buildCachePrefix({
      ...baseInput,
      agentsList: [
        { name: 'system-engineer', oneLine: 'Engineer (primary) — platform' },
        { name: 'code-reviewer', oneLine: 'Reviewer (specialist) — review' },
      ],
    })
    expect(sections.map((s) => s.key)).toContain('available-agents')
    expect(content).toContain('system-engineer')
    expect(content).toContain('Call run_specialist or handoff_to_colleague')
  })
...
describe('the tool inventory names EYAS tools the way the target host does', () => {
  const tools = [
    { name: 'memory_search', oneLine: 'Search EYAS memory.' },
    { name: 'memory_expand', oneLine: 'Open a memory hit.' },
  ]

  function build(toolAddressing?: ToolAddressing, toolsList = tools) {
    return buildCachePrefix({
      coreIdentity: 'You are EYAS.',
      coreRules: 'Be helpful.',
      personality: 'Be warm.',
  ...
  const toolsSection = (r: ReturnType<typeof build>) => r.sections.find((s) => s.key === 'available-tools')!.content

  it('adds the mcp-prefix note for a Claude Code target', () => {
    const section = toolsSection(build({ kind: 'mcp-prefix', prefix: 'mcp__eyas__' }))
    expect(section).toContain(toolAddressingNote({ kind: 'mcp-prefix', prefix: 'mcp__eyas__' }))
    expect(section).toContain('`mcp__eyas__memory_search`')
    expect(section).toContain('provider native tool API')
  })

  it('adds the use_tool note for a Grok target', () => {
    const section = toolsSection(build({ kind: 'meta-tool', via: 'use_tool', qualify: 'eyas__' }))
    expect(section).toContain('`use_tool` with tool_name `eyas__<name>`')
    expect(section).toContain('`search_tool`')
  })

  it('keeps the note when the inventory falls back to names only', () => {
    const many = Array.from({ length: 56 }, (_, i) => ({
      name: `a_tool_name_${String(i).padStart(3, '0')}`,
      oneLine: 'A description of roughly the length these actually run to in practice, which is not short at all.',
    }))
    const result = build({ kind: 'meta-tool', via: 'use_tool', qualify: 'eyas__' }, many)
    const tools = result.sections.find((s) => s.key === 'available-tools')!
    expect(tools.content).toContain('`use_tool` with tool_name `eyas__<name>`')
    expect(tools.truncated).toBe(false)
  })

  it('adds nothing for a native target', () => {
    const native = toolsSection(build({ kind: 'native' }))
    expect(native).not.toContain('MCP server')
    expect(native).not.toContain('use_tool')
    expect(native).not.toContain('mcp__eyas__')
  })

  it('adds nothing when no target addressing is known', () => {
    expect(toolsSection(build(undefined))).toBe(toolsSection(build({ kind: 'native' })))
  })

  it('puts no note anywhere when there is no tool inventory', () => {
    const { content } = build({ kind: 'meta-tool', via: 'use_tool', qualify: 'eyas__' }, [])
    expect(content).not.toContain('use_tool')
  })

  it('still reproduces the prompt byte-for-byte from its sections', () => {
    const { content, sections } = build({ kind: 'mcp-server', server: 'eyas' })

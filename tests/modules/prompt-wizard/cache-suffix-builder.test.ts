...
describe('buildCacheSuffix', () => {
  ...
  it('carries no clock: date and time travel in the per-message turn block', () => {
    const { content } = buildCacheSuffix({
      team: null,
      memory: null,
      runtime: runtime(),
      activeVoice: { scope: 'internal', reason: 'default', profile: profile() },
    ...
    expect(content).not.toMatch(/Current date|Current time/)
    expect(content).toContain('- Channel: owner_dm')
  })

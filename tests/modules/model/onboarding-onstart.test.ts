...
//
// The claude-code submodule is mocked ONLY to remove the dependency on a real
// Claude Code runtime being installed and signed in (isClaudeRuntimeUsable
// would otherwise make this test's outcome depend on the machine it runs on).
// The mock still respects the same `enabled` gate the real submodule checks,
// so the ordering assertion is meaningful: if the fresh-flip block didn't run
...
// Availability of the mocked Claude Code runtime (resolves AND signed in).
const claude = vi.hoisted(() => ({ usable: true, checks: 0 }))

vi.mock('@modules/model/submodules/claude-code/manifest.js', () => ({
  isClaudeRuntimeUsable: async () => {
    claude.checks++
    return claude.usable
  },
  claudeCodeManifest: {
...
describe('modelModule.onStart — Claude Code auto-enable needs a usable runtime', () => {
  it('leaves claude-code disabled on a fresh install when the runtime is not usable (e.g. not signed in)', async () => {
    claude.usable = false
    const { modelModule } = await import('@modules/model/index')
    const ctx = buildCtx()
    await modelModule.onRegister(ctx)
    ...
    expect(ctx.providerConfig.getDefault()).toBeNull()
  })

  it('does not re-check availability once the provider row exists', async () => {
    const { modelModule } = await import('@modules/model/index')
    const ctx = buildCtx()
    await modelModule.onRegister(ctx)
    await modelModule.onStart(ctx)

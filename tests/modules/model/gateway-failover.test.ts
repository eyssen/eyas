...
// An isolated request (a background one-shot) may only hop to a provider that
// honours isolation: an API provider, or a CLI advertising
// supportsIsolatedCompletion. A CLI without it would run the call with its own
// memory, config and tools loaded.
describe('ModelGateway failover — isolated requests', () => {
  function cli(id: string, script: Array<Error | 'ok'>, isolating: boolean) {
    const provider = scriptedProvider(id, script)
    return isolating ? Object.assign(provider, { supportsIsolatedCompletion: true }) : provider
  }

  it('does not hop an isolated request to a CLI that cannot isolate', async () => {
    const p1 = scriptedProvider('p1', [rateLimit(), rateLimit()])
    const grok = cli('grok-cli', ['ok'], false)
    const gateway = createModelGateway(undefined, {
      sleep: async () => {},
      getTierFallback: () => ({ providerId: 'grok-cli', modelId: 'grok-cli-default' }),
    })
    gateway.registerProvider(p1)
    gateway.registerProvider(grok)

    await expect(gateway.complete(req({ isolated: true, metadata: { tier: 'heartbeat' } }))).rejects.toThrow('rate limit')
    expect(grok.calls).toBe(0)
  })

  it('does not hop an isolated stream to a CLI that cannot isolate', async () => {
    const p1 = scriptedProvider('p1', [rateLimit(), rateLimit()])
    const grok = cli('grok-cli', ['ok'], false)
    const gateway = createModelGateway(undefined, {
      sleep: async () => {},
    ...
    let thrown: unknown
    try {
      for await (const _ of gateway.stream(req({ isolated: true, metadata: { tier: 'heartbeat' } }))) { /* drain */ }
    } catch (err) { thrown = err }
    expect(thrown).toBeInstanceOf(Error)
    expect(grok.calls).toBe(0)
  })

  it('hops an isolated request to an API provider', async () => {
    const p1 = scriptedProvider('p1', [rateLimit(), rateLimit()])
    const api = scriptedProvider('openai', ['ok'])
    const gateway = createModelGateway(undefined, {
      sleep: async () => {},
      getTierFallback: () => ({ providerId: 'openai', modelId: 'gpt-mini' }),
    })
    gateway.registerProvider(p1)
    gateway.registerProvider(api)

    const res = await gateway.complete(req({ isolated: true, metadata: { tier: 'heartbeat' } }))
    expect(res.provider).toBe('openai')
  })

  it('hops an isolated request to a CLI that advertises isolated completion', async () => {
    const p1 = scriptedProvider('p1', [rateLimit(), rateLimit()])
    const claude = cli('claude-code', ['ok'], true)
    const gateway = createModelGateway(undefined, {
      sleep: async () => {},
      getTierFallback: () => ({ providerId: 'claude-code', modelId: 'claude-code-haiku' }),
    })
    gateway.registerProvider(p1)
    gateway.registerProvider(claude)

    const res = await gateway.complete(req({ isolated: true, metadata: { tier: 'heartbeat' } }))
    expect(res.provider).toBe('claude-code')
  })

  it('still hops a non-isolated request to a CLI that cannot isolate, as before', async () => {
    const p1 = scriptedProvider('p1', [rateLimit(), rateLimit()])
    const grok = cli('grok-cli', ['ok'], false)
    const gateway = createModelGateway(undefined, {
      sleep: async () => {},
...
describe('ModelGateway failover — unpinned requests', () => {
  it('retries an unpinned request on the same default provider', async () => {
    const p1 = scriptedProvider('p1', ['ok'])
    const p2 = scriptedProvider('p2', [rateLimit(), 'ok'])
    const gateway = createModelGateway(undefined, {
      sleep: async () => {},
      // A model id no provider lists: the retry must not fall into model lookup.
      getDefault: () => ({ providerId: 'p2', modelId: 'unlisted-model' }),
    })
    gateway.registerProvider(p1)
    gateway.registerProvider(p2)

    const res = await gateway.complete({ messages: [{ role: 'user', content: 'hi' }] })
    expect(res.provider).toBe('p2')
...
describe('ModelGateway failover — privacy per destination (D4)', () => {
  /** Records what each attempt received; the script decides throw or succeed. */
  function recording(id: string, script: Array<Error | 'ok'>, host?: string) {
    const received: ModelRequest[] = []
    const provider = scriptedProvider(id, script)
    const complete = provider.complete.bind(provider)
    provider.complete = async (r: ModelRequest) => { received.push(r); return complete(r) }
    if (host) provider.egressHost = () => host
    return { provider, received }
  }

  it('a local primary gets the raw request; the remote tier fallback gets it masked', async () => {
    ...
    try {
      ...
      const local = recording('ollama', [rateLimit(), rateLimit()], 'localhost')
      const cloud = recording('openai', ['ok'], 'api.openai.com')
      const gateway = createModelGateway(undefined, {
        sleep: async () => {},
        egress,
        getTierFallback: () => ({ providerId: 'openai', modelId: 'gpt' }),
      })
      gateway.registerProvider(local.provider)
      gateway.registerProvider(cloud.provider)

      const content = 'contact alice@example.com on 2026-09-08'
      ...
      expect(res.provider).toBe('openai')
      expect(local.received.map((r) => r.messages[0].content)).toEqual([content, content])
      expect(cloud.received[0].messages[0].content).toBe('contact [EMAIL] on 2026-09-08')
    } finally {
      fx.cleanup()
    }
  })

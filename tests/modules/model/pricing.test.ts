...
describe('estimateCost', () => {
  ...
  it('bills cache tokens at the input rate for a model with no configured cache rates (never $0)', () => {
    ...
    // gpt-4o input is $2.5 per 1M: both cache counts at that rate.
    expect(cost - withoutCache).toBeCloseTo(2.5 + 2.5, 6)
  })

  describe('canonical input semantics (inputTokens = uncached)', () => {
    it('prices every prompt token once: uncached at input, cached at the cache-read rate', () => {
      // kimi-k3: $3 input, $0.3 cache read, $15 output. 1000 prompt tokens of
      // which 800 were cached → 200 uncached + 800 cache reads.
      const cost = estimateCost('kimi', 'kimi-k3', { inputTokens: 200, outputTokens: 100, cacheReadTokens: 800 })
      expect(cost).toBeCloseTo((200 * 3 + 800 * 0.3 + 100 * 15) / 1_000_000, 12)
    })

    it('the pre-canonical shape (cached share also inside inputTokens) costs more — the double count this removes (negative)', () => {
      const canonical = estimateCost('kimi', 'kimi-k3', { inputTokens: 200, outputTokens: 100, cacheReadTokens: 800 })
      const doubleCounted = estimateCost('kimi', 'kimi-k3', { inputTokens: 1000, outputTokens: 100, cacheReadTokens: 800 })
      expect(doubleCounted - canonical).toBeCloseTo((800 * 3) / 1_000_000, 12)
    })

    it('without a cache-read rate the split prompt costs what the whole prompt at the input rate costs', () => {
      const split = estimateCost('openai', 'gpt-4o', { inputTokens: 200, outputTokens: 0, cacheReadTokens: 800 })
      const whole = estimateCost('openai', 'gpt-4o', { inputTokens: 1000, outputTokens: 0 })
      expect(split).toBeCloseTo(whole, 12)
    })

    it('claude-code aliases price cache tokens like their underlying model when the SDK sent no cost', () => {
      const cc = estimateCost('claude-code', 'claude-code-sonnet', { inputTokens: 10, outputTokens: 10, cacheReadTokens: 1_000_000, cacheCreationTokens: 1_000_000 })
      const api = estimateCost('anthropic', 'claude-sonnet-4-6', { inputTokens: 10, outputTokens: 10, cacheReadTokens: 1_000_000, cacheCreationTokens: 1_000_000 })
      expect(cc).toBeCloseTo(api, 12)
    })

    it('local providers stay $0 with cache tokens too (negative)', () => {
      expect(estimateCost('ollama', 'llama3', { inputTokens: 10, outputTokens: 10, cacheReadTokens: 1_000_000 })).toBe(0)
    })
  })
  ...
  describe('corrected Anthropic rates (fix round 1 Critical 1)', () => {
    ...
    it('grok-cli uses the published $2/$6 card, not the unrecognized-model fallback', () => {
      const cost = estimateCost('grok-cli', 'grok-cli-default', {
        inputTokens: 164_240, outputTokens: 2_015,
      })
      expect(cost).toBeCloseTo((164_240 * 2 + 2_015 * 6) / 1_000_000, 6)

...
describe('OpenAIAdapter (v2)', () => {
  it('concatenates prefix + suffix into one system message; reminders as separate', async () => {
    ...
    await adapter.send({
      systemPrompt: { prefix: 'PREFIX', suffix: 'SUFFIX', reminders: ['REM1', 'REM2'], cacheBoundaryHint: 0, prefixHash: 'x', tokenEstimate: { prefix: 1, suffix: 1, reminders: 1 }, sections: [] },
  ...
  it('omits concatenated system message when prefix and suffix are empty', async () => {
    ...
    const adapter = new OpenAIAdapter({ chat: { completions: { create } } } as never)
    await adapter.send({
      systemPrompt: { prefix: '', suffix: '   ', reminders: ['ONLY'], cacheBoundaryHint: 0, prefixHash: 'x', tokenEstimate: { prefix: 0, suffix: 0, reminders: 1 }, sections: [] },
      messages: [{ role: 'user', content: 'hi' }],
      modelId: 'gpt-4o',
    })
    const arg = create.mock.calls[0][0]

...
describe('GET /api/v1/self-learning/insights model-pass gating on selfLearning.apply', () => {
  ...
  function buildApp(flagOn: boolean, aux: ReturnType<typeof createFakeAuxiliaryModel>) {
    const db = createMemoryDb()
    createTables(db)
    seedFlakyTool(db)
  ...
  it('flag OFF: returns the generic-sentence insight with no model call', async () => {
    const aux = createFakeAuxiliaryModel(auxOk('patch'))
    const res = await buildApp(false, aux).request('/api/v1/self-learning/insights')
    expect(res.status).toBe(200)
  ...
  it('flag ON: authors a concrete edit via a model call', async () => {
    const aux = createFakeAuxiliaryModel(auxOk('concrete patch'))
    const res = await buildApp(true, aux).request('/api/v1/self-learning/insights')
    expect(res.status).toBe(200)
    expect(aux.calls.length).toBeGreaterThan(0)
    const body = await res.json() as any

...
describe('Shell tool — security hardening (S3)', () => {
  ...
  it('runs a legitimate program with args and captures stdout', async () => {
    const r = await runCmd.execute(
      { command: 'echo', args: ['hello', 'world'] },
      { ...silentCtx(), workingDirectory: tmpdir() },
    )
...
describe('Tool executor — memory outside EYAS (B2)', () => {
  // The executor's gate is the choke point for EYAS tools called outside the
  // agent runner (the CLI-MCP bridge, pipelines). A conversation whose folder
  // is the EYAS install root still cannot read or write the data folder
  // through read_file: the jail would let it in, the memory-path policy not.
  ...
  async function executorWithRealGate() {
    fx.stubInstanceEnv()
    const registry = createToolRegistry()
    ...
    const gateCtx = { db: createMemoryDb(), model, permissions: createPermissionRegistry(), logger: silentCtx().logger, config: {} } as any
    await securityGateModule.onRegister!(gateCtx)
    const exec = createToolExecutor(registry, {
      authorization: { getSecurityGate: () => gateCtx.securityGate, getAbilityForRole: () => ({ can: () => true }) },
    })
    return { exec, complete: model.complete as ReturnType<typeof vi.fn> }
  }

  function repoCtx(): ToolContext {
    return {
      ...silentCtx(),
  ...
  it('(+) read_file of <dataDir>/vault is denied with the policy reason, the judge never asked', async () => {
    const { exec, complete } = await executorWithRealGate()
    const r = await exec.execute('read_file', { path: 'data/vault/semantic/fact.md' }, repoCtx())
    expect(r.success).toBe(false)
    expect(r.errorCode).toBe('DENIED')
    expect(r.error).toContain('EYAS data directory (vault) is read and written only by EYAS')
    expect(complete).not.toHaveBeenCalled()
  })

  it('(+) write_file into the EYAS vault is denied the same way (no judge for a yellow write)', async () => {
    const { exec, complete } = await executorWithRealGate()
    const r = await exec.execute('write_file', { path: join(fx.dataDir, 'vault', 'semantic', 'new.md'), content: 'kind: feedback' }, repoCtx())
    expect(r.success).toBe(false)
    expect(r.error).toContain('EYAS data directory (vault)')
    expect(complete).not.toHaveBeenCalled()
  })

  it('(+) read_file of another conversation\'s workspace is denied from the context\'s folders', async () => {
    const { exec } = await executorWithRealGate()
    const ctx: ToolContext = { ...silentCtx(), workingDirectories: [fx.ownWorkspace], actor: { kind: 'agent', role: 'agent' } }
    const r = await exec.execute('read_file', { path: fx.otherFile }, ctx)
    expect(r.success).toBe(false)
    expect(r.error).toMatch(/Not this conversation's workspace/)
  })

  it('(−) read_file inside the workspace is allowed and returns the file', async () => {
    const { exec } = await executorWithRealGate()
    const r = await exec.execute('read_file', { path: 'src/a.ts' }, repoCtx())
    expect(r.success).toBe(true)
    expect(String((r.output as { content?: string }).content)).toContain('export {}')
  })
})

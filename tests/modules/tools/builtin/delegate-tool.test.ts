...
describe('createDelegateTool', () => {
  ...
  it("hands the delegating turn's model binding to the delegation (H4)", async () => {
    const delegate = vi.fn().mockResolvedValue({ conversationId: 'child-1', result: 'ok' })
    const [run] = createDelegateTool({ delegate } as any, registry([specialist]))
    const logger = { info() {}, warn() {}, error() {}, debug() {} } as any
    const binding = { providerId: 'claude-code', modelId: 'claude-code-sonnet' }
    await run!.execute({ agentId: 'code-reviewer', task: 'review' }, { conversationId: 'c1', userId: 'u1', logger, modelBinding: binding })
    expect(delegate).toHaveBeenLastCalledWith('c1', 'code-reviewer', 'review', { binding })
    // (−) No binding on the turn: none is invented.
    await run!.execute({ agentId: 'code-reviewer', task: 'review' }, { conversationId: 'c1', userId: 'u1', logger })
    expect(delegate).toHaveBeenLastCalledWith('c1', 'code-reviewer', 'review', { binding: null })
  })

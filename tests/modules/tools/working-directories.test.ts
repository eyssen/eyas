...
// A conversation's workspace is a folder only for that conversation and for
// the conversations of its human owner (mayUseWorkspace); as written, below
// it, and through a symlink. Without the option (a project's folders, a CLI
// cwd check) nothing changes.
describe('validateWorkingDirectories — another conversation\'s workspace (mayUseWorkspace)', () => {
  let f: SovereigntyFixture
  /** conv-1 and conv-1b are u1's; conv-2 is u2's (conv-1 and conv-2 are the fixture's workspaces). */
  const owners: Record<string, string> = { 'conv-1': 'u1', 'conv-1b': 'u1', 'conv-2': 'u2' }
  const owns = (id: string, userId: string) => owners[id] === userId
  ...
  const codeFor = (path: string, mayUseWorkspace?: (id: string) => boolean) => {
    const r = validateWorkingDirectories([path], { policy: f.policy, homeDir: f.home, ...(mayUseWorkspace ? { mayUseWorkspace } : {}) })
    return r.ok ? 'ok' : r.code
  }

  it('(−) u2\'s conversation may not name u1\'s workspace, a folder in it, a symlink to it or a run\'s scratch folder', () => {
    const forConv2 = conversationWorkspaceAccess('conv-2', 'u2', owns)
    mkdirSync(join(f.ownWorkspace, 'src'), { recursive: true })
    mkdirSync(join(f.root, 'links'), { recursive: true })
    const link = join(f.root, 'links', 'conv-1')
    symlinkSync(f.ownWorkspace, link)
    const scratch = join(f.workspacesRoot, RUN_SCRATCH_DIR, 'run-7')
    mkdirSync(scratch, { recursive: true })
    const stray = join(f.workspacesRoot, 'not-a-conversation')
    mkdirSync(stray, { recursive: true })
    for (const path of [f.ownWorkspace, join(f.ownWorkspace, 'src'), link, scratch, stray]) {
      expect(codeFor(path, forConv2), path).toBe('otherWorkspace')
    }
    // A new conversation (no id yet) of u2: the same.
    expect(codeFor(f.ownWorkspace, conversationWorkspaceAccess(null, 'u2', owns))).toBe('otherWorkspace')
    // Unknown owner: only the conversation's own workspace.
    expect(codeFor(f.ownWorkspace, conversationWorkspaceAccess('conv-2', null, owns))).toBe('otherWorkspace')
    expect(codeFor(f.otherWorkspace, conversationWorkspaceAccess('conv-2', null, owns))).toBe('ok')
  })

  it('(+) its own workspace, and the workspace of another conversation of the same owner, are accepted', () => {
    expect(codeFor(f.otherWorkspace, conversationWorkspaceAccess('conv-2', 'u2', owns))).toBe('ok')
    expect(codeFor(f.ownWorkspace, conversationWorkspaceAccess('conv-1b', 'u1', owns))).toBe('ok')
    expect(codeFor(f.ownWorkspace, conversationWorkspaceAccess(null, 'u1', owns))).toBe('ok')
  })

  it('(+) without mayUseWorkspace (a project\'s folders, a CLI cwd check) a single workspace is still accepted', () => {
    expect(codeFor(f.ownWorkspace)).toBe('ok')
  })

  it('checkWorkingDirectoriesBody passes the rule on and answers 400 {code: otherWorkspace, path}', () => {
    const check = checkWorkingDirectoriesBody([f.ownWorkspace], { policy: f.policy, homeDir: f.home, mayUseWorkspace: conversationWorkspaceAccess('conv-2', 'u2', owns) })
    expect(check.ok).toBe(false)
    if (!check.ok) expect(check.body).toMatchObject({ code: 'otherWorkspace', path: f.ownWorkspace })
  })

  it('screenStoredWorkingDirectories leaves a stored folder in another user\'s workspace out, and keeps the rest', () => {
    const project = join(f.root, 'projects', 'app')
    mkdirSync(project, { recursive: true })
    const screened = screenStoredWorkingDirectories([f.ownWorkspace, project], { policy: f.policy, homeDir: f.home, mayUseWorkspace: conversationWorkspaceAccess('conv-2', 'u2', owns) })
    expect(screened.entries.map((e) => e.path)).toEqual([project])
    expect(screened.refused).toEqual([{ path: f.ownWorkspace, code: 'otherWorkspace' }])
  })

  it('foreignWorkspaceOf names the workspace a path reaches, or null', () => {
    const forConv2 = conversationWorkspaceAccess('conv-2', 'u2', owns)
    expect(foreignWorkspaceOf(join(f.ownWorkspace, 'deep', 'file.md'), forConv2, f.policy)).toEqual({ id: 'conv-1', scratch: false })
    expect(foreignWorkspaceOf(join(f.otherWorkspace, 'x'), forConv2, f.policy)).toBeNull()
    expect(foreignWorkspaceOf(join(f.root, 'projects'), forConv2, f.policy)).toBeNull()
    expect(foreignWorkspaceOf(join(f.workspacesRoot, RUN_SCRATCH_DIR, 'r1'), () => true, f.policy)).toEqual({ id: 'r1', scratch: true })
  })
})

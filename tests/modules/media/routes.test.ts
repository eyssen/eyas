...
describe('Media routes', () => {
  ...
  describe('POST /api/v1/media/providers/:id/connect', () => {
    ...
    it('starts OAuth and returns { url } for heygen', async () => {
      gw.registerProvider(createFakeMediaProvider({ id: 'heygen', name: 'HeyGen', configured: false }))
      const res = await app.request('/api/v1/media/providers/heygen/connect', {
        method: 'POST',
        headers: auth(),
      })
      expect(res.status).toBe(200)
      expect(await res.json()).toEqual({ url: 'https://auth.example/authorize?server=sid-heygen' })
      expect(startOAuthCalls).toEqual(['sid-heygen'])
    })
  ...
  describe('PUT /api/v1/media/settings expert flag', () => {
    it('reconnects media-owned MCP servers when expertRawMcpTools flips', async () => {
      ...
      expect(res.status).toBe(200)
      expect(mcpConnectCalls.sort()).toEqual(['sid-fal', 'sid-heygen', 'sid-magnific'])
    })
  })
})

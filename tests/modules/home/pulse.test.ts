...
describe('GET /api/v1/home/pulse — user scoping (Ruling 7 / fix round 1)', () => {
  ...
  it("a non-admin whose runs finished earlier today sees non-zero costTodayUsd — real agent/daily-stats wiring, not a mock", async () => {
    ...
    // "Today" is the UTC day (daily-stats.ts). A fixed "now - 2h" lands on the
    // previous UTC day during the first two hours after UTC midnight, so take
    // the midpoint between UTC midnight and now: always earlier, always today.
    const now = Date.now()
    const utcMidnight = Date.UTC(new Date(now).getUTCFullYear(), new Date(now).getUTCMonth(), new Date(now).getUTCDate())
    const finishedEarlierToday = new Date(Math.floor((utcMidnight + now) / 2)).toISOString()

...
beforeEach(() => { projectDir = mkdtempSync(join(tmpdir(), 'eyas-tag-proj-')) })
afterEach(() => {
  testDb.cleanup()
...
describe('PUT /api/v1/conversations/:id/tags', () => {
  it('sets tags on a conversation and returns them', async () => {
    ...
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Tag Test Project', typeId: projectType.id, workingDirectories: [projectDir] }),
    })
  ...
  it('returns 400 without tagIds array', async () => {
    ...
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Validation Project', typeId: projectType.id, workingDirectories: [projectDir] }),
    })
    const { project } = await projRes.json() as any

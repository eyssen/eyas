...
describe('PUT /api/v1/conversations/:id/tags', () => {
  it('sets tags on a conversation and returns them', async () => {
    ...
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Tag Test Project', typeId: projectType.id, workingDirectories: [tmpdir()] }),
    })
  ...
  it('returns 400 without tagIds array', async () => {
    ...
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Validation Project', typeId: projectType.id, workingDirectories: [tmpdir()] }),
    })
    const { project } = await projRes.json() as any

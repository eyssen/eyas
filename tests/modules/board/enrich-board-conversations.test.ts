...
describe('enrichBoardConversations', () => {
  ...
  it('attaches the latest composition size, not the cumulative tokensUsed', () => {
    ...
    db.run(sql`INSERT INTO context_compositions
      ...
      VALUES ('old', '2026-08-01T00:00:00.000Z', ${parent.id}, 'conversation', 'grok-cli', 'grok-cli-default', 500000, 999999)`)
    db.run(sql`INSERT INTO context_compositions
      (id, created_at, conversation_id, entry_point, provider, model, context_window, estimated_tokens)
      VALUES ('fresh', '2026-08-28T00:00:00.000Z', ${parent.id}, 'conversation', 'grok-cli', 'grok-cli-default', 500000, 50000)`)
    ...
    expect(enriched[0].measured).toBe(false)
  ...
  it('uses a CLI model\'s own catalog window, not the provider default (G11)', () => {
    const bus = createLocalBus()
    const svc = createConversationService(db, bus)
    const parent = svc.create({ userId: 'u1', title: 'Catalog row' })
    svc.update(parent.id, {
      stageId: 'stg-open',
      projectId: 'general-general',
      providerId: 'grok-cli',
    ...
    const refreshed = svc.listByProject('general-general', 'stg-open', 'u1')
    const enriched = enrichBoardConversations(db, refreshed)
    expect(enriched[0].contextWindow).toBe(256_000)
  })

  it('shows the measured size of the last model call when the provider reported one', () => {
    const bus = createLocalBus()
    const svc = createConversationService(db, bus)
    const parent = svc.create({ userId: 'u1', title: 'Measured' })
    svc.update(parent.id, { stageId: 'stg-open', projectId: 'general-general', providerId: 'anthropic', modelId: 'claude-x' })
    createContextTables(db as any)
    db.run(sql`INSERT INTO context_compositions
      (id, created_at, conversation_id, entry_point, provider, model, context_window, estimated_tokens,
       history_estimated_tokens, observed_prompt_tokens)
      VALUES ('m1', '2026-09-01T00:00:00.000Z', ${parent.id}, 'conversation', 'anthropic', 'claude-x', 200000, 3000, 5000, 9100)`)

    const enriched = enrichBoardConversations(db, svc.listByProject('general-general', 'stg-open', 'u1'))
    expect(enriched[0]).toMatchObject({ estimatedTokens: 9_100, contextWindow: 200_000, measured: true })
  })

...
beforeEach(async () => {
  ...
  createBoardRoutes(app, {
    ...
    // Only p/m is an enabled model of an active provider.
    getBindingResolver: () => ({ isSelectable: (pair) => pair.providerId === 'p' && pair.modelId === 'm' }),
  }, conversationService)
...
// A folder of its own for the projects (K2: the whole system temp dir can
// hold other tests' vaults and data dirs, which a folder may not contain).
let projectDir: string
beforeEach(() => { projectDir = mkdtempSync(join(tmpdir(), 'eyas-board-proj-')) })
afterEach(() => {
  testDb.cleanup()
...
describe('POST /api/v1/projects with typeId', () => {
  it('creates project and auto-generates stages from type', async () => {
    ...
    const res = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'My Project', typeId: projectType.id, workingDirectories: [projectDir] }),
    })
    expect(res.status).toBe(201)
    const body = await res.json() as any
  ...
  it('creates two projects under the same type (fictive names)', async () => {
    ...
    const alpha = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'alpha', typeId: projectType.id, workingDirectories: [projectDir] }),
    })
    const bravo = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'bravo', typeId: projectType.id, workingDirectories: [projectDir] }),
    })
  ...
  // A2: a conversation without folders gets its own EYAS workspace, so a
  // project does not need working directories.
  it('(+) creates a project with no working directories, and clears them with an empty list', async () => {
    const res = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'no-folders' }),
    })
    expect(res.status).toBe(201)
    const { project } = await res.json() as any
    expect(project.workingDirectories ?? []).toEqual([])

    const empty = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'empty-folders', workingDirectories: [] }),
    })
    expect(empty.status).toBe(201)

    const withDir = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'with-folder', workingDirectories: [projectDir] }),
    })
    const created = (await withDir.json() as any).project
    const cleared = await app.request(`/api/v1/projects/${created.id}`, {
      method: 'PATCH',
      headers: auth(),
      body: JSON.stringify({ workingDirectories: [] }),
    })
    expect(cleared.status).toBe(200)
    expect((await cleared.json() as any).project.workingDirectories ?? []).toEqual([])
  })

  it('(−) still refuses an invalid folder (relative path) on create and patch', async () => {
    const res = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'bad', workingDirectories: ['relative/path'] }),
    })
    expect(res.status).toBe(400)
    const ok = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'good' }),
    })
    const { project } = await ok.json() as any
    const patched = await app.request(`/api/v1/projects/${project.id}`, {
      method: 'PATCH',
      headers: auth(),
      body: JSON.stringify({ workingDirectories: ['relative/path'] }),
    })
...
describe('GET /api/v1/projects/:id', () => {
  it('returns project with stages', async () => {
    ...
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Sprint 1', typeId: projectType.id, workingDirectories: [projectDir] }),
    })
...
describe('GET /api/v1/projects/:id/board', () => {
  it('returns project with global stages and conversations', async () => {
    ...
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Dev Project', workingDirectories: [projectDir] }),
    })
    const { project } = await projRes.json() as any
  ...
  it('a colleague\'s card follows its colleague\'s model; a card without one, or with a named model, is fixed (D3)', async () => {
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Binding Project', workingDirectories: [projectDir] }),
    })
    const { project } = await projRes.json() as any
    const card = async (body: Record<string, unknown>) => {
      const res = await app.request(`/api/v1/projects/${project.id}/conversations`, {
        method: 'POST', headers: auth(), body: JSON.stringify({ title: 'c', ...body }),
      })
      return (await res.json() as any).conversation
    }
    expect((await card({ agentId: 'agent-1' })).modelBinding).toBe('inherit')
    expect((await card({})).modelBinding).toBe('pinned')
    expect(await card({ agentId: 'agent-1', providerId: 'p', modelId: 'm' })).toMatchObject({ modelBinding: 'pinned', providerId: 'p', modelId: 'm' })
  })

  it('a pair that is not an enabled model of an active provider is not stored (H4)', async () => {
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Binding Project 2', workingDirectories: [projectDir] }),
    })
    const { project } = await projRes.json() as any
    const card = async (body: Record<string, unknown>) => {
      const res = await app.request(`/api/v1/projects/${project.id}/conversations`, {
    ...
    // A colleague's card with an unknown pair follows its colleague instead.
    expect(await card({ agentId: 'agent-1', providerId: 'ghost', modelId: 'm' })).toMatchObject({ modelBinding: 'inherit', providerId: null, modelId: null })
    // An agentless card fixes the install default on its first run.
    expect(await card({ providerId: 'p', modelId: 'disabled' })).toMatchObject({ modelBinding: 'pinned', providerId: null, modelId: null })
    // Half a pair is no pair.
    expect(await card({ providerId: 'p' })).toMatchObject({ providerId: null, modelId: null })
  })
})
...
describe('PATCH /api/v1/conversations/:id/move', () => {
  it('moves conversation to a different stage', async () => {
    ...
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Move Test Project', workingDirectories: [projectDir] }),
    })
    const { project } = await projRes.json() as any
...
describe('stage autoAssigneeId over HTTP', () => {
  ...
  it('POST /api/v1/projects/:id/stages persists autoAssigneeId', async () => {
    const projRes = await app.request('/api/v1/projects', {
      method: 'POST',
      headers: auth(),
      body: JSON.stringify({ name: 'Auto Project', workingDirectories: [projectDir] }),
    })
    const { project } = await projRes.json() as any

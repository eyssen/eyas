...
describe('ProjectTypeService', () => {
  ...
  describe('seed protection', () => {
    ...
    it('prevents renaming a seed project type', () => {
      const now = new Date().toISOString()
      db.run(sql`INSERT INTO project_types (id, name, prompt, default_stages, default_priority, source, created_at)
    ...
    it('allows configuring a seed project type (prompt/sources/directories) without throwing', () => {
      const now = new Date().toISOString()
      db.run(sql`INSERT INTO project_types (id, name, prompt, default_stages, default_priority, source, created_at)
        VALUES ('test-seed-cfg', 'Seed Type', '', '["Done"]', 'normal', 'seed', ${now})`)
      expect(() => svc.update('test-seed-cfg', {
        prompt: 'Instance domain brief',
        indexedSources: ['src-a'],
        workingDirectories: ['/tmp/type-a'],
        defaultAgentId: 'a1',
      })).not.toThrow()
      const updated = svc.get('test-seed-cfg')!
      expect(updated.prompt).toBe('Instance domain brief')
      expect(updated.indexedSources).toEqual(['src-a'])
      expect(updated.workingDirectories).toEqual(['/tmp/type-a'])
      expect(updated.defaultAgentId).toBe('a1')
      expect(updated.name).toBe('Seed Type')
    })

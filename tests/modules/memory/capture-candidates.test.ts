...
describe('parseBatch salvage via the capture flow', () => {
  ...
  let answerOnce: (text: string) => void
  ...
  beforeEach(() => {
    ...
    fakeComplete = vi.fn()
    answerOnce = (text: string) => fakeComplete.mockResolvedValueOnce(reply(text))
  ...
  it('writes both siblings — the project note downgrades to reference instead of sinking', async () => {
    ...
    await capture({ conversationId: 'salvage-1', projectId: null, userMessage: LONG, assistantMessage: 'reply', author: 'owner', entryPath: 'interactive' })
  ...
  it('writes a project-kind note as reference when the conversation has no project', async () => {
    ...
    await capture({ conversationId: 'salvage-2', projectId: null, userMessage: LONG, assistantMessage: 'reply', author: 'owner', entryPath: 'interactive' })
  ...
  it('records notes_written 0, skipped_reason rejected-shape when the only note is genuinely schema-invalid', async () => {
    ...
    await capture({ conversationId: 'salvage-4', projectId: null, userMessage: LONG, assistantMessage: 'reply', author: 'owner', entryPath: 'interactive' })
  ...
  it('still records the healthy NULL-reason empty row for a clean {"notes":[]}', async () => {
    ...
    await capture({ conversationId: 'salvage-3', projectId: null, userMessage: LONG, assistantMessage: 'reply', author: 'owner', entryPath: 'interactive' })

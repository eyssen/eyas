...
describe('ModelBridge — the named embedding-tier provider', () => {
  it('canEmbed is false when the named provider cannot embed, even if another can', () => {
    const gateway = createMockGateway({ hasEmbed: true })
    expect(createModelBridge(gateway, { provider: 'ollama' }).canEmbed()).toBe(true)
    expect(createModelBridge(gateway, { provider: 'missing' }).canEmbed()).toBe(false)
  })

  it('modelId names the provider and model, and never collides with a local embedder id', () => {
    const gateway = createMockGateway()
    expect(createModelBridge(gateway, { provider: 'openai', model: 'text-embedding-3-small' }).modelId?.())
      .toBe('gateway:openai/text-embedding-3-small')
    expect(createModelBridge(gateway).modelId?.()).toBe('gateway:any/default')
    expect(createModelBridge(gateway, { provider: 'openai', model: 'a' }).modelId?.())
      .not.toBe(createModelBridge(gateway, { provider: 'openai', model: 'b' }).modelId?.())
  })
})

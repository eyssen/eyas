...
describe('EpisodicMemoryService', () => {
  ...
  describe('embed', () => {
    /** `onCreated` is what schedules the embedding, so it stands in for one here. */
    const withHook = () => {
      const created: string[] = []
      const svc = createEpisodicMemoryService(db, { onCreated: (m) => created.push(m.id) })
      return { svc, created }
    }

    it('fires onCreated by default', () => {
      const { svc, created } = withHook()
      const mem = svc.create({ content: 'ordinary row', sourceType: 'conversation' })
      expect(created).toEqual([mem.id])
    })

    it('stores the row without firing onCreated when embed is false', () => {
      // The importer's path: no model call per row, whatever the size of the
      // import. The row is still there, and a later pass picks it up.
      const { svc, created } = withHook()
      const mem = svc.create({ content: 'imported transcript', sourceType: 'system', embed: false })
      expect(created).toEqual([])
      expect(svc.get(mem.id)!.content).toBe('imported transcript')
    })
  })
})

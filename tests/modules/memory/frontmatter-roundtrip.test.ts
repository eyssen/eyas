...
describe('vault frontmatter round-trip', () => {
  ...
  it('keeps the enriching model across a write, a read and a rewrite', () => {
    const enriched_by = { provider: 'anthropic', model: 'claude-haiku-4-5', route: 'tier' }
    vault.write('semantic/enriched.md', fm({ enriched_by }), 'Body.')

    const back = vault.read('semantic/enriched.md')
    expect(back?.frontmatter.enriched_by).toEqual(enriched_by)

    vault.write('semantic/enriched.md', { ...back!.frontmatter, updated: '2026-09-06' }, 'Body two.')
    expect(vault.read('semantic/enriched.md')?.frontmatter.enriched_by).toEqual(enriched_by)
  })

  it('reads no enriched_by from a note that has none, or one without a provider', () => {
    expect(parseVaultFile(serializeVaultFile(fm(), 'Body.')).frontmatter.enriched_by).toBeUndefined()
    const noProvider = serializeVaultFile(fm(), 'Body.').replace('title:', 'enriched_by:\n  model: m\ntitle:')
    expect(parseVaultFile(noProvider).frontmatter.enriched_by).toBeUndefined()
    const notAnObject = serializeVaultFile(fm(), 'Body.').replace('title:', 'enriched_by: anthropic\ntitle:')
    expect(parseVaultFile(notAnObject).frontmatter.enriched_by).toBeUndefined()
    // Only string fields survive: a hand edit cannot smuggle structure in.
    const nested = serializeVaultFile(fm(), 'Body.').replace('title:', 'enriched_by:\n  provider: p\n  model:\n    x: 1\ntitle:')
    expect(parseVaultFile(nested).frontmatter.enriched_by).toEqual({ provider: 'p' })
  })

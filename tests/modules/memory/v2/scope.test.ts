...
import { findConversationScope, resolveConversationScope } from '@modules/memory/v2/scope'
...
describe('findConversationScope (fail-closed variant)', () => {
  it('returns the same scope as resolveConversationScope for a conversation that exists', () => {
    const db = createMemoryDb(); fullSchema(db)
    expect(findConversationScope(db, 'c1')).toEqual(resolveConversationScope(db, 'c1'))
    expect(findConversationScope(db, 'c2')?.projectId).toBeNull()
  })

  it('returns null — not "no project" — for an unknown conversation or an unreadable table', () => {
    const db = createMemoryDb(); fullSchema(db)
    expect(findConversationScope(db, 'nope')).toBeNull()
    expect(findConversationScope(createMemoryDb(), 'c1')).toBeNull()
  })
})

...
describe('isStopWord', () => {
  ...
  it("'multi' checks every stop list, and still keeps content words (J6)", () => {
    expect(isStopWord('Hogy', 'multi')).toBe(true)
    expect(isStopWord('the', 'multi')).toBe(true)
    expect(isStopWord('nicht', 'multi')).toBe(true)
    expect(isStopWord('para', 'multi')).toBe(true)
    expect(isStopWord('avec', 'multi')).toBe(true)
    expect(isStopWord('Számla', 'multi')).toBe(false)
    expect(isStopWord('invoice', 'multi')).toBe(false)
    expect(isStopWord('Rechnung', 'multi')).toBe(false)
    expect(isStopWord("qapla'", 'multi')).toBe(false)
  })
})

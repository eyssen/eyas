...
describe('heuristicLeafGist', () => {
  const first = 'Please plan the invoice module rollout for Contoso.'
...
describe('heuristicRollupGist', () => {
  ...
  const children = [
    { text: 'Invoice module rollout planned for Contoso with a staging rehearsal.', importance: 0.9, occurredAtMs: t0 },
  ...
  const scope = { label: 'Project Contoso', taskCount: 3, from: t0, to: t0 + 20 * day }
  it('renders the templated header, themes, key points by importance × recency and the latest line', () => {
    ...
    expect(lines[0]).toBe('Project Contoso — 3 tasks, 2026-01-10 → 2026-01-30.')
    expect(lines[1].startsWith('Themes: ')).toBe(true)
    expect(lines[2]).toBe('Key points:')
    expect(lines[3]).toBe('- Invoice module rollout planned for Contoso with a staging rehearsal.')
  ...
  it('ranks by importance x recency, not importance alone', () => {
    ...
    const lines = heuristicRollupGist(aged, { label: 'Project Contoso', taskCount: 3, from: t0, to }, db).split('\n')
  ...
  it('the closing clip catches a header that alone overruns the cap', () => {
    ...
    const long = { label: 'Contoso '.repeat(300).trim(), taskCount: 3, from: t0, to: t0 + 20 * day }
  ...
  it('stays within 1 200 characters for many long children and handles the empty case', () => {
    ...
    expect(heuristicRollupGist([], { ...scope, taskCount: 1 }, db)).toBe('Project Contoso — 1 task, 2026-01-10 → 2026-01-30.')
  })
})

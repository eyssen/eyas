...
const candidate = (facts: CandidateFact[] = []): ExtractionCandidate => ({
  gist: 'Invoice rollout planned for Contoso.', importance: 0.4, entities: [{ name: 'Contoso Kft', type: 'proper' }], topics: ['invoi', 'contoso kft'],
  facts, language: 'en', gistSource: 'heuristic', heuristicGist: 'Invoice rollout planned for Contoso.',
})
...
describe('arbitrate — task gist', () => {
  it('writes one current task gist with trust = min over the sources, tags, source rows and links', () => {
    ...
    const [g] = gistRows()
    expect(g).toMatchObject({ id: r.gistId, scope_type: 'task', scope_id: 'c1', tree_depth: 0, text: 'Invoice rollout planned for Contoso.',
    ...
    expect(JSON.parse(g.structured_json)).toMatchObject({ topics: ['invoi', 'contoso kft'], language: 'en', entities: ['Contoso Kft'], facts_pending: true })
    ...
    expect(tags).toEqual(expect.arrayContaining(['task=c1', 'layer=gist', 'language=en', 'trust_tier=ingested', 'topic=invoi', 'topic=contoso kft',
      'entity=Contoso Kft', 'source_type=user_message', 'source_type=tool_result']))
  ...
  it('poisoning gate: a high-level model gist is rejected and the heuristic gist is written instead', () => {
    ...
    expect(gistRows()[0]).toMatchObject({ text: 'Invoice rollout planned for Contoso.', gist_source: 'heuristic' })
  })

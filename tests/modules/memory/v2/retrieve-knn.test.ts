...
import { d1Keys } from '@modules/memory/v2/d1'
...
const GOLD = 'Contoso helyesbito szamla dontes: MODIFY uses corrected_invoice_id.'
...
describe('d1Keys', () => {
  it('always includes global 0 and looks up project ∪ project_type keys', () => {
    ...
    expect(d1Keys(db, null, null)).toEqual([0])
    ...
    const keys = d1Keys(db, 'p1', 't1')
    expect(keys[0]).toBe(0)
    expect(keys).toContain(1)
    expect(keys).toContain(2)
    expect(d1Keys(db, 'missing', null)).toEqual([0])
  })
})
...
describe.skipIf(!vecOk)('L3 KNN on a live sqlite-vec connection', () => {
  ...
  it('filters live_in_index next to MATCH — a cold vec0 row is not returned', async () => {
    ...
    const caps = probeSqliteCapabilities(raw)
    createMemoryV2Tables(db, caps)
    const liveRid = insertFact(db, 'f-live', 'Contoso', GOLD, 0)
    const coldRid = insertFact(db, 'f-cold', 'Contoso', GOLD, 0)
    raw.prepare('INSERT INTO memory_embedding_vec(rowid, project_key, embedding) VALUES (?, ?, vec_int8(?))')
      .run(liveRid, 0, floatToInt8(hashEmbed(`Contoso states ${GOLD}`)))
    raw.prepare('INSERT INTO memory_embedding_vec(rowid, project_key, embedding) VALUES (?, ?, vec_int8(?))')
      .run(coldRid, 0, floatToInt8(hashEmbed(`Contoso states ${GOLD}`)))
  ...
  it('restricts KNN to the D1 partition set (global ∪ project ∪ type), not other projects', async () => {
    ...
    const home = insertFact(db, 'f-home', 'Contoso', GOLD, p1)
    const other = insertFact(db, 'f-other', 'Contoso', GOLD, p9)
    const ins = raw.prepare('INSERT INTO memory_embedding_vec(rowid, project_key, embedding) VALUES (?, ?, vec_int8(?))')
    const blob = floatToInt8(hashEmbed(`Contoso states ${GOLD}`))

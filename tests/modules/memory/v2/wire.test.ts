...
describe('wireL0Capture', () => {
  ...
  it('warns at boot when reasoning capture is on, and not about tool results then', async () => {
    const { db, caps } = makeV2Db()
    const loud = { ...silentLogger, warn: vi.fn() }
    await wireL0Capture({ db, caps, logger: loud as any, instanceId: 'inst-test', config: () => config({ captureThinking: true }) })
    expect(loud.warn).toHaveBeenCalledTimes(1)
    expect(loud.warn).toHaveBeenCalledWith(expect.stringContaining('captureThinking is ON'))

    resetIngestBridge()
    const quiet = { ...silentLogger, warn: vi.fn() }
    await wireL0Capture({ db, caps, logger: quiet as any, instanceId: 'inst-test', config: () => config({ captureThinking: false }) })
    expect(quiet.warn).not.toHaveBeenCalled()
  })

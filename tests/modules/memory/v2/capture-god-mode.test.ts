...
describe('God Mode turn → L0 rows', () => {
  ...
  it('writes the user turn and the promoted winner to memory_raw with godMode provenance', async () => {
    ...
    const rows = db.all(sql`SELECT source_type, actor, trust_tier, meta_json FROM memory_raw WHERE conversation_id = ${conversationId} ORDER BY rid`) as any[]
    ...
    // The route is where the owner typed: the one path that stores owner trust (W8).
    expect(rows[0].trust_tier).toBe('owner')
    expect(JSON.parse(rows[0].meta_json)).toMatchObject({ author: 'owner', entryPath: 'interactive' })
    expect(rows[1].trust_tier).toBe('derived')

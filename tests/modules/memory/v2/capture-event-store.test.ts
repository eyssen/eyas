...
describe('agent_events LlmResponse → L0 capture', () => {
  ...
  it('carries the answering pair, the agent and the entry path from the payload into the provenance meta', async () => {
    const store = createEventStore(db)
    await store.append({
      sessionId: 'sess-bg', type: EventTypes.LlmResponse,
      payload: { response: { content: 'answer', stopReason: 'end', provider: 'kimi-cli', model: 'k2' }, entryPath: 'background' },
    })
    expect(enqueue.mock.calls[0][0].meta).toMatchObject({ provider: 'kimi-cli', model: 'k2', agentId: 'agent-1', entryPath: 'background' })
  })

  it('records null provenance for a payload that carries none (an older event)', async () => {
    const store = createEventStore(db)
    await store.append({ sessionId: 'sess-bg', type: EventTypes.LlmResponse, payload: { response: { content: 'old row' } } })
    expect(enqueue.mock.calls[0][0].meta).toMatchObject({ provider: null, model: null, entryPath: null })
  })
  ...
  it('a background run through the real runner produces an L0 row', async () => {
    ...
    expect(JSON.parse(rows[0].meta_json)).toMatchObject({ sessionId: 'sess-bg', origin: 'agent_events', provider: 'mock', model: 'm' })
  })
})

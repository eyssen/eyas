...
const candidate = (facts: CandidateFact[], entities: ExtractionCandidate['entities'] = []): ExtractionCandidate => ({
  gist: 'Invoice rollout planned for Contoso.', importance: 0.4, entities, topics: ['invoi'], facts,
  language: 'en', gistSource: 'heuristic', heuristicGist: 'Invoice rollout planned for Contoso.',
})
...
describe('arbitrate — facts', () => {
  ...
  it('tag invariant: a source lacking the scope project tag, or an unknown source, is a counted violation and no row', () => {
    const r = arbitrate(db, candidate([fact('customer', 'Contoso', [r4]), fact('x', 'y', ['nope']), fact('z', 'w', [r1, r4])]), scope(), 'run-1')
    ...
    const noProject = arbitrate(db, candidate([fact('customer', 'Contoso', [r4])]), { ...scope(), projectId: null }, 'run-2')
  ...
  it('entity stubs: created once, matched by canonical name or alias, linked from a fact whose subject names them', () => {
    const c = candidate([fact('contoso kft', 'customer', [r1])], [{ name: 'Contoso Kft', type: 'proper' }, { name: '2026-10-01', type: 'date' }])
    arbitrate(db, c, scope(), 'run-1')
    expect(count(db, 'memory_entity')).toBe(2)
    expect(count(db, 'memory_item', `item_type = 'entity'`)).toBe(2)
    const entity = (db.all(sql`SELECT id, rid, canonical_name, entity_type FROM memory_entity WHERE canonical_name = 'Contoso Kft'`) as any[])[0]
    expect(entity.entity_type).toBe('proper')
    expect(tagsOf(entity.rid)).toEqual(['layer=entity'])
    const [row] = factRows()
    expect(row.entity_id).toBe(entity.id)
    expect(tagsOf(row.rid)).toContain('entity=Contoso Kft')
    ...
    db.run(sql`INSERT INTO memory_entity (rid, id, content_hash, origin_instance_id, hlc_physical_ms, hlc_logical, revision, created_at, tombstoned,
      ...
      VALUES (${rid}, 'ent-alias', 'h', 'inst-test', 1, 0, 1, 1, 0, 'Contoso Kft Budapest', 'proper', '["ckft"]', NULL)`)
    arbitrate(db, candidate([fact('ckft', 'alias test', [r2])], [{ name: 'CKFT', type: 'proper' }]), scope(), 'run-3')
    expect(count(db, 'memory_entity')).toBe(3)
    expect(factRows().find((f: any) => f.subject === 'ckft').entity_id).toBe('ent-alias')
  })
  ...
  it('supersede and dedup are scoped to the task, not global', () => {
    ...
    arbitrate(db, candidate([fact('owner', 'alice', [r2])]), scope(), 'run-3')
    const deduping = arbitrate(db, candidate([fact('owner', 'alice', [other])]), otherScope, 'run-4')
  ...
  it('entity matching is case-symmetric and survives a malformed alias list', () => {
    ...
    const accented: ExtractionCandidate['entities'] = [{ name: '\u00dcgyf\u00e9l Port\u00e1l', type: 'proper' }, { name: 'Contoso Kft', type: 'proper' }]

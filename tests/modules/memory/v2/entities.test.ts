...
describe('extractEntities', () => {
  ...
  it('finds @mentions without trailing punctuation and never inside e-mail addresses', () => {
    expect(names('Ping @alice and @eyas-bot. Mail alice@example.com instead.', 'mention')).toEqual(['alice', 'eyas-bot'])
  })
  ...
  it('turns key: value and key = value lines into kv entities (keys only)', () => {
    ...
    expect(extractKeyValues(text)).toEqual([{ key: 'Deadline', value: '2026-10-01' }, { key: 'Customer', value: 'Contoso Kft' }])
  })
  ...
  it('finds capitalised multi-word phrases, dropping leading stop-words and single words', () => {
    expect(names('The Kubernetes Ingress broke for Contoso Kft in Budapest yesterday. A szállítási cím Contoso Kft.', 'proper'))
      .toEqual(['Kubernetes Ingress', 'Contoso Kft'])
  })

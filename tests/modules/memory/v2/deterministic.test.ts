...
const units: ExtractionUnit[] = [
  u('r1', 'user_message', 'Please plan the invoice module rollout for Contoso Kft.\nDeadline: 2026-10-01\nCustomer = Contoso Kft', 0),
...
describe('extractDeterministic', () => {
  it('turns key: value lines in USER messages into structural facts; tool results never yield facts', () => {
    ...
    expect(c.facts).toEqual(expect.arrayContaining([
      ...
      { subject: 'customer', predicate: 'is', object: 'Contoso Kft', confidenceHint: 0.5, sourceRawIds: ['r1'] },
  ...
  it('detects the language, extracts entities and unions TF-IDF stems with entity names into topics', () => {
    ...
    expect(c.entities).toEqual(expect.arrayContaining([{ name: '2026-10-01', type: 'date' }, { name: 'Contoso Kft', type: 'proper' }, { name: 'browser_click', type: 'code' }]))
    expect(c.topics).toContain('kuber')
    expect(c.topics).toContain('contoso kft')
  ...
  it('produces a heuristic gist under 280 chars and an importance that responds to close and pin', () => {
    ...
    expect(open.gist.startsWith('Please plan the invoice module rollout for Contoso Kft.')).toBe(true)
  ...
  it('never quotes recorded tool I/O in the gist, though it still shapes entities', () => {
    const secretLast: ExtractionUnit[] = [
      u('s1', 'user_message', 'Please check the harbor deployment settings for the staging cluster.', 0),
      u('s2', 'assistant_message', 'Reading the settings file now.', 1),
      u('s3', 'tool_result', '{"tool":"read_file","input":{"path":"/srv/harbor/.env"},"output":"HARBOR_TOKEN=TOKEN-GIST-3131. The harbor deployment settings are loaded."}', 2),
      u('s4', 'tool_result', '{"tool":"browser_fill","input":{"value":"PASSWORD-GIST-4242"},"output":"The harbor deployment form was filled."}', 3),
    ]
    const c = extractDeterministic(secretLast, { db, projectId: null, taskClosed: false })
    // (+) the gist is made of the messages
    expect(c.gist).toContain('Please check the harbor deployment settings')
    expect(c.gist).toContain('Reading the settings file now.')
    // (−) and never of a tool call's arguments or output, first, middle or last
    for (const leaked of ['TOKEN-GIST-3131', 'PASSWORD-GIST-4242', '/srv/harbor/.env', 'read_file', 'filled']) {
      expect(c.gist).not.toContain(leaked)
    }
    expect(c.entities).toEqual(expect.arrayContaining([{ name: 'read_file', type: 'code' }]))
    // (−) a batch of tool results alone has no gist
    expect(extractDeterministic(secretLast.slice(2), { db, projectId: null, taskClosed: false }).gist).toBe('')
  })
  ...
  it('mints key:value and a states fact from imported vault notes and episodic rows', () => {
    const imported: ExtractionUnit[] = [
      u('d1', 'document', '---\nkind: feedback\ntitle: Commit rule\n---\n# Commit rule\nNever commit or push automatically.\nCustomer: Contoso Kft\n', 0),
      u('e1', 'legacy_episodic', 'We decided the Contoso 4711 corrective invoice is a data fix, not a code change.', 1),
    ]
    const c = extractDeterministic(imported, { db, projectId: null, taskClosed: false })
    expect(c.facts).toEqual(expect.arrayContaining([
      { subject: 'customer', predicate: 'is', object: 'Contoso Kft', confidenceHint: 0.5, sourceRawIds: ['d1'] },
      expect.objectContaining({ subject: 'commit rule', predicate: 'states', sourceRawIds: ['d1'] }),
      expect.objectContaining({ subject: 'episode', predicate: 'states', sourceRawIds: ['e1'] }),
    ]))
    expect(c.facts.find((f) => f.sourceRawIds[0] === 'e1')?.object).toMatch(/Contoso 4711/)
    expect(c.importance).toBeGreaterThan(0.15)
    expect(c.gist.toLowerCase()).toMatch(/commit|contoso/)
  })
})

...
describe('createConsolidator — runOnce()', () => {
  ...
  // D-7 / P-19 — the nightly consolidator hands an episodic cluster to an LLM
  // summariser and then invalidates its rows. A flagged row must not be
  // clustered (it would be summarised into a recallable note) and must not be
  // invalidated either: it stays a valid, hidden episodic row.
  it('leaves a flagged episodic cluster out of semantic promotion and keeps its rows valid', async () => {
    const recent = new Date(now - 1 * DAY).toISOString()
    const memories = [0, 1, 2].map((i) =>
      makeEpisodic({
        id: `secret-${i}`,
        content: 'zxq secret cluster TOKEN=alphabravocharlie0001',
        tags: ['contains-secrets'],
        lastAccessedAt: recent,
      })
    )
    const { port, episodicStore } = makeFakeMemory({ blocks: [], episodic: memories })
    const promote = vi.fn(async (_members: EpisodicMemory[]) => ({ path: 'semantic/x.md' }))
    const consolidator = createConsolidator({
      memory: port,
      events: makeFakeEvents(),
      wiki: makeFakeWiki(),
      semanticPromoter: { promoteCluster: promote },
    })

    const report = await consolidator.runOnce()

    expect(promote).not.toHaveBeenCalled()
    expect(report.promotions.episodic).toBe(0)
    expect(episodicStore.filter((m) => m.tags.includes('contains-secrets'))).toHaveLength(3)
    expect(episodicStore.every((m) => m.validUntil === null)).toBe(true)
  })

  it('clusters a flagged episodic row once the owner opened recall', async () => {
    const recent = new Date(now - 1 * DAY).toISOString()
    const memories = [0, 1, 2].map((i) =>
      makeEpisodic({
        id: `open-${i}`,
        content: 'zxq open cluster TOKEN=alphabravocharlie0001',
        tags: ['contains-secrets'],
        lastAccessedAt: recent,
      })
    )
    const { port } = makeFakeMemory({ blocks: [], episodic: memories })
    const promote = vi.fn(async (_members: EpisodicMemory[]) => ({ path: 'semantic/x.md' }))
    const consolidator = createConsolidator({
      memory: port,
      events: makeFakeEvents(),
  ...
  it('still promotes an untagged cluster while a flagged one sits beside it', async () => {
    const recent = new Date(now - 1 * DAY).toISOString()
    const memories = [
      ...[0, 1, 2].map((i) => makeEpisodic({ id: `plain-${i}`, content: 'zxq plain cluster', lastAccessedAt: recent })),
      ...[0, 1, 2].map((i) => makeEpisodic({
        id: `secret-${i}`,
        content: 'zxq secret cluster TOKEN=alphabravocharlie0001',
        tags: ['contains-secrets'],
        lastAccessedAt: recent,
    ...
    expect(promote).toHaveBeenCalledTimes(1)
    expect(promote.mock.calls[0][0].every((m: any) => !m.tags.includes('contains-secrets'))).toBe(true)
    expect(episodicStore.filter((m) => m.tags.includes('contains-secrets')).every((m) => m.validUntil === null)).toBe(true)
  })

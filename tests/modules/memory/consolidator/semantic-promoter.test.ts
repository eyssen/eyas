...
describe('SemanticPromoter', () => {
  ...
  it('carries contains-secrets from a flagged member onto the promoted note', async () => {
    ...
    const promoter = createSemanticPromoter({ gateway: fakeGateway, vault, indexer })
    const result = await promoter.promoteCluster([flagged, makeMember('b', 'Deploy alpha again'), makeMember('c', 'Deploy alpha once more')])
  ...
  it('leaves the tag off a note whose cluster held no flagged member', async () => {
    ...
    const promoter = createSemanticPromoter({ gateway: fakeGateway, vault, indexer })
    const result = await promoter.promoteCluster([makeMember('a', 'Deploy bravo'), makeMember('b', 'Deploy bravo again')])

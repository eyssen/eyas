...
describe('graph-builder', () => {
  it('infers topics from path/title', () => {
    ...
    expect(inferTopic('semantic/project_globex_ticket4711.md', 'Globex')).toBe(
      'topic:project:globex',
    )
    expect(inferTopic('semantic/contoso-rollout-notes.md', 'Rollout notes')).toBe('topic:contoso-rollout')
  })
  ...
  it('builds soft topic edges when tags are only import noise', () => {
    const nodes = [
      ...
      { id: 'semantic/contoso-foo.md', label: 'Contoso Foo', tier: 'semantic', tags: ['imported'] },
      { id: 'semantic/contoso-bar.md', label: 'Contoso Bar', tier: 'semantic', tags: ['imported'] },
  ...
  it('resolves wikilinks with underscore/hyphen variants', () => {
    const nodes = [
      {
        id: 'semantic/erp-repo-location.md',
        label: 'Repo location',
        tier: 'semantic',
        tags: [],
      },
    ...
    const outgoing = new Map([
      [
        ...
        [{ targetId: 'erp_repo_location', context: 'see [[erp_repo_location]]' }],
      ],
    ])
    ...
    expect(wl?.target).toBe('semantic/erp-repo-location.md')
  })
})

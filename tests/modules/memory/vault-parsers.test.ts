...
describe('durable-memory frontmatter', () => {
  ...
  it('reads domain kind and projectType, and does not treat projectType as a project id', () => {
    const parsed = parseVaultFile([
      '---',
      'title: Shared tax groups',
      'kind: domain',
      'projectType: type-a',
    ...
    expect(parsed.frontmatter.kind).toBe('domain')
    expect(parsed.frontmatter.projectType).toBe('type-a')
    expect(parsed.frontmatter.project).toBeUndefined()
  })
})

...
describe('VaultIndexer', () => {
  ...
  it('indexes kind and summary, so the prompt index need not read every file', () => {
    service.write('semantic/owner.md', {
    ...
    const row = (db as any).all(sql`SELECT kind, summary FROM vault_index WHERE path = 'semantic/owner.md'`)[0]
    expect(row.kind).toBe('user')
    expect(row.summary).toBe('Answers in Hungarian')
  })

  it('carries an updated kind through a re-index', () => {
    service.write('semantic/note.md', {
      title: 'N', tags: [], tier: 'semantic', links: [],
      created: '2026-08-27', updated: '2026-08-27', summary: 'First',
    }, 'Body one.')
    indexer.indexAll()
    ...
    service.write('semantic/note.md', {
      ...
      kind: 'feedback', summary: 'Second',
    }, 'Body two.')
    indexer.indexAll()

    const row = (db as any).all(sql`SELECT kind, summary FROM vault_index WHERE path = 'semantic/note.md'`)[0]

...
// D-7 / P-19 — the importer stores a credential-bearing file verbatim and tags
// it; the protection is here, at recall. Related work is a model-facing
// consumer, so a tagged vault note or episodic row must not reach a prompt
// unless the owner turned `memory.recall.includeSecrets` on.
describe('contains-secrets (D-7)', () => {
  function seedSecretHits() {
    note('semantic/alpha-key.md', {
      title: 'Alpha key',
      summary: 'Direct MNB SOAP — Cloudflare 1010 blocked IAP',
      content_text: MNB_BODY,
      tags: '["contains-secrets"]',
    })
    db.run(sql`INSERT INTO episodic_memories (id, content, source_type, salience, access_count, valid_from, tags, created_at)
      VALUES ('ep-secret', ${MNB_BODY}, 'system', 1, 0, '2026-08-31T00:00:00Z', '["contains-secrets"]', '2026-08-31T00:00:00Z')`)
  }
  ...
  it('leaves tagged vault and episodic hits out by default and shows them on request', () => {
    ...
    const hidden = buildRelatedWork(db, { query: MNB_QUERY, conversationId: 'c-now', projectTypeId: null })!
    expect(hidden).not.toBeNull()
    expect(hidden.ids).not.toContain('vt:semantic/alpha-key.md')
    expect(hidden.ids).not.toContain('ep:ep-secret')

    const shown = buildRelatedWork(db, {
      query: MNB_QUERY, conversationId: 'c-now', projectTypeId: null, includeSecrets: true,
    })!
    expect(shown.ids).toEqual(expect.arrayContaining(['vt:semantic/alpha-key.md', 'ep:ep-secret']))
  })

  it('keeps an untagged hit when the tags column is malformed rather than losing it', () => {
    seedMnbScene()
    note('semantic/broken-tags.md', {
      title: 'Broken tags',
      summary: 'Direct MNB SOAP — Cloudflare 1010 blocked IAP twice',
      content_text: MNB_BODY,
      tags: 'not json',
    })
    const block = buildRelatedWork(db, { query: MNB_QUERY, conversationId: 'c-now', projectTypeId: null })!
    expect(block.ids).toContain('vt:semantic/broken-tags.md')
  })
})

describe('imported unscoped episodic and L0 raw', () => {
  it('includes a NULL-project episodic row in a project conversation', () => {
    db.run(sql`CREATE TABLE IF NOT EXISTS projects (id TEXT PRIMARY KEY, name TEXT, type_id TEXT)`)
    db.run(sql`INSERT INTO projects (id, name, type_id) VALUES ('alpha', 'Alpha', 'type-a')`)
    conv('c-now', { project_id: 'alpha' })
    db.run(sql`INSERT INTO episodic_memories (id, content, source_type, salience, access_count, valid_from, tags, created_at, project_id)
      VALUES ('ep-werth', ${MNB_BODY}, 'import', 1, 0, '2026-08-31T00:00:00Z', '[]', '2026-08-31T00:00:00Z', NULL)`)
    const block = buildRelatedWork(db, {
      query: MNB_QUERY,
      conversationId: 'c-now',
      projectId: 'alpha',
    ...
    expect(block?.ids).toContain('ep:ep-werth')
  })

  it('surfaces an L0 document hit from memory_raw_fts', () => {
    const caps = probeSqliteCapabilities(getRawFromDrizzle(db))
    createMemoryV2Tables(db, caps)
    if (!caps.fts5) return
    const now = Date.now()
    db.run(sql`INSERT INTO memory_item (item_type, id, created_at) VALUES ('raw', 'raw-werth', ${now})`)
    const rid = (db.all(sql`SELECT rid FROM memory_item WHERE id = 'raw-werth'`) as Array<{ rid: number }>)[0].rid
    db.run(sql`INSERT INTO memory_raw (
      ...
      shred_partition_id, source_type, actor, conversation_id, project_id, project_type_id, occurred_at, trust_tier
    ) VALUES (
      ${rid}, 'raw-werth', 'hash', 'inst', ${now}, 0, 1, ${now}, 0,
      'vault:semantic/werth.md', 'document', 'owner', 'vault:semantic/werth.md', NULL, NULL, ${now}, 'owner'
    )`)
    db.run(sql`INSERT INTO memory_raw_fts (rowid, body) VALUES (${rid}, ${MNB_BODY})`)
    db.run(sql`INSERT INTO memory_item (item_type, id, created_at) VALUES ('gist', 'gist-werth', ${now})`)
    const grid = (db.all(sql`SELECT rid FROM memory_item WHERE id = 'gist-werth'`) as Array<{ rid: number }>)[0].rid
    db.run(sql`INSERT INTO memory_gist (
      ...
      ${grid}, 'gist-werth', 'gh', 'inst', ${now}, 0, 1, ${now}, 0,
      'task', 'vault:semantic/werth.md', 0, ${MNB_BODY}, '{}', 0, 'owner', 10, 0.5, 'heuristic',
      1, 1.0, 'hot', 0, 0
    )`)
    conv('c-now')
    const block = buildRelatedWork(db, { query: MNB_QUERY, conversationId: 'c-now', projectTypeId: null })
    expect(block?.ids).toContain('raw:raw-werth')
    expect(block?.content).toMatch(/\[note\]/)
  })
})

...
beforeEach(() => {
  ...
  // I2: the drill-down scope comes from the conversation row on the server,
  // never from the context's projectId.
  db.run(sql`CREATE TABLE conversations (id TEXT PRIMARY KEY, project_id TEXT)`)
  db.run(sql`INSERT INTO conversations (id, project_id) VALUES
    ('conv-alpha', 'alpha'), ('conv-bravo', 'bravo'), ('conv-general', 'general-general')`)
  ...
  // Shaped like ctx.memory: memory/index.ts publishes the db on the service.
  ;(memory as unknown as { db: unknown }).db = db
...
describe('search_memory default scope (item 24)', () => {
  it('from alpha, default search does not return bravo project notes', async () => {
    const r = await harness.run('search_memory', { query: 'zebra' }, { conversationId: 'conv-alpha' })
  ...
  it('from bravo, default search does not return alpha project notes', async () => {
    const r = await harness.run('search_memory', { query: 'zebra' }, { conversationId: 'conv-bravo' })
  ...
  it('an unscoped project note is found from any project and from no project', async () => {
    ...
    const fromAlpha = await harness.run('search_memory', { query: 'zebra' }, { conversationId: 'conv-alpha' })
    expect(fromAlpha.success).toBe(true)
    expect(contents(fromAlpha.output)).toContain('zebra-unscoped')

    const fromNoProject = await harness.run('search_memory', { query: 'zebra' }, { conversationId: 'conv-general' })
    expect(fromNoProject.success).toBe(true)
    expect(contents(fromNoProject.output)).toContain('zebra-unscoped')
  })

  it('ignores a context projectId that disagrees with the conversation (spoofed bridge context)', async () => {
    const r = await harness.run('search_memory', { query: 'zebra' }, { conversationId: 'conv-alpha', projectId: 'bravo' })
  ...
  it('fails closed on a conversation id that has no row: an error and no results', async () => {
    const r = await harness.run('search_memory', { query: 'zebra' }, { conversationId: 'conv-unknown', projectId: 'alpha' })

    expect(r.success).toBe(true)
    expect((r.output as { error?: string }).error).toMatch(/memory scope unresolved/)
    expect(contents(r.output)).toBe('')
  })

  it('an outside MCP client (no EYAS conversation) reads global notes only', async () => {
    const r = await harness.run(
      'search_memory',
      { query: 'zebra' },
      { conversationId: 'mcp-external', projectId: 'alpha', actor: { kind: 'external', role: 'owner' } },
    )
...
describe('contains-secrets recall', () => {
  ...
  it('keeps a flagged note out of the tool result too, not only out of the service', async () => {
    ...
    const r = await harness.run('search_memory', { query: 'zxq bravo credential' }, { conversationId: 'conv-alpha' })
    expect(r.success).toBe(true)

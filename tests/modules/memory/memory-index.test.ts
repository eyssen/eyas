...
import { buildMemoryIndex, inferKind, MEMORY_SECTION_KEY, hasSecretsTag, recallIncludesSecrets } from '@modules/memory/memory-index'
...
describe('buildMemoryIndex', () => {
  ...
  it('within a kind, prefers a short MOC over a long dump so standing notes still fit', () => {
    ...
    note('semantic/werth.md', { kind: 'reference', title: 'Werth', summary: 'Customer Werth' })
    const built = buildMemoryIndex(db, { budgetChars: 280 })!
    expect(built.paths[0]).toBe('semantic/werth.md')
    expect(built.content).toContain('Customer Werth')
  })
  ...
  it('keeps short reference MOCs and the commit rule inside an 8000-char budget after many feedback notes', () => {
    ...
    expect(built.paths).toContain('semantic/werth.md')
...
describe('project-ranked recall (F1)', () => {
  ...
  it('a project note with no declared project id is global, ranked as project', () => {
    ...
    expect(buildMemoryIndex(db, { projectId: 'p1' })!.content).toContain('- [project] Orphan project note')
    expect(buildMemoryIndex(db, {})!.content).toContain('- [project] Orphan project note')
  })
})
...
describe('type-ranked recall (item 20)', () => {
  ...
  it('a domain note with no type id is global, ranked as domain', () => {
    ...
    expect(buildMemoryIndex(db, { projectId: 'alpha', projectTypeId: 'type-a' })!.content)
      .toContain('- [domain] Orphan type note')
    expect(buildMemoryIndex(db, {})!.content).toContain('- [domain] Orphan type note')
  })

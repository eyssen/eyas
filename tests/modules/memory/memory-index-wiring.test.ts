...
import { MEMORY_SECTION_KEY, buildMemoryIndex, recallIncludesSecrets } from '@modules/memory/memory-index'
...
describe('background path', () => {
  ...
  // D-7 / P-19 — the always-on index is the widest model-facing consumer there
  // is: every turn gets it. The accessor below mirrors the one the memory
  // module publishes as `ctx.memoryIndex` (config read per call, so a
  // `config reload` takes effect).
  const accessor = (config: unknown) => (opts?: Record<string, unknown>) =>
    buildMemoryIndex(db, { ...(opts ?? {}), includeSecrets: recallIncludesSecrets(config) })

  function seedTaggedNote() {
    db.run(sql`INSERT INTO vault_index (path, title, tier, tags, content_text, kind, summary, file_hash, indexed_at)
      VALUES ('semantic/alpha-key.md', 'N', 'semantic', '["contains-secrets"]', 'body', 'user',
              'Alpha deploy key is alphabravocharlie0001', 'h', '2026-08-27T00:00:00Z')`)
  }

  it('keeps a contains-secrets note out of the system prompt by default', async () => {
    seedTaggedNote()
    seedNote(db, 'semantic/owner.md', 'user', 'Answers in Hungarian')
    deps.memoryIndex = accessor({ memory: { recall: { includeSecrets: false } } })
    await runConversation('conv-1', deps)

    expect(runCalls[0].system).toContain('Answers in Hungarian')
    expect(runCalls[0].system).not.toContain('alphabravocharlie0001')
  })

  it('shows it once the owner opened memory.recall.includeSecrets', async () => {
    seedTaggedNote()
    deps.memoryIndex = accessor({ memory: { recall: { includeSecrets: true } } })
    await runConversation('conv-1', deps)

    expect(runCalls[0].system).toContain('alphabravocharlie0001')
  })

  it('excludes it on a build with no accessor at all, where no config is read', async () => {
    // conversation-runner falls back to a direct build; no config means the
    // default, and the default is the exclusion.
    seedTaggedNote()
    await runConversation('conv-1', deps)
    expect(runCalls[0].system ?? '').not.toContain('alphabravocharlie0001')
  })

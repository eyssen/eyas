...
import { RELATED_WORK_SECTION_KEY, buildRelatedWork } from '@modules/memory/related-work'
import { recallIncludesSecrets } from '@modules/memory/memory-index'
...
describe('background path', () => {
  ...
  // D-7 / P-19 — the accessor below mirrors the one the memory module publishes
  // as `ctx.relatedWork`: the config is read per call, so a `config reload`
  // changes what the next turn may see.
  const accessor = (config: unknown) => (opts: any) =>
    buildRelatedWork(db, { ...opts, includeSecrets: recallIncludesSecrets(config) })

  function seedTaggedMatch() {
    db.run(sql`INSERT INTO vault_index (path, title, tier, tags, content_text, kind, summary, file_hash, indexed_at)
      VALUES ('semantic/alpha-key.md', 'N', 'semantic', '["contains-secrets"]', ${MNB_BODY}, null,
              'MNB SOAP IAP key alphabravocharlie0001', 'h', '2026-08-31T00:00:00Z')`)
    db.run(sql`UPDATE conversations SET goal_description = ${GOAL} WHERE id = 'conv-1'`)
  }

  it('keeps a contains-secrets note out of the related-work block by default', async () => {
    seedTaggedMatch()
    deps.relatedWork = accessor({ memory: { recall: { includeSecrets: false } } })
    await runConversation('conv-1', deps)
  ...
  it('shows it once the owner opened memory.recall.includeSecrets', async () => {
    seedTaggedMatch()
    deps.relatedWork = accessor({ memory: { recall: { includeSecrets: true } } })
    await runConversation('conv-1', deps)

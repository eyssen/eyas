...
Bundled content.`
  ...
  describe('CRUD operations', () => {
    ...
    it('gets the newest user skill by name, ignoring bundled rows', async () => {
      writeFileSync(join(skillsDir, 'shared-name.md'), '---\nname: Shared Name\n---\nbundled body.')
      await loader.loadFromDirectory(skillsDir)

      expect(loader.getByName('Shared Name')).toBeNull()
      expect(loader.getByName('Never Created')).toBeNull()

      const older = loader.create({ name: 'Shared Name', content: 'older body' })
      db.run(sql`UPDATE skills SET created_at = '2020-01-01T00:00:00.000Z' WHERE id = ${older.id}`)
      const newer = loader.create({ name: 'Shared Name', content: 'newer body' })

      const found = loader.getByName('Shared Name')
      expect(found).not.toBeNull()
      expect(found!.id).toBe(newer.id)
      expect(found!.content).toBe('newer body')

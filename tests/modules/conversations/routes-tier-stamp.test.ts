...
// overridden. Only a conversation set to Auto-routing is routed at all (D3):
// a fixed or colleague's model is never triaged and never stamped.
...
describe('chat route — routing tier stamping', () => {
  ...
  beforeEach(async () => {
    ...
    const decisionEngine = {
      route: async () => {
        routeCalls++
        return { provider: 'p1', model: 'm1', tier: 'complex', strategy: 'triage', confidence: 1, reason: 'test' }
      },
  ...
  it('stamps the decision tier on a turn of an Auto-routing conversation', async () => {
    chatService.update(conversationId, { modelBinding: 'auto' })
  ...
  it('never routes or stamps a conversation fixed to a model', async () => {
    await send({ content: 'hello' })
  ...
  it('never routes or stamps a colleague-bound (inherit) conversation', async () => {
    chatService.update(conversationId, { modelBinding: 'inherit', agentId: 'someone' })
    await send({ content: 'hello' })
    expect(routeCalls).toBe(0)
    expect(captured[0].metadata?.tier).toBeUndefined()
  })

  it('leaves the tier unset when the caller pinned a provider, even on an Auto conversation', async () => {
    chatService.update(conversationId, { modelBinding: 'auto' })

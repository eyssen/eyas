...
describe('ConversationService', () => {
  ...
  describe('ownsConversation', () => {
    ...
    it('ownerOf names the user ownsConversation answers true for, or null', () => {
      const root = svc.create({ userId: 'user-1' })
      const child = svc.create({ userId: 'system' })
      svc.update(child.id, { parentConversationId: root.id })
      const orphan = svc.create({ userId: 'system' })
      expect(svc.ownerOf(root.id)).toBe('user-1')
      expect(svc.ownerOf(child.id)).toBe('user-1')
      expect(svc.ownerOf(orphan.id)).toBeNull()
      expect(svc.ownerOf('does-not-exist')).toBeNull()
    })
  })

...
describe('decideTeamAutoPropose', () => {
  ...
  it('nudges specialists on auto + complex first message (no team card)', () => {
    const d = decideTeamAutoPropose(base)
    expect(d.action).toBe('nudge')
  })

  it('proposes on auto + expert first message (epic)', () => {
  ...
  it('nudges on auto + multi-step moderate first message (no team card)', () => {
    const d = decideTeamAutoPropose({
      ...base,
      complexity: 'moderate',
  ...
  it('nudges on deep + moderate first message (no team card)', () => {
    const d = decideTeamAutoPropose({
      ...base,
      orchestration: 'deep',
      complexity: 'moderate',
...
describe('fireTeamProposal', () => {
  it('creates session and broadcasts team:proposed', async () => {
    ...
    expect(analyzeAndPropose).toHaveBeenCalledWith('Build a secure multi-tenant API', 'complex', { conversationId: 'c1' })

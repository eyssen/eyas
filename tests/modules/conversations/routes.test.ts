...
describe('POST /api/v1/conversations/:id/messages (SSE)', () => {
  ...
  it('increments conversations.total_cost_usd from the estimated cost of the turn', async () => {
    ...
    await res.text() // the turn ends (and records its cost) when the stream does
  ...
  it('fixes the install default on the first message when none was specified', async () => {
    const createRes = await app.request('/api/v1/conversations', {
      method: 'POST',
      headers: { Authorization: `Bearer ${ownerToken}`, 'Content-Type': 'application/json' },
    ...
    const conv = await createRes.json() as any
    // Nothing is stamped at creation (D3): the default is fixed by the first turn.
    ...
    expect(res.headers.get('Content-Type')).toBe('text/event-stream')
    await res.text()
    expect(conversationService.get(conv.id)!.providerId).toBe('anthropic')
  })
})
...
describe('POST /api/v1/conversations/:id/messages — composition entry_point + compositionId correlation (Task 10 fix round 1)', () => {
  ...
  /** What the stub runner yields; absent → one plain 'done'. */
  let scriptedEvents: any[] | undefined
    ...
    const getAgentRunner = () => ({
      run: (opts: any) => {
        ...
        return (async function* () {
          if (scriptedEvents) { yield* scriptedEvents; return }
  ...
  // G11 — context occupancy: the history estimate at record time, and the
  // provider's measured prompt size of the last call as the numerator.
  async function turn(content: string): Promise<{ conv: any; done: any }> {
    const createRes = await compApp.request('/api/v1/conversations', {
      method: 'POST',
      headers: { Authorization: `Bearer ${compToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'occupancy', providerId: 'anthropic', modelId: 'claude-3-opus' }),
    })
    const conv = await createRes.json() as any
    const res = await compApp.request(`/api/v1/conversations/${conv.id}/messages`, {
      method: 'POST',
    ...
    return { conv, done: frames.find((f) => f.type === 'done') }
  }

  it('(G11) records the history estimate and the measured prompt size; the done frame reads it as measured', async () => {
    scriptedEvents = [
      { type: 'text', text: 'ok' },
      { type: 'turn_complete', turn: 1, tokensUsed: 2, usage: { inputTokens: 40, outputTokens: 1, cacheReadTokens: 7_000, promptTokensLastCall: 7_040 } },
      { type: 'done', response: { content: [{ type: 'text', text: 'ok' }], usage: { inputTokens: 40, outputTokens: 1 }, contextWindow: 1_000_000 }, outcome: 'completed', stopReason: 'end' },
    ]
    const { conv, done } = await turn('x'.repeat(400))
    const comp = (db.all(sql`SELECT history_estimated_tokens, observed_prompt_tokens, observed_context_window
      FROM context_compositions WHERE conversation_id = ${conv.id}`) as any[])[0]
    expect(comp.history_estimated_tokens).toBeGreaterThanOrEqual(100)
    expect(comp).toMatchObject({ observed_prompt_tokens: 7_040, observed_context_window: 1_000_000 })
    expect(done.conversation).toMatchObject({ estimatedTokens: 7_040, contextWindow: 1_000_000, measured: true })
  })

  it('(G11, −) nothing measured: the done frame carries the estimate (sections + history), measured=false', async () => {
    const { conv, done } = await turn('y'.repeat(800))
    const comp = (db.all(sql`SELECT estimated_tokens, history_estimated_tokens, observed_prompt_tokens
      FROM context_compositions WHERE conversation_id = ${conv.id}`) as any[])[0]
    expect(comp.observed_prompt_tokens).toBeNull()
    expect(comp.history_estimated_tokens).toBeGreaterThanOrEqual(200)
    expect(done.conversation).toMatchObject({
      estimatedTokens: comp.estimated_tokens + comp.history_estimated_tokens,

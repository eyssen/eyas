...
describe('PATCH /api/v1/conversations/:id — status whitelist + totalCostUsd strip', () => {
  ...
  // A3 — a provider session id is not client-writable: there is no update()
  // path for it any more, so a PATCH carrying one is ignored (and on a legacy
  // DB that still has the inert column, nothing lands in it).
  it('ignores a client-supplied sdkSessionId (no resume id can be planted)', async () => {
    db.run(sql.raw('ALTER TABLE conversations ADD COLUMN sdk_session_id TEXT'))
    const res = await patch({ sdkSessionId: 'host-session-id', title: 'Still renamed' })
    expect(res.status).toBe(200)
    const body = await res.json() as any
    expect(body).not.toHaveProperty('sdkSessionId')
    expect(body.title).toBe('Still renamed')
    const row = (db.all(sql`SELECT sdk_session_id FROM conversations WHERE id = ${conversationId}`) as any[])[0]

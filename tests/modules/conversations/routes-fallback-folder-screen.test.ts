...
describe('chat route, direct gateway branch — stored folders screened (K2)', () => {
  ...
  it('(−) a stored folder in another user\'s conversation workspace is left out of the request, with a folderRefused notice', async () => {
    // The fixture's conv-1 workspace is not a conversation of this user.
    chatService.update(conversationId, { workingDirectories: [f.ownWorkspace, project] })
    const out = await send()
    expect(out).toContainEqual({ type: 'notice', code: 'folderRefused', params: { path: f.ownWorkspace, reason: 'otherWorkspace' } })
    expect(requests[0]?.metadata?.workingDirectories).toEqual([project])
  })

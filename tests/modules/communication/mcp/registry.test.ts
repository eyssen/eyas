...
describe('MCP Registry — data integrity', () => {
  it('contains exactly 34 server entries', () => {
    expect(mcpServerRegistry).toHaveLength(34)
  })
  ...
  it('ships Magnific, Higgsfield, fal, and HeyGen hosted MCP entries', () => {
    const ids = mcpServerRegistry.map((e) => e.id)
    expect(ids).toEqual(expect.arrayContaining(['magnific', 'higgsfield', 'fal', 'heygen']))
    for (const id of ['magnific', 'higgsfield', 'fal', 'heygen']) {
  ...
  it('tier distribution is correct', () => {
    expect(entriesByTier('bundled').length).toBe(11)
    expect(entriesByTier('one-click').length).toBe(13)
    expect(entriesByTier('manual').length).toBe(10)
  })

  it('memory-store entries are manual, never bundled or one-click', () => {
    const flagged = mcpServerRegistry.filter(e => e.memoryStore)
    expect(flagged.map(e => e.id).sort()).toEqual(['memory', 'obsidian', 'qdrant'])
    for (const entry of flagged) expect(entry.tier, entry.id).toBe('manual')
  })
})
...
describe('MCP Registry — API routes', () => {
  ...
  describe('GET /api/v1/mcp/registry', () => {
    it('returns all 34 registry entries', async () => {
      const res = await app.request('/api/v1/mcp/registry', { headers: auth() })
      expect(res.status).toBe(200)
      const body = await res.json() as any
      expect(body.servers).toHaveLength(34)
    })

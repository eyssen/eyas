...
function setup(opts: { ability?: { can: (a: string, s: string) => boolean }; role?: string } = {}) {
  ...
  const server = createMcpServer({
    toolRegistry,
    toolExecutor: { execute, renderForModel: createToolExecutor(toolRegistry, { authorization: 'disabled' }).renderForModel } as any,
    logger: silentLogger,
    http: app as any,
  })
...
describe('MCP server routes — authenticated + CASL-gated (F0 R2)', () => {
  ...
  it('answers a successful call as MCP text content with the request id', async () => {
    const { app } = await connected({ ability: allow })
    const res = await app.request('/api/v1/mcp/tools/call', callBody('echo', { a: 1 }))
    expect(await res.json()).toEqual({ jsonrpc: '2.0', result: { content: [{ type: 'text', text: '{"echoed":{"a":1}}' }] }, id: 7 })
  })

  it('accepts the bare { name, arguments } body too', async () => {
    const { app, execute } = await connected({ ability: allow })
    const res = await app.request('/api/v1/mcp/tools/call', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'echo', arguments: { a: 2 } }),
    })
    expect(res.status).toBe(200)
    expect(execute.mock.calls[0][1]).toEqual({ a: 2 })
  })

  it('rejects a malformed body with a JSON-RPC error (400) and never executes (negative)', async () => {
    const { app, execute } = await connected({ ability: allow })
    const post = (body: string) => app.request('/api/v1/mcp/tools/call', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body })
    const notJson = await post('not json')
    expect(notJson.status).toBe(400)
    expect(((await notJson.json()) as { error: { code: number } }).error.code).toBe(-32600)
    const badParams = await post(JSON.stringify({ id: 3, params: { name: 'echo', arguments: 'nope' } }))
    expect(badParams.status).toBe(400)
    expect(await badParams.json()).toEqual({ jsonrpc: '2.0', error: { code: -32602, message: 'Invalid params' }, id: 3 })
    expect((await post(JSON.stringify({ id: 4, params: {} }))).status).toBe(400)
    expect(execute).not.toHaveBeenCalled()
  })

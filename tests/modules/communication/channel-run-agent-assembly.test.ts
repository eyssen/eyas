...
describe('createChannelRunAgent — context recording', () => {
  ...
  it('(G11) records the history estimate and what the provider measured on the reply\'s last call', async () => {
    const record = vi.fn().mockReturnValue('comp-3')
    const observe = vi.fn()
    const { deps: d } = deps({
      contextRecorder: { record, observe },
      conversations: {
        get: () => ({ id: 'c1', projectId: 'p1', messages: [{ role: 'user', content: 'x'.repeat(400) }] }),
        addMessage: vi.fn(),
      },
      agentRunner: {
        run: async function* () {
          yield { type: 'text', text: 'ok' }
          yield { type: 'turn_complete', turn: 1, tokensUsed: 2, usage: { inputTokens: 1, outputTokens: 1, promptTokensLastCall: 5_500 } }
          yield { type: 'done', response: { content: [], usage: { inputTokens: 1, outputTokens: 1 }, contextWindow: 256_000 }, outcome: 'completed' }
        },
      },
    })
    await createChannelRunAgent(d)({ conversationId: 'c1', agentId: 'a1', mode: 'managed' })
    expect(record).toHaveBeenCalledWith(expect.objectContaining({ historyEstimatedTokens: 100 }))
    expect(observe).toHaveBeenCalledWith('comp-3', { promptTokens: 5_500, contextWindow: 256_000 })
  })

  it('(G11, −) no composition recorded: nothing is observed', async () => {
    const observe = vi.fn()
    const { deps: d } = deps({ contextRecorder: { record: vi.fn().mockReturnValue(null), observe } })
    await createChannelRunAgent(d)({ conversationId: 'c1', agentId: 'a1', mode: 'managed' })

...
describe('outboundToRfc822 — display names cannot break out of the header', () => {
  it('drops CR/LF from a display name instead of starting a new header', () => {
    const rfc = outboundToRfc822({
      to: [{ address: 'r@example.com', name: 'Real\r\nBcc: victim@example.com' }],
      subject: 'Hello',
      bodyText: 'Body',
    })
    // The injected header must not appear at the start of any line.
    expect(rfc).not.toMatch(/^Bcc:/m)
    expect(rfc).toContain('r@example.com')
  })

  it('escapes a backslash in a quoted display name', () => {
    const rfc = outboundToRfc822({
      to: [{ address: 'r@example.com', name: 'Back\\slash, Inc' }],
      subject: 'Hello',
      bodyText: 'Body',
    })
    // A lone backslash before the closing quote would end the quoted string early.
    expect(rfc).toContain('"Back\\\\slash, Inc" <r@example.com>')
  })

  it('quotes a display name containing an address separator', () => {
    const rfc = outboundToRfc822({
      to: [{ address: 'r@example.com', name: 'Ops <ops@evil.test>' }],
      subject: 'Hello',
      bodyText: 'Body',
    })
    const to = rfc.split(/\r?\n/).find((l) => l.startsWith('To:')) ?? ''
    expect(to).toBe('To: "Ops <ops@evil.test>" <r@example.com>')
  })
})

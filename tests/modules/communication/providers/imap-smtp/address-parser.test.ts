...
describe('formatAddressList — header safety', () => {
  it('drops CR/LF so a display name cannot start a new header', () => {
    const out = formatAddressList([
      { address: 'r@example.com', name: 'Real\r\nBcc: victim@example.com' },
    ])
    expect(out).not.toMatch(/[\r\n]/)
    expect(out).toContain('r@example.com')
  })

  it('escapes a backslash before the quote it would otherwise cancel', () => {
    const out = formatAddressList([{ address: 'r@example.com', name: 'Back\\slash, Inc' }])
    expect(out).toBe('"Back\\\\slash, Inc" <r@example.com>')
  })
})

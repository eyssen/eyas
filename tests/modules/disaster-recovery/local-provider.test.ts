...
describe('local backup provider — full data coverage', () => {
  ...
  it('packs everything needed for empty-system restore (data + config + .env + version)', async () => {
    ...
    // EYAS-owned CLI homes: the sign-in stays, the session stores do not.
    mkdirSync(join(root, 'data', 'cli-homes', 'grok-cli', '.grok', 'sessions', 'enc-cwd', 's1'), { recursive: true })
    mkdirSync(join(root, 'data', 'cli-homes', 'kimi-cli', '.kimi', 'sessions'), { recursive: true })
    writeFileSync(join(root, 'data', 'cli-homes', 'grok-cli', '.grok', 'auth.json'), '{}')
    writeFileSync(join(root, 'data', 'cli-homes', 'grok-cli', '.grok', 'sessions', 'enc-cwd', 's1', 'chat_history.jsonl'), 'transcript')
    writeFileSync(join(root, 'data', 'cli-homes', 'kimi-cli', '.kimi', 'sessions', 'k1.jsonl'), 'transcript')
    ...
    expect(listing).toMatch(/data\/cli-homes\/grok-cli\/\.grok\/auth\.json/)
    expect(listing).not.toMatch(/chat_history\.jsonl/)
    expect(listing).not.toMatch(/k1\.jsonl/)

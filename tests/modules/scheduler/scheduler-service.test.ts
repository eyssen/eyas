...
describe('SchedulerService', () => {
  ...
  describe('run (manual execution)', () => {
    it('executes a handler and records success', async () => {
      ...
      // The parsed config, and which job runs (its creator owns an agent_run's conversation).
      expect(handlerFn).toHaveBeenCalledWith({ key: 'value' }, { jobId: job.id, createdBy: 'user-7' })
    ...
    it('executes without handlerConfig', async () => {
      ...
      await scheduler.run(job.id)
      expect(handlerFn).toHaveBeenCalledWith(undefined, { jobId: job.id, createdBy: undefined })
    })
  })

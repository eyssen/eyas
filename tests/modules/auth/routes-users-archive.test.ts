...
describe('DELETE /api/v1/users/:id — archive semantics', () => {
  it('archives a regular user: 200, status becomes archived, disappears from default list, appears in archived list', async () => {
    ...
    // …nor an OpenCode terminal: the archive is reported as an access change.
    expect(userAccessChanged).toHaveBeenCalledWith(expect.objectContaining({ targetId: id, status: 'archived' }))
  })

...
describe('Module Interactions', () => {
  ...
  it('privacy scanner detects PII in text', () => {
    const scanner = createRegexScanner()

    const text = 'Szia, az email-em: test@example.com, TAJ: 123-456-788'
  ...
  it('privacy scanner + policy: an IBAN is masked on egress and refused as a new message', () => {
    const fx = createPrivacyFixture({ actions: { iban: 'block' } })
    try {
      const text = 'IBAN: HU42 1177 3016 1111 1018 0000 0000'
      expect(fx.service.redactText(text, { locality: 'remote' }).text).toBe('IBAN: [IBAN]')
      const verdict = fx.service.checkInbound(text, { localities: ['remote'] })

...
describe('classifyAuthError', () => {
  ...
  it('treats an EYAS-coded auth failure (a CLI not signed in for EYAS) as auth (positive)', () => {
    const err = new CodedModelError('auth', 'cliSignIn', { provider: 'grok-cli' }, { message: 'Grok CLI is not signed in for EYAS' })
    expect(classifyAuthError(err)).toEqual({ kind: 'auth', isAuth: true })
  })

  it('does not treat other coded failures as auth (negative)', () => {
    expect(classifyAuthError(new CodedModelError('isolation', 'cliIsolation', { provider: 'grok-cli' })).isAuth).toBe(false)
    expect(classifyAuthError({ name: 'CodedModelError', kind: 'auth' }).isAuth).toBe(false)
  })
})

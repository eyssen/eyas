...
describe('createSharedRunSeq', () => {
  it('without a persisted high-water mark it is a plain sequence from 1', () => {
    const next = createSharedRunSeq('r1')
    expect([next(), next(), next()]).toEqual([1, 2, 3])
  })

  it('continues above what other emitters of the run already persisted', () => {
    let persisted = 5
    const next = createSharedRunSeq('r1', () => persisted)
    expect(next()).toBe(6)
    // Another emitter wrote 7..9 on the same run meanwhile.
    persisted = 9
    expect(next()).toBe(10)
    // Nothing new persisted (a live-only sink): it keeps counting.
    expect(next()).toBe(11)
  })

  it('asks for the high-water mark of its own run', () => {
    const seen: string[] = []
    const next = createSharedRunSeq('conv-1', (runId) => { seen.push(runId); return 0 })
    next()
    expect(seen).toEqual(['conv-1'])
  })

  it('an unreadable or bogus high-water mark counts as none (negative)', () => {
    const throwing = createSharedRunSeq('r1', () => { throw new Error('db gone') })
    expect([throwing(), throwing()]).toEqual([1, 2])
    const bogus = createSharedRunSeq('r1', () => Number.NaN)
    expect(bogus()).toBe(1)
  })
})

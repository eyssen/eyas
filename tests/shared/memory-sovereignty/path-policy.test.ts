...
describe('workspacesOf', () => {
  it('names the workspace a path is or lies in, under the default and a relocated root; the root and _runs themselves name none', () => {
    for (const ws of [defaultWs, relocatedWs]) {
      const p = policy({ workspacesRoot: ws })
      expect(p.workspacesOf(join(ws, 'conv-1'))).toEqual([{ id: 'conv-1', scratch: false }])
      expect(p.workspacesOf(join(ws, 'conv-2', 'deep', 'file.md'))).toEqual([{ id: 'conv-2', scratch: false }])
      expect(p.workspacesOf(join(ws, RUN_SCRATCH_SEGMENT, 'run-a', 'x'))).toEqual([{ id: 'run-a', scratch: true }])
      expect(p.workspacesOf(ws)).toEqual([])
      expect(p.workspacesOf(join(ws, RUN_SCRATCH_SEGMENT))).toEqual([])
      expect(p.workspacesOf(join(ws, 'conv-1', '..', 'conv-2'))).toEqual([{ id: 'conv-2', scratch: false }])
    }
    const p = policy()
    expect(p.workspacesOf(join(repo, 'src'))).toEqual([])
    expect(p.workspacesOf('relative/conv-1')).toEqual([])
    expect(p.workspacesOf(`${join(defaultWs, 'conv-1')}\0`)).toEqual([])
  })

  it('judges a symlink where it leads too: a link inside one workspace into another names both', () => {
    const p = policy()
    const link = join(defaultWs, 'conv-1', 'to-conv-2')
    try {
      symlinkSync(join(defaultWs, 'conv-2'), link)
      expect(p.workspacesOf(link)).toEqual([{ id: 'conv-1', scratch: false }, { id: 'conv-2', scratch: false }])
      const outside = mk(root, 'links')
      symlinkSync(join(defaultWs, 'conv-2'), join(outside, 'ws'))
      expect(p.workspacesOf(join(outside, 'ws', 'file.md'))).toEqual([{ id: 'conv-2', scratch: false }])
    } finally {
  ...
  it('folds case only where the platform does, and keeps the id as the path spells it', () => {
    // A workspace that does not exist: realpath cannot re-spell it.
    const upper = join(defaultWs.toUpperCase(), 'Conv-9')
    expect(policy({ platform: 'darwin' }).workspacesOf(upper)).toEqual([{ id: 'Conv-9', scratch: false }])
    expect(policy({ platform: 'linux' }).workspacesOf(upper)).toEqual([])
  })
})

...
import { checkMemoryEmbedder } from '../../src/cli/commands/doctor'
...
describe('Doctor — memory embedder (recall always embeds locally)', () => {
  function cacheDir(withWeights: boolean): string {
    const dir = join(tmpdir(), `eyas-doctor-models-${Date.now()}-${Math.random().toString(36).slice(2)}`)
    mkdirSync(dir, { recursive: true })
    if (withWeights) {
      mkdirSync(join(dir, 'intfloat', 'multilingual-e5-small', 'onnx'), { recursive: true })
      writeFileSync(join(dir, 'intfloat', 'multilingual-e5-small', 'onnx', 'model_qint8_avx512_vnni.onnx'), 'x')
    }
    return dir
  }

  it('reports the hash embedder with the install remedy when @huggingface/transformers is missing', async () => {
    const dir = cacheDir(true)
    const r = await checkMemoryEmbedder({ cacheDir: dir, transformersInstalled: () => false })
    expect(r.name).toBe('Memory embedder')
    expect(r.status).toBe('warn')
    expect(r.message).toContain('stem5-fnv-384')
    expect(r.message).toContain('bun add @huggingface/transformers')
    rmSync(dir, { recursive: true })
  })

  it('reports e5 when the model dir holds the weights', async () => {
    const dir = cacheDir(true)
    const r = await checkMemoryEmbedder({ cacheDir: dir, transformersInstalled: () => true })
    expect(r.status).toBe('ok')
    expect(r.message).toContain('multilingual-e5-small')
    expect(r.message).toContain(join('multilingual-e5-small', 'onnx'))
    rmSync(dir, { recursive: true })
  })

  it('warns (not ok) when transformers is installed but the weights are not downloaded yet', async () => {
    const dir = cacheDir(false)
    const r = await checkMemoryEmbedder({ cacheDir: dir, transformersInstalled: () => true })
    expect(r.status).toBe('warn')
    expect(r.message).toContain('stem5-fnv-384')
    expect(r.message).toMatch(/next start downloads/)
    rmSync(dir, { recursive: true })
  })
})

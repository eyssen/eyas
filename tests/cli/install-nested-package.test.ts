...
describe('isUnresolvedPackageError', () => {
  ...
  it('matches Bun FileNotFound linking errors from a lockfile that records link:', () => {
    // bun.lock already lists @saker/core@link:@saker/core. Isolated install then
    // prints this instead of "is not linked" (Adel, public clone, bun 1.3.10).
    const log = [
      'FileNotFound: failed linking dependency/workspace to node_modules for package @saker/core',
      'FileNotFound: failed linking dependency/workspace to node_modules for package @saker/react',
      'FileNotFound: failed linking dependency/workspace to node_modules for package @saker/ui',
      'Failed to install 3 packages',
    ].join('\n')
    expect(isUnlinkedPackageError(log)).toBe(true)
    expect(isUnresolvedPackageError(log)).toBe(true)
  })
...
describe('installNestedPackage', () => {
  ...
  it('skips link: deps when bun.lock records them (FileNotFound linking)', async () => {
    const dir = tempDir('eyas-nested-lock-')
    const bunRoot = tempDir('eyas-bun-lock-root-')
    const pkg = {
      name: 't',
      private: true,
      dependencies: {
    ...
    writeFileSync(join(dir, 'bun.lock'), JSON.stringify({
      ...
      packages: {
        '@saker/core': ['@saker/core@link:@saker/core', {}],
        '@saker/react': ['@saker/react@link:@saker/react', {}],
        '@saker/ui': ['@saker/ui@link:@saker/ui', {}],
      },
    }, null, 2) + '\n')
    const env = { ...process.env, BUN_INSTALL: bunRoot, HOME: bunRoot }
    const r = await installNestedPackage(dir, { env })
    expect(r.ok).toBe(true)
    expect(r.skippedLinks.sort()).toEqual(['@saker/core', '@saker/react', '@saker/ui'].sort())
    const restored = JSON.parse(readFileSync(join(dir, 'package.json'), 'utf8'))
    expect(restored.dependencies['@saker/core']).toBe('link:@saker/core')
  })
})
...
describe('installer scripts know about the nested frontend package', () => {
  ...
  it('clone_or_update fast-forwards an existing checkout onto FETCH_HEAD', () => {
    const sh = readRepo('scripts/install.sh', 'utf8')
    expect(sh).toMatch(/checkout --force -B/)
    expect(sh).toMatch(/FETCH_HEAD/)
    const ps = readRepo('scripts/install.ps1', 'utf8')
    expect(ps).toMatch(/checkout --force -B/)
    expect(ps).toMatch(/FETCH_HEAD/)
  })
})

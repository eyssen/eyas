...
const META = {
  ...
  'daily/home': {
    ...
    intros: {
      en: 'Home screen after login: a per-user drag-and-drop widget grid — pulse, attention, running agents, schedule, and more — plus setup recommendations.',
      hu: 'Bejelentkezés utáni kezdőképernyő: felhasználónkénti húzd-és-ejtsd widget rács — pulzus, figyelem, futó ágensek, ütemezés és más — valamint setup ajánlások.',
      de: 'Startbildschirm nach dem Login: ein persönliches Drag-and-Drop-Widget-Raster — Puls, Aufmerksamkeit, laufende Agenten, Zeitplan und mehr — plus Setup-Empfehlungen.',
      es: 'Pantalla de inicio tras el login: una cuadrícula de widgets personal y arrastrable — pulso, atención, agentes en ejecución, programación y más — más recomendaciones de setup.',
      fr: 'Écran d’accueil après connexion : une grille de widgets personnelle, glisser-déposer — pouls, attention, agents en cours, planification et plus — et des recommandations de configuration.',
      tlh: 'yI\'el \'etlh: SoH mIw raS widget — tIq, qImnIS, ghoqwI\'pu\' QaptaH, poH nav je — \'ej lIng chupmey.',
    },
  },
  'daily/conversations': {
  ...
  'admin/settings': {
    ...
    intros: {
      en: 'System settings hub: appearance, language, brand (design system), model assignments, team agents, autonomy features, data port, and system update.',
      hu: 'Rendszerbeállítások központ: megjelenés, nyelv, arculat (design system), modell-hozzárendelések, team ágensek, autonómia, data port, frissítés.',
      de: 'System-Einstellungen: Erscheinungsbild, Sprache, Marke (Design-System), Modellzuweisungen, Team-Agenten, Autonomie, Data Port, Update.',
      es: 'Centro de ajustes: apariencia, idioma, marca (sistema de diseño), asignaciones de modelo, agentes de equipo, autonomía, data port, actualización.',
      fr: 'Hub des paramètres système : apparence, langue, marque (design system), affectations de modèles, agents d’équipe, autonomie, port de données et mise à jour système.',
      tlh: 'pat SeHmey juH: qal\'aq, Hol, Degh (design system), pat lIngmey, ghom ghoqwI\'pu\', SeH\'egh, De\' He, \'ej chu\'qa\'.',
    },
  },
  'admin/backup': {
...
function cell(s) {
  return String(s || '')
    .replace(/\r?\n/g, ' ')
    // Backslash first: a value ending in one would otherwise escape the
    // backslash we add for the pipe, and the pipe would split the table cell.
    .replace(/\\/g, '\\\\')
    .replace(/\|/g, '\\|')
    .trim()
}

...
| Goal | How |
|------|-----|
| Lives in the repo | Markdown under `src/content/docs/{en,hu,de,es,fr,tlh}/` (prefer `.md` over MDX — braces in prose break MDX) |
| Multi-language | Starlight locales: **en**, **hu**, **de**, **es**, **fr**, **tlh** (product languages) |
...
Architecture specs and contributor plans stay in the repo root `docs/` tree — not here.


## Screenshots

Locale-specific UI captures live under `src/assets/screenshots/{en,hu,de,es,fr,tlh}/`.
...
See **[OUTLINE.md](./OUTLINE.md)** for the locked section map (learning path, then product areas). Chapter shape: **[PAGE_TEMPLATE.md](./PAGE_TEMPLATE.md)**.

### Generate / refresh product docs

Do **not** run `bun run full-docs` / `generate-full-docs.mjs` on existing chapters — it overwrites handwritten prose. Edit the Markdown under `src/content/docs/{locale}/` instead. `generate-skeleton.mjs` skips custom pages.

```bash
bun run build
```

- `scripts/export-field-catalog.mjs` — reads `src/web/src/pages/**/locales/{en,hu,de,es,fr,tlh}.json` → `field-catalog.json`
...
```
src/content/docs/{en,hu,de,es,fr,tlh}/
...
1. Create the same relative path under **all six** locales (or accept English fallback).
2. Add a sidebar entry in `astro.config.mjs` (with `translations` for hu/de/es/fr/tlh).
...
`help-map.json` is the contract for in-app `?` icons:
...
- Architecture / design docs (`docs/eyas-architecture.md`, `docs/superpowers/…`)
- UI string i18n (`src/web/**/locales/*.json`)
- Architecture / design docs (`docs/eyas-architecture.md`, `docs/superpowers/…`) — this package only points at them

...
description: Edit IDENTITY, AGENTS, TOOLS, and your MEMORY notes — and restore a snapshot if you need to.
---

**What this is for.** Workspace files are the agent's long-lived prose: who they are, how they relate to the team, and how they should use tools — plus a **Memory** file for your own notes about the agent. This is deeper than the Configuration form. What EYAS remembers is not here: it records memory itself and shows it on the [Memory](/docs/en/knowledge/memory/) page. When autonomy forbids self-update, changes to identity come through [Forge](/docs/en/agents/forge/) instead of a silent rewrite.
...
| **Memory** (`MEMORY`) | Your own notes about this agent. EYAS does not send this file to the model, and agents cannot write it. What EYAS remembers is on the [Memory](/docs/en/knowledge/memory/) page. |

Choosing **Memory** in the file picker shows that hint: *Your own notes about this agent. EYAS does not send this file to the model, and agents cannot write it. What EYAS remembers is on the Memory page.* You can still edit and save it, and view its history, as before.

### What agents may edit themselves

The `workspace_append` and `workspace_edit` tools edit only **Team description** (`AGENTS.md`) and **Tools** (`TOOLS.md`). Agents can no longer write the Memory file (`MEMORY.md`) or daily memory files (`memory/YYYY-MM-DD.md`): EYAS records memory automatically, and agents never write memory themselves.

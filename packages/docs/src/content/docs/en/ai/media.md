---
title: Media
description: Connect Magnific, Higgsfield, fal, or HeyGen. Agents generate and upscale through five shared tools. Compare backends and pick one — or several.
---

**What this is for.** Media is how EYAS generates, upscales, and waits on images, video, audio, edits, and 3D. You pick the backends; the agent uses **one set of tools**. None of the four vendors is default. Zero connected providers is an empty, fail-closed state — never mock pixels.
...
- You have a Magnific, Higgsfield, fal, or HeyGen account and do **not** want fifty vendor tools dumped onto the model.
...
1. Open **Media** (`/media`).
2. Read **Which backend?** on that page, then **Connect** one (or several). Magnific, Higgsfield, and HeyGen open a browser OAuth window; fal uses an API key.
...
| Criterion | Magnific | Higgsfield | fal | HeyGen |
|-----------|----------|------------|-----|--------|
| **Best at** | Photoreal stills, prompt-guided **Creative** upscale, faithful **Precision** upscale | Cinematic video, character consistency (Soul) | Huge model catalog, price check before a run | Talking-head avatars; presenter video from a prompt or script |
| **Kinds in EYAS** | Upscale, image, edit (also video / audio / 3D) | Video, image (also audio) | Image, video, audio, 3D, upscale | Video, audio |
| **Sign-in** | OAuth (Magnific account) | OAuth (Higgsfield account) | Bearer API key (`fal-api-key`) | OAuth (HeyGen account) |
| **Credits** | Same balance as the Magnific site. Web **Unlimited does not apply** to MCP/API | MCP **always** spends credits, even if the website plan is unlimited | MCP itself is free; you pay per model run | MCP spends the HeyGen **web plan**, not the separate REST API balance |
| **Result files** | CDN URL — EYAS copies bytes locally | URLs expire in about **seven days** — ingest is required | CDN URL — still copied locally | CDN URL — EYAS copies bytes locally |
| **Connect first if…** | You upscale, retouch, or need stills | You need clips or a locked character | You want many models or a cost check first | You need a presenter, digital twin, or scripted avatar |

**Recommendations**

1. **Connect one backend for the job you actually have.** Stills and upscale → Magnific. Cinematic clip / character lock → Higgsfield. Talking-head / presenter → HeyGen. Broad catalog or “what does this cost?” → fal.
...
3. **Do not turn on raw vendor MCP tools** unless you are debugging. That dumps Magnific/Higgsfield/fal/HeyGen tool lists onto the agent and skips ingest.
...
Suggested defaults (applied only if that provider is connected, and you have not pinned the row): upscale / image / edit → Magnific; video → Higgsfield (pin **heygen** for talking-head); audio / 3D → fal.
...
**Expose raw vendor MCP tools.** Off by default. On = the agent also sees `mcp_magnific_*` / `mcp_higgsfield_*` / `mcp_fal_*` / `mcp_heygen_*`. Leave it off.

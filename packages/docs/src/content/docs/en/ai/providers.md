...
**Host Claude config (Claude Code CLI).** By default EYAS keeps conversations isolated from the host machine's Claude configuration — no settings.json (hooks, permission rules), no CLAUDE.md files, no host skills or project .mcp.json servers — so EYAS's own memory is the single source of truth. The "Load host Claude config" switch on the provider panel opts back in. The Grok and Kimi CLIs always load their own machine-level config; EYAS cannot disable that. Enterprise-managed policy settings, where deployed, still apply — that tier cannot be disabled from EYAS.

...
| Working directories | Ordered named folders (`name` + absolute path) where a conversation may read/write; first is the primary cwd. Set on the type and/or project; inherited by conversations. File tools jail here — empty list refuses |
| Plan first | Conversation composer mode: the model writes a plan and waits (**Approve** / **Skip plan** / **Reject**) before tools run |
| Skill import roots | Instance `skills.importRoots` / `agent.importRoots` in `local.yaml` — extra markdown folders. Default empty. Isolation stays on |

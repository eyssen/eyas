...
- You used to reach another tool's memory live — a host `CLAUDE.md` loaded by Claude Code, an import root inside `~/.claude`, a Memory / Qdrant / Obsidian MCP server — and EYAS now refuses it. The import is the sanctioned way in: one-way, once, with the origin recorded.
...
| Enrichment | **Off by default** — an opt-in checkbox, metadata only (kind, summary, tags, checked links), never the body; enriched notes say who answered (`enriched_by`) |
...
**Model enrichment is off by default.** The review step carries an **Enrich metadata with the model** checkbox. Left unchecked — the default — the whole import is deterministic and spends no model call at all; nothing changes for such imports. Ticked, notes *without* a declared type get a suggested kind, summary, tags and links from EYAS's **background model**: the Heartbeat routing tier first, then the install default, then API providers, then CLIs that can run isolated. There is no item cap; the cost is time. The body is never rewritten, nothing is skipped on the model's say-so, and an item tagged `contains-secrets` is never sent. The result panel reports how many notes were enriched.

- **Only an isolated answer is used.** An answer counts only when it comes from the background model through an isolated call (one turn, no tools): an API provider, or a CLI that can run isolated. If no such model is available (for example a Grok-only or Kimi-only install), if the call fails or comes back empty, or if the model refuses, the note keeps exactly its deterministic metadata and is counted as a **fallback** in the result panel.
- **Links are checked.** A link the model suggests is kept only when it names a note that already exists in the vault (by file name or title) or a note selected in the same import, and it is written as that note's file name. Every other link the model proposes is dropped. Links the source file itself contains are always kept unchanged.
- **Enriched notes say who answered.** Every enriched note carries an `enriched_by` field in its frontmatter, for example `enriched_by: {provider: anthropic, model: claude-haiku-4-5, route: tier}`. `provider` is the provider that answered; `model` is the model it reported, omitted when the backend named none (a CLI used with its own default); `route` is how the background model was chosen — `tier` (the routing tier, Heartbeat first), `default` (the install default), `api` (an API provider) or `isolated-cli` (a CLI that can run isolated). The field stays when the note is later rewritten. Notes imported without enrichment have no `enriched_by`.
- The background model is looked up when a job runs, not when the server starts, so enrichment works even when the model module became available after the Data Port.

**Cleaning an earlier enriched import.** Imports enriched before this update carry no `enriched_by` and may hold links that were never checked. To redo one without enrichment: open the job in the result panel or in **Previous imports** and choose **Roll back this import**, then scan the same source again and import with **Enrich metadata with the model** unticked. Notes whose body you edited since are kept and reported as skipped.
...
The importer reads other tools' stores itself, not through a model tool, so it is unaffected by the security gate's memory policy — which refuses **every model** reads and writes of `~/.claude`, `~/.grok`, `ai-memory` folders, Obsidian vaults and the rest (see [Security & privacy — Memory outside EYAS](/docs/en/admin/security-privacy/#memory-outside-eyas)). An agent that used to read such a file directly now gets a refusal; import the content once here instead. See [Memory](/docs/en/knowledge/memory/).

**This is the way in for content EYAS no longer reads live:**

- **Host `CLAUDE.md` and host skills.** Claude Code always runs isolated and no longer loads them; import them once here. See [Providers — Claude Code isolation](/docs/en/ai/providers/#claude-code-isolation).
- **Import roots inside another tool's folders.** `skills.importRoots` / `agent.importRoots` entries inside `~/.claude`, `~/.grok`, `~/.agents`, an Obsidian vault and similar are skipped at start; import that folder once here, then remove the entry from `local.yaml`. See [Configuration](/docs/en/deploy/configuration/#extra-skill-and-persona-roots).
- **Memory-store MCP servers.** The Memory, Qdrant, Obsidian and MCPVault MCP servers, and any server pointed at a vault or at another tool's memory, are blocked for every model; copy that memory into EYAS here. See [MCP](/docs/en/ai/mcp/#memory-store-servers-are-blocked).
- **Deleted imported agents.** An agent imported from `agent.importRoots` that you deleted is not created again at start; import its file here to get it back.
...
**What the tag does.** A note, episodic row or skill tagged `contains-secrets` is left out of everything the model reaches on its own: the standing memory index, recall, `search_memory`, the reflection job, the nightly consolidator, the skill matcher and the assembled system prompt. It is never handed to the optional enrichment model and it is never embedded. The tag also carries over to what EYAS derives from the item — its copy in the raw record, the facts extracted from it and the summaries built from it — so none of those reach the model either (see [Memory — Imported secrets stay out of recall](/docs/en/knowledge/memory/#imported-secrets-stay-out-of-recall)). It stays fully visible to you on the Memory page. To open it to the model, set `memory.recall.includeSecrets: true` in `config/local.yaml` and restart — see [Configuration](/docs/en/deploy/configuration/).
...
| **Enrich metadata with the model** | Off by default — opt-in, metadata only (kind, summary, tags, links to existing notes), never the body, never a flagged item; used only from an isolated background-model answer, otherwise counted as a fallback |

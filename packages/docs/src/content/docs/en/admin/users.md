...
| OpenCode terminal | Owner and admins only (the manage right on OpenCode): the terminal runs commands as the server's own user. The user role sees the OpenCode page, and colleagues' `opencode_run` tasks run in its conversations. The built-in roles' rights are fixed; to give someone the terminal, give them the admin role. Lowering their role, suspending or archiving them ends their open terminals at once — see [OpenCode — Who can open a terminal](/docs/en/automation/opencode/#who-can-open-a-terminal) |

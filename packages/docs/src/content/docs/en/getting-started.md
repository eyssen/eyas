...
When you are done, go to the [Setup wizard](/docs/en/setup-wizard/), then [Your first hour](/docs/en/first-hour/).
...
4. Open the web UI and begin work — next: [Your first hour](/docs/en/first-hour/)
...
| Disk space | SQLite DB, vault and agent files under `data/`; conversation workspaces (see below) |
...
| Home | [Home](/docs/en/daily/home/) |
| Product docs | Always at **`/docs/`** on the same host/port |

## Where data lives

Under the instance home (`EYAS_HOME` or the directory you started from). The data directory is `data/` there unless `EYAS_DATA_DIR` names another folder — everything below `data/` then lives in that folder:

| Path | Contents |
|------|----------|
| `data/sqlite/` | Main SQLite database (WAL mode, `synchronous = NORMAL`) |
| `data/vault/` | Semantic / procedural markdown vault — always `<data dir>/vault` |
| `data/agents/<id>/` | Per-agent workspace files (IDENTITY, SOUL, …) |
| `data/backups/` | Backup archives |
| `config/` | YAML defaults + local overlays |

Conversation workspaces — the folder each conversation without folders of its own works in — are in `data/workspaces/` on Docker, Kubernetes and packaged installs. A source install run from a git clone keeps them outside the checkout, in your user's application-data directory. See [Configuration — Conversation workspaces](/docs/en/deploy/configuration/#conversation-workspaces).

...
**Which model writes them.** Self-learning suggestions, Forge description proposals and skill authoring for Skill Evolution run on EYAS's background model, one isolated call each: the **Heartbeat** routing tier (primary, then fallback), then the install default, then API providers, then CLIs that can run isolated calls. They never go to a provider the gateway picks on its own. When no model qualifies (for example a Grok-only or Kimi-only install before their isolation is verified) or the budget is at *stop*, no model call is made: Self-learning shows its generic suggestion sentences, Forge keeps the concatenated description proposal, and Skill Evolution writes the template `SKILL.md`. The Autonomy feature flags still gate these calls. See [Routing & budget — The background model](/docs/en/ai/routing-budget/#background-model).

...
| `opencode_status` / `opencode_run` | Optional OpenCode coding-engine sidecar ([OpenCode](/docs/en/automation/opencode/)). Status is green; run is red + approval. |
...
| Tool | Purpose | Risk |
|------|---------|------|
| `run_specialist` | Spawn an enabled specialist and wait for the summary. Alias: `delegate_to_agent`. | green |
| `handoff_to_colleague` | Open another colleague’s home thread and start them. | green |

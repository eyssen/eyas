...
**User > Generated > Imported (`skills.importRoots`) > Bundled by an installed extension > Bundled with EYAS itself**
...
<h3 id="import-roots">Import extra skill roots</h3>

Host skills from another assistant are **not** loaded by Claude Code — it always runs isolated. Point this instance at extra markdown directories in `local.yaml`:

```yaml
skills:
  importRoots: []    # extra skill folders (empty = none)
agent:
  importRoots: []    # extra persona / agent markdown folders
```

Default is an **empty list**. Paths belong on the instance, never in shipped `src/`. Imported skills sit above core bundled copies on the ladder above. Personae from `agent.importRoots` appear in the UI as markdown, not as hardcoded names.

These roots are read on **every start**, so they are a live source — for ordinary folders only, such as a team folder like `/opt/team-skills`. A root that lies inside, or contains, another assistant's or note app's own folders is **skipped**: `~/.claude` (skills, agents, plugins), `~/.grok`, `~/.codex`, `~/.gemini`, `~/.kimi`, `~/.cursor`, `~/.codeium`, `~/.copilot`, `~/.agents`, `~/.config/agents`, OpenCode's folders, Obsidian's settings and vaults, `security.foreignMemoryPaths`, and EYAS's own CLI homes. A root such as your whole home folder is skipped too, because it contains them. Each skipped root logs a warning at start and shows as an **Import roots** warning in `eyas doctor`.

Skills already imported from such a folder stay in EYAS; they just stop refreshing from it. To bring that content in, or refresh it, run a [Data import](/docs/en/admin/data-port/) of the folder once — it is copied into EYAS with its origin recorded — then remove the entry from `local.yaml`. See [Configuration — Extra skill and persona roots](/docs/en/deploy/configuration/#extra-skill-and-persona-roots).

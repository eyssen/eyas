...
Host skills from another assistant are **not** loaded by turning Claude `settingSources` back on — isolation stays off by default. Point this instance at extra markdown directories in `local.yaml`:
...
Default is an **empty list**. Paths belong on the instance, never in shipped `src/`. Imported skills sit above core bundled copies on the ladder above. Personae from `agent.importRoots` appear in the UI as markdown, not as hardcoded names. See [Configuration](/docs/en/deploy/configuration/).

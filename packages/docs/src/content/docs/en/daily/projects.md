...
2. Create a **Project Type** if you need a template (prompt, optional **Working directories**), then **New Project** (name, type, default agent, optional **Working directories**, optional **Default code sources**, optional **Wiki auto-update**).
...
| **Working directories** | No | Named roots (`Name` + absolute path). First path is **Primary**. New conversations inherit this list. Empty list copies the **type** list when you pick that type. If nothing is set, each conversation works in its own EYAS workspace ([Folders](/docs/en/daily/conversations/#working-folders)). |
...
**Which folders can be saved.** The project, the project type and conversation Folders share one check. Saving refuses a folder and names it, with the reason in your language: the filesystem root, your home folder or any folder above it; a folder inside another AI tool's own storage (`~/.claude`, `~/.grok`, `~/.codex`, `~/.cursor`, …) or EYAS's CLI sign-in homes; a folder inside an Obsidian vault, an `ai-memory` folder or a `security.foreignMemoryPaths` entry; a folder inside EYAS's own data folder (except a single conversation workspace, Studio projects and browser downloads); sensitive locations (`.ssh`, `.env`, `master.key`, the database folder); and a relative path, a missing folder or a file. A folder is also refused when it **contains** such a place, and the message names what was found inside: EYAS's own home, data folder, database or workspaces folder (the EYAS checkout that holds `data/`), another AI tool's storage or an EYAS CLI sign-in folder, a notes vault, an `ai-memory` folder or a `security.foreignMemoryPaths` entry (`~/Documents` holding a vault). A CLI reads and searches inside its working folder without asking, so such a folder is not safe to hand over; choose a narrower one, such as the project folder inside `~/Documents` or a separate clone of the repository. The full list: [Conversations — Folders](/docs/en/daily/conversations/#working-folders).

Every save checks the whole list, so remove a now-refused folder before saving other changes. Folders saved earlier that are now refused are not rewritten, but every run leaves them out — EYAS's own file tools, the security gate and the CLI's working folder — and the chat turn shows a notice naming the folder. If every stored folder is refused, EYAS's own file tools have no folder, and a CLI works in the conversation's own EYAS workspace. The **Project Types** form now shows a save error in the form (before, a failed save was silent). The API answers a refused folder on `POST`/`PATCH /api/v1/projects` and `/api/v1/project-types` with `400 {error, code, path, found}` (`found` is the protected place inside the folder, for the codes `containsEyasData`, `containsProviderHome` and `containsVault`); a `workingDirectories` value that is not a list of non-blank paths (or `null`) is a `400` without a code, and `null` or `[]` still clears the list.

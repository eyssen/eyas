...
When an enabled, installed Grok CLI or Kimi Code CLI is not signed in for EYAS, a banner above the grid says *&lt;CLI&gt; needs a sign-in for EYAS* (owners and admins). This typically appears after an upgrade on an install that relied on the CLI's login on the computer, which EYAS no longer uses. **Sign in** opens the sign-in card in place; **Dismiss** hides the banner for this browser session. See [Providers — Sign in Grok and Kimi for EYAS](/docs/en/ai/providers/#sign-in-grok-and-kimi-for-eyas).

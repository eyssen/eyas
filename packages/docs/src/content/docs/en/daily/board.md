...
| **N% context** | How full the model's context window is — the same measurement as the conversation's context bar: the prompt size the provider reported for the last model call where available, otherwise an estimate (system prompt plus history), against the answering model's own window. Colours follow the theme |

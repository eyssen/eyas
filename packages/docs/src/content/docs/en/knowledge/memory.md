...
3. Have a conversation longer than ~40 characters that states a lasting fact. Return here after the reply — you should see a new vault note (kind `user`, `feedback`, `project`, or `reference`).
...
| **Client wiki** | Per-client delivery notes, not global memory. |
...
correct extractor answer (0–2 notes, kinds `user` / `feedback` / `project` /
`reference`).

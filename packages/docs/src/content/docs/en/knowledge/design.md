...
An AI edit runs on the install's default model: the **Standard** routing tier,
otherwise the default provider, otherwise the first active provider
(alphabetically) with an enabled model. It runs as one isolated call without
tools. Which provider registered first no longer affects it.

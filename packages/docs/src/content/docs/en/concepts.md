...
| **Colleague** | Primary or team agent you talk to — home thread, SOUL. Setup creates two (Personal Assistant + System Engineer). | Sidebar **Colleagues** / Agents |
| **Specialist** | Narrow worker from a shared pool; any colleague can spawn an enabled one (`run_specialist`) | Agents (tier: Specialist) |
...
3. The colleague may use **tools/skills**, write **memory**, **hand off / spawn a specialist**, or answer on a **channel**
...
| Control | Meaning |
|---------|---------|
| **Model** | Which model answers: a fixed model, Auto-routing, or the colleague's default |
| **Effort** | Reasoning depth vs cost/speed. Lists only the levels the conversation's model offers; **Auto** inherits (Deep → Max, the colleague's effort, the delegating conversation's) or uses the model's default |
| **Orchestration: Solo** | No specialists, hand-offs or team proposals |
| **Orchestration: Auto** | Model decides fan-out |
| **Orchestration: Deep** | Aggressive multi-agent fan-out; default effort Max |

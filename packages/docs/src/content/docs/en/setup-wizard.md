...
| Control | Meaning |
|---------|---------|
| **Language** | Product UI language (`en` / `hu` / `de` / `es`). Stored in the client language store. |
...
| Destination | Why |
|-------------|-----|
| [Dashboard](/docs/en/daily/dashboard/) | Setup recommendations for remaining optional work |

...
Une modification par IA tourne sur le modèle par défaut de l'install : le niveau
de routage **Standard**, sinon le fournisseur par défaut, sinon le premier
fournisseur actif (par ordre alphabétique) qui a un modèle activé. Elle s'exécute
en un seul appel isolé, sans outils. L'ordre d'enregistrement des fournisseurs
n'a plus d'effet.

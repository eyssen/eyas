...
**À quoi ça sert.** Vous installez le serveur EYAS pour que l’interface web existe sur votre machine. Cette page est le chemin d’installation, pas une visite du produit.

Quand vous avez terminé, passez à l’[assistant de configuration](/docs/fr/setup-wizard/), puis à [Votre première heure](/docs/fr/first-hour/).
...
4. Ouvrir l’interface web et commencer à travailler — ensuite : [Votre première heure](/docs/fr/first-hour/)
...
| Espace disque | Base SQLite, coffre et fichiers d’agent sous `data/` ; espaces de travail des conversations (voir plus bas) |
...
Non interactif : ajoutez `bash -s -- --yes`. Épingler une version : `--version 0.8.16-beta`.
...
Sous le répertoire d’instance (`EYAS_HOME` ou le répertoire depuis lequel vous avez démarré). Le répertoire de données y est `data/`, sauf si `EYAS_DATA_DIR` nomme un autre dossier — tout ce qui est sous `data/` vit alors dans ce dossier :

| Chemin | Contenu |
|--------|---------|
| `data/sqlite/` | Base SQLite principale (mode WAL, `synchronous = NORMAL`) |
| `data/vault/` | Coffre markdown sémantique / procédural — toujours `<répertoire de données>/vault` |
...
Les espaces de travail des conversations — le dossier dans lequel travaille chaque conversation sans dossiers propres — sont dans `data/workspaces/` sur les installs Docker, Kubernetes et packagées. Une install depuis les sources lancée depuis un clone git les garde hors du checkout, dans le répertoire de données d’application de votre utilisateur. Voir [Configuration — Espaces de travail des conversations](/docs/fr/deploy/configuration/#conversation-workspaces).

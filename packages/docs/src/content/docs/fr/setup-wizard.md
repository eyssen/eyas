...
**À quoi ça sert.** Premier démarrage uniquement. L’assistant crée le mot de passe maître, le propriétaire racine, vos deux agents principaux et un premier backend de modèle pour déverrouiller l’application. Ensuite, changez cela dans **Paramètres**, **Fournisseurs** et **Agents** — n’attendez pas de relancer l’assistant.

## Quand l’utiliser

- Le navigateur vous a envoyé vers `/setup` parce que la configuration est incomplète
- Vous avez ignoré une étape facultative et voulez la liste des champs
- Vous restaurez une instance neuve

Pas pour les changements du quotidien une fois l’application ouverte.
...
**Objectif :** créer les deux **collègues** always-on à qui vous parlez (barre **Collègues**, un fil d’accueil chacun).
...
Vous pouvez les renommer et les reconfigurer plus tard sous **Agents**. Ouvrez-les depuis la barre **Collègues** après l’assistant. L’Assistant coordonne et n’édite pas le code source ; l’Ingénieur possède la plateforme/le code. Voir [Équipes et délégation](/docs/fr/agents/teams/).

---

## Agents d’équipe (facultatif)

**Objectif :** activer des **collègues** supplémentaires (niveau équipe) et des **spécialistes** (pool partagé qu’un collègue peut lancer). Les agents principaux n’ont pas besoin d’une carte de proposition pour appeler un spécialiste activé.
...
|-------------|----------|
| [Votre première heure](/docs/fr/first-hour/) | Parcourir l’UI en direct : Accueil, une conversation, Tableau, Mémoire |
...
| [Agents](/docs/fr/agents/overview/) | Relire les collègues et les spécialistes |
| [Équipes et délégation](/docs/fr/agents/teams/) | Handoff et `run_specialist` |

...
**Répertoire de données sur le volume.** Avec le montage par défaut du chart (`/app/data`), rien à régler : la base et le coffre de mémoire sont sur le volume persistant. Si tu fais pointer `EYAS_DATA_DIR` vers un autre montage, le coffre y part aussi — c’est toujours `<répertoire de données>/vault` — avec la base. Voir [Configuration — Répertoire de données et coffre](/docs/fr/deploy/configuration/#data-directory-and-vault).

**Sandbox de fichiers du noyau.** L’image EYAS n’inclut pas bubblewrap (il est sous LGPL ; l’installer est le choix de l’opérateur). Pour le sandbox de fichiers du noyau des fournisseurs CLI, le conteneur a besoin de bubblewrap (`bwrap`), de `socat` pour Claude Code, et des espaces de noms utilisateur non privilégiés — que beaucoup de clusters et de runtimes de conteneurs désactivent. Sans eux, EYAS refuse toujours chaque requête qu’il voit et affiche le sandbox comme *Indisponible* ; avec `security.cliSandbox: required`, les tours CLI avec outils sont refusés. `eyas doctor` le signale sur sa ligne **CLI sandbox**. Voir [Fournisseurs — Sandbox de fichiers du noyau](/docs/fr/ai/providers/#kernel-file-sandbox).

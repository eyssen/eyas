...
**À quoi ça sert.** Troisième chemin : chart Helm pour un vrai cluster (testé sur OCI OKE). PVC pour `data/`, Ingress, probes, secrets en objets Kubernetes. Les YAML bruts en haut de `deploy/k8s/` sont **héritage** — en prod, utilise **Helm**.

## Quand l'utiliser

- Plusieurs réplicas ou cluster partagé (quand même **jamais** deux écrivains sur un SQLite).
- Ingress + StorageClass (`oci-bv` sur OKE).

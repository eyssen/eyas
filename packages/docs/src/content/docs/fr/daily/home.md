---
title: Accueil
description: Votre grille après connexion — pouls, attention, agents en cours, planning et coût d’un coup d’œil.
---

**À quoi ça sert.** Accueil est le premier écran après la connexion : une grille de widgets personnelle qui montre ce qui vous attend, ce qui tourne et ce qui arrive. Tout le monde part de la disposition d’usine. Vous la réarrangez quand les neuf tuiles par défaut ne correspondent plus à votre travail.

## Quand l’utiliser

- Un coup d’œil aux approbations, aux exécutions bloquées et au coût du jour avant d’ouvrir une conversation.
- Mettre en pause ou interrompre un agent en cours sans chercher le Contrôle de mission.
- Le planning du jour, y compris un job dont la dernière exécution a échoué.
- Vous installez encore EYAS — la bande de configuration recommandée parcourt le reste.
- Un tableau ou un briefing sur la même page que le pouls.

## Déroulement typique

1. Ouvrez **Accueil** dans la barre latérale (**Principal**) — route `/`.
2. Lisez **Pouls** et **Nécessite votre attention** (approbations, reprises bloquées, en retard).
3. Cliquez un chiffre pour sa liste, ou **Modifier la page d'accueil** pour réarranger.
4. Cliquez **Terminé**. Les changements s’enregistrent seuls. À la visite suivante, vous devez voir la même disposition.
...
Quand une Grok CLI ou une Kimi Code CLI activée et installée n'est pas connectée pour EYAS, un
bandeau au-dessus de la grille indique *&lt;CLI&gt; doit être connecté pour EYAS* (propriétaires
et administrateurs). Il apparaît en général après une mise à jour, sur une install qui s'appuyait
sur la connexion de la CLI sur l'ordinateur, qu'EYAS n'utilise plus. **Se connecter** ouvre la
carte de connexion sur place ; **Ignorer** masque le bandeau pour cette session du navigateur.
Voir [Fournisseurs — Connecter Grok et Kimi pour EYAS](/docs/fr/ai/providers/#sign-in-grok-and-kimi-for-eyas).

---
title: Recherche
description: Cherchez tout ce qu’EYAS a indexé — et épinglez quel arbre de code un agent peut utiliser.
---

**À quoi ça sert.** La recherche a deux métiers. La requête de la barre supérieure trouve des hits sur le tableau, la mémoire, les documents et le code. **Sources de recherche** est où vous enregistrez ces arbres (un checkout Odoo par source) et les réindexez. Les épingles de conversation et de projet décident quel arbre un agent peut chercher, pour que les versions ne se mélangent pas en silence.

## Quand l’utiliser

- Un fichier, une note de mémoire ou un document, sans savoir quel module le tient.
- Ajouter un checkout Odoo (ou n’importe quelle base de code) pour que les agents citent au lieu de deviner.
- Plusieurs versions Odoo sont indexées et il faut épingler 18c vs 18e sur une conversation ou un projet.
- Un agent a renvoyé `needsPin` — choisir des sources avant qu’il cherche.
- Réindexer après un changement de l’arbre sur disque.

## Déroulement typique

1. Tapez dans la barre de recherche du shell (*Search across all indexed sources…*) ou ouvrez les résultats — c’est la recherche globale.
2. Pour ajouter un arbre : **Paramètres → Sources de recherche** (barre latérale **Paramètres**, groupe **Modules**) — route `/search-sources`.
3. **Add Source** (une racine absolue, **Label** p. ex. `18c`, **Family :** `odoo`) → **Create Source** → **Reindex** jusqu’à **ready**.
4. Sur le projet, cochez **Default code sources** ; ouvrez une conversation et confirmez l’onglet **Sources**. Les hits et citations doivent rester dans ces arbres.
...
- [Élément de configuration Accueil : Répertoires à indexer](/docs/fr/daily/home/)

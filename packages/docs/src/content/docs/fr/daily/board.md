---
title: Tableau
description: Déplacez les conversations entre étapes — kanban, liste, chronologie, graphe et tableau de bord.
---

**À quoi ça sert.** Le Tableau est la surface de travail des conversations. Chaque carte est un fil : vous la glissez entre étapes, filtrez par projet et l’ouvrez pour parler. Kanban, liste, chronologie, graphe et tableau de bord lisent le même ensemble filtré.

## Quand l’utiliser

- Voir tous les fils ouverts d’un projet en colonnes (ou tous les projets d’un coup).
- Faire avancer le travail, épingler une carte, ou repérer les en retard / bloqués / en attente d’approbation.
- Une liste triable, une chronologie d’échéances, ou le graphe d’une exécution d’orchestration.
- Le débit du jour, le coût et les exécutions live sans quitter le tableau.
- Démarrer une conversation dans un projet et une étape donnés.

## Déroulement typique

1. Ouvrez **Tableau** dans la barre latérale (**Principal**) — route `/board`.
2. Choisissez un projet (ou **All projects**). Passez **Kanban** / **List** / **Timeline** / **Graph** / **Dashboard**.
3. Glissez une carte vers une nouvelle étape, ou cliquez pour ouvrir la conversation.
4. La carte doit être dans la nouvelle colonne (et le champ **Stage** de la conversation doit correspondre). Les filtres restent au changement de vue.
...
| **N% contexte** | Le remplissage de la fenêtre de contexte du modèle — la même mesure que la barre de contexte de la conversation : la taille de prompt que le fournisseur a signalée pour le dernier appel au modèle quand elle existe, sinon une estimation (prompt système plus historique), rapportée à la fenêtre propre du modèle qui répond. Les couleurs suivent le thème |
...
**Les étiquettes sont un filtre du tableau, pas un arbre de projets.** Les noms de catégorie tels que `module` et `area` sont des exemples que tu crées sur cette instance ; les valeurs vivent sur la conversation. Choisis un projet, puis une étiquette, pour découper le travail dans le projet — pas via un sous-projet. Les étiquettes de conversation apparaissent aussi comme une ligne `tags: …` dans le suffixe du prompt (pas dans le préfixe de cache).

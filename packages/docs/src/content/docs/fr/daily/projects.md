...
- Un nouveau corpus de travail avec son agent par défaut et (pour Odoo) ses arbres de code par défaut.
- Un type réutilisable (priorité, icône, prompt) pour que les nouveaux projets démarrent pareil.
- Ajouter, plier ou fermer une étape que chaque tableau montrera.
- Les conversations créées dans ce projet doivent hériter des sources indexées.
...
2. Créez un **Project Type** si un modèle est nécessaire, puis **New Project** (nom, type, agent par défaut, **Default code sources** optionnel).
3. Sous **Stages**, ajoutez ou réordonnez les colonnes (**Closed**, **Folded**, **Bot**, **Auto-assign**).
4. Ouvrez **Tableau**, choisissez le projet — ces étapes doivent être les colonnes, et une nouvelle conversation doit épingler les mêmes sources.

...
Routage d’agents (enregistré par le module agent, non dupliqué ici) :

| Outil | Rôle | Risque |
|-------|------|--------|
| `run_specialist` | Lance un spécialiste activé et attend le résumé. Alias : `delegate_to_agent`. | vert |
| `handoff_to_colleague` | Ouvre le fil d’accueil d’un autre collègue et le démarre. | vert |
| `assign_task` | Carte asynchrone du tableau pour un agent activé. | vert |
| `propose_team` | Carte pour rôles manquants, travail épique, ou demande explicite d’équipe. | jaune |
| `propose_agent_creation` | Propose un modèle de spécialiste. | jaune |

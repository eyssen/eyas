...
2. Connectez OpenCode **pour EYAS** (le propriétaire ou un administrateur) : ouvrez une conversation, cliquez sur l’icône terminal, puis utilisez `/connect` dans le terminal OpenCode (voir [Connexion](#sign-in)).
...
| Terminal web | `@xterm/xterm` sur `/api/v1/opencode/terminal/:id` (JWT). Réservé au propriétaire et aux administrateurs (le droit de gestion d’OpenCode — voir [Qui peut ouvrir un terminal](#who-can-open-a-terminal)). La déconnexion tue le PTY |
...
OpenCode se connecte lui-même à ses fournisseurs de modèles ; EYAS ne glisse pas de clés API dans ce processus. La connexion vit désormais dans le dossier EYAS : **les utilisateurs existants d’OpenCode sont donc déconnectés une fois**. Le propriétaire ou un administrateur ouvre le terminal OpenCode d’une conversation et utilise `/connect` (voir [Qui peut ouvrir un terminal](#who-can-open-a-terminal)). N’utilisez pas `opencode auth login` dans un terminal à vous, hors d’EYAS : il utilise votre environnement normal et connecterait votre OpenCode de tous les jours, pas celui d’EYAS.

### Qui peut ouvrir un terminal {#who-can-open-a-terminal}

Dans le terminal OpenCode, c’est vous qui approuvez les appels d’outils d’OpenCode — le portail de sécurité d’EYAS ne répond qu’aux tâches qu’EYAS lance — et OpenCode n’a pas de bac à sable noyau : le terminal est donc en pratique un shell sur le serveur, qui tourne sous son utilisateur système. C’est pourquoi tout terminal est réservé au propriétaire et aux administrateurs, comme l’était déjà le shell simple.

| Droit sur OpenCode | Rôles par défaut | Ce qu’il permet |
|--------------------|------------------|-----------------|
| read (lecture) | owner, admin, user | La page OpenCode (état, modèles, réglages enregistrés) ; lister et fermer ses propres terminaux |
| create (création) | owner, admin, agent | L’usage headless : l’API mémoire EYAS qu’appelle le plugin d’OpenCode, pour un appelant connecté |
| manage (gestion) | owner, admin | Enregistrer les réglages (**Modèle et raisonnement**), et ouvrir un terminal ou s’y connecter — le terminal OpenCode et le shell simple |

- Sans le droit de gestion, `POST /api/v1/opencode/sessions` répond `403` pour les deux types, et le socket du terminal (`/api/v1/opencode/terminal/:id`) refuse la connexion. La barre du haut d’une conversation n’affiche l’icône terminal qu’à qui le serveur autorise les terminaux ; si le serveur refuse malgré tout, le panneau du terminal le dit.
- Le droit est vérifié avec le rôle et le statut actuels de l’utilisateur à chaque connexion du socket, puis tant qu’elle reste ouverte : avant chaque frappe, redimensionnement ou ping envoyé par le terminal, toutes les 15 secondes, et aussitôt que le propriétaire ou un administrateur change le rôle de la personne, la suspend ou l’archive. Un terminal dont l’utilisateur a perdu le droit prend fin — le panneau dit pourquoi, la connexion se ferme et le processus du terminal est arrêté — et on ne peut pas s’y reconnecter.
- Les tâches headless ne changent pas : l’`opencode_run` d’un collègue tourne toujours dans la conversation de n’importe quel utilisateur, car chaque appel d’outil qu’il contient interroge le portail de sécurité d’EYAS ([Les tâches headless demandent à EYAS](#headless-tasks-ask-eyas)).
- `GET /api/v1/opencode/access` (droit de lecture) renvoie `{terminal, settings}` : ce que permettent les propres droits de l’appelant. La page web s’en sert pour afficher ou non l’icône terminal.

Les droits des rôles intégrés sont fixés dans EYAS ; aucun réglage ne donne au rôle user le droit de gestion sur OpenCode. Pour qu’une personne utilise le terminal, donnez-lui le rôle admin (`PATCH /api/v1/users/<id>` avec `{"role": "admin"}`, par le propriétaire ou un administrateur) — ce qui lui donne aussi tous les autres droits d’administrateur.
...
**Pas un bac à sable pour son propre utilisateur.** Les dossiers ci-dessus décident seulement où le terminal démarre. Ce qui y tourne tourne sous l’utilisateur système du serveur EYAS : une commande que vous approuvez dans le terminal OpenCode (une commande shell, ou l’accès à un dossier hors du dossier de la tâche) atteint tout ce que cet utilisateur atteint, y compris d’autres dossiers du serveur. C’est pourquoi tout terminal est réservé au propriétaire et aux administrateurs — voir [Qui peut ouvrir un terminal](#who-can-open-a-terminal). Le shell simple de l’API des sessions (`POST /api/v1/opencode/sessions` avec `kind: "shell"` ; la page web n’ouvre que le terminal OpenCode) démarre avec l’environnement du terminal OpenCode et le dossier personnel propre à EYAS, pas avec l’environnement du serveur : la clé maîtresse d’EYAS et les clés API des fournisseurs n’y sont donc pas.
...
- Les versions précédentes laissaient le rôle user ouvrir le terminal OpenCode (le droit de création sur OpenCode). Désormais tout terminal exige le droit de gestion : le propriétaire et les administrateurs le gardent, un user perd l’icône terminal et reçoit `403`. Les tâches `opencode_run` des collègues dans les conversations de ce user tournent comme avant. Il n’y a rien à migrer — les droits des rôles intégrés ne sont pas stockés — et aucun réglage ne rétablit l’ancien défaut ; donnez le rôle admin à qui a besoin du terminal (voir [Qui peut ouvrir un terminal](#who-can-open-a-terminal)).

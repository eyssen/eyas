...
| Working directories | Dossiers nommés (nom + chemin absolu) lecture/écriture ; le premier est cwd. Type et/ou projet ; la conversation hérite. Les outils fichier y sont enfermés |
| Plan d’abord | Mode du compositeur : le modèle écrit un plan et attend **Approuver** / **Ignorer le plan** / **Rejeter** avant les outils |
| Skill import roots | `skills.importRoots` / `agent.importRoots` dans `local.yaml`. Défaut vide. L’isolation reste active |

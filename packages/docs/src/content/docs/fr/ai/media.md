---
title: Médias
description: Connectez Magnific, Higgsfield, fal ou HeyGen. Les agents génèrent avec cinq outils partagés. Comparez les backends et choisissez-en un — ou plusieurs.
---

**À quoi ça sert.** Médias, c’est la façon dont EYAS génère, agrandit et attend images, vidéo, audio, retouches et 3D. Vous choisissez les backends ; l’agent utilise **un seul jeu d’outils**. Aucun des quatre vendeurs n’est le défaut. Zéro connecté = vide, fail-closed — jamais de pixels fictifs.
...
- Vous avez un compte Magnific, Higgsfield, fal ou HeyGen et **ne** voulez **pas** cinquante outils vendeur sur le modèle.
...
2. Lisez **Quel backend ?** sur la page, puis **Connecter** un (ou plusieurs). Magnific, Higgsfield et HeyGen : OAuth dans le navigateur ; fal : clé API.
...
| Critère | Magnific | Higgsfield | fal | HeyGen |
|---------|----------|------------|-----|--------|
| **Point fort** | Stills photoréalistes, upscale **Creative** guidé par prompt, upscale fidèle **Precision** | Vidéo cinématique, consistance de personnage (Soul) | Catalogue énorme, vérification du prix avant l’exécution | Avatars talking-head ; vidéo présentateur à partir d’un prompt ou d’un script |
| **Types dans EYAS** | Agrandissement, image, édition (aussi vidéo / audio / 3D) | Vidéo, image (aussi audio) | Image, vidéo, audio, 3D, agrandissement | Vidéo, audio |
| **Connexion** | OAuth (compte Magnific) | OAuth (compte Higgsfield) | Clé API Bearer (`fal-api-key`) | OAuth (compte HeyGen) |
| **Crédits** | Le même solde que le site Magnific. L’**Unlimited web ne s’applique pas** à MCP/API | MCP **débite toujours** des crédits, même si le plan web est illimité | MCP lui-même est gratuit ; vous payez l’exécution du modèle | MCP débité sur le **plan web** HeyGen, pas le solde REST API séparé |
| **Résultat** | URL CDN — EYAS copie les octets | Les URL expirent en environ **sept jours** — l’ingest est obligatoire | URL CDN — copiée quand même | URL CDN — EYAS copie les octets |
| **Le brancher d’abord si…** | Vous agrandissez, retouchez ou voulez des stills | Vous voulez des clips ou un personnage figé | Vous voulez beaucoup de modèles ou le prix d’abord | Vous voulez un présentateur, un jumeau numérique ou un avatar scripté |

**Recommandation**

1. **Branchez un backend pour le travail que vous avez vraiment.** Stills et agrandissement → Magnific. Clip cinématique / personnage → Higgsfield. Talking-head / présentateur → HeyGen. Large catalogue ou « ça coûte combien ? » → fal.
...
Défauts suggérés (seulement si ce fournisseur est connecté et que la ligne n’est pas épinglée) : agrandissement / image / édition → Magnific ; vidéo → Higgsfield (talking-head : épingler **heygen**) ; audio / 3D → fal.

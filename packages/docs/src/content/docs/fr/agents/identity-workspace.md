...
description: Éditer IDENTITY, AGENTS, TOOLS et vos notes MEMORY — et restaurer un instantané si besoin.
---

**À quoi ça sert.** Les fichiers workspace sont la prose durable de l’agent : qui il est, comment il se relie à l’équipe, comment utiliser les outils — plus un fichier **Mémoire** pour vos propres notes sur l’agent. Plus profond que le formulaire Configuration. Ce dont EYAS se souvient n’est pas ici : EYAS enregistre la mémoire lui-même et l’affiche sur la page [Mémoire](/docs/fr/knowledge/memory/). Si l’autonomie interdit l’auto-mise à jour, les changements d’identité passent par [Forge](/docs/fr/agents/forge/), pas par une réécriture silencieuse.
...
| **Mémoire** (`MEMORY`) | Vos propres notes sur cet agent. EYAS n'envoie pas ce fichier au modèle et les agents ne peuvent pas l'écrire. Ce dont EYAS se souvient se trouve sur la page [Mémoire](/docs/fr/knowledge/memory/). |

Choisir **Mémoire** dans le sélecteur de fichier affiche cette indication : *Vos propres notes sur cet agent. EYAS n'envoie pas ce fichier au modèle et les agents ne peuvent pas l'écrire. Ce dont EYAS se souvient se trouve sur la page Mémoire.* Vous pouvez toujours l'éditer, l'enregistrer et consulter son historique, comme avant.

### Ce que les agents peuvent éditer eux-mêmes

Les outils `workspace_append` et `workspace_edit` n'éditent que **Description de l'équipe** (`AGENTS.md`) et **Outils** (`TOOLS.md`). Les agents ne peuvent plus écrire le fichier Mémoire (`MEMORY.md`) ni les fichiers de mémoire quotidiens (`memory/YYYY-MM-DD.md`) : EYAS enregistre la mémoire automatiquement, et les agents n'écrivent jamais la mémoire eux-mêmes.

...
**À quoi ça sert.** L’onglet **Configuration** est l’identité SQL : nom, rôle, modèle, effort, outils, contraintes, budget mensuel de tokens. Les fichiers workspace et la voix sont d’autres onglets. C’est ce que vous remplissez à la création, et ce que vous changez quand le métier bascule.

## Quand l’utiliser

- Vous créez un agent : nom, type, modèle, liste d’outils.
- Un agent de code doit avoir `read_file` / `edit_file` / `grep` sans CLI.
- Un plafond mensuel de tokens, ou `0` = illimité.
- Telegram (ou un autre canal) entrant doit aboutir ici.
- Le prompt coach doit resserrer le system prompt — pas la voix, pas le domaine projet.

## Déroulement typique

1. Ouvrez **Agents** → l’agent (ou **Create Agent**) — route `/agents/:id`, onglet **Configuration**.
2. Remplissez **Name**, **Role**, **Tier**, **Agent Type**, **Model** (ou **Auto (routing decides)**), **Tools**, **Constraints**.
3. **Monthly Token Budget** si un plafond est voulu. Sur **Channels**, liez un canal si l’inbound doit atterrir ici.
4. **Save Changes**. Une nouvelle conversation assignée à cet agent doit utiliser ce modèle, ces outils et ce prompt.
...
| Champ | Signification |
|-------|---------------|
| **Niveau** | Principal / Équipe = collègues à qui vous parlez ; Spécialiste = pool partagé (voir [vue d'ensemble](/docs/fr/agents/overview/)) |
...
L’**Assistant personnel** (principal, type assistant) coordonne — ne lui donnez pas `write_file` / `edit_file` / `run_command`. L’**Ingénieur système** et les spécialistes de code possèdent ces outils. Voir [Équipes et délégation](/docs/fr/agents/teams/).

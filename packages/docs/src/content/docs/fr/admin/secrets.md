...
La clé API de Grok CLI vit ici sous `grok-cli-api-key` (portée **Système**) quand tu choisis **Utiliser plutôt une clé d’API** sur la carte de connexion de Grok CLI. EYAS ne la transmet qu’aux exécutions de Grok CLI, en `XAI_API_KEY` ; tu peux aussi l’ajouter ici sous ce nom, et **Se déconnecter** sur le panneau du fournisseur la supprime. Voir [Fournisseurs — Connecter Grok et Kimi pour EYAS](/docs/fr/ai/providers/#sign-in-grok-and-kimi-for-eyas). Les connexions par code d’appareil de Grok et Kimi ne sont pas des secrets de ce magasin : ce sont des fichiers dans les homes CLI propres à EYAS.

...
Lisez ceci une fois après [Votre première heure](/docs/fr/first-hour/). Revenez quand un chapitre ultérieur emploie un mot que vous ne reconnaissez pas. C’est le modèle mental, pas un guide écran par écran.
...
| **Collègue** | Agent principal ou d’équipe à qui vous parlez — fil d’accueil, SOUL. L’installation en crée deux (Assistant personnel + Ingénieur système). | Barre **Collègues** / Agents |
| **Spécialiste** | Travailleur étroit d’un pool partagé ; tout collègue peut lancer un spécialiste activé (`run_specialist`) | Agents (niveau : Spécialiste) |
...
3. Le collègue peut utiliser des **outils/compétences**, écrire dans la **mémoire**, **passer la main / lancer un spécialiste**, ou répondre sur un **canal**  
...
| **Markdown du coffre** | Import / agents / vous / **auto-capture après un tour de conversation** (activée par défaut depuis 0.8.16-beta) | Notes sémantiques et procédurales durables |
| **Base de connaissances** | Vous (éditeur) | Wiki organisé |
| **Documents** | Téléversement | PDF, fichiers bureautiques, dumps de sources |

Un fait durable énoncé dans le chat peut devenir une note du coffre sans que personne le demande. La capture s’exécute après la livraison de la réponse ; une capture ratée coûte une note, jamais la réponse. Détails : [Mémoire](/docs/fr/knowledge/memory/).
...
|----------|---------------|
| **Modèle** | Quel modèle répond : un modèle fixe, le routage automatique, ou le modèle par défaut du collègue |
| **Effort** | Profondeur de raisonnement vs coût/vitesse. Ne liste que les niveaux que propose le modèle de la conversation ; **Auto** hérite (Deep → Max, l’effort du collègue, celui de la conversation qui délègue) ou utilise le défaut du modèle |
| **Orchestration : Solo** | Ni spécialistes, ni passations, ni propositions d’équipe |
| **Orchestration : Auto** | Le modèle décide du déploiement |
| **Orchestration : Deep** | Déploiement multi-agent agressif ; effort par défaut Max |
...
- [Votre première heure](/docs/fr/first-hour/)

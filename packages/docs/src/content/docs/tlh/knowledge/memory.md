...
3. ~40 pongmey tlhoS ja'chuq nI' wanI' ja'. jang ret naDev chegh — chu' vault ghItlhHom (`user`, `feedback`, `project`, pagh `reference`).
...
| **jeSwI' wiki** | wa' jeSwI' nob ghItlhHommey, qo' qawHaqbe'. |
...
cha' frontmatter Dochmey SeH: `kind` (`user`, `feedback`, `project`,
`reference` — 'ej mIw tlhegh je) 'ej `summary` (tetlhDaq Dov). `user`
`feedback` je nIH. `kind` Hutlhchugh: `procedural/` bIngDaq `feedback`,
...
DaqmeyDaq: `data/vault/semantic|procedural|projects/`.
...
- [jeSwI' wiki](/docs/tlh/knowledge/client-wiki/)

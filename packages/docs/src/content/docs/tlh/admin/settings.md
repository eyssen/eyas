---
title: SeHmey Del
description: pat potlh — qab, Hol, navmey, Hemey.
---

**Qu' 'oH.** SeHmey (`/settings`) pat potlh 'oH: qab, Hol, pat nobmey, Qun mIw tetlh, 'ej tlhop ghommey latlh admin navmey poSmoHbogh. mI'mey 'ej pat De' navvamDaq yIn. [Qum chu'](/docs/tlh/admin/notifications/), [chelmeH janmey](/docs/tlh/admin/extensions/), [hopbogh Daqmey](/docs/tlh/admin/nodes/), [ghopmey](/docs/tlh/admin/hands/) je navchaj 'oH, tlhopvo' He — naDev juHbe'.

**He:** `/settings`.

## mI'mey

nobwI'pu' QaptaH/Hoch · patmey chu'lu'/Hoch · pegh mI' · lo'wI' mI'.

## nobwI' Del

nobwI' tetlh, QaptaH cha' 'ej pat mI' tlhej (naQ SeH nobwI' navDaq).

## pat De'

| mIw | Del |
|-----|-----|
| **mI'** | EYAS mI' |
| **Dotlh** | yIn |
| **QapmeH poH** | Bun |
| **De' qawHaq** | SQLite (WAL, `synchronous = NORMAL`) |

## navvamDaq navmey

| nav | Qu' |
|-----|-----|
| **pat chu'choH** | GitHubvo' chu'choH yIchov / yIlo' |
| **De' lojmIt** | lI' taghwI' ([De' lI'](/docs/tlh/admin/data-port/)) |
| **qab** | qab pat + wov / Hurgh |
| **Hol** | en / hu / de / es / fr / tlh |
| **pat nobmey** | ghoqwI' pat wIvmey |
| **Qun mIw** | cha'–vagh pat wa' Qu' qaD, quS, Huch 'aqroS, pa' qaw. yIlegh [ja'chuqmey — Qun mIw](/docs/tlh/daily/conversations/#qun-miw). |
| **ghom ghoqwI'pu'** | tejwI' wIv |
| **nIteb laHmey** | laH wIvmey |

## tlhop SeHmey ghommey

| ghom | Hemey |
|------|-------|
| motlh | pat, lo'wI'pu', API peghmey, peghmey |
| AI 'ej pat | nobwI'pu', mu'tlheghmey, qawHaq, MCP |
| patHommey | Qu'mey, ghItlhmey, nej Halmey, [Qum chu'](/docs/tlh/admin/notifications/) (`/notifications-settings`), tlhop, 'egh ghoj, [chelmeH janmey](/docs/tlh/admin/extensions/) (`/extensions`) |
| rarmey | [rarmey](/docs/tlh/admin/connections/) (`/connections`) — Hur pat tetlh |
| pat chen | [ghopmey](/docs/tlh/admin/hands/) (`/hands`), [Ingress](/docs/tlh/admin/ingress/), [hopbogh Daqmey](/docs/tlh/admin/nodes/) (`/nodes`), qon, ghommey |

## latlh

- [nobwI'pu'](/docs/tlh/ai/providers/)
- [nIteb Qap](/docs/tlh/agents/autonomy/)
- [rarmey](/docs/tlh/admin/connections/)
- [Qum chu'](/docs/tlh/admin/notifications/)
- [chelmeH janmey](/docs/tlh/admin/extensions/)
- [hopbogh Daqmey](/docs/tlh/admin/nodes/)
- [ghopmey](/docs/tlh/admin/hands/)

...
mInDu': jan mI' + chaw' poQ mI'. Seghmey `system`/`file`/`network`/`compute`/`data`. tetlh QIH **green / yellow / red**. ghItlh Daq (`read_file`, `edit_file`, `grep`, `glob`, `run_command`), nej/`needsPin`, qawHaq 'ay'mey + `search_memory`/`save_memory`, QIn draft→chaw'→ngeH, Odoo chaw', rar tetlh. **nagh beQ** (chaw'): `media_generate`, `media_wait`, `media_catalog`, `media_balance`, `media_history` — [nagh beQ](/docs/tlh/ai/media/). jIH Qum chIch jan 'oHbe': Recordly AGPL tlhej [cheltaHghachmey](/docs/tlh/admin/extensions/#recordly) — `recordly_*` tu'lu'be'. CLI MCP rap: [MCP](/docs/tlh/ai/mcp/).

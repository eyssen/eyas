...
mInDu': jan mI' + chaw' poQ mI'. Seghmey `system`/`file`/`network`/`compute`/`data`. tetlh QIH **green / yellow / red**. ghItlh Daq (`read_file`, `edit_file`, `grep`, `glob`, `git_status`/`git_diff`, `run_command` — He'mey **vum pa'mey**Daq ngaQ). `run_command`/`Bash` `git status` pagh `git diff` (metachar Hutlh, `-C`/`--git-dir` Hutlh, naQ He Hutlh) SuD janvaD choH — **qIH Hutlh**. `git commit` / `git add` / `ls` Doq taH. nej/`needsPin`, qawHaq 'ay'mey + `search_memory`/`save_memory` (`scope` motlh `current`: Qu'vam + Segh + Hoch user/feedback/reference; `all` = latlh Qu'mey; `search_memory` latlh ja'chuq QInmey (user + assistant) Qu'vamDaq nej je; `scope=all` Qu'mey tetlh; qawHaq jaj nej pe'be'), QIn draft→chaw'→ngeH, Odoo chaw', rar tetlh. **nagh beQ** (chaw'): `media_generate`, `media_wait`, `media_catalog`, `media_balance`, `media_history` — [nagh beQ](/docs/tlh/ai/media/). jIH Qum chIch jan 'oHbe': Recordly AGPL tlhej [cheltaHghachmey](/docs/tlh/admin/extensions/#recordly) — `recordly_*` tu'lu'be'. CLI MCP rap: [MCP](/docs/tlh/ai/mcp/).

ghoqwI' He (ghoqwI' pat cher; naDev rurbe'):

| jan | meq | QIH |
|-----|-----|-----|
| `run_specialist` | chu'bogh laHwI' chenmoH 'ej Del loS. Alias: `delegate_to_agent`. | SuD |
| `handoff_to_colleague` | latlh jup home thread poSmoH 'ej tagh. | SuD |
| `assign_task` | poH Hutlh Qu' nav chovnatlh. | SuD |
| `propose_team` | Hutlh Segh / Qu' tIn / ghom poQ chaw'. | SuD (yellow) |

...
2. OpenCode **EYASvaD** yI'el: ja'chuq yIpoSmoH, QonoS mI' yIchu', 'ej OpenCode QonoSDaq `/connect` yIlo' (['el](#sign-in)).
...
| QonoS | `@xterm/xterm`, `/api/v1/opencode/terminal/:id` (JWT). rarHa'DI' PTY Hegh |
...
pat nobwI'meyDaq 'el'egh OpenCode; OpenCodeDaq API ngoqmey ngeHbe' EYAS. DaH EYAS pa'Daq 'el yIn, vaj **OpenCode lo'wI'pu' tu'lu'bogh wa'logh mejmoHlu'**. ja'chuq OpenCode QonoS yIpoSmoH 'ej `/connect` yIlo'. shell QonoS naDev `opencode auth login` yIlo'Qo': shellvetlh motlh envlIj lo', vaj jaj OpenCodelIj 'elmoH, EYAS OpenCode ghobe'.
...
**Qu' pa'.** ja'chuq qoDDaq neH Qap `opencode_run`, 'ej latlh DaqDaq lajQo'. Daponglu'bogh pa' ja'chuq pa'mey qoDDaq tu'lu'nIS. pa' Hutlhchugh, ja'chuq wa'DIch pa' lo' EYAS, pagh ja'chuq vum pa''egh — EYAS lIng pa' not. OpenCode QonoS ja'chuq vum pa'Daq cheghqa' rap. latlh Qap pa'mey chov rap Suq QonoS pa'mey: Hublu'bogh Daq 'oHbogh, Hublu'bogh DaqDaq tu'lu'bogh pagh Hublu'bogh Daq ngaSbogh pa' (EYAS De''egh, latlh AI jan pol Daq, ghItlhHom vault, pagh juH pa'lIj) chIllu', chaw'lu'bogh veb pa'Daq pagh ja'chuq vum pa'Daq poSchoH QonoS, 'ej chIllu'bogh pa' ponglu' QonoS Dung tlhegh.

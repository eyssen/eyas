...
**De' tetlh:** wa' tetlh skill id Hoch. patlh **User > Generated > Imported (`skills.importRoots`) > Bundled (cheltaHghach) > Bundled (EYAS)**. host laHmey `settingSources` chu'qa'meH 'elbe'. latlh pa'mey `local.yaml`Daq (`skills.importRoots` / `agent.importRoots`, motlh **tetlh chIm**). tu'wI' **chup neH**: Hubbe', So'lu', lo'lu'pu'be' (0, 90 jaj tIQ), Qong (180+). [SeH'egh](/docs/tlh/agents/autonomy/). **chu'Ha', teqbe'.** Hubbe'/So'lu' DaH; lo'lu'pu'be'/Qong 30 jaj; lo'wI' laHmey poH chutmeyvo' tlhejbe'.

...
| ngaSwI' logh | SQLite DB, toDpa', ghoqwI' teywI'mey `data/` bIngDaq; ja'chuq vum pa'mey (bIng) |
...
pat juH bIngDaq (`EYAS_HOME` pagh taghpu'bogh pa'). naDev De' paq `data/` 'oH, `EYAS_DATA_DIR` latlh paq pongbe'chugh — vaj `data/` bIngDaq Hoch paqvetlhDaq yIn:

| He | ngaS |
|----|------|
| `data/sqlite/` | potlh SQLite De' pa' (WAL mIw, `synchronous = NORMAL`) |
| `data/vault/` | QIj / tIgh markdown toDpa' — reH `<data dir>/vault` |
| `data/agents/<id>/` | ghoqwI' vum Daq teDwI'mey (IDENTITY, SOUL, …) |
| `data/backups/` | qon ngaSwI'mey |
| `config/` | YAML motlh + juH chel |

ja'chuq vum pa'mey — pa'mey Daj ghajbe'bogh Hoch ja'chuq vumbogh paq — Docker, Kubernetes, ngaSlu'bogh lIngmeyDaq `data/workspaces/`Daq tu'lu'. git clonevo' Qapbogh Hal lIng checkout Hurvo' pol, lo'wI' ghun De' paqDaq. [SeH — ja'chuq vum pa'mey](/docs/tlh/deploy/configuration/#conversation-workspaces).

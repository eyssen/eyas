...
Una edición por IA corre en el modelo por defecto de la instalación: el nivel de
enrutado **Standard**, si no el proveedor por defecto, si no el primer proveedor
activo (por orden alfabético) con un modelo activado. Corre como una llamada
aislada sin herramientas. Qué proveedor se registró primero ya no influye.

...
| Working directories | Carpetas con nombre (nombre + ruta absoluta) de lectura/escritura; la primera es cwd. Tipo y/o proyecto; la conversación hereda. Las herramientas de archivo quedan encerradas aquí |
...
| Skill import roots | `skills.importRoots` / `agent.importRoots` en `local.yaml`. Por defecto vacío. El aislamiento sigue activo |

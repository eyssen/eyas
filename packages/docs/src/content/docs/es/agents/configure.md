...
**Classification:** Tier (Primary/Team = colegas; Specialist = pool compartido), Agent Type.  
...
Para coding (cualquier modelo): `read_file, write_file, edit_file, grep, glob, git_status, git_diff, run_command`. El **Asistente personal** coordina — no le des `write_file` / `edit_file` / `run_command`. El **Ingeniero de sistema** y los especialistas de código los poseen. Véase [Equipos y delegación](/docs/es/agents/teams/). Los agentes creados antes de 0.8.6 **no** los heredan solos — hay que añadirlos. Catálogo: [Herramientas](/docs/es/automation/tools/).  

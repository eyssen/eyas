...
**Directorio de datos en el volumen.** Con el montaje por defecto del chart (`/app/data`) no hay nada que definir: la base de datos y el vault de memoria están en el volumen persistente. Si apuntas `EYAS_DATA_DIR` a otro montaje, el vault se mueve allí también — siempre es `<data dir>/vault` — junto con la base de datos. Ver [Configuración — Directorio de datos y vault](/docs/es/deploy/configuration/#data-directory-and-vault).

**Sandbox de archivos del kernel.** La imagen de EYAS no incluye bubblewrap (es LGPL; instalarlo es decisión del operador). Para el sandbox de archivos del kernel de los proveedores CLI, el contenedor necesita bubblewrap (`bwrap`), `socat` para Claude Code y user namespaces sin privilegios — que muchos clústeres y runtimes de contenedores desactivan. Sin ellos, EYAS sigue rechazando cada petición que ve y muestra el sandbox como *No disponible*; con `security.cliSandbox: required`, los turnos de CLI con herramientas se rechazan. `eyas doctor` lo informa en su línea **CLI sandbox**. Ver [Proveedores — Sandbox de archivos del kernel](/docs/es/ai/providers/#kernel-file-sandbox).

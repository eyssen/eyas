...
- Antes llegabas en vivo a la memoria de otra herramienta — un `CLAUDE.md` del host cargado por Claude Code, una raíz de importación dentro de `~/.claude`, un servidor MCP Memory / Qdrant / Obsidian — y ahora EYAS lo rechaza. La importación es la vía autorizada: unidireccional, una vez, con el origen registrado.
...
| Enriquecimiento | **Desactivado por defecto** — una casilla opcional, solo metadatos (kind, resumen, etiquetas, enlaces comprobados), nunca el cuerpo; las notas enriquecidas dicen quién respondió (`enriched_by`) |
...
**El enriquecimiento por modelo está desactivado por defecto.** El paso de revisión lleva la casilla **Enriquecer los metadatos con el modelo**. Sin marcar — el caso normal — toda la importación es determinista y no gasta ni una llamada al modelo; para esas importaciones no cambia nada. Marcada, las notas *sin* tipo declarado reciben un kind sugerido, un resumen, etiquetas y enlaces del **modelo en segundo plano** de EYAS: primero el nivel de enrutado Heartbeat, luego el valor por defecto de la instalación, luego los proveedores de API, luego las CLIs que pueden ejecutarse aisladas. No hay tope de elementos; el coste es tiempo. El cuerpo nunca se reescribe, nada se omite por decisión del modelo, y un elemento etiquetado `contains-secrets` nunca se envía. El panel de resultado indica cuántas notas se enriquecieron.

- **Solo se usa una respuesta aislada.** Una respuesta cuenta únicamente cuando llega del modelo en segundo plano a través de una llamada aislada (un turno, sin herramientas): un proveedor de API, o una CLI que puede ejecutarse aislada. Si no hay ningún modelo así (por ejemplo, una instalación solo con Grok o solo con Kimi), si la llamada falla o vuelve vacía, o si el modelo se niega, la nota conserva exactamente sus metadatos deterministas y se cuenta como **reserva** (fallback) en el panel de resultado.
- **Los enlaces se comprueban.** Un enlace que sugiere el modelo solo se conserva cuando nombra una nota que ya existe en el vault (por nombre de archivo o título) o una nota seleccionada en la misma importación, y se escribe como el nombre de archivo de esa nota. Cualquier otro enlace que proponga el modelo se descarta. Los enlaces que contiene el propio archivo de origen se conservan siempre sin cambios.
- **Las notas enriquecidas dicen quién respondió.** Toda nota enriquecida lleva un campo `enriched_by` en su frontmatter, por ejemplo `enriched_by: {provider: anthropic, model: claude-haiku-4-5, route: tier}`. `provider` es el proveedor que respondió; `model` es el modelo que informó, y se omite cuando el backend no nombró ninguno (una CLI usada con su propio predeterminado); `route` es cómo se eligió el modelo en segundo plano — `tier` (el nivel de enrutado, primero Heartbeat), `default` (el predeterminado de la instalación), `api` (un proveedor de API) o `isolated-cli` (una CLI que puede ejecutarse aislada). El campo se mantiene cuando la nota se reescribe después. Las notas importadas sin enriquecimiento no tienen `enriched_by`.
- El modelo en segundo plano se busca cuando se ejecuta un trabajo, no cuando arranca el servidor, así que el enriquecimiento funciona aunque el módulo de modelos haya pasado a estar disponible después del Data Port.

**Limpiar una importación enriquecida anterior.** Las importaciones enriquecidas antes de esta actualización no llevan `enriched_by` y pueden contener enlaces que nunca se comprobaron. Para rehacer una sin enriquecimiento: abre el trabajo en el panel de resultado o en **Importaciones anteriores** y elige **Revertir esta importación**; después escanea de nuevo el mismo origen e importa con **Enriquecer los metadatos con el modelo** sin marcar. Las notas cuyo cuerpo hayas editado desde entonces se conservan y se informan como omitidas.
...
El importador lee él mismo los almacenes de otras herramientas, no a través de una herramienta del modelo, así que no le afecta la política de memoria del security gate — que rechaza a **todos los modelos** leer y escribir en `~/.claude`, `~/.grok`, las carpetas `ai-memory`, las bóvedas de Obsidian y el resto (ver [Seguridad y privacidad — Memoria fuera de EYAS](/docs/es/admin/security-privacy/#memory-outside-eyas)). Un agente que antes leía un archivo así directamente ahora recibe un rechazo; importa el contenido una vez aquí. Véase [Memoria](/docs/es/knowledge/memory/).

**Esta es la vía de entrada para el contenido que EYAS ya no lee en vivo:**

- **`CLAUDE.md` y skills del host.** Claude Code siempre corre aislado y ya no los carga; impórtalos una vez aquí. Ver [Proveedores — Aislamiento de Claude Code](/docs/es/ai/providers/#claude-code-isolation).
- **Raíces de importación dentro de las carpetas de otra herramienta.** Las entradas de `skills.importRoots` / `agent.importRoots` dentro de `~/.claude`, `~/.grok`, `~/.agents`, una bóveda de Obsidian y similares se omiten al arrancar; importa esa carpeta una vez aquí y luego quita la entrada de `local.yaml`. Ver [Configuración](/docs/es/deploy/configuration/#extra-skill-and-persona-roots).
- **Servidores MCP de almacén de memoria.** Los servidores MCP Memory, Qdrant, Obsidian y MCPVault, y cualquier servidor que apunte a una bóveda o a la memoria de otra herramienta, están bloqueados para todos los modelos; copia esa memoria a EYAS aquí. Ver [MCP](/docs/es/ai/mcp/#memory-store-servers-are-blocked).
- **Agentes importados borrados.** Un agente importado desde `agent.importRoots` que borraste no se vuelve a crear al arrancar; importa aquí su archivo para recuperarlo.
...
**Qué hace la etiqueta.** Una nota, fila episódica o skill etiquetada `contains-secrets` queda fuera de todo lo que el modelo alcanza por sí solo: el índice permanente de memoria, el recall, `search_memory`, la tarea de reflexión, el consolidador nocturno, el emparejador de skills y el prompt de sistema ensamblado. Nunca se entrega al modelo de enriquecimiento opcional y nunca se incrusta. La etiqueta también pasa a lo que EYAS deriva del elemento — su copia en el registro en bruto, los hechos extraídos de él y los resúmenes construidos a partir de él —, así que nada de eso llega tampoco al modelo (ver [Memoria — Los secretos importados quedan fuera del recall](/docs/es/knowledge/memory/#imported-secrets-stay-out-of-recall)). En la página de Memoria la sigues viendo entera. Para abrirla al modelo, pon `memory.recall.includeSecrets: true` en `config/local.yaml` y reinicia — véase [Configuración](/docs/es/deploy/configuration/).
...
| **Enriquecer los metadatos con el modelo** | Desactivado por defecto — opcional, solo metadatos (kind, resumen, etiquetas, enlaces a notas existentes), nunca el cuerpo, nunca un elemento marcado; solo se usa a partir de una respuesta aislada del modelo en segundo plano, si no se cuenta como reserva |

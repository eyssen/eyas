---
title: Media
description: Conecta Magnific, Higgsfield, fal o HeyGen. Los agentes generan con cinco herramientas compartidas. Compara backends y elige uno — o varios.
---

**Para qué sirve.** Media es cómo EYAS genera, amplia y espera imágenes, vídeo, audio, ediciones y 3D. Tú eliges los backends; el agente usa **un solo conjunto de herramientas**. Ninguno de los cuatro proveedores es el predeterminado. Cero conectados = vacío, fail-closed — nunca píxeles inventados.
...
- Tienes cuenta Magnific, Higgsfield, fal o HeyGen y **no** quieres cincuenta herramientas del vendedor en el modelo.
...
1. Abre **Media** (`/media`).
2. Lee **¿Qué backend?** en esa página y **Conecta** uno (o varios). Magnific, Higgsfield y HeyGen: OAuth en el navegador; fal: clave API.
...
| Criterio | Magnific | Higgsfield | fal | HeyGen |
|----------|----------|------------|-----|--------|
| **Mejor en** | Fotos fotorrealistas, upscale **Creative** guiado por prompt, upscale fiel **Precision** | Vídeo cinematográfico, consistencia de personaje (Soul) | Catálogo enorme, comprobar el precio antes de ejecutar | Avatares talking-head; vídeo de presentador desde un prompt o un guion |
| **Tipos en EYAS** | Ampliación, imagen, edición (también vídeo / audio / 3D) | Vídeo, imagen (también audio) | Imagen, vídeo, audio, 3D, ampliación | Vídeo, audio |
| **Acceso** | OAuth (cuenta Magnific) | OAuth (cuenta Higgsfield) | Clave API Bearer (`fal-api-key`) | OAuth (cuenta HeyGen) |
| **Créditos** | El mismo saldo que el sitio Magnific. El **Unlimited web no cubre** MCP/API | MCP **siempre** gasta créditos, aunque el plan web sea ilimitado | El MCP es gratis; pagas la ejecución del modelo | MCP gasta el **plan web** de HeyGen, no el saldo REST API aparte |
| **Resultado** | URL CDN — EYAS copia los bytes | Las URL caducan en unos **siete días** — el ingest es obligatorio | URL CDN — igual se copia | URL CDN — EYAS copia los bytes |
| **Conéctalo primero si…** | Amplías, retocas o necesitas stills | Necesitas clips o un personaje fijo | Quieres muchos modelos o el precio primero | Necesitas un presentador, gemelo digital o avatar con guion |

**Recomendación**

1. **Conecta un backend para el trabajo que realmente tienes.** Stills y ampliación → Magnific. Clip cinematográfico / personaje → Higgsfield. Talking-head / presentador → HeyGen. Catálogo amplio o «¿cuánto cuesta?» → fal.
...
Valores sugeridos (solo si ese proveedor está conectado y no has fijado la fila): ampliación / imagen / edición → Magnific; vídeo → Higgsfield (talking-head: fija **heygen**); audio / 3D → fal.

...
2. Inicia sesión de OpenCode **para EYAS** (el propietario o un administrador): abre una conversación, pulsa el icono de terminal y usa `/connect` en la terminal de OpenCode (ver [Inicio de sesión](#sign-in)).
...
| Terminal web | `@xterm/xterm` sobre `/api/v1/opencode/terminal/:id` (JWT). Solo el propietario y los administradores (el permiso de gestión de OpenCode — ver [Quién puede abrir una terminal](#who-can-open-a-terminal)). Al desconectar se mata el PTY |
...
OpenCode inicia sesión él mismo en sus proveedores de modelos; EYAS no pasa claves de API a ese proceso. El inicio de sesión vive ahora en la carpeta de EYAS, así que **los usuarios existentes de OpenCode quedan desconectados una vez**. El propietario o un administrador abre la terminal de OpenCode de una conversación y usa `/connect` (ver [Quién puede abrir una terminal](#who-can-open-a-terminal)). No uses `opencode auth login` en una terminal propia fuera de EYAS: usa tu entorno normal e iniciaría sesión en tu OpenCode de diario, no en el de EYAS.

### Quién puede abrir una terminal {#who-can-open-a-terminal}

En la terminal de OpenCode apruebas tú mismo las llamadas a herramientas de OpenCode — el gate de seguridad de EYAS solo responde a las tareas que EYAS inicia — y OpenCode no tiene sandbox de kernel, así que la terminal es en la práctica una shell en el servidor que corre como su usuario del sistema operativo. Por eso toda terminal es solo para el propietario y los administradores, como ya lo era la shell simple.

| Permiso sobre OpenCode | Roles por defecto | Qué permite |
|------------------------|-------------------|-------------|
| read (lectura) | owner, admin, user | La página de OpenCode (estado, modelos, ajustes guardados); listar y cerrar tus propias terminales |
| create (creación) | owner, admin, agent | Uso sin interfaz: la API de memoria de EYAS que llama el plugin de OpenCode, para un llamante con sesión iniciada |
| manage (gestión) | owner, admin | Guardar los ajustes (**Modelo y razonamiento**) y abrir una terminal o conectarse a ella — la terminal de OpenCode y la shell simple |

- Sin el permiso de gestión, `POST /api/v1/opencode/sessions` responde `403` para ambos tipos, y el socket de la terminal (`/api/v1/opencode/terminal/:id`) rechaza la conexión. La barra superior de la conversación muestra el icono de terminal solo a quien el servidor deja abrir terminales; si el servidor aun así la rechaza, el panel de la terminal lo dice.
- El permiso se comprueba con el rol y el estado actuales del usuario cada vez que el socket se conecta, y de nuevo mientras siga abierto: antes de cada pulsación, cambio de tamaño o ping que envíe la terminal, cada 15 segundos, y al instante cuando el propietario o un administrador cambia el rol de la persona, la suspende o la archiva. Una terminal cuyo usuario ha perdido el permiso termina —el panel dice por qué, la conexión se cierra y el proceso de la terminal se detiene— y no se puede volver a conectar.
- Las tareas sin interfaz no cambian: el `opencode_run` de un colega sigue corriendo en la conversación de cualquier usuario, porque cada llamada a herramienta dentro de él pregunta al gate de seguridad de EYAS ([Las tareas sin interfaz preguntan a EYAS](#headless-tasks-ask-eyas)).
- `GET /api/v1/opencode/access` (permiso de lectura) devuelve `{terminal, settings}`: lo que permiten los propios permisos del llamante. La página web lo usa para decidir si muestra el icono de terminal.

Los permisos de los roles integrados son fijos en EYAS; ningún ajuste da al rol user el permiso de gestión sobre OpenCode. Para que alguien use la terminal, dale el rol admin (`PATCH /api/v1/users/<id>` con `{"role": "admin"}`, por el propietario o un administrador), que también le da todos los demás permisos de administrador.
...
**No es un sandbox para su propio usuario.** Las carpetas de arriba solo deciden dónde arranca la terminal. Lo que corre en ella corre como el usuario del sistema operativo del servidor de EYAS: un comando que apruebas en la terminal de OpenCode (un comando de shell o el acceso a una carpeta fuera de la carpeta de la tarea) llega a todo lo que llega ese usuario, también a otras carpetas del servidor. Por eso toda terminal es solo para el propietario y los administradores — ver [Quién puede abrir una terminal](#who-can-open-a-terminal). La shell simple de la API de sesiones (`POST /api/v1/opencode/sessions` con `kind: "shell"`; la página web solo abre la terminal de OpenCode) arranca con el entorno de la terminal de OpenCode y la carpeta personal propia de EYAS, no con el entorno propio del servidor, así que ni la clave maestra de EYAS ni las claves de API de los proveedores están en ella.
...
- Las versiones anteriores dejaban que el rol user abriera la terminal de OpenCode (el permiso de creación de OpenCode). Ahora toda terminal necesita el permiso de gestión: el propietario y los administradores lo conservan, y un user pierde el icono de terminal y recibe `403`. Las tareas `opencode_run` de los colegas en las conversaciones de ese user corren como antes. No hay nada que migrar — los permisos de los roles integrados no se guardan — ni ningún ajuste que devuelva el valor por defecto anterior; a quien necesite la terminal, dale el rol admin (ver [Quién puede abrir una terminal](#who-can-open-a-terminal)).

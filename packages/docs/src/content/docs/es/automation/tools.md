...
Cabecera: recuento y cuántas **requieren aprobación**. Categorías `system`/`file`/`network`/`compute`/`data`. El catálogo imprime riesgo **green / yellow / red**. Superficie de código (`read_file`, `edit_file`, `grep`, `glob`, `git_status`/`git_diff`, `run_command` — rutas encerradas en las **carpetas de trabajo**). Un `run_command`/`Bash` que es inequívocamente `git status` o `git diff` (sin metacaracteres, sin `-C`/`--git-dir`/`--no-index`, sin ruta absoluta) se remapea a las herramientas verdes **sin clic**. `git commit` / `git add` / `ls` siguen en rojo. búsqueda/`needsPin`, bloques de memoria + `search_memory`/`save_memory` (`scope` por defecto `current`: este proyecto + tipo + notas globales; `all` = otros proyectos; `search_memory` también busca mensajes de conversaciones anteriores (user + assistant) en el proyecto actual; `scope=all` cruza proyectos; la página Memoria no filtra), email draft→approve→send, Odoo opcional, inventario de conexiones. **Media** (opcional): `media_generate`, `media_wait`, `media_catalog`, `media_balance`, `media_history` — [Media](/docs/es/ai/media/). El pulido de captura de pantalla no es un tool: Recordly es compañero AGPL en [Extensiones](/docs/es/admin/extensions/#recordly) — no hay `recordly_*`. Paridad CLI MCP: [MCP](/docs/es/ai/mcp/).

Enrutado de agentes (lo registra el módulo agent, no se duplica aquí):

| Herramienta | Propósito | Riesgo |
|-------------|-----------|--------|
| `run_specialist` | Lanza un especialista activo y espera el resumen. Alias: `delegate_to_agent`. | verde |
| `handoff_to_colleague` | Abre el hilo de inicio de otro colega y lo arranca. | verde |
| `assign_task` | Tarjeta asíncrona del tablero para un agente activo. | verde |
| `propose_team` | Tarjeta si faltan roles, el trabajo es épico, o pediste un equipo. | amarillo |
| `propose_agent_creation` | Propone una plantilla de especialista nueva. | amarillo |

Véase [Equipos y delegación](/docs/es/agents/teams/).

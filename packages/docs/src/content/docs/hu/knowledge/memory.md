...
3. Folytass egy ~40 karakternél hosszabb beszélgetést, ami tartós tényt mond. A válasz után ide vissza: új vault-jegyzetet kell látnod (`user`, `feedback`, `project` vagy `reference`).
...
| **Ügyfél wiki** | Ügyfél-specifikus delivery jegyzetek, nem globális memória. |

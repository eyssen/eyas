...
- Korábban élőben értél el egy másik eszköz memóriáját — a Claude Code által betöltött host `CLAUDE.md`-t, egy `~/.claude`-on belüli import-gyökeret, egy Memory / Qdrant / Obsidian MCP szervert —, és az EYAS ezt most elutasítja. Az import a jóváhagyott út befelé: egyirányú, egyszeri, az eredet rögzítésével.
...
| Dúsítás | **Alapból kikapcsolva** — opcionális kapcsoló, csak metaadat (kind, összefoglaló, címkék, ellenőrzött linkek), a törzs soha; a dúsított jegyzetek megmondják, ki válaszolt (`enriched_by`) |
...
**A modell-dúsítás alapból ki van kapcsolva.** Az átnézés lépésben ott a **Metaadatok dúsítása modellel** kapcsoló. Bejelöletlenül — ez az alapállapot — az egész import determinisztikus, és egyetlen modellhívást sem használ; az ilyen importoknál semmi nem változik. Bejelölve a *deklarált típus nélküli* jegyzetek javasolt kind-ot, összefoglalót, címkéket és linkeket kapnak az EYAS **háttérmodelljétől**: először a Heartbeat routing-szint, aztán a telepítés alapértelmezése, aztán az API providerek, végül az izoláltan futni képes CLI-k. Tételkorlát nincs; ideje van ára. A törzs sosem íródik át, a modell szavára semmi nem marad ki, és a `contains-secrets` címkéjű tétel sosem kerül elküldésre. Az eredménypanel mutatja, hány jegyzet dúsult.

- **Csak izolált válasz számít.** Egy válasz csak akkor használható, ha a háttérmodelltől izolált hívásban (egy kör, toolok nélkül) érkezik: API providertől, vagy izoláltan futni képes CLI-től. Ha nincs ilyen modell (például csak Grokos vagy csak Kimis telepítésen), ha a hívás hibára fut vagy üresen tér vissza, vagy ha a modell megtagadja a választ, a jegyzet pontosan a determinisztikus metaadatait tartja meg, és **tartalékként** számít az eredménypanelen.
- **A linkeket az EYAS ellenőrzi.** A modell által javasolt link csak akkor marad meg, ha olyan jegyzetet nevez meg, amely már létezik a vaultban (fájlnév vagy cím alapján), vagy amelyet ugyanabban az importban kiválasztottál; ilyenkor annak a jegyzetnek a fájlnevével íródik ki. Minden más, a modell által javasolt link elmarad. A forrásfájl saját linkjei mindig változatlanul megmaradnak.
- **A dúsított jegyzet megmondja, ki válaszolt.** Minden dúsított jegyzet frontmatterében ott egy `enriched_by` mező, például `enriched_by: {provider: anthropic, model: claude-haiku-4-5, route: tier}`. A `provider` a válaszoló provider; a `model` az általa jelentett modell, és elmarad, ha a backend nem nevezett meg modellt (a saját alapértelmezésével használt CLI); a `route` azt mondja meg, hogyan választódott a háttérmodell — `tier` (a routing-szint, elöl a Heartbeat), `default` (a telepítés alapértelmezése), `api` (egy API provider) vagy `isolated-cli` (izoláltan futni képes CLI). A mező a jegyzet későbbi átírásakor is megmarad. A dúsítás nélkül importált jegyzeteknek nincs `enriched_by` mezője.
- A háttérmodellt az EYAS a job futásakor keresi meg, nem a szerver indulásakor, így a dúsítás akkor is működik, ha a modell modul a Data Port után vált elérhetővé.

**Egy korábbi dúsított import kitakarítása.** Az ezen frissítés előtt dúsított importoknak nincs `enriched_by` mezőjük, és olyan linkeket is tartalmazhatnak, amelyeket senki nem ellenőrzött. Ha dúsítás nélkül akarod újra elvégezni: nyisd meg a jobot az eredménypanelen vagy a **Korábbi importok** listában, válaszd az **Import visszavonása** lehetőséget (ehhez `delete` jogosultság kell a Data Porton), majd szkenneld újra ugyanazt a forrást, és importáld a **Metaadatok dúsítása modellel** kapcsoló nélkül. Azok a jegyzetek, amelyek törzsét azóta szerkesztetted, megmaradnak, és kihagyottként jelennek meg.
...
Az importer maga olvassa a többi eszköz tárait, nem modell-toolon keresztül, ezért a biztonsági kapu memória-policyje nem érinti — az **minden modell** számára elutasítja a `~/.claude`, a `~/.grok`, az `ai-memory` mappák, az Obsidian vaultok és a többi olvasását és írását (lásd [Biztonság és adatvédelem — Memória az EYAS-on kívül](/docs/hu/admin/security-privacy/#memory-outside-eyas)). Az az agent, amely korábban közvetlenül olvasott egy ilyen fájlt, most elutasítást kap; a tartalmat inkább itt importáld egyszer. Lásd [Memória](/docs/hu/knowledge/memory/).

**Ez az út befelé azokhoz a tartalmakhoz, amelyeket az EYAS már nem olvas élőben:**

- **Host `CLAUDE.md` és host skillek.** A Claude Code mindig izoláltan fut, és már nem tölti be őket; importáld őket itt egyszer. Lásd [Providerek — Claude Code izoláció](/docs/hu/ai/providers/#claude-code-isolation).
- **Egy másik eszköz mappáiban lévő import-gyökerek.** A `~/.claude`, `~/.grok`, `~/.agents` mappán, egy Obsidian vaulton és hasonlókon belüli `skills.importRoots` / `agent.importRoots` bejegyzéseket az EYAS induláskor kihagyja; importáld itt egyszer azt a mappát, majd töröld a bejegyzést a `local.yaml`-ból. Lásd [Konfiguráció](/docs/hu/deploy/configuration/#extra-skill-and-persona-roots).
- **Memóriatár MCP szerverek.** A Memory, Qdrant, Obsidian és MCPVault MCP szerverek, valamint minden vaultra vagy egy másik eszköz memóriájára mutató szerver minden modell számára tiltott; azt a memóriát itt másold be az EYAS-ba. Lásd [MCP](/docs/hu/ai/mcp/#memory-store-servers-are-blocked).
- **Törölt importált agentek.** Az `agent.importRoots`-ból importált, általad törölt agentet az EYAS induláskor nem hozza létre újra; ha vissza akarod kapni, importáld itt a fájlját.
...
**Mit csinál a címke.** A `contains-secrets` címkéjű jegyzet, epizodikus sor vagy skill kimarad mindenből, amit a modell magától elér: az állandó memóriaindexből, a felidézésből, a `search_memory`-ból, a reflexiós jobból, az éjszakai konszolidálóból, a skill-illesztőből és az összeállított rendszerpromptból. Az opcionális dúsító modellnek sosem adódik át, és sosem kap beágyazást. A címke arra is átöröklődik, amit az EYAS a tételből származtat — a nyers naplóbeli másolatára, a belőle kinyert tényekre és a belőle épített összefoglalókra —, így ezek sem jutnak el a modellhez (lásd [Memória — Az importált titkok kimaradnak a felidézésből](/docs/hu/knowledge/memory/#imported-secrets-stay-out-of-recall)). A Memória oldalon viszont teljes egészében látod. Ha meg akarod nyitni a modell felé, állítsd a `memory.recall.includeSecrets` kulcsot `true`-ra a `config/local.yaml`-ben, és indítsd újra — lásd a [Konfigurációt](/docs/hu/deploy/configuration/).
...
| **Metaadatok dúsítása modellel** | Alapból ki — opcionális, csak metaadat (kind, összefoglaló, címkék, meglévő jegyzetekre mutató linkek), a törzs soha, és megjelölt tétel soha; csak izolált háttérmodell-válaszból kerül át, különben tartalékként számít |

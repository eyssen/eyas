...
A Grok CLI API-kulcsa itt van `grok-cli-api-key` néven (**Rendszer** hatókör), ha a Grok CLI bejelentkezési kártyáján az **Inkább API-kulccsal** lehetőséget választod. Az EYAS csak a Grok CLI futásainak adja át, `XAI_API_KEY`-ként; ezen a néven itt is felveheted, és a provider panel **Kijelentkezés** gombja törli. Lásd [Providerek — Bejelentkezés a Grokba és a Kimibe az EYAS számára](/docs/hu/ai/providers/#sign-in-grok-and-kimi-for-eyas). A Grok és a Kimi eszközkódos bejelentkezése nem titok ebben a tárban: fájlok az EYAS saját CLI-home-jaiban.

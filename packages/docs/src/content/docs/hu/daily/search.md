---
title: Keresés
description: Keresés minden indexelt forrásban — és rögzítés, melyik kódfát használhatja az ágens.
---

**Mire való.** A keresésnek két feladata van. A felső sáv lekérdezése találatokat ad a táblán, memórián, dokumentumokon és kódon. A **Keresési források** az, ahol ezeket a fákat regisztrálod (egy Odoo-checkout forrásonként) és újraindexeled. A beszélgetés- és projekt-rögzítés dönti el, melyik fát keresheti az ágens, hogy a verziók ne keveredjenek csendben.

## Mikor használd

- Fájlt, memória-jegyzetet vagy dokumentumot keresel anélkül, hogy tudnád, melyik modulban van.
- Odoo-checkoutot (vagy bármely kódbázist) adsz hozzá, hogy az ágensek citálhassák találgatás helyett.
- Több Odoo-verzió van indexelve, és a beszélgetésen vagy projekten rögzíteni kell a 18c vs 18e-t.
- Az ágens `needsPin` választ adott — forrásokat kell választanod, mielőtt keres.
- A lemezen változott a fa, és újraindexelni akarod.

## Tipikus munkafolyamat

1. Írj a shell keresősávjába (*Search across all indexed sources…*), vagy nyisd a találatokat — ez a globális keresés.
2. Fa hozzáadása: **Beállítások → Keresési források** (oldalsáv **Beállítások**, **Modulok** csoport) — útvonal `/search-sources`.
3. **Add Source** (egy abszolút gyökér, **Label** pl. `18c`, **Family:** `odoo`) → **Create Source** → **Reindex**, amíg a státusz **ready**.
4. A projekten pipáld a **Default code sources** mezőt; nyiss beszélgetést, és ellenőrizd, hogy a **Sources** fül ugyanazokat a rögzítéseket mutatja. A találatoknak és citációknak ezután ezeken a fákon belül kell maradniuk.

...
2. Jelentkeztesd be az OpenCode-ot **az EYAS számára** (a tulajdonos vagy egy admin): nyiss egy beszélgetést, kattints a terminál ikonra, és az OpenCode terminálban használd a `/connect` parancsot (lásd [Bejelentkezés](#sign-in)).
...
| Webes terminál | `@xterm/xterm` a `/api/v1/opencode/terminal/:id` útvonalon (JWT). Csak a tulajdonosé és az adminoké (az OpenCode kezelési joga — lásd [Ki nyithat terminált](#who-can-open-a-terminal)). A kapcsolat bontása leállítja a PTY-t |
...
Az OpenCode maga jelentkezik be a modellproviderekhez; az EYAS nem ad át API-kulcsot abba a folyamatba. A bejelentkezés most az EYAS mappájában van, ezért **a meglévő OpenCode-felhasználók egyszer kijelentkeznek**. A tulajdonos vagy egy admin megnyitja egy beszélgetés OpenCode terminálját, és kiadja a `/connect` parancsot (lásd [Ki nyithat terminált](#who-can-open-a-terminal)). Ne használd az `opencode auth login` parancsot egy saját, EYAS-on kívüli terminálban: az a normál környezetedet használja, és a mindennapi OpenCode-odat jelentkeztetné be, nem az EYAS-osat.

### Ki nyithat terminált {#who-can-open-a-terminal}

Az OpenCode terminálban az OpenCode eszközhívásait te magad hagyod jóvá — az EYAS biztonsági kapuja csak az EYAS által indított feladatoknak válaszol —, és az OpenCode-nak nincs kernel-sandboxa, így a terminál gyakorlatilag egy shell a szerveren, amely annak operációsrendszer-felhasználójaként fut. Ezért minden terminál csak a tulajdonosé és az adminoké, ahogy a sima shell eddig is.

| OpenCode jog | Alapértelmezett szerepkörök | Mit enged |
|--------------|-----------------------------|-----------|
| olvasás (read) | owner, admin, user | Az OpenCode oldal (állapot, modellek, mentett beállítások); a saját terminálok listázása és bezárása |
| létrehozás (create) | owner, admin, agent | Fej nélküli használat: az OpenCode plugin által hívott EYAS-memória API, bejelentkezett hívónak |
| kezelés (manage) | owner, admin | A beállítások mentése (**Modell és gondolkodás**), valamint terminál megnyitása vagy csatlakozás hozzá — az OpenCode terminál és a sima shell |

- Kezelési jog nélkül a `POST /api/v1/opencode/sessions` mindkét fajtára `403`-at ad, és a terminál socketje (`/api/v1/opencode/terminal/:id`) nem enged csatlakozni. A beszélgetés felső sávja csak annak mutatja a terminál ikont, akinek a szerver megengedi a terminált; ha a szerver mégis elutasítja, a terminálpanel ezt kiírja.
- A jogot az EYAS minden socket-csatlakozáskor a felhasználó aktuális szerepkörével és állapotával ellenőrzi, majd újra, amíg a kapcsolat nyitva van: a terminál minden billentyűleütése, átméretezése és pingje előtt, 15 másodpercenként, és azonnal, amikor a tulajdonos vagy egy admin megváltoztatja az illető szerepkörét, felfüggeszti vagy archiválja. Az a terminál, amelynek a felhasználója elvesztette a jogot, véget ér — a panel kiírja, miért, a kapcsolat bezárul, és az EYAS leállítja a terminál folyamatát —, újra csatlakozni pedig nem lehet hozzá.
- A fej nélküli feladatokat ez nem érinti: egy kolléga `opencode_run` feladata továbbra is fut bármely felhasználó beszélgetésében, mert minden eszközhívása az EYAS biztonsági kapujától kérdez ([A fej nélküli feladatok az EYAS-t kérdezik](#headless-tasks-ask-eyas)).
- A `GET /api/v1/opencode/access` (olvasási jog) `{terminal, settings}` választ ad: amit a hívó saját jogai megengednek. A webes felület ebből dönti el, mutatja-e a terminál ikont.

A beépített szerepkörök jogai rögzítettek az EYAS-ban; egyik beállítás sem ad a user szerepkörnek kezelési jogot az OpenCode-on. Ha valakinek terminál kell, add neki az admin szerepkört (`PATCH /api/v1/users/<id>` `{"role": "admin"}` törzzsel, a tulajdonos vagy egy admin által) — ez minden más admin jogot is megad neki.
...
**Nem sandbox a saját felhasználója számára.** A fenti mappák csak azt döntik el, hol indul a terminál. Ami benne fut, az EYAS szerver operációsrendszer-felhasználójaként fut: egy parancs, amelyet az OpenCode terminálban jóváhagysz (egy shell-parancs vagy hozzáférés a feladatmappán kívüli mappához), mindent elér, amit az a felhasználó — a szerver más mappáit is. Ezért minden terminál csak a tulajdonosé és az adminoké — lásd [Ki nyithat terminált](#who-can-open-a-terminal). A session-API sima shellje (`POST /api/v1/opencode/sessions` `kind: "shell"`-lel; a webes oldal csak az OpenCode terminált nyitja meg) az OpenCode terminál környezetével és az EYAS saját home mappájával indul, nem a szerver saját környezetével, így sem az EYAS mesterkulcsa, sem a providerek API-kulcsai nincsenek benne.
...
- A korábbi verziókban a user szerepkör is megnyithatta az OpenCode terminált (az OpenCode létrehozási joga). Most minden terminálhoz kezelési jog kell: a tulajdonosnak és az adminoknak megmarad, egy user elveszíti a terminál ikont, és `403`-at kap. A kollégák `opencode_run` feladatai az ő beszélgetéseiben ugyanúgy futnak, mint eddig. Nincs mit migrálni — a beépített szerepkörök jogai nincsenek eltárolva —, és nincs beállítás, amely visszaállítaná a régi alapértelmezést; akinek terminál kell, annak add az admin szerepkört (lásd [Ki nyithat terminált](#who-can-open-a-terminal)).

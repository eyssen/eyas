...
| `opencode_status` / `opencode_run` | Opcionális OpenCode kódoló-motor sidecar ([OpenCode](/docs/hu/automation/opencode/)). A status zöld; a run piros + jóváhagyás. |
...
**Média** (opcionális): `media_generate`, `media_wait`, `media_catalog`, `media_balance`, `media_history` — Magnific / Higgsfield / fal / HeyGen a [Média](/docs/hu/ai/media/) alatt. Talking-head / presenter: `provider: heygen`. A kész fájlok a Dokumentumokba ingestelődnek.

További csoportok, ha a modul be van kapcsolva: board, conversation, document, knowledge, research, schedule, channel, A2A delegate, opcionális Google Docs.

Agent-routing (az agent-modul regisztrálja, itt nem duplikálva):

| Tool | Cél | Kockázat |
|------|-----|----------|
| `run_specialist` | Enabled specialista, összefoglalóra vár. Alias: `delegate_to_agent`. | zöld |
| `handoff_to_colleague` | Másik kolléga home-szála. | zöld |
| `assign_task` | Aszinkron board-kártya. | zöld |
| `propose_team` | Kártya hiányzó szerepre, epicre, vagy explicit csapatkérésre. | sárga |
| `propose_agent_creation` | Új specialista sablon. | sárga |

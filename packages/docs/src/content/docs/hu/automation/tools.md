...
| `search_memory` / `save_memory` | Tartós vault jegyzetek — EYAS memória, nem a host CLI-é. A `search_memory` `scope` alapból `current` (ez a projekt, a típusa, plusz globális user/feedback/reference); `all` a többi projektet is. A Memória oldal keresése szűretlen. |

...
Útvonalak a workspace-re vagy az agent **worktree**-jére vannak zárva. Érzékeny pathok (`.env`, `master.key`, `.ssh`, …) tiltva. Inkább `edit_file`, mint teljes fájl-újraírás.
...
| `search_memory` / `save_memory` | Tartós vault jegyzetek — EYAS memória, nem a host CLI-é |

...
**Mire való.** Csak az első indításkor. A varázsló létrehozza a mesterjelszót, a root ownert, a két elsődleges ágenst és az első modell-backendet, hogy a fő app kinyíljon. Utána ezeket a **Beállítások**, a **Providerek** és az **Agentek** alatt változtatod — ne számíts arra, hogy a varázslót újra futtatod.

## Mikor használd

- A böngésző a `/setup`ra küldött, mert a setup nincs kész
- Kihagytál egy opcionális lépést, és a mezőlistát akarod
- Friss példányt állítasz helyre

Nem napi változtatásokra, ha az app már nyitva van.
...
| Vezérlő | Jelentés |
|---------|----------|
| **Nyelv** | Termék UI nyelve (`en` / `hu` / `de` / `es` / `fr` / `tlh`). |
...
**Cél:** a két always-on **kolléga**, akikkel beszélsz (sidebar **Kollégák**, kollégánként egy home-szál).
...
Mindkettőhöz létrejön: `agent_definitions` sor, `data/agents/<id>/` workspace, és agent user rekord. Később az **Agentek** oldalon finomhangolható. A varázsló után a sidebar **Kollégák** listájáról nyithatók. Az Asszisztens koordinál, nem szerkeszt forrást; a Mérnök a platform/kód. Lásd [Csapatok és delegálás](/docs/hu/agents/teams/).

---

## Csapat-agentek (opcionális)

**Cél:** extra **kollégák** (team tier) és **specialisták** (közös pool, akiket bármelyik kolléga indíthat). A primary-knak nem kell javaslatkártya egy enabled specialistához.
...
| Hova | Miért |
|------|-------|
| [Az első órád](/docs/hu/first-hour/) | Élő UI: Kezdőlap, egy beszélgetés, Tábla, Memória |
| [Kezdőlap](/docs/hu/daily/home/) | Hátralévő ajánlott setup tételek |
| [Providerek](/docs/hu/ai/providers/) | További backendek |
| [Ágensek](/docs/hu/agents/overview/) | Kollégák és specialisták |
| [Csapatok és delegálás](/docs/hu/agents/teams/) | Handoff és `run_specialist` |
| [Felhasználók](/docs/hu/admin/users/) | További emberi userek |

## Biztonság

- A mesterjelszó a **secretekeket** védi, nem helyettesíti a lemez/backup védelmet.

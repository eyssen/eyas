---
title: Média
description: Csatlakoztasd a Magnific, Higgsfield, fal vagy HeyGen szolgáltatást. Az ágensek öt közös toolon generálnak. Hasonlítsd össze a backendeket, és válassz egyet — vagy többet.
---

**Mire való.** A Média az, ahogy az EYAS képet, videót, hangot, szerkesztést és 3D-t generál, nagyít, és megvár. Te választod a backendeket; az ágens **egy tool-készletet** használ. A négy vendor közül egyik sem alapértelmezett. Nulla csatlakoztatott provider üres, fail-closed állapot — soha nincsenek hamis pixelek.
...
- Van Magnific, Higgsfield, fal vagy HeyGen fiókod, és **nem** akarod a vendor 50 toolját a modellre önteni.
...
2. Olvasd el a **Melyik backend?** blokkot, majd **Csatlakozás** egy (vagy több) szolgáltatóhoz. Magnific, Higgsfield és HeyGen böngészős OAuth; fal API-kulcs.
...
| Szempont | Magnific | Higgsfield | fal | HeyGen |
|----------|----------|------------|-----|--------|
| **Erősség** | Fotórealisztikus still, promptos **Creative** upscale, hű **Precision** upscale | Filmes videó, karakter-azonosság (Soul) | Óriás modellkatalógus, ár ellenőrzése futás előtt | Talking-head avatar; presenter videó promptból vagy scriptből |
| **Típusok az EYAS-ban** | Nagyítás, kép, szerkesztés (videó / hang / 3D is) | Videó, kép (hang is) | Kép, videó, hang, 3D, nagyítás | Videó, hang |
| **Belépés** | OAuth (Magnific-fiók) | OAuth (Higgsfield-fiók) | Bearer API-kulcs (`fal-api-key`) | OAuth (HeyGen-fiók) |
| **Kredit** | Ugyanaz az egyenleg, mint a Magnific-weben. A webes **Unlimited nem érvényes** MCP/API-ra | Az MCP **mindig** kreditet von, még unlimited webes csomagnál is | Maga az MCP ingyenes; a modellfutásért fizetsz | Az MCP a HeyGen **webes csomag** kreditjét vonja, nem a külön REST API-egyenleget |
| **Eredmény** | CDN-URL — az EYAS lemásolja | Az URL kb. **hét nap** múlva lejár — az ingest kötelező | CDN-URL — akkor is másoljuk | CDN-URL — az EYAS lemásolja |
| **Először ezt, ha…** | Nagyítás, retus, still kell | Klipp vagy rögzített karakter kell | Sok modell vagy előzetes ár kell | Presenter, digitális iker vagy scriptelt avatar kell |

**Ajánlás**

1. **Egy backendet köss be arra a munkára, ami tényleg van.** Still és nagyítás → Magnific. Filmes klipp / karakterzár → Higgsfield. Talking-head / presenter → HeyGen. Széles katalógus vagy „mennyibe kerül?” → fal.
...
3. **Ne kapcsold be a nyers vendor-MCP toolokat**, hacsak nem debugolsz. Az ágensre önti a Magnific/Higgsfield/fal/HeyGen tool-listát, és kikerüli az ingestet.
...
Javasolt defaultok (csak ha a provider csatlakoztatva van, és a sort nem pined): nagyítás / kép / szerkesztés → Magnific; videó → Higgsfield (talking-headhez pineld a **heygen**-t); hang / 3D → fal.
...
**Nyers vendor-MCP toolok.** Alapból ki. Be = az ágens látja a `mcp_magnific_*` / `mcp_higgsfield_*` / `mcp_fal_*` / `mcp_heygen_*` toolokat is. Hagyd ki.

...
| **Projekttípus** | Egy munkatípus defaultjai (a típus **Prompt** mezője, lemezen `AGENTS.md`) |
| **Projekt** | Egy projekt felülírásai. Üres = örökli a típust. `+` előtag = kiterjeszti. Minden más = felülírja. Az űrlap a szerkesztő; a nemüres DB érték nyer a testvér `AGENTS.md` fölött. |

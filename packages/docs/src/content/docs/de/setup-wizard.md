...
**Wozu das gut ist.** Nur beim ersten Start. Der Assistent legt Master-Passwort, Root-Owner, die zwei primären Agenten und ein erstes Modell-Backend an, damit die Haupt-App aufgeht. Danach änderst du das unter **Einstellungen**, **Anbieter** und **Agenten** — rechne nicht damit, den Assistenten erneut zu durchlaufen.

## Wann du es nutzt

- Der Browser hat dich nach `/setup` geschickt, weil Setup unvollständig ist
- Du hast einen optionalen Schritt übersprungen und willst die Feldliste
- Du richtest eine frische Instanz wieder ein

Nicht für Alltagsänderungen, sobald die App offen ist.
...
| Steuerung | Bedeutung |
|-----------|-----------|
| **Sprache** | UI-Sprache (`en` / `hu` / `de` / `es` / `fr` / `tlh`) |
...
**Zweck:** die zwei immer-an **Kollegen**, mit denen du sprichst (Sidebar **Kollegen**, je ein Home-Thread).
...
Pro Agent: DB-Zeile, Workspace `data/agents/<id>/`, Agent-User-Datensatz. Später unter **Agenten** umbenennen. Nach dem Assistenten in der Sidebar **Kollegen** öffnen. Der Assistant koordiniert und editiert keinen Quellcode; der Engineer besitzt Plattform/Code. Siehe [Teams und Delegation](/docs/de/agents/teams/).

## Team-Agenten (optional)

**Zweck:** weitere **Kollegen** (Team-Tier) und **Spezialisten** (gemeinsamer Pool, den jeder Kollege starten kann). Primaries brauchen keine Vorschlagskarte für einen aktivierten Spezialisten.
...
- [Die erste Stunde](/docs/de/first-hour/) — laufende UI: Start, ein Gespräch, Board, Speicher
- [Start](/docs/de/daily/home/) · [Provider](/docs/de/ai/providers/) · [Agenten](/docs/de/agents/overview/) · [Teams und Delegation](/docs/de/agents/teams/)

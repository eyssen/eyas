...
**Tier** (Primary/Team = Kollegen; Specialist = Spawn-Pool) · **Agent Type**.
...
Für Coding: `read_file, write_file, edit_file, grep, glob, git_status, git_diff, run_command` freigeben (modellunabhängig). Der **Personal Assistant** koordiniert — kein `write_file` / `edit_file` / `run_command`. Der **System Engineer** und Coding-Spezialisten besitzen diese Tools. Siehe [Teams und Delegation](/docs/de/agents/teams/). Bestehende Agenten vor 0.8.6 erben das **nicht** automatisch. Katalog: [Tools](/docs/de/automation/tools/).

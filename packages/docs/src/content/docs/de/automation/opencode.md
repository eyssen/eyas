...
**Keine Sandbox für den eigenen Benutzer.** Die Ordner oben entscheiden nur, wo das Terminal startet. Was darin läuft, läuft als Betriebssystem-Benutzer des EYAS-Servers: Ein Befehl, den du im OpenCode-Terminal freigibst (ein Shell-Befehl oder der Zugriff auf einen Ordner außerhalb des Aufgabenordners), erreicht alles, was dieser Benutzer erreicht, auch andere Ordner auf dem Server. Gib das OpenCode-Recht daher nur Personen, denen du das anvertrauen würdest. Die einfache Shell der Session-API (`POST /api/v1/opencode/sessions` mit `kind: "shell"`; die Webseite öffnet nur das OpenCode-Terminal) ist nur für den Eigentümer und Admins — sie braucht das Verwaltungsrecht für OpenCode, alle anderen erhalten `403` — und startet mit der Umgebung des OpenCode-Terminals und dem EYAS-eigenen Home-Ordner, nicht mit der eigenen Umgebung des Servers; EYAS' Master-Schlüssel und die API-Schlüssel der Anbieter sind darin also nicht enthalten. Standardmäßig kann auch die Rolle user das OpenCode-Terminal öffnen (das Erstellrecht für OpenCode).

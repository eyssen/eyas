...
2. OpenCode **für EYAS** anmelden (der Eigentümer oder ein Admin): Gespräch öffnen, Terminal-Icon klicken und im OpenCode-Terminal `/connect` nutzen (siehe [Anmeldung](#sign-in)).
...
| Webterminal | `@xterm/xterm` über `/api/v1/opencode/terminal/:id` (JWT). Nur Eigentümer und Admins (das Verwaltungsrecht für OpenCode — siehe [Wer ein Terminal öffnen darf](#who-can-open-a-terminal)). Trennen beendet die PTY |
...
OpenCode meldet sich selbst bei seinen Modellanbietern an; EYAS reicht keine API-Schlüssel in diesen Prozess. Die Anmeldung liegt jetzt im EYAS-Ordner, daher werden **bestehende OpenCode-Nutzer einmal abgemeldet**. Der Eigentümer oder ein Admin öffnet das OpenCode-Terminal eines Gesprächs und nutzt `/connect` (siehe [Wer ein Terminal öffnen darf](#who-can-open-a-terminal)). Nutze nicht `opencode auth login` in einem eigenen Terminal außerhalb von EYAS: Es nutzt deine normale Umgebung und würde dein alltägliches OpenCode anmelden, nicht das von EYAS.

### Wer ein Terminal öffnen darf {#who-can-open-a-terminal}

Im OpenCode-Terminal gibst du OpenCodes Tool-Aufrufe selbst frei — das EYAS-Security-Gate beantwortet nur die Aufgaben, die EYAS startet —, und OpenCode hat keine Kernel-Sandbox. Das Terminal ist also praktisch eine Shell auf dem Server, die als dessen Betriebssystem-Benutzer läuft. Deshalb ist jedes Terminal nur für den Eigentümer und Admins, wie die einfache Shell schon bisher.

| Recht auf OpenCode | Standardrollen | Was es erlaubt |
|--------------------|----------------|----------------|
| read (Lesen) | owner, admin, user | Die OpenCode-Seite (Status, Modelle, gespeicherte Einstellungen); eigene Terminals auflisten und schließen |
| create (Erstellen) | owner, admin, agent | Headless-Nutzung: die EYAS-Speicher-API, die OpenCodes Plugin aufruft, für einen angemeldeten Aufrufer |
| manage (Verwalten) | owner, admin | Die Einstellungen speichern (**Modell und Reasoning**) sowie ein Terminal öffnen oder sich damit verbinden — das OpenCode-Terminal und die einfache Shell |

- Ohne Verwaltungsrecht antwortet `POST /api/v1/opencode/sessions` für beide Arten mit `403`, und der Terminal-Socket (`/api/v1/opencode/terminal/:id`) lehnt die Verbindung ab. Die obere Leiste eines Gesprächs zeigt das Terminal-Icon nur, wenn der Server dem Benutzer Terminals erlaubt; lehnt der Server trotzdem ab, sagt das Terminal-Panel es.
- Das Recht wird bei jeder Socket-Verbindung mit der aktuellen Rolle und dem Status des Benutzers geprüft, und erneut, solange die Verbindung offen ist: vor jedem Tastendruck, jeder Größenänderung und jedem Ping des Terminals, alle 15 Sekunden und sofort, wenn der Eigentümer oder ein Admin die Rolle der Person ändert, sie sperrt oder archiviert. Ein Terminal, dessen Benutzer das Recht verloren hat, endet — das Panel sagt, warum, die Verbindung wird geschlossen und der Prozess des Terminals beendet; neu verbinden lässt es sich nicht.
- Headless-Aufgaben sind nicht betroffen: Das `opencode_run` eines Kollegen läuft weiterhin im Gespräch jedes Benutzers, weil jeder Tool-Aufruf darin das EYAS-Security-Gate fragt ([Headless-Aufgaben fragen EYAS](#headless-tasks-ask-eyas)).
- `GET /api/v1/opencode/access` (Leserecht) liefert `{terminal, settings}`: was die eigenen Rechte des Aufrufers erlauben. Die Webseite entscheidet damit, ob sie das Terminal-Icon zeigt.

Die Rechte der eingebauten Rollen sind in EYAS fest vorgegeben; keine Einstellung gibt der Rolle user das Verwaltungsrecht für OpenCode. Wer das Terminal nutzen soll, bekommt die Rolle admin (`PATCH /api/v1/users/<id>` mit `{"role": "admin"}`, durch den Eigentümer oder einen Admin) — und damit auch alle anderen Admin-Rechte.
...
**Aufgabenordner.** `opencode_run` läuft nur innerhalb eines Gesprächs und lehnt überall sonst ab. Der Ordner, den du nennst, muss innerhalb der Ordner des Gesprächs liegen — oder, wenn es keine hat, in seinem eigenen Workspace. Ohne Angabe nutzt EYAS den ersten Gesprächsordner, sonst den eigenen Workspace des Gesprächs — nie den EYAS-Installationsordner. Das OpenCode-Terminal öffnet sich nur für ein eigenes Gespräch: Das Gespräch eines anderen Benutzers wird *nicht gefunden*, auch für einen Admin. Es arbeitet in den gespeicherten Ordnern dieses Gesprächs, die EYAS selbst liest — die Seite kann keine anderen nennen — und fällt genauso auf den Gesprächs-Workspace zurück. Seine Ordner durchlaufen dieselbe Prüfung wie die jedes anderen Laufs: Ein Ordner, der ein geschützter Ort ist, in einem liegt oder einen enthält (EYAS' eigene Daten, der Speicher eines anderen KI-Werkzeugs, ein Notiz-Vault oder dein Home-Ordner), wird ausgelassen, das Terminal öffnet sich im nächsten erlaubten Ordner oder im Gesprächs-Workspace, und eine Zeile oben im Terminal nennt den ausgelassenen Ordner. Genauso wird ein gespeicherter Ordner ausgelassen, der der Workspace des Gesprächs eines anderen Benutzers ist, darin liegt oder (über einen Link) dorthin führt — gespeichert, bevor EYAS solche Ordner abgelehnt hat, oder von einem Projekt geerbt; auch `opencode_run` lässt ihn aus und lehnt einen dort genannten Aufgabenordner ab. Die Workspaces deiner eigenen anderen Gespräche bleiben nutzbar.

**Keine Sandbox für den eigenen Benutzer.** Die Ordner oben entscheiden nur, wo das Terminal startet. Was darin läuft, läuft als Betriebssystem-Benutzer des EYAS-Servers: Ein Befehl, den du im OpenCode-Terminal freigibst (ein Shell-Befehl oder der Zugriff auf einen Ordner außerhalb des Aufgabenordners), erreicht alles, was dieser Benutzer erreicht, auch andere Ordner auf dem Server. Deshalb ist jedes Terminal nur für den Eigentümer und Admins — siehe [Wer ein Terminal öffnen darf](#who-can-open-a-terminal). Die einfache Shell der Session-API (`POST /api/v1/opencode/sessions` mit `kind: "shell"`; die Webseite öffnet nur das OpenCode-Terminal) startet mit der Umgebung des OpenCode-Terminals und dem EYAS-eigenen Home-Ordner, nicht mit der eigenen Umgebung des Servers; EYAS' Master-Schlüssel und die API-Schlüssel der Anbieter sind darin also nicht enthalten.
...
- Frühere Versionen ließen auch die Rolle user das OpenCode-Terminal öffnen (das Erstellrecht für OpenCode). Jetzt braucht jedes Terminal das Verwaltungsrecht: Eigentümer und Admins behalten es, ein User verliert das Terminal-Icon und bekommt `403`. Die `opencode_run`-Aufgaben von Kollegen in den Gesprächen dieses Users laufen wie bisher. Es gibt nichts zu migrieren — die Rechte der eingebauten Rollen werden nicht gespeichert — und keine Einstellung, die den alten Standard zurückholt; wer das Terminal braucht, bekommt die Rolle admin (siehe [Wer ein Terminal öffnen darf](#who-can-open-a-terminal)).

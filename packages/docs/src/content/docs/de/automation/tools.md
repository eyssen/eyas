...
Coding-Oberfläche (`read_file`, `write_file`, `edit_file`, `grep`, `glob`, `git_status`/`git_diff`, `run_command` — Pfade sind auf die **Arbeitsordner** der Conversation beschränkt). `run_command`/`Bash` mit eindeutigem `git status` / `git diff` (kein Metachar, kein `-C`/`--git-dir`/`--no-index`, kein absoluter Pfad) wird auf die grünen Tools umgebogen — **ohne Klick**. `git commit` / `git add` / `ls` bleiben rot. Search/Grounding (`needsPin`), Memory-Blöcke + `search_memory`/`save_memory` (`scope` default `current`: dieses Projekt + Typ + globale Notizen; `all` = andere Projekte; `search_memory` durchsucht auch frühere Gesprächsnachrichten (user + assistant) im aktuellen Projekt; `scope=all` projektübergreifend; die Memory-Seite sucht ungefiltert). E-Mail draft→approve→send, optionales Odoo, Connections-Inventar, Board/Conversation/Document/Knowledge/Research/Schedule/Channel/A2A-Delegate. **Studio** `hyperframes_*` / `videouse_*`: [Studio](/docs/de/studio/). Screen-Capture-Politur ist kein Tool: Recordly ist AGPL-Begleiter unter [Erweiterungen](/docs/de/admin/extensions/#recordly) — kein `recordly_*`. **Medien** (optional): `media_generate`, `media_wait`, `media_catalog`, `media_balance`, `media_history` — [Medien](/docs/de/ai/media/). CLI-MCP-Parität für Grok/Kimi: [MCP](/docs/de/ai/mcp/).

Agent-Routing (vom Agent-Modul registriert, hier nicht dupliziert):

| Tool | Zweck | Risiko |
|------|-------|--------|
| `run_specialist` | Aktivierten Spezialisten starten, auf die Zusammenfassung warten. Alias: `delegate_to_agent`. | grün |
| `handoff_to_colleague` | Home-Thread eines anderen Kollegen öffnen und starten. | grün |
| `assign_task` | Asynchrone Board-Karte für einen aktivierten Agenten. | grün |
| `propose_team` | Karte für fehlende Rollen, epische Arbeit oder explizite Teamanfrage. | gelb |
| `propose_agent_creation` | Neuen Spezialisten-Template vorschlagen. | gelb |

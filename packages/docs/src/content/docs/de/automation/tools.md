...
| Suche | `list_search_sources`, `get_search_context`, `set_search_context`, `search_indexed` (Pin / Multi-Version; bei Konflikt `needsPin`) |
...
| Odoo (Quelle) | `odoo_search_model` / `field` / `xml_id` — Roots aus Conversation/Projekt-Pin, Search Sources oder Env; Cite `odoo-src:label:…` |

---
title: Medien
description: Magnific, Higgsfield, fal oder HeyGen verbinden. Agenten erzeugen über fünf gemeinsame Tools. Backends vergleichen und eines — oder mehrere — wählen.
---

**Wozu das da ist.** Medien ist, wie EYAS Bilder, Video, Audio, Edits und 3D erzeugt, hochskaliert und abwartet. Du wählst die Backends; der Agent nutzt **einen Tool-Satz**. Keiner der vier Vendoren ist Default. Null verbundene Provider = leer, fail-closed — niemals Mock-Pixel.
...
- Du hast ein Magnific-, Higgsfield-, fal- oder HeyGen-Konto und willst **nicht** fünfzig Vendor-Tools auf das Modell schütten.
...
2. Lies **Welches Backend?** auf der Seite, dann **Verbinden** (eins oder mehrere). Magnific, Higgsfield und HeyGen: OAuth im Browser; fal: API-Schlüssel.
...
| Kriterium | Magnific | Higgsfield | fal | HeyGen |
|-----------|----------|------------|-----|--------|
| **Stärke** | Fotorealistische Stills, promptgeführtes **Creative**-Upscale, treues **Precision**-Upscale | Filmische Videos, Charakter-Konsistenz (Soul) | Riesiger Modellkatalog, Preischeck vor dem Lauf | Talking-Head-Avatare; Presenter-Video aus Prompt oder Skript |
| **Arten in EYAS** | Upscale, Bild, Edit (auch Video / Audio / 3D) | Video, Bild (auch Audio) | Bild, Video, Audio, 3D, Upscale | Video, Audio |
| **Anmeldung** | OAuth (Magnific-Konto) | OAuth (Higgsfield-Konto) | Bearer-API-Schlüssel (`fal-api-key`) | OAuth (HeyGen-Konto) |
| **Credits** | Dasselbe Guthaben wie auf der Magnific-Website. Web-**Unlimited gilt nicht** für MCP/API | MCP **zieht immer** Credits, auch bei Unlimited-Webplan | MCP selbst ist kostenlos; du zahlst den Modelllauf | MCP zieht Credits vom HeyGen-**Webplan**, nicht vom REST-API-Guthaben |
| **Ergebnis** | CDN-URL — EYAS kopiert die Bytes | URLs laufen in etwa **sieben Tagen** ab — Ingest ist Pflicht | CDN-URL — wird trotzdem kopiert | CDN-URL — EYAS kopiert die Bytes |
| **Zuerst verbinden, wenn…** | Du hochskalierst, retuschierst oder Stills brauchst | Du Clips oder einen festen Charakter brauchst | Du viele Modelle oder erst den Preis wissen willst | Du einen Presenter, digitalen Zwilling oder ein Skript-Avatar brauchst |

**Empfehlung**

1. **Verbinde ein Backend für den Job, den du wirklich hast.** Stills und Upscale → Magnific. Filmischer Clip / Charakter-Lock → Higgsfield. Talking-Head / Presenter → HeyGen. Breiter Katalog oder „was kostet das?“ → fal.
...
Vorgeschlagene Defaults (nur wenn der Provider verbunden ist und die Zeile nicht gepinnt): Upscale / Bild / Edit → Magnific; Video → Higgsfield (Talking-Head: **heygen** pinnen); Audio / 3D → fal.
...
**Rohe Vendor-MCP-Tools.** Standard aus. An = der Agent sieht auch `mcp_magnific_*` / `mcp_higgsfield_*` / `mcp_fal_*` / `mcp_heygen_*`. Aus lassen.

...
| **Kollege** | Primary- oder Team-Agent, mit dem du sprichst — Home-Thread, SOUL. Setup legt zwei an (Assistant + Engineer). | Sidebar **Kollegen** / Agenten |
| **Spezialist** | Schmaler Arbeiter aus dem gemeinsamen Pool; jeder Kollege kann einen aktivierten starten (`run_specialist`) | Agenten (Spezialist) |
...
3. Kollege: Tools/Skills, Memory, Handoff / Spezialist, Kanal
...
| Steuerung | Bedeutung |
|-----------|-----------|
| **Modell** | Welches Modell antwortet: ein festes Modell, automatisches Routing oder der Standard des Kollegen |
| **Effort** | Reasoning-Tiefe vs. Kosten. Listet nur die Stufen, die das Modell des Gesprächs anbietet; **Automatisch** erbt (Deep → *Maximum*, der Effort des Kollegen, der des delegierenden Gesprächs) oder nutzt den Default des Modells |
| **Solo** | Keine Spezialisten, Übergaben oder Teamvorschläge |
| **Auto** | Modell entscheidet Fan-out |
| **Deep** | Aggressiver Multi-Agent-Fan-out; Default-Effort *Maximum* |

...
export default defineConfig({
  test: {
    ...
    // isolated-workspaces gives each test file a throw-away workspaces root
    // (EYAS_WORKSPACES_DIR), so conversation workspaces and run scratch
    // folders never land in the developer's real one.
    // cli-sandbox pins the kernel-sandbox host the CLI providers detect, so
    // their tests do not depend on the machine (bubblewrap or not).
    setupFiles: ['@testing-library/jest-dom/vitest', './tests/helpers/isolated-workspaces.setup.ts', './tests/helpers/cli-sandbox.setup.ts'],
    ...
    server: {
      deps: {
        ...
        //
        // '@tanstack/react-virtual' (and the '@tanstack/virtual-core' it wraps)
        // is the same shape once more: installed only under
        // src/web/node_modules, and the data-port wizard's folder tree and
        // candidate list both mount `useVirtualizer`, which calls React hooks
        // of its own. Externalized it resolves 'react' to the nested copy and
        // every virtualised render dies on "resolveDispatcher().useState is
        // null" before an assertion is reached.
        inline: [
          /react-grid-layout/,
          /react-resizable/,
          /react-draggable/,
  ...
  resolve: {
    alias: {
      ...
      // Same again for the virtualiser the data-port wizard's tree and list use
      // (Task 14/15): '@tanstack/react-virtual' exists only under
      // src/web/node_modules, so a root-level test file importing a component
      // that uses it — or mocking it — must resolve the one copy that exists,
      // and it must be the copy the 'react' alias above applies to.
      '@tanstack/react-virtual': resolve(__dirname, 'src/web/node_modules/@tanstack/react-virtual'),

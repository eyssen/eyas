...
/**
 * Usage shape enough to price a call, in the canonical ModelUsage semantics:
 * inputTokens are the UNCACHED prompt tokens and the cache reads/writes are
 * counted separately, so every prompt token is priced exactly once. Providers
 * that don't report a field simply omit it.
 */
 ...
 * rate: cacheRead = 0.1x, cacheWrite (5-minute TTL) = 1.25x — except where a
 * model publishes its own cache-read rate (Fable 5.1 $0.25, Opus 5.5 $0.20).
 * A longer id precedes its prefix (claude-opus-5-5 before claude-opus-5), so
 * a dated variant prefix-matches its own family.
 */
export const DEFAULT_MODEL_PRICING: PricingTable = {
  // Anthropic — direct API (claude-api reference, verified 2026-09-22)
  ...
  'claude-code/claude-code-fable': { input: 10, output: 50, cacheRead: 1, cacheWrite: 12.5 },
  'claude-code/claude-code-opus': { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 },
  'claude-code/claude-code-sonnet': { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
  'claude-code/claude-code-haiku': { input: 1, output: 5, cacheRead: 0.1, cacheWrite: 1.25 },
  ...
  // Discovered models (kimi-cli-<key>): the default row's rate — which
  // Moonshot model a key of the CLI's config runs is not known to EYAS.
  'kimi-cli/kimi-cli-': { input: 0.95, output: 4 },
  // OpenAI
 ...
 *
 * Cache tokens are part of the prompt: a model without a configured cache
 * rate bills them at its input rate — never at $0, and never on top of an
 * input count that already contains them (inputTokens is uncached).
 */
export function estimateCost(
  ...
  if (usage.cacheReadTokens) {
    cost += (usage.cacheReadTokens * (rates.cacheRead ?? rates.input)) / 1_000_000
  }
  if (usage.cacheCreationTokens) {
    cost += (usage.cacheCreationTokens * (rates.cacheWrite ?? rates.input)) / 1_000_000
  }
  return cost
}

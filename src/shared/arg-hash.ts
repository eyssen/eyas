...
import { canonicalToolName, normalizeToolInput } from './canonical-tool-name.js'
...
/**
 * argHash of a tool input after normalizeToolInput (Claude Code's `file_path`
 * becomes EYAS's `path`). For an input without runtime-specific keys this is
 * exactly argHash(input), so hashes recorded before normalization still match.
 */
export function toolArgHash(input: unknown): string {
  const isRecord = input !== null && typeof input === 'object' && !Array.isArray(input)
  return argHash(isRecord ? normalizeToolInput(input as Record<string, unknown>) : input)
}

/**
 * The do-not-repeat ledger key of one tool call: canonical name + toolArgHash.
 * The same key whether EYAS's executor ran the call or a CLI ran it itself
 * (Bash → run_command), so a resumed run recognises what its predecessor did
 * on any provider. The agent runner records ToolResult events under it and
 * both the runner and the CLI permission bridge check a resumed run against it.
 */
export function toolLedgerKey(toolName: string, input: unknown): string {
  return `${canonicalToolName(toolName)}:${toolArgHash(input)}`
}

...
/**
 * A coded model error (classify-model-error CodedModelError) EYAS raised
 * itself, e.g. 'cliSignIn' for a CLI that is not signed in. Recognised by
 * shape: that module imports this one.
 */
function codedAuthKind(e: unknown): boolean {
  if (!e || typeof e !== 'object') return false
  const err = e as { name?: unknown; kind?: unknown; code?: unknown }
  return err.name === 'CodedModelError' && err.kind === 'auth' && typeof err.code === 'string'
}

export function classifyAuthError(e: unknown): AuthErrorClassification {
  if (codedAuthKind(e)) return { kind: 'auth', isAuth: true }

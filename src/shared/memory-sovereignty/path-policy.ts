...
export interface PathPolicy {
  ...
  /**
   * The workspaces a path is, or lies inside: `<root>/<id>` (a conversation's)
   * and `<root>/_runs/<id>` (a run's scratch folder), judged by each of the
   * path's forms — as written and after realpath — so a symlink into a
   * workspace names that workspace. Ids as the path spells them. Empty for a
   * path outside the workspaces root, and for the root or its `_runs` folder
   * itself. Who may use a workspace is the caller's rule (folder validation's
   * mayUseWorkspace).
   */
  workspacesOf(absPath: string): WorkspaceRef[]
  /** What the policy protects, for the UI. */
  describe(): PathPolicyDescription
}

/** One workspace under the workspaces root (PathPolicy.workspacesOf). */
export interface WorkspaceRef {
  /** The conversation id (`<root>/<id>`) or the run id (`<root>/_runs/<id>`). */
  id: string
  /** A run's scratch folder, which belongs to no conversation. */
  scratch: boolean
}
...
export function createPathPolicy(options: PathPolicyOptions): PathPolicy {
  ...
  /**
   * The workspaces a path is or lies inside, by its lexical and real forms.
   * Compared segment by segment against the root's canonical forms, so the
   * id keeps the path's own spelling where the filesystem folds case.
   */
  const workspaceRootSegments = workspacesForms.map((r) => r.split('/').filter(Boolean)).filter((s) => s.length > 0)
  const foldSegment = (s: string): string => (caseInsensitive ? s.toLowerCase() : s)
  const workspacesOf = (absPath: string): WorkspaceRef[] => {
    if (typeof absPath !== 'string' || !isAbsolute(absPath) || absPath.includes('\0')) return []
    const out: WorkspaceRef[] = []
    const seen = new Set<string>()
    for (const form of new Set([resolve(absPath), realOf(absPath)])) {
      const segments = form.replace(/\\/g, '/').split('/').filter(Boolean)
      for (const root of workspaceRootSegments) {
        if (segments.length <= root.length || !root.every((seg, i) => foldSegment(segments[i]) === seg)) continue
        const rest = segments.slice(root.length)
        const scratch = foldSegment(rest[0]) === foldSegment(RUN_SCRATCH_SEGMENT)
        const id = scratch ? rest[1] : rest[0]
        if (!id) continue
        const key = `${scratch ? 'run' : 'conversation'}:${id}`

import type { EyasModule, FrontendManifest, ModuleContext } from './types.js'
...
/**
 * Projection served by ctx.listModules(): id + frontend manifest + enabled flag.
 * Returns all registered modules, each flagged by whether it is currently active.
 * The widget catalogue uses this to show disabled modules' widgets dimmed in the drawer.
 */
export function buildModuleList(loader: ModuleLoader, disabled: string[]): ModuleListing[] {
  const disabledSet = new Set(disabled)
  return loader.getAllModules().map((m: EyasModule) => ({
    id: m.id,
    frontend: m.frontend,

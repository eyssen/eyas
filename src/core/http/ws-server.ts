...
export interface WsServer {
  ...
  /** GET /api/v1/opencode/terminal/:sessionId?token= — PTY stream (JWT; manage OpenCode; own session). */
  handleTerminalUpgrade(req: Request, server: Server<WSData>): Promise<Response | undefined>
...
export async function setupWsServer(ctx: ModuleContext): Promise<WsServer> {
  ...
  async function handleTerminalUpgrade(req: Request, server: Server<WSData>): Promise<Response | undefined> {
    const url = new URL(req.url)
    const match = url.pathname.match(/^\/api\/v1\/opencode\/terminal\/([^/]+)$/)
    ...
    const opencode = (ctx as {
      opencode?: {
        pty?: { get: (id: string) => { userId: string } | undefined }
        wsHandler?: unknown
        mayUseTerminal?: (userId: string) => boolean
      }
    }).opencode
    if (!opencode?.pty) {
      return new Response('OpenCode module unavailable', { status: 503 })
    }
    ...
    // A terminal is effectively a shell as the server's OS user: the socket
    // attaches only for a user who may manage OpenCode now (role read fresh,
    // opencode/permissions.ts) — asked before the session is looked up, so a
    // refusal says nothing about which sessions exist. No check wired = no
    // terminal (fail closed).
    if (typeof opencode.mayUseTerminal !== 'function' || !opencode.mayUseTerminal(userId)) {
      return new Response('Forbidden', { status: 403 })
    }
    const rec = opencode.pty.get(sessionId)
    if (!rec || rec.userId !== userId) {
      return new Response('Not found', { status: 404 })
    }
    const upgraded = server.upgrade(req, { data: { userId, isTerminal: true, terminalSessionId: sessionId } })
    if (!upgraded) {
      return new Response('WebSocket upgrade failed', { status: 400 })
    }
    return undefined
  ...
  return {
    ...
    websocket: {
      open(ws) {
        ...
        if (ws.data.isTerminal && ws.data.terminalSessionId) {
          const opencode = (ctx as any).opencode
          if (opencode?.wsHandler) {
            opencode.wsHandler.onOpen(ws, ws.data.terminalSessionId, ws.data.userId)
          }
          return
        }
      ...
      message(ws, rawMessage) {
        ...
        if (ws.data.isTerminal && ws.data.terminalSessionId) {
          const opencode = (ctx as any).opencode
          if (opencode?.wsHandler) {
            opencode.wsHandler.onMessage(ws, ws.data.terminalSessionId, rawMessage)
          }
          return
        }
        try {
      ...
      close(ws) {
        ...
        if (ws.data.isTerminal && ws.data.terminalSessionId) {
          const opencode = (ctx as any).opencode
          if (opencode?.wsHandler) {
            opencode.wsHandler.onClose(ws, ws.data.terminalSessionId)
          }
          return
        }

...
import { checkCliMcpBridge } from '@modules/model/cli-mcp/bridge-routes'
...
export async function bootstrap(options: BootstrapOptions = {}): Promise<ModuleContext> {
  ...
  // data paths (master.key, documents, …) land under the instance home. The
  // memory vault does not rely on this: it is InstancePaths.vaultDir.
  ...
  // Privacy installs its egress filter into the raw gateway's egress slot
  // (ctx.modelEgress) and no longer wraps ctx.model, so a gateway reference
  // captured before its onStart is masked too. It starts right after 'auth'
  // (a declared dependency: its routes must sit behind auth's middleware),
  // still before every module that reads ctx.privacy (all read it lazily);
  // observability's tracing wrapper (registered later) wraps ctx.model.
  ...
  if (!moduleLoader.hasModule(opencodeModule.id)) {
    moduleLoader.register(opencodeModule)
  }
  ...
  // Prove the Grok/Kimi tool bridge answers through the full middleware
  // stack (setup guard, session-auth catch-all, bridge auth) once every
  // module has mounted its routes. A failure is a warning, never a boot stop.
  ctx.cliMcpBridge = await checkCliMcpBridge({
    http: ctx.http,
    logger,

...
import { isValidTimeZone } from '@shared/clock.js'
...
export const configSchema = z.object({
  ...
  i18n: z.object({
    ...
    // IANA time zone for the clock EYAS tells a model (e.g. 'America/New_York').
    // Unset: the server's own zone (the TZ variable, else the OS setting).
    timezone: z.string().trim().min(1)
      .refine(isValidTimeZone, { message: 'Unknown time zone — use an IANA name such as Europe/Berlin or UTC' })
      .optional(),
  }).default({}),
  modules: z.object({
  ...
  security: z.object({
    ...
    // Memory outside EYAS: further stores a model may neither read nor write,
    // on top of the built-in list (src/shared/memory-sovereignty/
    // foreign-stores.ts). Absolute paths; a leading ~ is expanded; anything
    // else is ignored with a warning. The security gate refuses reads and
    // writes under them on every tool call it checks (read at start).
    foreignMemoryPaths: z.array(z.string().min(1)).default([]),
    // Kernel file sandbox for the CLI providers' own tools (macOS Seatbelt,
    // Linux bubblewrap — plus socat for Claude Code). 'auto' runs a CLI turn
    // with tools in it where one is available, otherwise without it and says
    // so; a Claude Code command that asks to leave it waits for a human.
    // 'required' refuses a CLI turn with tools when none is available (Kimi
    // has none) and lets no Claude Code command leave it. There is
    // deliberately no 'off' (model/cli-runtime/sandbox).
    cliSandbox: z.enum(['auto', 'required']).default('auto'),
  }).default({}),
  ...
  agent: z.object({
     ...
     * read on every start. Empty by default — instance overlay lists the
     * paths. Content is never burned into src/. A folder inside another
     * tool's own folders (~/.claude/agents, …) is skipped: import it once
     * with Data port. An agent edited or deleted in EYAS is never overwritten.
     */
  ...
  model: z.object({
    ...
    // CLI turn timeouts (Claude Code, Grok, Kimi): a CLI turn is stopped only
    // when the CLI goes silent — idleTimeoutMs while no tool runs,
    // toolTimeoutMs while one does — never by a fixed whole-turn clock.
    // toolTimeoutMs stays above run_specialist's 15 minutes
    // (model/cli-turn-watchdog.ts).
    cli: z.object({
      idleTimeoutMs: z.number().int().positive().default(600_000),
      toolTimeoutMs: z.number().int().positive().default(1_200_000),
    }).default({}),
  }).default({}),
  ...
  memory: z.object({
    // Sovereign layered memory (spec 2026-09-03 §14). This switch gates ONE
    // thing: deterministic L1 extraction. 'v2' always extracts; 'legacy'
    // extracts only while `l0.extractInLegacy` is on (the default). L0 capture
    // runs under both, and recall is always the v2 layered recall (standing
    // recall, memory_search / memory_expand) whichever value is set.
    // Recall always embeds locally (multilingual-e5-small, else the hashed
    // stem embedder); the 'embedding' routing tier feeds only the legacy
    // vault/episodic search index, never recall (memory/embeddings/select-bridges.ts).
    ...
    l0: z.object({
      ...
      // One switch for the output of every tool an agent run calls, whatever
      // the model: EYAS tools (also when a CLI calls them over the bridge) and
      // the CLIs' own built-in tools (Claude Code, Grok, Kimi), OpenCode's
      // tools inside an opencode_run task and the OpenCode terminal panel's
      // output (opencode/memory-bridge.ts). Refused,
      // skipped or approval-waiting calls ran nothing and are not recorded.
      // Off for a reason: a captured tool result (output plus 2,048 characters
      // of its arguments) is stored verbatim and unredacted, and nothing scans
      // or encrypts it at rest. run_command's stdout, read_file's contents,
      // browser_fill values and browser_totp's one-time code would all land in
      // memory_raw as-is. Turn it on only where that is acceptable. It is never
      // recalled or quoted into a prompt (retrieve.ts NEVER_RECALLED_SOURCE_TYPES,
      // extract/gist.ts): only its output shapes the topics and entities
      // extraction derives from the conversation. The arguments stay on the row
      // as provenance (memory_raw.meta_json) — never indexed, never extracted.
      captureToolResults: z.boolean().default(false),
      // Model reasoning ('thinking'), one row per model call, for audit only:
      // never extracted into facts, never recalled into a prompt. Stored
      // verbatim and unredacted like tool results, hence off by default.
      captureThinking: z.boolean().default(false),
    ...
    // (memory.relatedWork is gone: prior work is part of the recalled-memory
    // block below. zod strips unknown keys, so an old local.yaml still loads.)
    // Size of the recalled-memory block attached to each message, in
    // characters: standing notes first (~100 per line), then notes retrieved
    // for the message, then the full text of the best matches — frame
    // included. 2 400 (~600 tokens) is the size at a 100k-token context
    // window; the prompt budget scales it with the answering model's window
    // (token-budget.ts budgetForWindow → memoryRecall: up to 2.5× from 250k,
    // less below 100k). Read per call from ctx.config, which is loaded once at
    // startup, so a change takes a restart. Same value and wording as
    // config/default.yaml.
    index: z.object({
    ...
    // but left out of the recalled-memory block and memory search unless the
    // owner turns this on.
    recall: z.object({
  ...
  skills: z.object({
    ...
    /**
     * Extra markdown skill directories scanned after config/skills on every
     * start. Imported files win on id collision (recorded as shadows). Empty
     * by default — instance overlay lists the paths. A folder inside another
     * tool's own folders (~/.claude/skills, ~/.agents/skills, …) is skipped:
     * import those once with Data port.
     */
    importRoots: z.array(z.string()).default([]),
  }).default({}),
})

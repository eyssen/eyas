...
export interface StreamPlanProposalEvent {
  type: 'plan_proposal'
  plan: {
    id: string
    goal: string
...
export function useStreaming(options?: UseStreamingOptions) {
  ...
  const sendMessage = useCallback(
    async (
       ...
       */
      opts?: { resume?: boolean; plan?: boolean },
    ) => {
...
export function processStreamEvent(event: StreamEvent, state: ReturnType<typeof useConversationStore.getState>) {
  switch (event.type) {
    ...
    case 'done':
      ...
      if (event.conversation) {
        state.updateConversation({
          ...
          estimatedTokens: event.conversation.estimatedTokens,
          contextWindow: event.conversation.contextWindow,
        })
      }
      break
    ...
    case 'plan_proposal':
      state.setPlanProposal(event.plan)
      state.finishAgentProgress()

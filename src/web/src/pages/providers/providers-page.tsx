...
import { BackgroundCallsCard } from './background-calls-card'
import { TierEffortField, tierUpdate, type TierModelInfo } from './tier-effort'
import { effortAdjustedText, type EffortLevel } from '@/lib/effort-options'
import { t as tCommon } from '@/i18n'
...
interface TierConfig {
  ...
  description: string
  enabled: boolean
  /** The tier's default effort (null = Auto). */
...
function RoutingTiersTab() {
  ...
  // Per tier: the "effort adjusted" line after a model change clamped its rung.
  const [effortNotices, setEffortNotices] = useState<Record<string, string>>({})
  ...
  const handleTierUpdate = useCallback(async (tier: string, field: string, value: string) => {
    ...
    if (!current) return
    setSaving(tier)
    // A provider change picks its first model; a model change clamps an
    // effort the new model does not offer, visibly, before the PUT.
    const { body, adjusted } = tierUpdate(current, field, value, modelsData ? allModels : null)
    try {
      await api.put(`/routing/tiers/${tier}`, body)
      setEffortNotices((prev) => {
        const next = { ...prev }
        if (adjusted) next[tier] = effortAdjustedText(adjusted)
        else delete next[tier]
        return next
      })
      refetch()
    } catch (err) {
      toast.error(err instanceof ApiError && err.code === 'EFFORT_UNSUPPORTED'
        ? tCommon('common.effort.rejected')
        : tCommon('common.saveFailed'))
      refetch()
    } finally {
      setSaving(null)
    }
  }, [tiers, allModels, modelsData, refetch])

  return (
    <div className="space-y-4">
      {/* Where background calls go now; refetched after every tier update */}
      <BackgroundCallsCard refreshKey={tiersData} providers={providersData?.providers} models={allModels} />
      ...
      {tiers.map((tier) => {
        ...
        return (
          <div key={tier.tier} className={`glass-card p-4 ${!tier.enabled ? 'opacity-50' : ''}`}>
            ...
            {/* Default effort — only the rungs the tier's model offers */}
            <TierEffortField
              tier={tier}
              models={modelsData ? allModels : null}
              notice={effortNotices[tier.tier] ?? null}
              onChange={(v) => handleTierUpdate(tier.tier, 'effort', v)}
            />
          </div>
        )
      })}

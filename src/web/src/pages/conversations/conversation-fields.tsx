...
import { useEffortOptions, type ConversationEffortOptions } from '@/hooks/use-effort-options'
import { effortFromSelect } from '@/lib/effort-options'
...
interface ConversationFieldsProps {
  ...
  effort: string | null
  /**
   * Changes whenever the effort options may have changed without the
   * conversation id changing: its model or binding.
   */
  effortRefreshKey?: string
...
export function ConversationFields({
  ...
  // The rungs the conversation's model offers and what Auto means here. The
  // stored level is not part of the key: what Auto inherits excludes it.
  const { options: effortOptions } = useEffortOptions<ConversationEffortOptions>({
    conversationId,
    refreshKey: `${effortRefreshKey ?? ''}|${orchestration}|${agentId ?? ''}`,
  })
  ...
  const { data: agentData } = useApi<{ agents: AgentItem[] }>('/agents?enabled=true')
  ...
  const agents = (agentData?.agents ?? []).filter(
    (a) => a.tier === 'primary' || a.tier === 'team' || a.id === (agentId ?? ''),
  )
  ...
  return (
    <div className="flex items-center gap-4 px-4 py-2 border-b border-border/50 text-xs flex-shrink-0 overflow-x-auto">
      ...
      {/* Reasoning effort — safe to change mid-conversation, so never locked.
          Only the rungs the conversation's model offers; Auto says what it
          means here (Deep, the colleague, the delegating conversation). */}
      <div className="flex items-center gap-1.5">
        <Brain className="h-3 w-3 text-muted-foreground" />
        <EffortSelect
          value={effortFromSelect(effort ?? '')}
          options={effortOptions}
          inherited={effortOptions?.inherited ?? null}
          hint={t('conversations.fields.effortHint')}
          onChange={(level) => onUpdate({ effort: level })}
        />
      </div>

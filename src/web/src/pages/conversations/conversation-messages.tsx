...
import { TurnNotices } from './components/stream-notice'
import { noticesOf } from './turn-notices'
import { useConversationStore } from '@/stores/conversation-store'
import { AnsweredBy } from './components/answered-by'
import { MessageMeta } from './components/message-meta'
import { StreamError, type ChatErrorView } from './components/stream-error'
import { ApprovalInlineCard } from './components/approval-inline-card'
...
interface Message {
  ...
  /** A reply's per-turn metadata; its notices, outcome, usage and binding show under the reply. */
  turnMeta?: unknown
  /** A failed turn (client-side bubble): rendered by stream-error.tsx, the only error renderer. */
  error?: ChatErrorView
  /** Who answered a reply (conversation_messages.provider/model). */
...
export function ConversationMessages({
  ...
  // The notices of the turn in flight (notice frames), shown under it.
  const streamNotices = useConversationStore((s) => s.streamNotices)
  // Which model the turn in flight runs on (agent_start.binding).
  const streamBinding = useConversationStore((s) => s.streamBinding)
  // Tool calls of the latest turn waiting for a human (approval_required).
  const approvals = useConversationStore((s) => s.approvals)
  ...
  }, [messages.length, streamingText, streamingThinking, backgroundWorking, toolCalls.length, approvals.length])
  ...
  return (
    <div ref={paneRef} className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4">
      {messages.map((msg) => (
        <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
          ...
          >
            ...
            {(() => {
              if (msg.role !== 'user' && msg.error) return <StreamError error={msg.error} />
            ...
            {msg.role !== 'user' && <TurnNotices notices={noticesOf(msg.turnMeta)} separated />}
            {msg.role === 'assistant' && !msg.error && (
              <MessageMeta provider={msg.provider} model={msg.model} turnMeta={msg.turnMeta} />
            )}
          </div>
          {msg.role === 'user' && (
      ...
      {isStreaming && (
        <div className="flex gap-3">
          ...
          <div className="max-w-[75%]">
            ...
            <TurnNotices notices={streamNotices} />
            {streamBinding && <AnsweredBy provider={streamBinding.providerId} model={streamBinding.modelId} binding={streamBinding} />}
            {onCancel && (
      ...
      {/* Tool calls waiting for a human: decided right here, or in the approval queue */}
      {approvals.length > 0 && (
        <div className="ml-10 max-w-[75%] space-y-2" data-testid="approval-cards">
          {approvals.map((approval) => (
            <ApprovalInlineCard key={approval.key} approval={approval} />
          ))}
        </div>
      )}
      ...
      {backgroundWorking && (
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-xs font-medium text-muted-foreground mb-1">{t('conversations.messages.assistant')}</div>

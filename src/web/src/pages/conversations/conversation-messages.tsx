...
export function ConversationMessages({
  ...
  }, [messages.length, streamingText, streamingThinking, backgroundWorking])
          ...
          >
            {msg.attachmentIds && msg.attachmentIds.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-1.5">
                {msg.attachmentIds.map(docId => (
                  <a key={docId} href={`/api/v1/documents/${docId}/download`} target="_blank" rel="noreferrer">
                    <img
                      src={`/api/v1/documents/${docId}/download`}
                      className="max-h-48 rounded-lg cursor-pointer hover:opacity-90 transition-opacity"
                      alt={t('conversations.messages.attachmentAlt')}
                      onError={(e) => {
                        // Hide broken images (non-image attachments)

...
import { ArrowUp, Paperclip, Loader2, X, FileText, Wand2, Map, ImageOff } from 'lucide-react'
...
export function ConversationInput({
  ...
  // The model the next turn runs on cannot see images (its catalog Vision
  // flag): attached images reach it only as a note, so say so before sending.
  // God Mode sends to its own roster, not to this pair.
  const blindModel = useConversationStore((s) => {
    const conv = s.activeConversation
    const binding = conv?.id === conversationId ? conv.effectiveBinding : null
    return binding?.supportsImages === false ? binding.modelId : null
  })
  const showImagesNotVisible = !godMode && blindModel !== null && attachments.some(a => a.mimeType.startsWith('image/'))
  ...
  // A refused message put back for editing (the privacy card's "Edit
  // message"): its text and attachments return to this composer, once.
  const composerDraft = useConversationStore((s) => s.composerDraft)
  useEffect(() => {
    if (!composerDraft || composerDraft.conversationId !== conversationId) return
    useConversationStore.getState().setComposerDraft(null)
    setValue(composerDraft.content)
    void addLinkedDocuments(composerDraft.attachmentIds)
    textareaRef.current?.focus()
  }, [composerDraft, conversationId, addLinkedDocuments])
  ...
  return (
    ...
    >
      ...
      {showImagesNotVisible && (
        <div
          role="status"
          className="mx-1 mb-2 inline-flex items-center gap-1.5 rounded-full border border-border/60 bg-accent/40 px-2.5 py-1 text-xs text-muted-foreground"
        >
          <ImageOff className="h-3.5 w-3.5 shrink-0" aria-hidden />
          <span>{t('conversations.input.imagesNotVisible', { model: blindModel ?? '' })}</span>
        </div>
      )}

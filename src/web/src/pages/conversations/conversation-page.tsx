...
import { useStreaming, type SendMessageOptions } from '@/hooks/use-streaming'
...
import { SkillProposalCard } from './components/skill-proposal-card'
import { PlanProposalCard } from './components/plan-proposal-card'
...
import { formatErrorBody } from './components/stream-error'
...
import { rememberGodRuns } from './god-mode-tab'
import { WebTerminal } from '@/components/terminal/web-terminal'
import type { BindingPatch } from './model-picker'

/** The localized text of a failed conversation PATCH. */
function patchErrorText(err: unknown): string {
  if (err instanceof ApiError) return formatErrorBody(err.details ?? { error: err.message, code: err.code }, err.message)
  return formatErrorBody(null, err instanceof Error ? err.message : String(err))
}
...
export default function ConversationPage() {
  ...
  const { sendMessage, cancel, abortRef } = useStreaming({ conversationId })
  ...
  const { data: projectsPayload } = useApi<{ projects: { id: string; workingDirectories?: string[] | null }[] }>('/projects')
  const [seenGodRuns, setSeenGodRuns] = useState(false)
  ...
  // Every conversation emits its own run tree (the agent runner, whatever the
  // provider) — mount the rail whenever the store has roots, not only for
  // team sessions.
  ...
  // God Mode SSE closes immediately; clear the live strip once the run idles.
  useEffect(() => {
    if (activeConversation?.status === 'idle' || activeConversation?.status === 'waiting') {
  ...
  // Every conversation's runs emit orchestration events under its own
  // conversationId as runId, for every provider (API and CLI; a Grok/Kimi
  // plan adds its steps) — subscribe in addition to the team topic, but
  ...
  const handleSend = useCallback(
    async (content: string, attachmentIds?: string[], opts?: Pick<SendMessageOptions, 'plan' | 'privacy' | 'displayContent'>) => {
      if (!conversationId || !activeConversation) return
      await sendMessage(conversationId, content, attachmentIds, opts)
    },
    [conversationId, activeConversation, sendMessage],
  )

  // The model picker: a fixed model ({modelBinding:'pinned', providerId,
  // modelId}), Auto-routing or the colleague's default. A refusal (the model
  // is not available, …) is shown in the user's language; the picker snaps
  // back on the refetch.
  const handleBindingChange = useCallback(async (patch: BindingPatch) => {
    if (!conversationId) return
    try {
      await api.patch(`/conversations/${conversationId}`, patch)
    } catch (err) {
      toast.error(patchErrorText(err))
    }
    refetch()
  ...
  const handleFieldUpdate = useCallback(async (fields: Record<string, unknown>) => {
    ...
    try {
      ...
      // A coded refusal (an effort the model does not offer, a model that is
      // not available) in the user's language; the select snaps back on refetch.
      toast.error(patchErrorText(err))
      refetch()
      return
    }
  ...
  const godModeOn = Boolean((conv as any)?.godMode)
  const working = conv?.status === 'working'
  const hasGodRuns = seenGodRuns

  useEffect(() => {
    setSeenGodRuns(false)
  }, [conversationId])

  // Fetch on mount / conversation switch / God-off; poll 1s while working so the
  // first run is latched before God can be turned off. Sticky — empty refetches
  // do not clear a seen run.
  useEffect(() => {
    if (!conversationId) return
    let cancelled = false
    const tick = async () => {
      try {
        const data = await api.get<{ runs: unknown[] }>(
          `/conversations/${conversationId}/god-mode/runs`,
        )
        if (!cancelled) {
          setSeenGodRuns((prev) => rememberGodRuns(prev, data.runs?.length ?? 0))
        }
      } catch {
        /* keep last known latch */
      }
    }
    void tick()
  ...
  }, [conversationId, working, godModeOn])

  const contextWindow = conv?.contextWindow ?? 0
  const estimatedTokens = conv?.estimatedTokens ?? undefined
  const measured = conv?.measured === true
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <ConversationTopBar
        ...
        modelBinding={(conv as any)?.modelBinding ?? null}
        effectiveBinding={(conv as any)?.effectiveBinding ?? null}
        bindingError={(conv as any)?.bindingError ?? null}
        autoRoutingEnabled={(conv as any)?.autoRoutingEnabled ?? null}
        ...
        godMode={(conv as any)?.godMode ?? false}
        ...
        <Panel defaultSize={66} minSize={30}>
          <div className="flex h-full min-h-0 flex-col overflow-hidden">
            <ConversationFields
              ...
              // The effort options follow the model the next turn runs on (H3 binding).
              effortRefreshKey={[
                conv?.providerId ?? '',
                conv?.modelId ?? '',
                (conv as any)?.modelBinding ?? '',
                (conv as any)?.effectiveBinding?.providerId ?? '',
                (conv as any)?.effectiveBinding?.modelId ?? '',
              ].join('|')}
              orchestration={(conv as any)?.orchestration ?? 'auto'}
              godMode={(conv as any)?.godMode ?? false}
              hasMessages={(conv?.messages?.length ?? 0) > 0}
              workingDirectories={(conv as any)?.workingDirectories ?? null}
              onUpdate={handleFieldUpdate}
            />
            ...
            <ConversationChat
              ...
              onSend={handleSend}
              onCancel={cancel}
              disabled={isStreaming || conv?.status === 'working' || conv?.status === 'waiting_plan'}
            ...
            {planProposal && (
              <div className="px-4 pb-2">
                <PlanProposalCard
                  ...
                  onAnswered={(decision) => {
                    useConversationStore.getState().setPlanProposal(null)
                    if (decision === 'rejected') return
                    void sendMessage(conversationId, '', undefined, { resume: true })
                  }}
                />
              </div>
            )}
            ...
            {skillProposal && (
              <div className="px-4 pb-2">
                <SkillProposalCard
                  ...
                  onAnswered={() => {
                    // The decision is recorded; the turn that stopped for it
                    // is re-run without re-sending the user's message.
                    useConversationStore.getState().setSkillProposal(null)
                    void sendMessage(conversationId, '', undefined, { resume: true })
                  }}
                />
              </div>
          ...
          {isTeamExpanded && teamSessionId ? (
            ...
            <div className="flex flex-col h-full min-h-0">
              {terminalOpen && conversationId && (
                <div className="h-[42%] min-h-[8rem] border-b border-border/50">
                  <WebTerminal
                    conversationId={conversationId}
                    cwd={(conv as any)?.workingDirectories?.[0] ?? null}
                    workingDirectories={(conv as any)?.workingDirectories ?? null}
                  />
                </div>
              )}
              ...
              <div className="flex-1 min-h-0 flex flex-col">
                <ChatterPanel
                  conversationId={conversationId}
                  externalRefreshKey={chatterRefreshKey}
                  searchContext={(conv as any)?.searchContext ?? null}
                  onSearchContextUpdate={handleFieldUpdate}
                  workingDirectories={(conv as any)?.workingDirectories ?? null}
                  projectHasDirectories={Boolean(
                    projectsPayload?.projects.find((p) => p.id === (conv as any)?.projectId)
                      ?.workingDirectories?.length,
                  )}
                  godMode={(conv as any)?.godMode ?? false}
                  conversationStatus={conv?.status}
                  hasGodRuns={hasGodRuns}
                />
              </div>
            </div>
          )}

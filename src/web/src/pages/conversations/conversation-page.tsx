...
export default function ConversationPage() {
  ...
  const { data: providerModels } = useApi<{ models: { modelId: string; contextWindow?: number | null }[] }>(
    conv?.providerId ? `/model/providers/${conv.providerId}` : '',
  )
  const modelWindow = providerModels?.models?.find((m) => m.modelId === conv?.modelId)?.contextWindow
  const contextWindow = resolveContextWindow(modelWindow, conv?.providerId)

  // Latest composition for this conversation — its estimated_tokens is the
  // ContextBar's numerator (composed context size), not the cumulative
  // conversation total. See composition-panel.tsx for the full drill-down.
  const { data: compositionData } = useApi<{ items: { estimatedTokens: number }[] }>(
    conversationId ? `/observability/compositions?conversationId=${conversationId}&limit=1` : '',
  )
  const estimatedTokens = compositionData?.items?.[0]?.estimatedTokens

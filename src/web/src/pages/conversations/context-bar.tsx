...
interface ContextBarProps {
  ...
  /**
   * The occupancy numerator: the prompt size the provider reported for the
   * last model call (`measured`), otherwise the estimate of the system
   * prompt plus the conversation history.
   */
  estimatedTokens?: number
  /** true: `estimatedTokens` was measured by the provider, not estimated. */
  measured?: boolean
  onClick?: () => void
}

export function ContextBar({ tokensUsed, contextWindow, estimatedTokens, measured = false, onClick }: ContextBarProps) {
  const occupancy = resolveContextOccupancy(contextWindow, estimatedTokens, measured)
  ...
  const title = occupancy.known
    ? t(occupancy.tooltipKey, {
        tokens: (estimatedTokens as number).toLocaleString(),

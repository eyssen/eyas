...
export default function SetupRecommendationsCard() {
  ...
  const ingress = useApi<{
    status?: string | { running?: boolean; active?: boolean }
    running?: boolean
    active?: boolean
    enabled?: boolean
  }>('/ingress/status')
  ...
  const ingressDone = useMemo(() => {
    ...
    if (!s) return false
    if (s.running === true || s.active === true || s.enabled === true) return true
    if (s.status && typeof s.status === 'object' && (s.status.running || s.status.active)) return true

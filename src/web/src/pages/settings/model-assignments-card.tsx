...
import { assignmentsBody, modelPairOptions, proposedPairValue } from '@/lib/model-pair'
import { useProviderDisplay } from '@/lib/provider-display'
import { t } from './i18n'

interface AiModelsResponse {
  providers: Array<{ id: string; models: Array<{ id: string; name: string }> }>
  agents: Array<{ id: string; name: string; agentType: string; proposedProviderId?: string | null; proposedModelId: string | null }>
}
...
export default function ModelAssignmentsCard() {
  ...
  // Each option names its provider too: the value is the encoded pair.
  const allModels = modelPairOptions(data?.providers ?? [])
  const modelFor = (a: AiModelsResponse['agents'][number]) => overrides[a.id] ?? proposedPairValue(a)

  async function save() {
    const assignments = assignmentsBody(Object.fromEntries(agents.map((a) => [a.id, modelFor(a)])))
    setSaving(true)
    setStatus(null)
    try {

...
import GodModeReport from './god-mode-report'
import ContextTab from './context-tab'
import { PURPOSE_ALL, PURPOSE_GROUPS, type PurposeGroup } from './purpose-groups'
...
interface AiTrace {
  ...
  model: string
  /** The concrete model the backend reported answering; null when it named none. */
  ...
  error: string | null
  /** A background call's purpose group; null for a conversation turn. */
  purposeGroup?: PurposeGroup | null
}
...
export default function ObservabilityPage() {
  ...
  const [purposeFilter, setPurposeFilter] = useState<PurposeGroup | typeof PURPOSE_ALL>(PURPOSE_ALL)
  ...
  if (purposeFilter !== PURPOSE_ALL) queryParams.set('purposeGroup', purposeFilter)
    ...
    <div className="flex-1 overflow-auto p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Tabs defaultValue="usage">
          <TabsList>
            <TabsTrigger value="usage">{t('observability.tab.usage')}</TabsTrigger>
            <TabsTrigger value="god" className="data-[state=active]:text-god">
              {t('observability.tab.god')}
            </TabsTrigger>
            <TabsTrigger value="context">{t('observability.tab.context')}</TabsTrigger>
          </TabsList>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="space-y-1">
            <label className="text-xs text-muted-foreground">{t('observability.col.purpose')}</label>
            <Select
              value={purposeFilter}
              onValueChange={(v) => { setPurposeFilter(v as PurposeGroup | typeof PURPOSE_ALL); setPage(0) }}
            >
              <SelectTrigger size="sm" className="w-44 text-sm">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={PURPOSE_ALL}>{t('observability.filter.purposeAll')}</SelectItem>
                {PURPOSE_GROUPS.map((group) => (
                  <SelectItem key={group} value={group}>{t(`observability.purpose.${group}`)}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
        ...
        <div className="rounded-xl border bg-card overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/30">
                  ...
                  <th className="text-left px-4 py-2.5 font-medium text-muted-foreground">{t('observability.col.purpose')}</th>
                ...
                {data?.traces.map((trace) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 font-mono text-xs">
                      {trace.model}
                      {trace.resolvedModel && trace.resolvedModel !== trace.model && (
                        <div className="text-[10px] text-muted-foreground">
                          {t('observability.col.resolvedModel', { id: trace.resolvedModel })}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-xs">
                      {trace.purposeGroup ? (
                        <Badge variant="secondary" className="text-xs">{t(`observability.purpose.${trace.purposeGroup}`)}</Badge>
                      ) : (
          ...
          <TabsContent value="god" className="mt-6">
            <GodModeReport />
          </TabsContent>

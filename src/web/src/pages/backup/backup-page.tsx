            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <select
                  ...
                  value={form.type}
                  onChange={(e) =>
                    setForm({ type: e.target.value, name: '', settings: {}, secrets: {} })
                  }
                >
                  {typeMeta.map((tp) => (
              ...
              {selectedType.settings.map((key) => (
                <div key={key}>
                  ...
                  <Input
                    ...
                    placeholder={SETTING_PLACEHOLDERS[key] ?? key}
                  />
                </div>
              ))}
            </div>
                ...
                <div key={key}>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        secrets: { ...form.secrets, [key]: e.target.value },
                      })
                    }
                    placeholder={
                      key === 'accessKeyId'
                        ? t('backup.dest.phAccessKey')
                        : key === 'secretAccessKey'
                          ? t('backup.dest.phSecretKey')
                          : key === 'accessToken'
                            ? t('backup.dest.phToken')
                            : key
                    }
                  />
                </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--vibrancy-border)]">
                ...
                <th className="text-left p-3 section-label">{t('backup.colRemote')}</th>
            ...
            <tbody>
              {backups.map((b) => (
                <tr key={b.id} className="border-b border-[var(--vibrancy-border)] last:border-0">
                  ...
                  <td className="p-3">
                    {b.remoteUploads && b.remoteUploads.length > 0 ? (
                      <Badge variant="outline" className="text-[10px]">
                        <Cloud className="h-3 w-3 mr-1" />
                        {t('backup.remoteBadge')}
                      </Badge>
                    ) : (
                      <span className="text-muted-foreground text-xs">{t('backup.localOnlyBadge')}</span>
                    )}
                  </td>

...
export default function SchedulerPage() {
  ...
  const jobs = useMemo(() => {
    let list = data?.jobs ?? []
    if (!showInfra) {
      list = list.filter((j) => j.source !== 'system' || j.kind === 'agent_run')
      // Still show system if user explicitly filters system
      if (sourceFilter === 'system') list = data?.jobs ?? []
    }
    return list
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => void handleRunNow(job.id)} title={t('scheduler.runNow')}>

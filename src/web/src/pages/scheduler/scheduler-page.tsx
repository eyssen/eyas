...
import type { EffortLevel } from '@/lib/effort-options'
import {
  AgentRunEffortField,
  agentRunConfigOf,
  agentRunHandlerConfig,
  withAgentRunEffort,
} from './agent-run-effort'
...
interface AgentRef {
  id: string
  name: string
  /** The agent's own effort (a scheduled run's Auto falls back to it). */
  effort?: string | null
}
...
/** The conversation a scheduled agent run used, read from its execution result. */
function executionConversationId(result?: string): string | undefined {
  if (!result) return undefined
  try {
    const parsed = JSON.parse(result) as { conversationId?: unknown } | null
    return typeof parsed?.conversationId === 'string' && parsed.conversationId ? parsed.conversationId : undefined
  } catch {
    return undefined
  }
}

/** Opens the conversation an agent_run execution ran in (completed or failed). */
function ExecutionConversationLink({ result }: { result?: string }) {
  const navigate = useNavigate()
  const conversationId = executionConversationId(result)
  ...
  return (
    <Button
      ...
      onClick={() => navigate({ to: '/conversations/$conversationId', params: { conversationId } })}
    >
      {t('scheduler.openConversation')}
    </Button>
  )
}
...
export default function SchedulerPage() {
  ...
  const [createForm, setCreateForm] = useState({
    ...
    agentId: '',
    prompt: '',
    /** agent_run only: the run's effort; null = Auto (the agent's). */
  ...
  // The create form's effort appears once the agent id names a known agent:
  // its options are that agent's model's (no lookup per keystroke).
  const createAgent = createForm.kind === 'agent_run' ? agentById.get(createForm.agentId.trim()) ?? null : null
  ...
  const handleCreate = async () => {
    ...
    if (createForm.kind === 'agent_run') {
      ...
      body.handlerConfig = agentRunHandlerConfig({
        agentId: createForm.agentId,
        prompt: createForm.prompt,
        title: createForm.name,
        // Only for an agent the form shows the effort for; Auto adds no key.
        effort: createAgent ? createForm.effort : null,
      })
  ...
  /** An agent_run job's effort, saved at once (the stored config keeps every other key). */
  const handleEffortChange = async (job: ScheduledJob, effort: EffortLevel | null) => {
    const handlerConfig = withAgentRunEffort(job.handlerConfig, effort)
    if (!handlerConfig) return
    setEffortSaving(true)
    try {
      await api.patch(`/scheduler/jobs/${job.id}`, { handlerConfig })
    } catch {
            ...
            {createForm.kind === 'handler' ? (
              ...
              <>
                ...
                {createAgent && (
                  <AgentRunEffortField
                    agent={createAgent}
                    value={createForm.effort}
                    onChange={(effort) => setCreateForm((prev) => ({ ...prev, effort }))}
                  />
                )}
              </>
            )}
      ...
      <div className={`grid gap-4 ${selectedId ? 'lg:grid-cols-[1fr_360px]' : ''}`}>
        ...
        {detail && selectedId && (
          <div className="glass-card p-4 h-fit sticky top-4 space-y-3">
            ...
            {detail.job.handler === 'scheduler.agent_run' && (() => {
              const cfg = agentRunConfigOf(detail.job.handlerConfig)
              if (!cfg.agentId) return null
              return (
                <AgentRunEffortField
                  key={detail.job.id}
                  agent={agentById.get(cfg.agentId) ?? { id: cfg.agentId }}
                  value={cfg.effort}
                  disabled={effortSaving}
                  onChange={(effort) => void handleEffortChange(detail.job, effort)}
                />
              )
            })()}
              ...
              <div className="space-y-1 max-h-80 overflow-y-auto">
                ...
                {detail.executions.map((exec) => (
                  ...
                  >
                    ...
                    <ExecutionConversationLink result={exec.result} />
                    {exec.result && (

...
import { resolveContextOccupancy } from '../conversations/context-bar-occupancy'
import { t } from './i18n'
...
export function BoardCard({
  ...
  const occupancy = resolveContextOccupancy(
    contextWindow,
    typeof estimatedTokens === 'number' ? estimatedTokens : undefined,
  )
  ...
  return (
    ...
    >
      ...
      {/* Context occupancy — same formula as the conversation ContextBar. */}
      {occupancy?.known && (
        <div
          className="group/bar relative mb-1.5"
          title={t('board.card.contextHint', {
        ...
        >
          <div className="h-[3px] rounded-full bg-muted overflow-hidden">
            <div
              className={cn('h-full rounded-full transition-all', occupancy.color)}
              style={{ width: `${Math.max(occupancy.pct, 2)}%` }}
            />
          </div>
          <span className="absolute -top-4 right-0 text-[9px] text-muted-foreground opacity-0 group-hover/bar:opacity-100 transition-opacity">
            {t('board.card.contextPct', { pct: Math.round(occupancy.pct) })}
          </span>
        </div>
      )}

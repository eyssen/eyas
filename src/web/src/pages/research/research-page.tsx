...
import { Search, FileText, Loader2, AlertCircle, AlertTriangle, ChevronRight } from 'lucide-react'
...
interface ReportSummary {
  ...
  /** Assembled without model synthesis: the sections list the top sources. */
  degraded?: boolean
  error?: string
  createdAt: string
  completedAt?: string
      ...
      <div className="w-[340px] border-r border-[var(--vibrancy-border)] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-4 border-b border-[var(--vibrancy-border)]">
          <div className="text-sm font-medium mb-3 inline-flex items-center gap-1.5">
            {t('research.newResearch')} <ContextualHelp helpId="automation.research" />
          </div>
          <input
            type="text"
            value={query}
      ...
      <div className="flex-1 overflow-y-auto">
        {!selectedReport ? (
          ...
          <div className="p-8 max-w-3xl mx-auto">
            ...
            {selectedReport.degraded && (
              <div
                role="status"
                className="mb-6 flex items-start gap-2 rounded-lg border border-[var(--vibrancy-border)] bg-accent/40 px-3 py-2 text-sm text-foreground"
              >
                <AlertTriangle className="h-4 w-4 mt-0.5 flex-shrink-0 text-muted-foreground" aria-hidden="true" />
                <span>{t('research.degradedBanner')}</span>
              </div>
            )}

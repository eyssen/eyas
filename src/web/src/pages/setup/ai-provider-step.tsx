...
import { CliSignInCard, isCliSignInProvider } from '@/components/providers/cli-sign-in-card'
...
function isCli(p: ProviderItem): boolean {
  return p.kind === 'cli'
}
...
// The wizard's CLI copy is i18n'd and keyed by id; any other CLI reads as its product name.
function cliLabel(p: ProviderItem): string {
  if (p.id === 'claude-code') return t('aiProvider.cliName.claude')
  if (p.id === 'grok-cli') return t('aiProvider.cliName.grok')
  if (p.id === 'kimi-cli') return t('aiProvider.cliName.kimi')
  return p.name || p.id
}
...
function detectedBadge(p: ProviderItem): string {
  if (p.id === 'claude-code') return t('aiProvider.detectedBadge.claude')
  if (p.id === 'grok-cli') return t('aiProvider.detectedBadge.grok')
  if (p.id === 'kimi-cli') return t('aiProvider.detectedBadge.kimi')
  return p.name || p.id
}
...
export function AiProviderStep({ onComplete, isLast }: Props) {
  ...
  const load = () => {
    ...
    api.get<{ providers: ProviderItem[] }>('/model/providers')
      .then((d) => {
        ...
        setProviders(sorted)
        const activeCli = sorted.filter((p) => isCli(p) && p.active)
  ...
  const activeCliProviders = providers?.filter((p) => isCli(p) && p.active) ?? []
  ...
  if (multiCli && !showAlternatives) {
    return (
      <div className="space-y-4">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">{cliLabel(p)}</span>
                    {selected && (
        ...
        {/* Grok / Kimi run in an EYAS-owned home: sign in for EYAS here. */}
        {activeCliProviders.map((p) => (
          isCliSignInProvider(p.id) ? <CliSignInCard key={p.id} providerId={p.id} /> : null
        ))}
  ...
  if (singleCli && !showAlternatives) {
    return (
      <div className="space-y-4">
        ...
        {isCliSignInProvider(singleCli.id) && <CliSignInCard providerId={singleCli.id} />}
        ...
        {providers.map((p) => {
          ...
          return (
            ...
            >
              ...
              {cli && p.active && isCliSignInProvider(p.id) && (
                <CliSignInCard providerId={p.id} className="mt-3" />
              )}

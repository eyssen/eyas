...
import { canSaveProject, projectSavePayload, type ProjectForm } from './project-form'
import { t as coachT } from '@/components/prompt-coach/i18n'
import { t, seedTypeName, folderErrorText } from './i18n'
...
export function ProjectsTab() {
  ...
  const handleSave = useCallback(async () => {
    if (!editing || !canSaveProject(editing)) return
    // Folders are optional: conversations without them get their own EYAS workspace.
    const payload = projectSavePayload(editing)
    try {
      setSaveError(null)
      if (editing.id) {
      ...
      setSaveError(folderErrorText(err, t('projects.saveFailed')))
    }
  }, [editing, refetch])

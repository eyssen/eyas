...
const EMPTY_PROJECT: ProjectForm = {
  name: '', typeId: '', description: '', prompt: '', color: '', defaultAgentId: '', indexedSourceIds: [], workingDirectories: [''],
}
...
export function ProjectsTab() {
  ...
  const handleEdit = useCallback((p: Project) => {
    setEditing({
      ...
      workingDirectories: p.workingDirectories?.length ? [...p.workingDirectories] : [''],
      source: p.source,
    })
  }, [])
  ...
  const handleSave = useCallback(async () => {
    ...
    const dirs = editing.workingDirectories.map((p) => p.trim()).filter(Boolean)
    if (dirs.length === 0) {
    ...
    <div>
      ...
      {editing && (
        <div className="glass-card p-4 mb-4 space-y-3">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs">{t('projects.form.workingDirs')}</Label>
            <p className="text-[11px] text-muted-foreground leading-snug">
              {t('projects.form.workingDirsHint')}
            </p>
            {editing.source === 'seed' && editing.workingDirectories.every((p) => !p.trim()) && (
              <p className="text-[11px] text-amber-600 dark:text-amber-400">
                {t('projects.form.workingDirsSeedHint')}
              </p>
            )}
            <div className="space-y-1.5">
              {editing.workingDirectories.map((path, i) => (
                <div key={i} className="flex items-center gap-1.5">
                  <div className="flex flex-col">
                    <button
                      ...
                      onClick={() => setEditing((prev) => {
                        if (!prev) return prev
                        const next = [...prev.workingDirectories]
                        ;[next[i - 1], next[i]] = [next[i], next[i - 1]]
                        return { ...prev, workingDirectories: next }
                      })}
                    ...
                    <button
                      type="button"
                      className="text-[10px] text-muted-foreground hover:text-foreground disabled:opacity-30"
                      disabled={i === editing.workingDirectories.length - 1}
                      onClick={() => setEditing((prev) => {
                        if (!prev) return prev
                        const next = [...prev.workingDirectories]
                        ;[next[i + 1], next[i]] = [next[i], next[i + 1]]
                  ...
                  <Button
                    ...
                    onClick={() => setEditing((prev) => {
                      if (!prev) return prev
                      return { ...prev, workingDirectories: prev.workingDirectories.filter((_, idx) => idx !== i) }
                    })}
              ...
              <div className="flex items-center gap-2">
                <DirectoryBrowseButton
                  startPath={editing.workingDirectories.filter((p) => p.trim()).at(-1)}
                  onPick={(picked) => setEditing((prev) => {
                    if (!prev) return prev
                    const next = prev.workingDirectories.map((p) => p.trim()).filter(Boolean)
                    next.push(picked)
                    return { ...prev, workingDirectories: next }
                  })}
                />
                <Button
                  ...
                  onClick={() => setEditing((prev) => prev ? { ...prev, workingDirectories: [...prev.workingDirectories, ''] } : prev)}
                >

...
import RecallEngineCard from './recall-engine-card'
import { t } from './i18n'
...
interface EpisodicMemory {
  id: string
  /** Present on the detail fetch only — the list ships `preview` + `contentLength`. */
  content?: string
  /** Whitespace-collapsed head of the body (400 chars), from the list endpoint. */
  preview?: string
  /** Full body length in characters, so the list can say what it is not showing. */
  contentLength?: number
  sourceType: string
...
export default function MemoryDashboard() {
  ...
  return (
    <div className="p-6 max-w-5xl">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-foreground">{mem.preview ?? mem.content}</p>
                    ...
                    <div className="text-right flex-shrink-0 space-y-1 w-28">
                      ...
                      {mem.contentLength !== undefined && (
                        <div className="text-[10px] text-muted-foreground">
                          {t('memory.episodic.chars', { count: mem.contentLength })}
                        </div>
                      )}
                      <div className="text-[10px] text-muted-foreground">
      ...
      {/* J13: read-only status of the recall engine (embedder, vectors, capture) */}
      <div className="mt-4">
        <RecallEngineCard />
      </div>

      {/* B10: owner-only quarantine of memory a provider wrote */}
      <div className="mt-4">
        <QuarantineCard />
      </div>
      </>

...
const navBtnClass =
  'flex items-center gap-2 px-3 py-2 rounded-[7px] text-[13px] transition-colors text-muted-foreground hover:text-foreground hover:bg-accent/50 w-full text-left cursor-pointer relative z-10'

function ColleaguesNav() {
  const navigate = useNavigate()
  const { data } = useApi<{ agents: { id: string; name: string; tier: string }[] }>('/agents?enabled=true')
  const colleagues = (data?.agents ?? []).filter((a) => a.tier === 'primary' || a.tier === 'team')

  const openHome = async (agentId: string) => {
    try {
      const conv = await api.post<{ id: string }>(`/agents/${agentId}/home-thread`, {})
      navigate({ to: '/conversations/$conversationId', params: { conversationId: conv.id } })
    } catch (err) {
      console.error('Failed to open colleague home thread:', err)
    }
  }
  ...
  return (
    <>
      <div className="section-label px-3 mb-1 mt-3">{t('nav.section.colleagues')}</div>
      {colleagues.map((a) => (
        <button
          key={a.id}
          type="button"
          className={navBtnClass}
          onClick={() => void openHome(a.id)}
        >
...
function NewConversationMenu() {
  const navigate = useNavigate()
  const { data: projectData, isLoading } = useApi<{ projects: { id: string; name: string; typeId: string | null }[] }>('/projects')
  const { data: typeData } = useApi<{ projectTypes: { id: string; name: string }[] }>('/project-types')
  const projects = projectData?.projects ?? []
  const types = typeData?.projectTypes ?? []

  const createIn = async (projectId: string) => {
    try {
      const conv = await api.post<{ id: string }>('/conversations', { projectId })
      navigate({ to: '/conversations/$conversationId', params: { conversationId: conv.id } })
    } catch (err) {
  ...
  const ungrouped = projects.filter((p) => !p.typeId || !types.some((tp) => tp.id === p.typeId))
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" side="right" className="min-w-[220px]">
        <DropdownMenuLabel>{t('nav.newConversation.choose')}</DropdownMenuLabel>
        {isLoading && (
          <DropdownMenuItem disabled>{t('nav.newConversation.loading')}</DropdownMenuItem>
        )}
        {!isLoading && projects.length === 0 && (
          <DropdownMenuItem onSelect={() => navigate({ to: '/projects' })}>
            {t('nav.newConversation.empty')}
          </DropdownMenuItem>
        )}
        {grouped.map((g, i) => (
          <DropdownMenuGroup key={g.id}>
            ...
            {g.projects.map((p) => (
              <DropdownMenuItem key={p.id} onSelect={() => createIn(p.id)}>
                {p.name}
              </DropdownMenuItem>
            ))}
          </DropdownMenuGroup>
        ...
        {ungrouped.length > 0 && (
          <DropdownMenuGroup>
            {(grouped.length > 0) && <DropdownMenuSeparator />}
            <DropdownMenuLabel className="text-[11px] text-muted-foreground font-normal">
              {projectsT('projects.types.ungrouped')}
            </DropdownMenuLabel>
            {ungrouped.map((p) => (
              <DropdownMenuItem key={p.id} onSelect={() => createIn(p.id)}>
                {p.name}
  ...
  return (
    <aside className="w-[220px] vibrancy border-r border-[var(--vibrancy-border)] flex flex-col flex-shrink-0">
      <nav className="flex-1 p-2 pt-3 flex flex-col gap-0.5 overflow-y-auto">
        ...
        {renderNavLink({ path: '/opencode', labelKey: 'nav.opencode', icon: SquareTerminal })}

...
interface WorkingDirectoriesEditorProps {
  ...
  label?: string
  hint?: string
  /** Which default label/hint to show: a project's own folders, or a project type's defaults. */
...
export function WorkingDirectoriesEditor({
  ...
  return (
    <div className="space-y-1.5">
      <Label className="text-xs">{label ?? t(variant === 'project' ? 'projects.form.workingDirs' : 'projects.types.workingDirs')}</Label>
      <p className="text-[11px] text-muted-foreground leading-snug">
        {hint ?? t(variant === 'project' ? 'projects.form.workingDirsHint' : 'projects.types.workingDirsHint')}
      </p>

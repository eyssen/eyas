...
import { a2aTaskPrompt } from './submodules/a2a/task-prompt.js'
...
import { createActiveVoiceResolver, createVoiceScopeResolver, type VoiceScopeResolverDeps } from './active-voice-resolver.js'
import { FALLBACK_CHANNEL_CONTEXT } from '@modules/prompt-wizard/active-voice-adapter.js'
...
import { createApprovalPing } from './approval-ping.js'
...
export const communicationModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    const handleTelegramCallback = async (input: { chatId: string; senderId: string; data: string }) => {
      const ping = (ctx as any).communication?.approvalPing as ReturnType<typeof createApprovalPing> | undefined
      if (!ping) return { text: 'Not ready' }
      return ping.handleCallback(input)
    }
    ...
    if (telegramManifest.enabled) {
      ...
      const telegram = createTelegramBot({
        ...
        onCallbackQuery: handleTelegramCallback,
        pairing: {
    ...
    if (a2aManifest.enabled) {
      ...
      const a2aExecutor = async (task: { id: string; description: string; skill?: string }) => {
        ...
        // The peer's text is fenced before it becomes the run's message.
        const taskPrompt = a2aTaskPrompt(task.description)
        // Bound to the answering agent: the run follows its model, else fixes
        // the install default on this conversation (H4).
        ...
        convService.update(conversationId, { agentId: primary.id, goalDescription: taskPrompt })

        // A peer agent, not the owner, reads the answer: no recalled owner
        // memory is pushed into this run (pull tools stay under the gate).
        const result = await agents.executeAgent(
          conversationId,
          primary.id,
          taskPrompt,
          { origin: 'delegation', audience: 'external' },
        )
    ...
    const voiceScopeDeps: VoiceScopeResolverDeps = {
    ...
    // The scope half alone, for channel replies deciding whether recalled
    // owner memory may go into the prompt. Same inputs the reply's voice is
    // resolved with (the owner-DM context, no channel id), so a conversation
    // forced to the external voice also answers without owner memory.
    const voiceScopeResolver = createVoiceScopeResolver(voiceScopeDeps)
    ...
    const runAgent = createChannelRunAgent({
      ...
      // F0 — lazy getters: prompt-wizard and observability publish these in
      // their own onStart, which may run after this call site.
      get promptAssembler() { return (ctx as any).promptAssembler },
      get contextRecorder() { return (ctx as any).contextRecorder },
      // H4 — the reply's model is one binding of the channel conversation.
      get modelBinding() { return ctx.modelBinding },
      // The reply's effort intent walks the conversation's parents.
      db: ctx.db,
      // The durable-fact pass on each channel exchange, read per reply.
      get memoryCapture() { return (ctx as any).memoryCapture },
      resolveVoiceScope: async ({ conversationId }: { conversationId: string }) =>
        (await voiceScopeResolver({ conversationId, channelId: null, channelContext: FALLBACK_CHANNEL_CONTEXT })).scope,
      logger: ctx.logger,
    } as any)
    ...
    const inboundCoordinator = createInboundCoordinator({
      ...
      createConversation: ({ source, channelId, senderId, senderName, agentId, mode, connectorId }) => {
        ...
        // Agent-bound: it follows the bound agent's model, else fixes the
        // install default on its first reply (H4).
        const conv = (ctx as any).conversations.create({
          userId: 'system',
          title: `${label}: ${senderName ?? senderId}`,
          modelBinding: agentId ? 'inherit' : 'pinned',
        })
        ;(ctx as any).conversations.update(conv.id, { agentId, mode })
        return conv.id
      },
      addMessage: (conversationId, role, content) => {
        // Inbound channel text is a third party's: peer trust (an assistant reply stays derived).
        ;(ctx as any).conversations.addMessage(conversationId, { role, content, author: 'peer', entryPath: 'channel' })
      },
      // Privacy ingress: read per message (the privacy module may start
      // later). A channel's model counts as remote — the sender cannot know
      // or choose where the agent's model runs.
      checkInbound: (text) => ctx.privacy?.checkInbound(text, { localities: ['remote'] }),
      privacyNotice,
      ...
      reply: async (msg, text) => {
        ...
        // F3 — an email reply carries a branded HTML alternative alongside the
        // text. `ChannelContent.html` has been plumbed all the way to MIME
        // since the channel layer was written and was never populated; the
        // renderer owns the markup, so nothing the model wrote becomes HTML.
        let html: string | undefined
        if (channel.type === 'email' && text) {
          try {
            html = renderHtmlEmail({ body: text }).html
          } catch (err) {
            // A rendering failure costs the reply its HTML part, never the reply.
            ctx.logger.warn({ err }, 'HTML email body failed; sending text only')
          }
        }
    ...
    const approvalPing = createApprovalPing({
      db: ctx.db,
      logger: ctx.logger,
      ...
      sendTelegram: async (chatId, content) => {
        const ch = (
          router.getChannel('telegram') ??
          router.listChannels().find((c: any) => c.type === 'telegram')
        ) as { send(id: string, body: import('./types.js').ChannelContent): Promise<void> } | undefined
        if (!ch?.send) return
        await ch.send(chatId, content)
      },
      getApproval: (id) => (ctx as any).securityGate?.autonomyPolicy?.getApproval?.(id) ?? null,
      decide: (id, status, actor) =>
        (ctx as any).securityGate?.autonomyPolicy?.decide?.(id, status, actor) ?? { ok: false },
      emitResolved: (payload) => {
        ctx.bus.emit('autonomy:approval-resolved', payload)
      },
    })
    ;(ctx as any).communication.approvalPing = approvalPing
    ctx.bus.on('autonomy:approval-requested', async (data) => {
      ...
      const id = Number(payload?.approvalId)
      if (!Number.isFinite(id)) return
      try {
        await approvalPing.notify({
          id,
        ...
        ctx.logger.warn({ err, approvalId: id }, 'Telegram approval ping failed')
      }
    })

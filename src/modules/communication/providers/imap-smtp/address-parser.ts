...
export function formatAddressList(list: EmailAddress[]): string {
  return list.map(a => {
    // CR/LF would terminate the header and let the rest be read as a new one.
    const address = a.address.replace(/[\r\n]+/g, '')
    const raw = a.name?.replace(/[\r\n]+/g, ' ').trim()
    if (!raw) return address
    // Quote the name if it contains special chars. The backslash is escaped
    // first: otherwise a name ending in one would escape the closing quote.
    const needsQuote = /[",()<>:;@\\]/.test(raw)

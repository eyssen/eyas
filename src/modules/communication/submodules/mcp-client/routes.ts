...
/** 409 body for a memory-store refusal (install, add, update, refresh). */
function memoryStoreBlockedBody(message: string) {
  return { error: message, code: MEMORY_STORE_BLOCKED }
}

/** Public server shape — never echo apiKey or env. */
function toPublicServer(s: McpServerRecord) {
  const memoryStore = mcpMemoryStoreVerdict({ name: s.name, command: s.command, args: s.args, url: s.url, env: s.env })
  return {
    id: s.id,
    name: s.name,
    ...
    status: memoryStore ? 'blocked' : s.status === 'blocked' ? 'disconnected' : s.status,
    error: memoryStore ? memoryStoreBlockedMessage(memoryStore.label) : s.status === 'blocked' ? null : s.error,
    /** 'memory_store' when EYAS refuses to run this server (a memory store outside EYAS). */
    blocked: memoryStore ? MEMORY_STORE_BLOCK : null,
...
export function createMcpRoutes(app: Hono, mcpClient: McpClient, deps?: McpRouteDeps) {
  ...
  api.post('/api/v1/mcp/servers', requirePermission('manage', 'Settings'), async (c: any) => {
    ...
    try {
      ...
      if (isMemoryStoreBlockedError(err)) return c.json(memoryStoreBlockedBody(err.message), 409)
  ...
  api.post('/api/v1/mcp/servers/:id/refresh', requirePermission('manage', 'Settings'), async (c: any) => {
    try {
      ...
      if (isMemoryStoreBlockedError(err)) return c.json({ ok: false, ...memoryStoreBlockedBody(err.message) }, 409)
  ...
  api.post('/api/v1/mcp/registry/:id/install', requirePermission('manage', 'Settings'), async (c: any) => {
    ...
    // A memory store outside EYAS is never installable, for any model.
    if (entry.memoryStore) {
      return c.json(memoryStoreBlockedBody(memoryStoreBlockedMessage(entry.name)), 409)
    }

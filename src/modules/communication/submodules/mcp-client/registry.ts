...
import type { McpAuthType, McpTransportType } from './types.js'
import { CHROME_DEVTOOLS_MCP_CATALOG_ARGS } from '@shared/chrome-devtools-mcp.js'
...
export interface McpRegistryEntry {
  ...
  args?: string[]
  envKeys?: string[]
  /** Default env merged on catalog install (stdio sidecars). */
  ...
  setupGuide?: string
  /**
   * The server's job is a second memory store outside EYAS. Such entries
   * stay listed (so the reason is visible) but can never be installed or
   * connected — see memory-store.ts. Always tier 'manual'.
   */
  memoryStore?: true
}
 ...
 *
 * Entries flagged `memoryStore` (a second memory outside EYAS) are listed
 * with the reason and are never installable — for any model.
 */
export const mcpServerRegistry: McpRegistryEntry[] = [
  ...
  },
  {
    ...
    name: 'Playwright MCP',
    description:
      'Microsoft @playwright/mcp — accessibility snapshot and element refs. Optional Playwright MCP Bridge extension for a live Chrome/Edge tab. Apache-2.0 npx sidecar. Tools arrive through the EYAS MCP bridge. Not the Python browser-use MCP.',
    tier: 'bundled',
    license: 'Apache-2.0',
    licenseCompat: 'mit-compatible',
    author: 'Microsoft',
    ...
    category: 'Browser',
    icon: '🎭',
    tags: ['browser', 'automation', 'playwright', 'a11y', 'mcp'],
    setupGuide:
      '## Playwright MCP\n\n' +
      'Apache-2.0 npx sidecar. EYAS does not vendor the package. Agent tools are discovered by the MCP client and registered as `mcp_playwright_*`.\n\n' +
      '### Doctor (fail-closed)\n' +
      'Node.js 18+ and npx must be on PATH. Telemetry is off (`DO_NOT_TRACK=1`). `--no-sandbox` and `PLAYWRIGHT_MCP_NO_SANDBOX` are stripped and refused.\n\n' +
      '### Live tab\n' +
      'Install the Playwright MCP Bridge extension, then replace `--isolated` with `--extension`. Never point `--user-data-dir` at the daily Chrome/Edge profile (Chrome 136+ blocks Default-profile CDP).\n\n' +
      '### Do not install\n' +
      'The Python `browser-use` MCP (`uvx browser-use --mcp`, `python -m browser_use`, …). It asks for an LLM API key and exposes `retry_with_browser_use_agent`. Use this catalog row, native `browser_*`, the agent-browser sidecar, or the Browser Use CLI sidecar instead.',
  },
  {
    id: 'agent-browser',
    name: 'Agent Browser',
    description:
      'Vercel agent-browser — Rust CLI MCP with @e1 snapshot refs, state save/load, domain allowlist (Apache-2.0). No LLM in the sidecar. Tools arrive through the EYAS MCP bridge. Never chat / --tools all. Never the daily Chrome profile.',
    tier: 'bundled',
    license: 'Apache-2.0',
    licenseCompat: 'mit-compatible',
    author: 'Vercel',
    ...
    category: 'Browser',
    icon: '🧭',
    tags: ['browser', 'automation', 'agent-browser', 'a11y', 'mcp'],
    setupGuide:
      '## Agent Browser\n\n' +
      'Apache-2.0 Rust CLI. EYAS does not vendor the crate. Install the published binary (`npm i -g agent-browser` or `brew install agent-browser`), then `agent-browser install` for Chrome for Testing. Set `EYAS_AGENT_BROWSER_BIN` if it is not on PATH.\n\n' +
      '### Doctor (fail-closed)\n' +
      'The extra-module doctor is `EYAS_AGENT_BROWSER_BIN` → PATH. Missing/empty binary, timeout, or `doctor --offline --quick --json` with `ok:false` → unavailable. Telemetry off (`DO_NOT_TRACK=1`). `--no-sandbox` is stripped. AI Gateway keys are stripped — do not call `chat`.\n\n' +
      '### Profile\n' +
      'Uses an EYAS-owned `--profile` directory (`data/browser/agent-browser/profile`). Never `--profile Default` or the daily Chrome/Edge profile (Chrome 136+ blocks Default-profile CDP). Prefer `state save` / `state load` for auth.\n\n' +
      '### MCP tools\n' +
      'Catalog install is `agent-browser mcp --tools core,state`. `--tools all` and `debug` are rewritten (they include `chat`). Agent tools appear as `mcp_agent_browser_*`.\n\n' +
      '### Do not install\n' +
      'The npm `mcp-agent-browser` wrapper, the Python `browser-use` MCP, Lightpanda, captcha/stealth plugins, or Vercel AI Gateway chat.',
  },
  {
    id: 'chrome-devtools',
    name: 'Chrome DevTools MCP',
    description:
      'Google chrome-devtools-mcp — live Chrome console, network, Lighthouse, and WebMCP (Apache-2.0 npx sidecar). Coding/debug lane, not form-filling. Tools arrive through the EYAS MCP bridge. WebMCP tools only if the sidecar advertises them.',
    tier: 'bundled',
    license: 'Apache-2.0',
    licenseCompat: 'mit-compatible',
    author: 'Google',
    ...
    category: 'DevTools',
    icon: '🐞',
    tags: ['chrome', 'devtools', 'debug', 'lighthouse', 'network', 'console', 'webmcp', 'mcp'],
    setupGuide:
      '## Chrome DevTools MCP\n\n' +
      'Apache-2.0 npx sidecar. EYAS does not vendor the package. This is the **coding/debug** lane (console, HAR/network, Lighthouse, WebMCP) — **not** form-filling. For public pages use native `browser_*`. For persistent auth use agent-browser. For a11y refs / a live tab use Playwright MCP.\n\n' +
      '### Doctor (fail-closed)\n' +
      'Node.js 18+ and npx must be on PATH. Telemetry is off (`DO_NOT_TRACK=1`, `--no-usage-statistics`, `--no-performance-crux`). `--no-sandbox` and `--chrome-arg=--no-sandbox` are stripped and refused. `--autoConnect` is refused (daily Chrome profile, Chrome 136+).\n\n' +
      '### Isolated profile\n' +
      'Catalog install uses `--isolated` (temporary user-data-dir). An explicit `--user-data-dir` must be EYAS-owned. Never the daily Chrome/Edge profile.\n\n' +
      '### WebMCP (fail-closed)\n' +
      'Catalog enables `--categoryExperimentalWebmcp=true`. `list_webmcp_tools` / `execute_webmcp_tool` appear as `mcp_chrome-devtools_*` **only if the sidecar advertises them** (Chrome 150+ with `--enable-features=WebMCP`, and the page registered `navigator.modelContext` tools). If they are missing, do not invent them and do not fall back to click/fill on this server.\n\n' +
      '### Do not install\n' +
      'The Python `browser-use` MCP. Do not use this server\'s `click` / `fill` / `fill_form` for forms — that is the `browser_*` lane.',
  },
  {
    id: 'docker',
    name: 'Docker',
  ...
  },
  {
    id: 'heygen',
    name: 'HeyGen',
    description: 'Talking-head avatar video, Video Agent, and speech via the hosted HeyGen MCP server. Prefer Settings → Media for generation; this catalog row is the raw MCP connection.',
    tier: 'manual',
    license: 'Proprietary',
    licenseCompat: 'proprietary',
    author: 'HeyGen',
    repoUrl: 'https://developers.heygen.com/mcp/overview',
    transport: 'sse',
    url: 'https://mcp.heygen.com/mcp/v1/',
    authType: 'oauth',
    category: 'AI',
    icon: '🗣️',
    tags: ['heygen', 'video', 'avatar', 'tts', 'mcp'],
    setupGuide:
      '## HeyGen MCP\n\n' +
      'Hosted Streamable HTTP at `https://mcp.heygen.com/mcp/v1/`. Sign in with OAuth (no API key).\n\n' +
      'Prefer **Settings → Media** to generate and ingest files. Enable this catalog row for expert/raw HeyGen tools.\n\n' +
      'MCP usage draws on your HeyGen **web plan** credits — not the separate REST API balance. Talking-head video belongs here; HTML composition is Studio / Hyperframes.',
  },

  // ── Not available: memory outside EYAS ──────────────────────────────────
  // Listed so the reason is visible; install/add/connect are refused.

  {
    ...
    setupGuide: memoryStoreGuide('a knowledge-graph JSON file'),
  },
  {
    id: 'qdrant',
    name: 'Qdrant',
    ...
    setupGuide: memoryStoreGuide('a Qdrant collection'),
  },
  {
    id: 'obsidian',
    name: 'Obsidian',
    ...
    setupGuide: memoryStoreGuide('an Obsidian vault'),
  },
]

/** Setup guide of a memoryStore entry: why it is blocked and what to use instead. */
function memoryStoreGuide(store: string): string {
  return '## Not available in EYAS\n\n' +
    `This server keeps memory outside EYAS (${store}) and would make it a live read/write source for every model. ` +
    'EYAS reads and writes memory only through its own stores, so the server cannot be installed or connected — for any model.\n\n' +
    '### Bring the memory in\n' +
    'Use **Settings → System → Data portability → Import data** for a one-way import into EYAS memory.'
}

...
import {
  ...
  isMemoryStoreBlockedError,
  mcpMemoryStoreVerdict,
  memoryStoreBlockedMessage,
  type McpServerShape,
  type MemoryStoreVerdict,
 ...
 */
export function shouldRegisterRawMcpTools<R extends { ownedBy?: string | null }>(
  record: R,
  check?: (record: R) => boolean,
): boolean {
...
export function createMcpClient(deps: {
  ...
  function shapeOf(record: McpServerRecord): McpServerShape {
    return { name: record.name, command: record.command, args: record.args, url: record.url, env: record.env }
  }

  function memoryStoreVerdictOf(record: McpServerRecord): MemoryStoreVerdict | null {
    return mcpMemoryStoreVerdict(shapeOf(record))
  }

  function markBlocked(record: McpServerRecord, verdict: MemoryStoreVerdict) {
    db.run(sql`UPDATE mcp_servers SET status = 'blocked', error = ${memoryStoreBlockedMessage(verdict.label)},
      updated_at = ${new Date().toISOString()} WHERE id = ${record.id}`)
  }

  function logBlocked(serverName: string, verdict: MemoryStoreVerdict, action: string) {
    logger.warn(
      { server: serverName, action, match: verdict.match, rule: verdict.ruleId, field: verdict.field, code: MEMORY_STORE_BLOCKED },
      'MCP server blocked: memory store outside EYAS',
    )
  }

  /**
   * Status sweep for stored rows: a flagged row becomes 'blocked' (with the
   * reason), a row the policy no longer flags leaves 'blocked'. Returns the
   * ids that are blocked now.
   */
  function syncMemoryStoreStatus(): Set<string> {
    const blocked = new Set<string>()
    for (const record of getAll()) {
      const verdict = memoryStoreVerdictOf(record)
      if (verdict) {
        blocked.add(record.id)
        if (record.status !== 'blocked') {
          markBlocked(record, verdict)
          logBlocked(record.name, verdict, 'boot')
        }
      } else if (record.status === 'blocked') {
        db.run(sql`UPDATE mcp_servers SET status = 'disconnected', error = NULL,
          updated_at = ${new Date().toISOString()} WHERE id = ${record.id}`)
      }
    }
    return blocked
  ...
  async function connectServer(record: McpServerRecord): Promise<{
    ...
    // A memory store is never spawned and its tools never reach any model.
    const verdict = memoryStoreVerdictOf(record)
    if (verdict) {
      const live = transports.get(record.id)
      if (live) {
        transports.delete(record.id)
        await live.disconnect().catch(() => {})
      }
      unregisterToolsFromRegistry(record.id)
      markBlocked(record, verdict)
      logBlocked(record.name, verdict, 'connect')
      throw new McpMemoryStoreBlockedError(verdict)
    }
  ...
  return {
    ...
    /** Add a new MCP server. A memory store is refused (McpMemoryStoreBlockedError). */
    async add(input: McpServerInput): Promise<McpServerRecord> {
      const blocked = mcpMemoryStoreVerdict(input)
      if (blocked) {
        logBlocked(input.name, blocked, 'add')
        throw new McpMemoryStoreBlockedError(blocked)
      }
    ...
    /** Update an existing MCP server. A result that is a memory store is refused (McpMemoryStoreBlockedError). */
    async update(id: string, input: Partial<McpServerInput>): Promise<McpServerRecord | null> {
      ...
      const blocked = mcpMemoryStoreVerdict({
        name: nextName,
        command: nextCommand,
      ...
      if (blocked) {
        logBlocked(nextName, blocked, 'update')
        throw new McpMemoryStoreBlockedError(blocked)
      }
    ...
    async test(id: string): Promise<{ ok: boolean; error?: string; code?: string; tools?: number }> {
      const record = getById(id)
      ...
      try {
        ...
        if (isMemoryStoreBlockedError(err)) return { ok: false, error: err.message, code: MEMORY_STORE_BLOCKED }
    ...
    /** Mark stored memory-store rows 'blocked' (and un-mark rows no longer flagged). */
    syncMemoryStoreStatus,

    /** Auto-connect all enabled servers (called on startup). Memory-store rows are marked, never connected. */
    async autoConnect() {
      const blocked = syncMemoryStoreStatus()

...
// A new message carrying a value the privacy policy blocks is refused before
// any conversation or message exists: the sender gets a localized notice and
// the row ends as skipped / privacy_blocked.
...
export interface InboundCoordinatorDeps {
  ...
  /**
   * Privacy ingress check of a NEW inbound message (a channel's model counts
   * as remote). `blocked`: it carries a block-class value and is refused
   * before a conversation, message or run exists. Absent, or returning
   * null/undefined (the privacy module is off): no check.
   */
  checkInbound?(text: string): { blocked: boolean; types: string[]; maskedText: string } | null | undefined
  /** The refusal notice for the sender, in the language of their message; it names types, never values. */
  privacyNotice?(types: string[], text: string): string
  /** Bus emit for eyas.privacy.inbound_refused (types and identity only). */
...
/** Used only when no localized notice is wired (privacy/notice.ts is the real one). */
const PRIVACY_REFUSED_FALLBACK = 'Your message was not delivered: it contains values the privacy policy does not allow to be sent to an AI model. Nothing was stored.'
...
export function createInboundCoordinator(deps: InboundCoordinatorDeps): InboundCoordinator {
  ...
  /**
   * True when the message was refused: the notice is sent first (a failed
   * send throws into the retry path, so the sender is not left without an
   * answer), then the row is closed. The notice is remembered like a reply
   * so a channel that echoes it back cannot loop.
   */
  async function refusedByPrivacy(row: InboundEventRow, msg: InboundMessage): Promise<boolean> {
    const verdict = deps.checkInbound?.(msg.content)
    if (!verdict?.blocked) return false
    const notice = deps.privacyNotice?.(verdict.types, msg.content) ?? PRIVACY_REFUSED_FALLBACK
    rememberSent(msg, notice)
    await deps.reply(msg, notice)
    db.run(
      sql`UPDATE channel_inbound_events SET status='skipped', content=${verdict.maskedText}, last_error=${PRIVACY_BLOCKED_CODE}, delivered_at=${iso(now())} WHERE id=${row.id}`
    )
    try {
      deps.emit?.(PRIVACY_INBOUND_REFUSED_EVENT, {
        targetId: null,
        conversationId: null,
        source: msg.source,
        types: verdict.types,
    ...
    } catch { /* the audit trail must never fail an inbound row */ }
    logger.info({ id: row.id, source: msg.source, types: verdict.types }, 'Inbound message refused by the privacy policy')
    return true
  }
  ...
  async function processRow(row: InboundEventRow): Promise<void> {
    ...
    try {
      // Privacy ingress: a NEW message (Phase A not done yet) with a
      // block-class value is refused before a conversation, a message or a
      // run exists. The sender gets a notice in their own language; the row
      // is closed as skipped / privacy_blocked and keeps only the masked text.
      if (!row.conversation_id && (await refusedByPrivacy(row, msg))) return

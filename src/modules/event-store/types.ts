...
/**
 * Who executed a recorded tool call: EYAS's own executor ('eyas', also a CLI's
 * bridged EYAS tool), or the provider's runtime ('provider', a CLI-native
 * tool). toolName is always the canonical name; rawName the provider's own.
 */
const ToolExecutedBy = z.enum(['eyas', 'provider'])
...
export const ToolCallPayload = z.object({
  ...
  executedBy: ToolExecutedBy.optional(),
})
...
export const LlmResponsePayload = z.object({
  response: z.object({
    ...
    usage: z
      ...
      .optional(),
    // The pair that answered this call (the response's own, not the requested
    // one). Provenance only: a malformed value is dropped, never the event.
    provider: z.string().min(1).max(200).optional().catch(undefined),
    model: z.string().min(1).max(300).optional().catch(undefined),
  }),
  // How the run entered EYAS (memory provenance: interactive, background, team…).
  entryPath: z.string().min(1).max(64).optional().catch(undefined),
})

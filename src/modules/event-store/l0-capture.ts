...
// Provenance comes from the payload the runner writes: the provider and model
// that answered the call, and how the run entered EYAS (agent-runner.ts). A
// run's tool I/O and reasoning are captured by memory/v2/run-capture.ts.
  ...
  try {
    ...
    captureUnit({
      ...
      meta: {
        ...
        provider: textOrNull(response?.provider),
        model: textOrNull(response?.model),
        agentId: session.agent_id ?? scope.agentId ?? null,
        entryPath: textOrNull((payload as { entryPath?: unknown }).entryPath),
      },
    })
  } catch {

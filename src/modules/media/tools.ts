...
const EMPTY_PROVIDER =
  'No media provider configured for this kind. Open /media and connect Magnific, Higgsfield, fal, or HeyGen.'
  ...
  return [
    {
      name: 'media_generate',
      description:
        'Generate, upscale, or edit media via a configured provider (Magnific, Higgsfield, fal, or HeyGen). Returns job id(s) immediately — call media_wait for a terminal result. Use media_catalog before inventing model ids. Talking-head / presenter video: provider heygen. HTML composition is Studio (hyperframes_*), not this tool.',
      category: 'custom',
      riskTier: 'yellow',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          provider: {
            type: 'string',
            description: 'Pin a single provider: magnific | higgsfield | fal | heygen',
          },
          providers: {
            type: 'array',
            items: { type: 'string' },
          ...
          options: {
            ...
            properties: {
              ...
              avatar_id: { type: 'string', description: 'HeyGen avatar look id — scripted avatar video instead of Video Agent' },
              voice_id: { type: 'string', description: 'HeyGen voice id for avatar video or speech' },
              engine: { type: 'string', description: 'HeyGen avatar engine (e.g. avatar_v)' },
            },
          },
        },
        required: ['kind'],

...
const PROVIDER_ID = 'higgsfield'
const SERVER_NAME = 'higgsfield'
const MCP_URL = 'https://mcp.higgsfield.ai/mcp'
const REST_KEY = 'higgsfield-api-key'
const REST_SECRET = 'higgsfield-api-secret'
export const HIGGSFIELD_REST_FALLBACK_ERROR =
  'Higgsfield REST fallback is not implemented in v1 — connect MCP via /media'
const OAUTH_REQUESTER = { userId: 'system', role: 'owner', trusted: true } as const

const GENERATE_TOOLS: Partial<Record<MediaKind, string>> = {
  image: 'generate_image',
  upscale: 'generate_image',
  edit: 'generate_image',
...
export function createHiggsfieldAdapter(deps: {
  ...
  async function readRestKeys(): Promise<boolean> {
    try {
      const key = await secrets.get(REST_KEY, 'system', OAUTH_REQUESTER)
      const secret = await secrets.get(REST_SECRET, 'system', OAUTH_REQUESTER)
      restKeysKnown =
        typeof key === 'string' && key.length > 0
        && typeof secret === 'string' && secret.length > 0
      return restKeysKnown
    } catch {
      restKeysKnown = false
      return false
    }
  ...
  const adapter: MediaProvider = {
    ...
    async catalog(kind?: MediaKind): Promise<MediaModel[]> {
      // v1 fixture has no models_list — do not invent 40 Soul/model endpoints.
      void kind
      return []
    },

    async generate(req: MediaGenerateRequest): Promise<MediaJob> {
      const rest = await readRestKeys()
      if (!mcpReady() && rest) {
        throw new Error(HIGGSFIELD_REST_FALLBACK_ERROR)
      }
      const ts = new Date().toISOString()
      const tool = GENERATE_TOOLS[req.kind]
      if (!tool) {
        return failedJob(req, `Higgsfield does not support kind ${req.kind}`, ts)
      }
    ...
    async status(providerJobId: string) {
      try {
        const resp = await invoke('creations_wait', { task_id: providerJobId, id: providerJobId })
        const payload = parseMcpPayload(resp)
        if (payload.error) {
    ...
    async cancel(_jobId: string) {
      // Higgsfield MCP has no cancel tool in the v1 fixture.
    },
    ...
    async balance(): Promise<MediaBalance | null> {
      try {
        ...
        const payload = parseMcpPayload(resp)
        if (payload.error) {
          logger.warn({ error: payload.error }, 'Higgsfield account_balance failed')
          return null
        }
        return {
        ...
        logger.warn({ err: err?.message }, 'Higgsfield account_balance failed')
        return null
      }
    },
  }
...
export function parseMcpPayload(resp: unknown): Record<string, unknown> {
  ...
  if (obj.isError) {
    ...
    return { error: text ?? 'Higgsfield tool error' }
  }

...
import { secretsFilterSql } from './v2/d1.js'
import { decodeRawBlob, NEVER_RECALLED_SOURCE_SQL } from './v2/retrieve.js'
...
export function createMemoryService(deps: MemoryServiceDeps) {
  ...
  /**
   * L0 rows plus L1/L2 facts and gists. Unless includeSecrets, a row carrying
   * the contains-secrets marker (v2/d1.ts) is left out. A raw hit is its own
   * row's text (J4, as in v2/retrieve.ts): quarantined rows, model reasoning,
   * captured tool I/O (NEVER_RECALLED_SOURCE_TYPES) and a vault note's L0 rows
   * (the note is the vault hit) never come back.
   */
  function ftsLayered(
    query: string,
    limit: number,
    ...
    try {
      ...
      for (const row of raw) {
        ...
        if (text === null) continue // nothing readable to show
        ...
        out.push({
          ...
          metadata: {
            ...
            trust: row.trust,
            ...(ex.truncated ? { truncated: true, contentLength: text.length } : {}),
          },
        })
      }
    ...
    if (needle) {
      ...
      try {
        const facts = safeAll<{ id: string; subject: string; predicate: string; objectText: string }>(sql`
          SELECT f.id, f.subject, f.predicate, f.object_text AS objectText
          FROM memory_fact f
          WHERE f.tombstoned = 0 AND f.valid_until IS NULL AND f.trust_tier != 'quarantined'
            AND ${secretsFilterSql(sql`f.rid`, includeSecrets)}
            AND (lower(f.subject) LIKE ${needle} OR lower(f.object_text) LIKE ${needle})
  ...
  return {
    ...
    vault: deps.vault,
    /**
     * Folder of the vault (in a running server the absolute
     * InstancePaths.vaultDir). Published for readers outside this module, such as the Data
     * Port undo's raw note reader, so no one has to guess where it is.
     */
    vaultPath: deps.vault.getBasePath(),
    ...
    async search(query: MemorySearchOptions): Promise<MemorySearchResult[]> {
      ...
      const layered = ftsLayered(query.query, perSource, projectId, query.excludeConversationId ?? null, includeSecrets)

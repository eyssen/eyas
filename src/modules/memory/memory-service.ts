...
export function createMemoryService(deps: MemoryServiceDeps) {
  ...
  function ftsVault(query: string, limit: number): Array<{ path: string; score: number; title: string; tier: string; tags: string[]; content: string }> {
    if (!deps.db) return []
    const fts = escapeFtsQuery(query)
    const rows = safeAll<any>(sql`
  ...
  return {
    ...
    async search(query: MemorySearchOptions): Promise<MemorySearchResult[]> {
      ...
      // Filter vault by requested tiers (semantic/procedural)
      const vaultFiltered = vaultFtsRaw.filter(v => tiers.includes(v.tier as any))

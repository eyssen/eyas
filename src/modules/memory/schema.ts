...
import { ensureConversationFts } from './search/conversation-fts.js'
...
export function createMemoryTables(db: EyasDb) {
  ...
  addColumnIfMissing(db, 'vault_index', 'project_type_id', 'project_type_id TEXT')
  db.run(sql`CREATE INDEX IF NOT EXISTS idx_vault_index_project_type ON vault_index(project_type_id)`)
  // The note's trust tier (vault/vault-trust.ts), derived by the indexer.
  // NULL = not derived yet: the indexer re-reads such a row even when its file
  // is unchanged, which is how an existing install fills the column.
  addColumnIfMissing(db, 'vault_index', 'trust_tier', 'trust_tier TEXT')
  ...
  // Which model produced this outcome, as `provider/model` (F1.2). Without it,
  // an instance with several providers can count unparsable runs but cannot say
  // which model is failing to answer in JSON — the question that took a live
  // retest to answer once already. NULL when no model was called: a gate skip
  // spends nothing, and an errored call may have failed before any provider
  // answered.
  addColumnIfMissing(db, 'memory_capture_runs', 'provider', 'provider TEXT')
  // Which run path handed the exchange over (interactive, background,
  // delegation, pipeline, a2a, team, channel, …; ingest-bridge
  // CaptureEntryPath). Every path reaches capture through one entry
  // (capture/run-end.ts); this makes "did path X capture at all" answerable.
  // NULL on rows written before the column existed.
  addColumnIfMissing(db, 'memory_capture_runs', 'entry_path', 'entry_path TEXT')
  ...
  // L0 conversation FTS — triggers only; chunked backfill is a later boot path.

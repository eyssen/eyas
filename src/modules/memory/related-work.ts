...
// episodic, then L0 conversations, then L0 memory_raw_fts (imported notes
// and transcripts). Vault window matches search_memory scope=current
// (project + type + global kinds).
...
export interface RelatedWorkOptions {
  ...
  /**
   * Default false: a vault note or episodic row tagged `contains-secrets` is
   * left out. The importer stores such a file verbatim (R11.4) and this is the
   * only place the owner's credentials are kept away from a prompt (D-7).
   */
...
function ftsVault(db: EyasDb, fts: string, limit: number): Array<{
  ...
  const rows = safeAll<{
    ...
    SELECT vi.path AS path, vi.title AS title, vi.tier AS tier,
           ...
           vi.summary AS summary, vi.tags AS tags,
...
function ftsEpisodic(
  ...
  limit: number,
  projectId: string | null,
): Array<{ id: string; content: string; tags: string | null }> {
  if (!fts || limit <= 0) return []
  // Unscoped imported rows (project_id NULL) are global, same as an unscoped
  // vault note: hiding them in a project conversation would drop the owner's
  // imported memory from every real task.
  const projectFilter = projectId
    ? sql`AND (em.project_id = ${projectId} OR em.project_id IS NULL)`
    : sql`AND em.project_id IS NULL`
  const rows = safeAll<{ id: string; content: string; tags: string | null }>(db, sql`
    SELECT em.id AS id, em.content AS content, em.tags AS tags,
...
): Array<{ id: string; conversationId: string; sourceType: string; snippet: string }> {
  if (!fts || limit <= 0) return []
  const projectFilter = projectId
    ? sql`AND (r.project_id = ${projectId} OR r.project_id IS NULL)`
    : sql`AND r.project_id IS NULL`
  return safeAll<{ id: string; conversationId: string; sourceType: string; snippet: string }>(db, sql`
    SELECT r.id AS id,
    ...
    WHERE memory_raw_fts MATCH ${fts}
      ...
      AND r.conversation_id != ${excludeConversationId}
      ${projectFilter}
    ORDER BY fts_score DESC
    LIMIT ${limit}
  `)
...
export function buildRelatedWork(db: EyasDb, opts: RelatedWorkOptions): RelatedWorkResult | null {
  ...
  // One decision for both hit lists. A malformed `tags` column reads as "no
  // tags" (hasSecretsTag), which keeps the note: losing a note to a broken
  // JSON blob would be worse than showing one the heuristic never flagged.
  ...
  // L0 conversation hits carry no secrets filter on purpose: the importer
  // never writes a conversation message. Its sinks are vault notes, episodic
  // rows, skills and agent personas — the two it can reach here are filtered
  // above.
  ...
  let rawHits: RelatedHit[] = []
  try {
    rawHits = ftsRaw(db, fts, fetchLimit, projectId, opts.conversationId)
      .filter((row) => hasQuerySignal(blob(row.snippet, row.conversationId), queryToks))
      .filter((row) => !isQueryEcho(row.snippet ?? '', queryToks))
      .map((row) => {
        const label = row.sourceType === 'document' ? 'note' : row.sourceType === 'legacy_episodic' ? 'episode' : 'l0'
        return {
          id: `raw:${row.id}`,
          line: `- [${label}] ${clipSnippet(row.snippet ?? '', maxSnippetChars)}`,
        }
      })
  } catch {
    rawHits = []
  ...
  take(conversationHits, 1, selected)
  take(rawHits, 1, selected)
  take(vaultHits, maxHits, selected)
  take(episodicHits, maxHits, selected)
  take(conversationHits, maxHits, selected)
  take(rawHits, maxHits, selected)

  const merged: RelatedHit[] = []
  for (const hit of [...vaultHits, ...episodicHits, ...conversationHits, ...rawHits]) {

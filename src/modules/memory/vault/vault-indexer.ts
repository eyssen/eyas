...
import { deriveVaultTrust, noteHasCaptureLink } from './vault-trust.js'
  ...
  return {
    indexAll(): number {
      ...
      for (const filePath of files) {
        ...
        // A row whose trust was never derived (written before the column
        // existed) is re-read once even though its file is unchanged.
        const existing = (db as any).all(
          sql`SELECT file_hash, trust_tier FROM vault_index WHERE path = ${filePath}`
        ) as any[]
        if (existing.length > 0 && existing[0].file_hash === fileHash && existing[0].trust_tier) continue
        ...
        // Who wrote the note decides how far it is trusted, once, here.
        const trustTier = deriveVaultTrust({
          frontmatter: entry.frontmatter,
          captureLinked: noteHasCaptureLink(db, filePath),
        })
        ...
        if (existingIndex.length > 0) {
          ...
          db.run(sql`INSERT INTO vault_index
            (path, title, tier, tags, content_text, kind, summary, project_id, project_type_id, trust_tier, embedding_hash, file_hash, indexed_at)

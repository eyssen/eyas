...
import {
  ...
  releaseQuarantine,
  type QuarantineDeps,
} from './v2/quarantine.js'
import { readMemoryEngineStatus, type MemoryEngineSources } from './v2/engine-status.js'
import type { EyasDb } from '@core/types'
import type { LogEntryInput } from '@modules/audit/service'
import { z } from 'zod'
...
/** What the quarantine routes (B10) reach outside the memory service; each is resolved per call and may be absent. */
export interface MemoryQuarantineHooks {
  /** Write the L0 units still buffered, so a preview and an apply see them. */
  flushPending?: () => void
  /** After a committed quarantine or release (the L3 worker's kick). */
  afterChange?: () => void
  /** The audit module's service, when it is loaded. */
  audit?: () => { log(entry: LogEntryInput): unknown } | undefined
}

/** What the recall engine card (J13) reads outside the database; each is resolved per request. */
export type MemoryEngineHooks = Omit<MemoryEngineSources, 'db'>

/** A memory_purge_log id as the history lists it (a ULID). */
const QuarantineIdSchema = z.string().regex(/^[0-9A-Za-z]{10,64}$/)

const QUARANTINE_ERROR_STATUS = {
  not_found: 404,
  already_released: 409,
  invalid_log: 422,
  note_move_failed: 500,
} as const

function userIdOf(c: unknown): string {
  try {
    const id = (c as { get?: (k: string) => unknown }).get?.('userId')
    return typeof id === 'string' && id ? id : 'owner'
  } catch {
    return 'owner'
  }
}
...
export function createMemoryRoutes(
  ...
  logger: Logger,
  extras?: { db?: EyasDb; wikilinks?: WikilinkService; quarantine?: MemoryQuarantineHooks; engine?: MemoryEngineHooks },
) {
  ...
  // ─── Quarantine (B10) ────────────────────────────────────────────────
  // The owner takes the memory a provider wrote out of recall — its L0 rows,
  // the facts and summaries derived from them, its capture notes — and can
  // put it back exactly. `delete` on MemoryEntry is held by the owner alone
  // (roles.ts): admins and users get 403. Every change is audited.
  if (extras?.db) {
    const db = extras.db
    const hooks = extras.quarantine
    const deps = (): QuarantineDeps => ({
      db,
      vaultRoot: memory.vault.getBasePath(),
      indexer: memory.indexer,
      logger,
      flushPending: hooks?.flushPending,
      afterChange: hooks?.afterChange,
    })
    const audit = (entry: LogEntryInput): void => {
      logger.info({ audit: entry.action, userId: entry.userId, target: entry.target, details: entry.details }, 'memory quarantine audit')
      try {
        hooks?.audit?.()?.log(entry)
      } catch (err) {
        logger.warn({ err, action: entry.action }, 'memory quarantine: the audit entry could not be written (the log line above records it)')
      }
    }
    const failed = (c: any, err: unknown, action: string) => {
      if (err instanceof QuarantineError) {
        return c.json({ error: err.message, code: err.code }, QUARANTINE_ERROR_STATUS[err.code])
      }
      logger.error({ err, action }, 'memory quarantine failed; nothing was changed')
      return c.json({ error: 'Quarantine failed; nothing was changed', code: 'failed' }, 500)
    }
    const selectionOf = async (c: any) => QuarantineSelectionSchema.safeParse(await c.req.json().catch(() => null))

    app.get('/api/v1/memory/quarantine', requirePermission('delete', 'MemoryEntry'), (c) => {
      return c.json({ entries: listQuarantines(db), providers: listQuarantineProviders(db) })
    })

    app.post('/api/v1/memory/quarantine/preview', requirePermission('delete', 'MemoryEntry'), async (c) => {
      const parsed = await selectionOf(c)
      if (!parsed.success) return c.json({ error: 'Invalid selection', issues: parsed.error.issues }, 400)
      try {
        return c.json({ counts: previewQuarantine(deps(), parsed.data) })
      } catch (err) {
        return failed(c, err, 'preview')
      }
    })

    app.post('/api/v1/memory/quarantine', requirePermission('delete', 'MemoryEntry'), async (c) => {
      const parsed = await selectionOf(c)
      if (!parsed.success) return c.json({ error: 'Invalid selection', issues: parsed.error.issues }, 400)
      const userId = userIdOf(c)
      try {
        const result = applyQuarantine(deps(), parsed.data, userId)
        if (result.id) {
          audit({
            userId,
            action: 'memory.quarantine.apply',
            module: 'memory',
            target: result.id,
            details: { selection: parsed.data, counts: result.counts },
            result: 'success',
            reversible: true,
          })
        }
        ...
        return failed(c, err, 'apply')
      }
    })

    app.post('/api/v1/memory/quarantine/:id/release', requirePermission('delete', 'MemoryEntry'), (c) => {
      const id = QuarantineIdSchema.safeParse(c.req.param('id'))
      if (!id.success) return c.json({ error: 'Invalid quarantine id', issues: id.error.issues }, 400)
      const userId = userIdOf(c)
      try {
        const result = releaseQuarantine(deps(), id.data, userId)
        audit({
          userId,
          action: 'memory.quarantine.release',
          module: 'memory',
          target: id.data,
          details: { counts: result.counts, renamed: result.renamed, missing: result.missing },
          result: 'success',
        })
        return c.json(result)
      } catch (err) {
        return failed(c, err, 'release')
      }
    })
  }

  // ─── Recall engine status (J13) ──────────────────────────────────────
  // Read-only: the embedder recall runs on, L3 vector coverage, D1
  // partitions, capture switches and recall settings. Counts and flags only,
  // no memory content, so `read` on MemoryEntry is enough.
  if (extras?.db && extras.engine) {
    const sources: MemoryEngineSources = { db: extras.db, ...extras.engine }
    app.get('/api/v1/memory/engine', requirePermission('read', 'MemoryEntry'), (c) => {
      try {
        return c.json(readMemoryEngineStatus(sources))
      } catch (err) {
        logger.error({ err }, 'memory engine status could not be read')
        return c.json({ error: 'Memory engine status unavailable' }, 500)
      }
    })
  }

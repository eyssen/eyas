...
/** The same MIT checkpoint with transformers.js-layout file names. */
export const E5_XENOVA_ID = 'Xenova/multilingual-e5-small'
/**
 * Where the weights are cached (transformers.js cacheDir). Relative to the
 * working directory the server runs in; `eyas doctor` reads the same path.
 */
export const E5_CACHE_DIR = 'data/models'

/** True when @huggingface/transformers can be resolved. Resolves only — never loads the runtime. */
...
/**
 * The folder holding downloaded multilingual-e5-small ONNX weights under
 * `cacheDir`, or null when none is there yet (the first load downloads them).
 */
export function findE5Weights(cacheDir: string = E5_CACHE_DIR): string | null {
  for (const repo of [E5_HF_ID, E5_XENOVA_ID]) {
    const onnxDir = join(resolve(cacheDir), ...repo.split('/'), 'onnx')
    try {
      if (existsSync(onnxDir) && readdirSync(onnxDir).some((f) => f.endsWith('.onnx'))) return onnxDir
...
export async function tryCreateE5Embedder(opts?: {
  ...
  const attempts: Array<{ id: string; options: Record<string, unknown> }> = [
    ...
    { id: E5_XENOVA_ID, options: { dtype: 'q8', session_options: session } },
  ]

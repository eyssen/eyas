...
export interface ConsolidatorDeps {
  ...
  /**
   * `memory.recall.includeSecrets` (D-7 / P-19), passed by the memory module.
   * Default false: an episodic row tagged `contains-secrets` is dropped before
   * clustering, so it is neither summarised into a vault note nor invalidated.
   * It stays a valid episodic row — hidden, not lost.
   */
...
export function createConsolidator(deps: ConsolidatorDeps): SleepTimeConsolidator {
  ...
  async function runOnce(): Promise<ConsolidationReport> {
    ...
    try {
      const episodic = deps.memory.episodic
        .list({ validOnly: true, limit: 10_000 })
        .filter((m) => deps.includeSecrets || !hasSecretsTag(m.tags))

 ...
 *
 * Returning null tells the consolidator to keep the cluster untouched (nothing
 * is invalidated), so data is not silently lost when no model can summarise
 * it; a later run tries again. Only a consolidator built WITHOUT a promoter
 * falls back to invalidate-only.
 */

...
//   secrets marker = ANY source carries it (a fact or gist derived from a
//   contains-secrets row is itself contains-secrets, see d1.ts).
...
import { markSecrets, SECRETS_TAG, SECRETS_TAG_TYPE } from './d1.js'
...
interface SourceRow {
  ...
  project: Set<string>
  task: Set<string>
  /** The row carries the contains-secrets marker. */
  secrets: boolean
}
...
function loadSources(db: EyasDb, scope: ArbitrationScope): Map<string, SourceRow> {
  ...
  scope.sourceRawIds.forEach((id, i) => {
    ...
    const tags = db.all<{ tag_type: string; tag_value: string }>(
      sql`SELECT tag_type, tag_value FROM memory_tag WHERE memory_rid = ${row.rid}
        AND (tag_type IN ('project', 'task') OR (tag_type = ${SECRETS_TAG_TYPE} AND tag_value = ${SECRETS_TAG}))`,
    )
    const source: SourceRow = {
      id, rid: row.rid, trust: scope.sourceTrustTiers[i] ?? row.trust_tier, sourceType: row.source_type,
      occurredAt: row.occurred_at, project: new Set(), task: new Set(), secrets: false,
    }
    for (const t of tags) {
      if (t.tag_type === 'project') source.project.add(t.tag_value)
      else if (t.tag_type === 'task') source.task.add(t.tag_value)
      else source.secrets = true
    }
    out.set(id, source)
  })
  return out
...
function commitFacts(
  ...
  for (const fact of candidate.facts) {
    ...
    //    hid both gaps.
    const dup = w.db.all<{ id: string; rid: number }>(sql`SELECT f.id, f.rid FROM memory_fact f
    ...
    if (dup) {
      ...
      // A new source that holds secrets makes the existing fact one too: from
      // now on it is also evidence of that content.
      if (own.some((s) => s.secrets)) markSecrets(w.db, dup.rid, 'fact')
      out.linked++
      continue
    }
    ...
    if (own.some((s) => s.secrets)) markSecrets(w.db, rid, 'fact')
    out.inserted++
...
function commitGist(
  ...
  if (sources.some((s) => s.secrets)) markSecrets(w.db, rid, 'gist')
  out.gistId = id
  return out
}

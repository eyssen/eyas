...
export interface L0WireConfig {
  ...
  chunkTokens: number
  /** Read only to warn at boot; the run capture reads the flag itself per call (capturePolicy). */
  captureToolResults: boolean
  /** Same, for model reasoning (memory.l0.captureThinking); absent = off. */
...
export async function wireL0Capture(ctx: L0WireContext): Promise<MemoryIngest | null> {
  ...
  if (ctx.config().captureToolResults) {
    ...
    logger.warn('L0 capture: memory.l0.captureToolResults is ON — tool results (arguments and output) are stored verbatim and unredacted, and nothing scans or encrypts them at rest (they are never recalled or quoted into a prompt)')
  }
  if (ctx.config().captureThinking) {
    logger.warn('L0 capture: memory.l0.captureThinking is ON — model reasoning is stored verbatim and unredacted (audit only: never extracted, never recalled)')
  }

...
// re-run is a no-op. The one exception is migrateMemoryV2Schema(): SQLite
// cannot alter a CHECK constraint, so widening memory_raw.source_type is a
// guarded, single-transaction table rebuild that keeps every rid. Three rules
...
/**
 * '1' = the Phase 0 tables. '2' = memory_raw.source_type also accepts
 * 'thinking' (a fresh database is created at '2'; an existing one is rebuilt
 * by migrateMemoryV2Schema).
 */
export const MEMORY_V2_SCHEMA_VERSION = '2'
export const EMBEDDING_DIMENSIONS = 384

/**
 * The L0 source vocabulary — the single list behind the memory_raw CHECK and
 * RawSourceType. 'thinking' (model reasoning) stays in L0 only: it is never
 * extracted and never recalled, so a model cannot reinforce its own reasoning.
 * There is deliberately no 'compaction' type: no compaction summary is
 * written to memory.
 */
export const RAW_SOURCE_TYPES = [
  'user_message', 'assistant_message', 'tool_result', 'document', 'r6_sync', 'legacy_episodic', 'thinking',
] as const
...
/** memory_raw's DDL under a given name, so the v2 rebuild creates exactly the table a fresh database gets. */
function memoryRawDdl(table: string): string {
  const sourceTypes = RAW_SOURCE_TYPES.map((t) => `'${t}'`).join(',')
  return `CREATE TABLE IF NOT EXISTS ${table} (${SYNC_COLUMNS},
...
/** The memory_raw indexes; DROP TABLE removes them, so the v2 rebuild recreates them from here. */
function createRawIndexes(db: EyasDb): void {
...
function createCoreTables(db: EyasDb): void {
  ...
  run(db, memoryRawDdl('memory_raw'))
  createRawIndexes(db)
}
...
/**
 * Creates the v2 tables at the current version. It never alters an existing
 * table, so a database created at an older version additionally needs
 * migrateMemoryV2Schema() — memory/index.ts runs both at boot, in that order.
 */
...
export interface MemoryV2MigrationResult {
  /** True when this call rebuilt memory_raw. */
  rebuilt: boolean
  /** memory_raw rows carried over by the rebuild; 0 when nothing was rebuilt. */
  rows: number
  /** memory_meta schema_version before the call; null when unset or when there is no memory_raw. */
  fromVersion: string | null
  /** memory_meta schema_version after the call; null when there is no memory_raw to migrate. */
  toVersion: string | null
}

/** Name of the transient table the rebuild copies into; it only ever exists inside the migration's transaction. */
const RAW_REBUILD_TABLE = 'memory_raw_new'

/**
 * The source types the stored memory_raw CHECK accepts, read from its CREATE
 * statement in sqlite_master. undefined = there is no memory_raw table;
 * null = a table whose source_type CHECK could not be read.
 */
function storedRawSourceTypes(db: EyasDb): Set<string> | null | undefined {
  const ddl = db.all<{ sql: string | null }>(sql`SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'memory_raw'`)[0]?.sql
  if (ddl === undefined || ddl === null) return undefined
  const check = /CHECK\s*\(\s*source_type\s+IN\s*\(([^)]*)\)\s*\)/i.exec(ddl)
  if (!check) return null
  return new Set([...check[1].matchAll(/'([^']*)'/g)].map((m) => m[1]))
}

/**
 * Whether the stored memory_raw CHECK accepts a source type. null means
 * nothing is known — no memory_raw table (createMemoryV2Tables failed) or an
 * unreadable CHECK — and callers must not treat that as a refusal.
 */
export function memoryRawAcceptsSourceType(db: EyasDb, sourceType: string): boolean | null {
  const types = storedRawSourceTypes(db)
  if (!types) return null
  return types.has(sourceType)
}

/** Raise schema_version to the current one; never lowers a newer value. */
function raiseSchemaVersion(db: EyasDb, current: string | null): string {
  if (current !== null && Number(current) >= Number(MEMORY_V2_SCHEMA_VERSION)) return current
  setMemoryMeta(db, 'schema_version', MEMORY_V2_SCHEMA_VERSION)
  return MEMORY_V2_SCHEMA_VERSION
}

/**
 * v1 → v2: widen memory_raw.source_type with every RAW_SOURCE_TYPES entry the
 * stored CHECK lacks (today: 'thinking'). SQLite cannot alter a CHECK, so the
 * table is rebuilt in ONE `BEGIN IMMEDIATE` transaction: create the new table,
 * copy every row by explicit column list, drop the old table (which drops its
 * indexes), rename, recreate the indexes, record schema_version. rid is
 * copied unchanged, so memory_raw_fts (rowid = rid), memory_tag, memory_item
 * and every blob reference stay valid; nothing references memory_raw itself.
 * Foreign keys stay ON: each copied row re-proves its memory_item parent, and
 * any violation rolls the whole rebuild back.
 *
 * Idempotent: a table whose CHECK already accepts every type is not touched
 * (only a stale schema_version is raised). Throws after rolling back, leaving
 * the v1 table in place; memory/index.ts logs it and the ingest then drops
 * the units the old CHECK would refuse instead of failing whole flushes. The
 * BEGIN is outside the try on purpose: inside a caller's transaction it
 * throws before anything runs, and the catch can never roll back their work.
 */
export function migrateMemoryV2Schema(db: EyasDb): MemoryV2MigrationResult {
  const stored = storedRawSourceTypes(db)
  if (stored === undefined) return { rebuilt: false, rows: 0, fromVersion: null, toVersion: null }
  const fromVersion = getMemoryMeta(db, 'schema_version')
  // An unreadable CHECK (null) is rebuilt to the canonical table too.
  const missing = RAW_SOURCE_TYPES.filter((t) => !stored?.has(t))
  if (missing.length === 0) return { rebuilt: false, rows: 0, fromVersion, toVersion: raiseSchemaVersion(db, fromVersion) }

  db.run(sql`BEGIN IMMEDIATE`)
  let rows = 0
  try {
    run(db, `DROP TABLE IF EXISTS ${RAW_REBUILD_TABLE}`)
    run(db, memoryRawDdl(RAW_REBUILD_TABLE))
    const columns = db.all<{ name: string }>(sql.raw(`PRAGMA table_info(${RAW_REBUILD_TABLE})`)).map((c) => c.name).join(', ')
    run(db, `INSERT INTO ${RAW_REBUILD_TABLE} (${columns}) SELECT ${columns} FROM memory_raw`)
    const before = db.all<{ c: number }>(sql`SELECT COUNT(*) AS c FROM memory_raw`)[0]?.c ?? 0
    rows = db.all<{ c: number }>(sql.raw(`SELECT COUNT(*) AS c FROM ${RAW_REBUILD_TABLE}`))[0]?.c ?? 0
    if (rows !== before) throw new Error(`memory_raw rebuild copied ${rows} of ${before} rows`)
    run(db, 'DROP TABLE memory_raw')
    run(db, `ALTER TABLE ${RAW_REBUILD_TABLE} RENAME TO memory_raw`)
    createRawIndexes(db)
    setMemoryMeta(db, 'schema_version', MEMORY_V2_SCHEMA_VERSION)
    db.run(sql`COMMIT`)
  } catch (err) {
  ...
  return { rebuilt: true, rows, fromVersion, toVersion: MEMORY_V2_SCHEMA_VERSION }
}

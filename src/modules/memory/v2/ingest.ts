...
// memory_tag(layer=raw); secrets-backfill.ts only adds the contains-secrets
// marker to rows written before it existed. Units arrive from the bridge with
// a capture-time ULID; they are buffered per conversation and flushed on task close, idle
...
import { allocateRid, memoryRawAcceptsSourceType } from './schema.js'
import { detectLanguage } from './language.js'
import { markSecrets } from './d1.js'
...
export function createMemoryIngest(deps: MemoryIngestDeps): MemoryIngest {
  ...
  // Source types the stored memory_raw CHECK refuses (a v1 table left in place by
  // a failed migrateMemoryV2Schema). Such a unit would fail its whole flush and be
  // re-queued forever, blocking the conversation's L0, so it is dropped at enqueue
  // instead. Answers are cached for the ingest's life: the schema only changes at
  // boot, before the ingest exists. An unknown answer (no table yet) is not cached
  // and never drops anything — the flush then fails and retries as before.
  const acceptedSourceTypes = new Map<string, boolean>()
  const refusedWarned = new Set<string>()

  function schemaAccepts(sourceType: string): boolean {
    const cached = acceptedSourceTypes.get(sourceType)
    if (cached !== undefined) return cached
    let answer: boolean | null = null
    try {
      answer = memoryRawAcceptsSourceType(db, sourceType)
    } catch {
      /* unreadable schema: behave as before and let the flush report it */
    }
    if (answer === null) return true
    acceptedSourceTypes.set(sourceType, answer)
    return answer
  }
  ...
  function writeUnit(unit: BufferedUnit): 'inserted' | 'skipped' | 'inserted_new_blob' {
    ...
    if (unit.secrets === true) markSecrets(db, rid, 'raw')
  ...
  const ingest: MemoryIngest = {
    enqueue(raw: CaptureUnit): void {
      if (!schemaAccepts(raw.sourceType)) {
        if (!refusedWarned.has(raw.sourceType)) {
          refusedWarned.add(raw.sourceType)
          logger.warn({ sourceType: raw.sourceType }, 'L0 ingest: memory_raw is still at an older schema that refuses this source type (the boot migration failed); such units are dropped')
        }
        return
      }

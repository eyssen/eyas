...
import { clipAtSentence, heuristicLeafGist } from './gist.js'
...
/** Spec §14: one `states` fact per imported document / legacy episodic row. */
export const IMPORTED_STATES_MAX_CHARS = 400
const IMPORTED_STATES_CONFIDENCE = 0.4
const IMPORTED_SOURCE_TYPES = new Set<RawSourceType>(['document', 'legacy_episodic'])
...
function structuralFacts(units: ExtractionUnit[], language: string): CandidateFact[] {
  const facts: CandidateFact[] = []
...
function noteSubject(content: string, sourceType: RawSourceType): string {
  const title = content.match(/^title:\s*["']?(.+?)["']?\s*$/m)?.[1]?.trim()
  if (title && title.length >= 2 && title.length <= 80) return title.toLowerCase()
  const heading = content.match(/^#\s+(.+)$/m)?.[1]?.trim()
  if (heading && heading.length >= 2 && heading.length <= 80) return heading.toLowerCase()
  return sourceType === 'document' ? 'note' : 'episode'
}

/** One `states` fact per imported vault note / episodic row (spec §14). */
export function importedNoteFacts(units: ExtractionUnit[]): CandidateFact[] {
  const facts: CandidateFact[] = []
  const seen = new Set<string>()
  for (const unit of units) {
    if (!IMPORTED_SOURCE_TYPES.has(unit.sourceType)) continue
    const object = clipAtSentence(stripFrontmatter(unit.content), IMPORTED_STATES_MAX_CHARS)
    if (object.length < 8) continue
    const subject = noteSubject(unit.content, unit.sourceType)
    const key = `${subject}|states|${object.toLowerCase()}`
    if (seen.has(key)) continue
    seen.add(key)
    facts.push({
      subject,
      predicate: 'states',
      object,
      confidenceHint: IMPORTED_STATES_CONFIDENCE,
      sourceRawIds: [unit.id],
    })
  }
  return facts
}
...
export function extractDeterministic(units: ExtractionUnit[], ctx: ExtractionContext): ExtractionCandidate {
  ...
  const importedUnits = units.filter((u) => IMPORTED_SOURCE_TYPES.has(u.sourceType))
  const kvUnits = [...userUnits, ...importedUnits]
  ...
  const facts = structuralFacts(kvUnits, language)
  facts.push(...importedNoteFacts(units))
  ...
  const countable = conversational.length > 0 ? conversational : importedUnits
  const importance = scoreImportance({
    messageCount: countable.length,
    userChars: (userUnits.length > 0 ? userUnits : importedUnits).reduce((n, u) => n + u.content.length, 0),
    decisionMarkers: countDecisionMarkers((conversational.length > 0 ? conversational : importedUnits).map((u) => u.content).join('\n')),

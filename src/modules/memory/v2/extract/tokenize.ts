...
// Keep rules: 4+ chars, or an ALL-CAPS code of 2+ (from the retired
// related-work lexer), on a Unicode-aware split — a Latin-only split turns
// "Szállítási" into "Sz", "ll", "t", "si".

import { escapeFtsQuery } from '../../schema.js'
import { MULTI_LANGUAGE } from '../language.js'
...
/**
 * The tokens tokenize() keeps, lazily and with their offsets in `text`, so a
 * reader can stop early (the recall gate) or cut the text around a token (a
 * recall snippet) without splitting the whole text first.
 */
export function* tokenSpans(text: string): Generator<{ token: string; index: number }> {
  const re = /[\p{L}\p{N}_]+/gu
  for (let m = re.exec(text); m !== null; m = re.exec(text)) {
    if (keepToken(m[0])) yield { token: m[0], index: m.index }
  }
}
...
const ALL_STOP_LISTS = Object.values(STOP_WORDS)

/**
 * Stop-word check in one language. 'multi' (a read-time query whose language
 * nothing reveals — language.resolveQueryLanguage) checks every list, so the
 * glue of whichever language it is stays out of the lexical query and the
 * ≥ 2-stem gate. Klingon and 'und' have no list.
 */
export function isStopWord(token: string, lang: string): boolean {
  if (lang === MULTI_LANGUAGE) {
    const folded = fold(token)
    return ALL_STOP_LISTS.some((list) => list.has(folded))
  }

...
/**
 * Units whose text a gist never quotes. Recorded tool I/O
 * (memory.l0.captureToolResults) is stored verbatim and unredacted — a file's
 * contents, a command's stdout, a browser_fill value — and a gist is recalled
 * into prompts, so it would replay exactly what retrieve.ts
 * NEVER_RECALLED_SOURCE_TYPES keeps out. It still shapes topics and entities.
 */
const NEVER_QUOTED_SOURCE_TYPES: ReadonlySet<string> = new Set(['tool_result'])

/**
 * First + last message + up to three TF-IDF sentences from the middle,
 * ≤ LEAF_GIST_MAX_CHARS. Tool output is never quoted: a batch of tool results
 * alone has no gist ('').
 */
export function heuristicLeafGist(units: GistUnit[], lang: string, db: EyasDb): string {
  const texts = units
    .filter((u) => !NEVER_QUOTED_SOURCE_TYPES.has(u.sourceType))
    .map((u) => flatten(u.content))
    .filter((t) => t.length > 0)

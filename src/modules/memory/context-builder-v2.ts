 ...
 *
 * NOT WIRED. Nothing in `src` or `tests` imports this module today, and the
 * D-7 recall sweep (P-19) therefore left it ungated. Anyone wiring it MUST
 * gate it first: it puts episodic rows and vault notes into a prompt block,
 * which makes it a model-facing consumer, and its `memory.search?.(query, …)`
 * call below does not even match the real `MemoryService.search({ … })`
 * signature. The gate is `recallIncludesSecrets(config)` plus `hasSecretsTag`
 * from `./memory-index.js`, exactly as memory-service.ts applies it.
 */

 ...
 * `rejected-shape` one (the call was made, every note failed the schema), a
 * `poison_gate` one (the call was made, the poison gate refused a note), and
 ...
 * repeatedly is exactly what a runaway guard is for). A `too-short`, `cap-reached`,
 * `no_eligible_model` or `budget_stop` row never reached a model and must not
 * consume the budget — counting those
  ...
  try {
    const row = (db.all(sql`SELECT COUNT(*) AS n FROM memory_capture_runs
      WHERE conversation_id = ${conversationId}
        AND (skipped_reason IS NULL OR skipped_reason IN ('unparsable', 'error', 'rejected-shape', 'poison_gate'))`) as Array<{ n: number }>)[0]

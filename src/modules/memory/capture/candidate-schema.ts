 ...
 */
export const CANDIDATE_KINDS = ['user', 'feedback', 'project', 'domain', 'reference'] as const
...
/** The kinds only the owner's own words can open: who the owner is, and how to work. */
export const OWNER_KINDS: readonly CandidateNote['kind'][] = ['user', 'feedback']

export interface CandidateSchemaOptions {
  allowProject: boolean
  allowDomain?: boolean
  /**
   * `false` when a peer (a channel contact, an A2A agent) wrote the message:
   * a `user` or `feedback` note is then REJECTED, not downgraded. A peer's
   * self-description is not the owner's, and a peer's "always do X" is not a
   * rule for how to work; as a `reference` it would still reach every prompt.
   * Default true.
   */
  allowOwnerKinds?: boolean
}
 ...
 * its siblings.
 */
export function createCandidateSchema(opts: CandidateSchemaOptions) {
  return base
    .refine(
    ...
    .refine(
      (n) => opts.allowOwnerKinds !== false || !OWNER_KINDS.includes(n.kind),
      { message: 'a peer\'s message cannot open a user or feedback note' },
    )
    ...
    // steering toward correct kinds, this is the net under it. Same net for
    // `domain` when the conversation has no type to scope a type-note to.
    .transform((n) => {
      if (!opts.allowProject && n.kind === 'project') return { ...n, kind: 'reference' as const }
      if (!opts.allowDomain && n.kind === 'domain') return { ...n, kind: 'reference' as const }
      return n
    })
}

export function createCandidateBatchSchema(opts: CandidateSchemaOptions) {

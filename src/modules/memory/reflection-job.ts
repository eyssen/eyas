...
import { hasSecretsTag, recallIncludesSecrets } from './memory-index.js'

export interface ReflectionJobEpisodicPort {
  list(opts: { limit: number }): Array<{ content: unknown; tags?: unknown }>
}
...
export function registerReflectionJob(
  ...
  scheduler.registerHandler('memory.reflection', async () => {
    ...
    try {
      // D-7 / P-19 — this list becomes a model prompt below, so a row the
      // importer flagged as holding a credential must not enter it at all.
      // Fetch 40 and keep 20: a burst of freshly imported flagged transcripts
      // must cost the digest some rows, never all of them.
      const includeSecrets = recallIncludesSecrets(ctx.config)
      recentMemories = episodic.list({ limit: 40 })
        .filter((m: any) => includeSecrets || !hasSecretsTag(m.tags))
        .slice(0, 20)
        .map((m: any) => String(m.content))
        .filter(Boolean)
    } catch { /* episodic unavailable */ }

    // The summariser runs on the background model service (purpose
    // 'reflection'): an API provider or a CLI that runs isolated, never a
    // gateway-chosen provider. Fail-open — no eligible model, a budget stop or
    // a model error throws here, and buildReflectionBuckets keeps the valid
    // deterministic digest.
    const summarize = async (prompt: string): Promise<string> => {
      const aux = ctx.auxiliaryModel
      if (!aux) throw new Error('background model service unavailable')
      ...
      if (!result.ok) throw new Error(`reflection model pass skipped: ${result.reason}`)
      return result.text
    }

...
export const ODOO_TYPE_PROMPT = 'ERP workspace for source-backed implementation. Prefer the version-pinned source indexer before reading files; confirm behaviour against the checkout, not from memory. Default to local changes; treat remote or pod writes as explicit. Do not invent host paths, connection identifiers, or project-specific facts — those belong on the project. Capture facts that apply to every project of this type as domain notes, and project-only facts as project notes.'
...
export const boardModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    try { ctx.db.run(sql`ALTER TABLE project_types ADD COLUMN working_directories TEXT`) } catch { /* already exists */ }
    ...
    try { ctx.db.run(sql`ALTER TABLE projects ADD COLUMN default_connection_id TEXT`) } catch { /* already exists */ }
    try { ctx.db.run(sql`ALTER TABLE projects ADD COLUMN ticket_connection_id TEXT`) } catch { /* already exists */ }
    try { ctx.db.run(sql`ALTER TABLE projects ADD COLUMN wiki_auto_tickets INTEGER NOT NULL DEFAULT 0`) } catch { /* already exists */ }
    try { ctx.db.run(sql`ALTER TABLE projects ADD COLUMN wiki_auto_decisions INTEGER NOT NULL DEFAULT 0`) } catch { /* already exists */ }
    try { ctx.db.run(sql`ALTER TABLE projects ADD COLUMN wiki_ticket_body TEXT NOT NULL DEFAULT 'title'`) } catch { /* already exists */ }
    ...
    const projectTypeService = createProjectTypeService(ctx.db, { dataDir })
    const projectService = createProjectService(ctx.db, projectTypeService, { dataDir })
  ...
  async onStart(ctx: ModuleContext) {
    ...
    const dataDir = (ctx.config as any)?.dataDir ?? 'data'
    const { syncMissingProjectAgentsFiles } = await import('@modules/prompt-wizard/project-agents-sync.js')
    syncMissingProjectAgentsFiles(dataDir, board.projectTypes.list(), board.projects.list())
    ...
    createBoardRoutes(
      ctx.http,
      { ...board, getDesigns: () => (ctx as any).designs, getBindingResolver: () => ctx.modelBinding },
      ctx.conversations,
    )

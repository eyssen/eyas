 ...
 * occupancy inputs the conversation header uses (numerator, whether it was
 * measured, model window). Uses lightweight bulk reads — suitable for typical kanban sizes.
 */
export function enrichBoardConversations(db: any, list: ConversationWithCount[]): BoardConversationDto[] {
  ...
  return list.map((c) => {
    const ctx = occupancy.get(c.id) ?? emptyConversationContext(c)

...
export function createProjectService(db: any, typeService: ProjectTypeService): ProjectService {
  return {
    create(input: CreateProjectInput): Project {
      ...
      db.run(sql`INSERT INTO projects (id, name, description, type_id, prompt, default_agent_id, color, sort_order, created_at, updated_at)
        VALUES (${id}, ${input.name}, ${input.description ?? null}, ${input.typeId ?? null}, ${input.prompt ?? null}, ${input.defaultAgentId ?? null}, ${input.color ?? null}, 0, ${now}, ${now})`)

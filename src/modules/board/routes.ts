...
import { emptyConversationContext } from '@modules/conversations/context-occupancy.js'
import { checkWorkingDirectoriesBody, conversationWorkspaceAccess } from '@modules/tools/working-directories.js'
...
interface BoardServices {
  ...
  db?: any
  /** Lazy: the design module publishes this in its own onRegister. */
  getDesigns?: () => import('@modules/design/design-service.js').DesignService | undefined
  /**
   * Lazy ctx.modelBinding: a pair sent for a new card is stored only when it
   * is an enabled model of an active provider (the POST /conversations rule).
   * Absent: no pair is stored.
   */
  getBindingResolver?: () => Pick<import('@modules/model/binding.js').BindingResolver, 'isSelectable'> | undefined
}
...
export function createBoardRoutes(app: Hono, board: BoardServices, conversationService: ConversationService): void {
  ...
  app.post('/api/v1/project-types', requirePermission('create', 'ProjectType'), async (c) => {
    ...
    if (body.workingDirectories !== undefined) {
      const dirs = checkWorkingDirectoriesBody(body.workingDirectories)
      if (!dirs.ok) return c.json(dirs.body, 400)
      body.workingDirectories = dirs.stored
    }
  ...
  app.patch('/api/v1/project-types/:id', requirePermission('update', 'ProjectType'), async (c) => {
    ...
    if ((existing as any).source === 'seed') {
      // Seed types: identity (name/stages) stays locked; instance fills
      // prompt, sources, directories, agent, color, icon.
      const { prompt, color, icon, defaultAgentId, indexedSources, workingDirectories, defaultPriority } = body
      ...
      if (workingDirectories !== undefined) allowed.workingDirectories = workingDirectories
      if (defaultPriority !== undefined) allowed.defaultPriority = defaultPriority
      if (Object.keys(allowed).length === 0) {
        throw new HTTPException(400, { message: 'Cannot modify system resource' })
      }
      board.projectTypes.update(id, allowed)
    } else {
  ...
  app.post('/api/v1/projects', requirePermission('create', 'Project'), async (c) => {
    ...
    // Folders are optional: a conversation in a project without folders works
    // in its own EYAS workspace (created with the conversation).
    const dirs = checkWorkingDirectoriesBody(body.workingDirectories)
    if (!dirs.ok) return c.json(dirs.body, 400)
    body.workingDirectories = dirs.stored
  ...
  app.patch('/api/v1/projects/:id', requirePermission('update', 'Project'), async (c) => {
    ...
    if ((existing as any).source === 'seed') {
      ...
      const { defaultAgentId, prompt, color, description, indexedSources, workingDirectories, designSystemId, defaultConnectionId, ticketConnectionId, wikiAutoTickets, wikiAutoDecisions, wikiTicketBody } = body
      const allowed: any = {}
      if (defaultAgentId !== undefined) allowed.defaultAgentId = defaultAgentId
      if (prompt !== undefined) allowed.prompt = prompt
      if (color !== undefined) allowed.color = color
      ...
      if (indexedSources !== undefined) allowed.indexedSources = indexedSources
      if (workingDirectories !== undefined) allowed.workingDirectories = workingDirectories
      // F1 — without this the brand is unsettable on the seed projects, and
      // `general-general` is where most conversations actually land.
      if (designSystemId !== undefined) allowed.designSystemId = designSystemId
      if (defaultConnectionId !== undefined) allowed.defaultConnectionId = defaultConnectionId
      if (ticketConnectionId !== undefined) allowed.ticketConnectionId = ticketConnectionId
      if (wikiAutoTickets !== undefined) allowed.wikiAutoTickets = wikiAutoTickets
      if (wikiAutoDecisions !== undefined) allowed.wikiAutoDecisions = wikiAutoDecisions
      if (wikiTicketBody !== undefined) allowed.wikiTicketBody = wikiTicketBody
      if (Object.keys(allowed).length === 0) {
        throw new HTTPException(400, { message: 'Cannot modify system resource' })
      }
  ...
  app.post('/api/v1/projects/:id/conversations', requirePermission('create', 'Conversation'), async (c) => {
    ...
    // Folders sent with the card pass the conversation Folders check before
    // the row exists (the POST /conversations rule): the protection rules,
    // and a conversation workspace only of the caller's own conversations.
    const requestedFolders = body.workingDirectories === undefined ? null : checkWorkingDirectoriesBody(body.workingDirectories, {
      mayUseWorkspace: conversationWorkspaceAccess(null, userId, (cid, uid) => conversationService.ownsConversation(cid, uid)),
    })
    if (requestedFolders && !requestedFolders.ok) return c.json(requestedFolders.body, 400)

    // The card's model binding, the POST /conversations rule: a pair is stored
    // only when both ids name an enabled model of an active provider;
    // otherwise nothing is stamped and the card fixes the install default on
    // its first run. A colleague's card without a pair follows its colleague.
    ...
    let pair: { providerId: string; modelId: string } | null = null
    if (providerId && modelId) {
      try {
        pair = board.getBindingResolver?.()?.isSelectable({ providerId, modelId }) ? { providerId, modelId } : null
      } catch {
        pair = null
      }
    }
    const conv = conversationService.create({
      userId,
      title: body.title,
      ...(pair ? { providerId: pair.providerId, modelId: pair.modelId } : {}),
      modelBinding: (body.agentId ?? project.defaultAgentId) && !pair ? 'inherit' : 'pinned',
    })
    ...
    const workingDirectories =
      requestedFolders?.ok
        ? requestedFolders.stored
        : project.workingDirectories?.length
          ? [...project.workingDirectories]
          : null

    // Designs the project carries are copied onto the new conversation, the
    // same way indexedSources and workingDirectories are just below.
    try {
      board.getDesigns?.()?.adoptProjectDesigns(conv.id, projectId)
    } catch {
      // A design copy failing must not stop the conversation being created.
    }

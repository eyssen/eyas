...
import { serializeWorkingDirectories, validateWorkingDirectories } from '@modules/tools/working-directories.js'
...
export function createBoardRoutes(app: Hono, board: BoardServices, conversationService: ConversationService): void {
  const enrich = (list: ReturnType<ConversationService['listByProject']>) =>
    board.db ? enrichBoardConversations(board.db, list) : list.map((c) => ({
      ...
      childCount: 0,
      childrenDone: 0,
      estimatedTokens: null as number | null,
  ...
  app.post('/api/v1/project-types', requirePermission('create', 'ProjectType'), async (c) => {
    ...
    if (body.workingDirectories !== undefined) {
      const dirs = validateWorkingDirectories(body.workingDirectories, { requireNonEmpty: false })
      if (!dirs.ok) throw new HTTPException(400, { message: dirs.error })
      body.workingDirectories = dirs.entries.length ? serializeWorkingDirectories(dirs.entries) : null
    }
  ...
  app.post('/api/v1/projects', requirePermission('create', 'Project'), async (c) => {
    ...
    const dirs = validateWorkingDirectories(body.workingDirectories, { requireNonEmpty: true })
    if (!dirs.ok) throw new HTTPException(400, { message: dirs.error })
    body.workingDirectories = serializeWorkingDirectories(dirs.entries)

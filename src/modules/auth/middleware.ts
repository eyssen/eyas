...
import { acceptsDelegatedBearer } from './delegated-bearer.js'
...
export function createAuthMiddleware(deps: AuthMiddlewareDeps): MiddlewareHandler {
  return async (c, next) => {
    ...
    // isPublicApiRoute also exempts this path; skip here so the later
    // `/api/v1/mcp/*` authenticate mount does not 401 the redirect.
    if (c.req.path === '/api/v1/mcp/oauth/callback') return next()
    ...
    if (authHeader?.startsWith('Bearer ')) {
      ...
      // 2·. A module's own valid bearer on one of its exact paths (the
      //     OpenCode memory plugin's session proof, delegated-bearer.ts):
      //     passed on WITHOUT a user; the route redeems it and grants only
      //     what it allows.
      if (acceptsDelegatedBearer(c.req.path, token)) {
        c.set('authMethod', 'delegated')
        return next()
      }

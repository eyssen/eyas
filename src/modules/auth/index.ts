...
import { USER_ACCESS_CHANGED, type UserAccessChangedEvent } from './events.js'
...
export const authModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    createAuthRoutes(ctx.http, {
      ...
      // A role or status change (or an archive) goes out on the bus: modules
      // that hold a long-lived right of that user ask it again now (the
      // OpenCode terminal socket ends if its user may no longer manage
      // OpenCode). userId is the actor, targetId the changed user (audit).
      userAccessChanged: ({ actorId, targetId, role, status }) => {
        const event: UserAccessChangedEvent = {
          userId: actorId,
          targetId,
          ...(role !== undefined ? { role } : {}),
          ...(status !== undefined ? { status } : {}),
        }
        ctx.bus.emit(USER_ACCESS_CHANGED, event)
      },
    })
    ...
    createModelRoutes(ctx.http, ctx.model, undefined, ctx.providerConfig, ctx.providerReload, (ctx as any).reauthHealer, ctx.db, ctx.reasoningRegistry, ctx.cliSignIn)

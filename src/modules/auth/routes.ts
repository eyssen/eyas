...
export interface AuthRouteDeps {
  ...
  /**
   * A user's role or status was changed, or the user was archived: whoever
   * holds a long-lived right of theirs (the OpenCode terminal socket) asks
   * it again now instead of at the next connect. The auth module emits
   * USER_ACCESS_CHANGED on the bus (events.ts). Best-effort — a failure
   * must never fail the auth action itself.
   */
  userAccessChanged?: (change: { actorId: string; targetId: string; role?: string; status?: string }) => void
}
...
export function createAuthRoutes(app: Hono, deps: AuthRouteDeps): void {
  ...
  const isPublicApiRoute = (path: string): boolean => {
    ...
    // MCP OAuth callback: the IdP redirects the browser here with ?code&state.
    if (path === '/api/v1/mcp/oauth/callback') return true
    // CLI-MCP tool bridge (the Grok/Kimi stdio MCP child): authenticated by
    // the bridge's own per-call secret, bound server-side to one turn, plus a
    // loopback check (model/cli-mcp/bridge-routes.ts). The child has no
    // session. Exact paths only: every other /api/v1/internal path still
    // requires session auth.
    if (path === '/api/v1/internal/cli-mcp/tools/list') return true
    if (path === '/api/v1/internal/cli-mcp/tools/call') return true
    return false
  }
  ...
  // Auth + CSRF on privacy endpoints: the policy editor (PUT /privacy/policy
  // changes what is masked and refused) and the scan tester. The privacy
  // module depends on 'auth', so its routes are registered after these.
  router.use('/api/v1/privacy/*', authenticate)
  router.use('/api/v1/privacy/*', csrfProtection)
  ...
  // Auth + CSRF on media endpoints — generate/cancel/connect spend vendor
  // credits and egress files, so the pairing is explicit here rather than
  // left to the deny-by-default catch-all (which applies `authenticate` only).
  router.use('/api/v1/media/*', authenticate)
  router.use('/api/v1/media/*', csrfProtection)

  // Auth + CSRF on studio endpoints — render executes AI-authored HTML in
  // Chromium, so the pairing is explicit here rather than left to the
  // deny-by-default catch-all (which applies `authenticate` only).
  router.use('/api/v1/studio/*', authenticate)
  router.use('/api/v1/studio/*', csrfProtection)

  router.use('/api/v1/browser-use/*', authenticate)
  router.use('/api/v1/browser-use/*', csrfProtection)

  // Auth + CSRF on the OpenCode endpoints — settings PUT, terminal sessions
  // POST/DELETE. The two memory-plugin paths also accept the plugin's own live
  // key (delegated-bearer.ts, registered by the opencode module); it carries
  // no cookie, so csrfProtection lets it through.
  router.use('/api/v1/opencode/*', authenticate)
  router.use('/api/v1/opencode/*', csrfProtection)
  ...
  router.delete('/api/v1/users/:id', authenticate, requirePermission('delete', 'User'), (c) => {
    ...
    try { deps.closeUserSockets?.(id) } catch { /* best-effort */ }
    try { deps.userAccessChanged?.({ actorId: c.get('userId'), targetId: id, status: 'archived' }) } catch { /* best-effort */ }

...
/**
 * The `enriched_by` frontmatter block: provider, the model when the backend
 * named one, and the rung of the background-model ladder that chose it.
 */
function enrichedByFrontmatter(by: EnrichmentProvenance): Record<string, string> {
  return {
  ...
  try {
    ...
    if (input.target === 'vault.semantic' || input.target === 'vault.procedural') {
      ...
      const frontmatterOf = (noteTags: string[]): Record<string, unknown> => ({
        ...
        source: t.source,
        // Which background model suggested the metadata above, so an enriched
        // note can always be told from a deterministic one (MCI-7).
        ...(t.enrichedBy ? { enriched_by: enrichedByFrontmatter(t.enrichedBy) } : {}),

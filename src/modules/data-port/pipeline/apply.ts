...
export async function applyMemoryItem(
  ...
  const pathHint = input.relativePath ?? ''
  const source = input.sourceContent ?? input.transformed.body
  if (pathHint && (isMemoryIndexFile(pathHint, source) || isImportJunk(pathHint, source))) {
    return { status: 'skipped', reason: 'index or junk file' }
  }
  if (input.transformed.skip || !input.transformed.body.trim()) {
    return { status: 'skipped', reason: 'normalizer skipped' }
  }
  ...
  const kind = safeImportedKind(input.transformed.kind)
    ...
    if (input.target === 'vault.semantic' || input.target === 'vault.procedural') {
      ...
      deps.vault.write(
        path,
        {
          ...
          tier,
          kind,
          summary: input.transformed.summary_one_line || undefined,

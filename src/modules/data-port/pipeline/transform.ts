...
export function safeImportedKind(raw: unknown): ImportedNoteKind {
  if (typeof raw === 'string' && NOTE_KINDS.has(raw as ImportedNoteKind)) return raw as ImportedNoteKind
  return 'reference'
}
...
export async function transformMemory(
  ...
  if (parsed?.skip === true) {
    return { ...fallback, skip: true, body: '', kind: 'reference' }
  }
  if (!parsed || typeof parsed.body !== 'string' || !parsed.body.trim()) return fallback
  return {
    skip: false,
    kind: safeImportedKind(parsed.kind),

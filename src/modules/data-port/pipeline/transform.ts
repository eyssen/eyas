...
/**
 * A link as the vault stores it: the target alone, without the `[[ ]]`, the
 * embed `!`, the display text, the heading or the block anchor.
 */
export function linkTarget(raw: string): string {
  let target = raw.trim()
  if (target.startsWith('!')) target = target.slice(1).trim()
  const wrapped = /^\[\[([\s\S]*)\]\]$/.exec(target)
  if (wrapped) target = wrapped[1]!
  for (const mark of ['|', '#', '^']) {
    const at = target.indexOf(mark)
    if (at >= 0) target = target.slice(0, at)
  }
  return target.trim()
}

/**
 * One comparison key for a slug, a file stem or a title: `user_profile`,
 * `user-profile.md` and `User Profile` meet on `user-profile`. NFC first, so a
 * name read decomposed off a macOS filesystem matches its composed form.
 */
...
/**
 * The notes a model-proposed link may point at, each under the slug the vault
 * resolves it by (its file stem). The runner fills it once per job with every
 * note the vault already holds and every note the same import is about to
 * write, so a link survives only when it names a note that exists.
 */
export interface LinkIndex {
  /** Registers a note under its slug and any other name that should reach it (its title). */
  add(slug: string, names?: ReadonlyArray<string | null | undefined>): void
  /** The slug a link target names, or null when it names no known note. */
  resolve(target: string): string | null
}

export function createLinkIndex(): LinkIndex {
  const byKey = new Map<string, string>()
  // First registration wins: the vault's own notes are added before the batch,
  // so a name both use resolves to the note that is already there.
  ...
  return {
    add(slug, names = []) {
      if (!slug) return
      put(slug, slug)
      for (const name of names) if (name) put(name, slug)
    },
    resolve(raw) {
      const target = linkTarget(raw)
      if (!target) return null
      const direct = byKey.get(linkKey(target))
      if (direct) return direct
      // A path (`semantic/user-profile.md`) resolves by its last segment, the
      // way the vault graph resolves one.
      const leaf = target.replace(/\\/g, '/').split('/').filter(Boolean).pop()
      return leaf && leaf !== target ? (byKey.get(linkKey(leaf)) ?? null) : null
    },
  }
}

/**
 * What enrichment needs: the background model service. Absent, or with no
 * eligible model behind it, every note keeps its deterministic metadata.
 */
export interface EnrichmentContext {
  aux?: Pick<AuxiliaryModelService, 'complete'>
...
export interface EnrichOptions {
  ...
  sourceProfile: SourceProfile
  /**
   * The slug a model-proposed link resolves to (`LinkIndex.resolve`), or null.
   * Absent: every link the model proposes is dropped — a link is kept only when
   * something confirms the note it names exists.
   */
 ...
 * change the body, and it cannot skip a note the user selected. The call goes
 * through the background model service (purpose 'data_port_enrichment'): an
 * API provider or a CLI that can run isolated, never the chat model.
 *
 * The note records who answered (`enrichedBy`, written as `enriched_by`), and a
 * link the model proposes is kept only when it resolves to a note in the vault
 * or in the same import — as the slug that note is filed under.
 */
export async function enrichMemory(
  ...
  const unenriched = { result: base, enriched: false }
  const aux = ctx?.aux
  if (!aux) return unenriched
  ...
  if (base.tags.includes(SECRETS_TAG)) return unenriched

  let answer: AuxResult
  try {
    answer = await aux.complete({
      purpose: 'data_port_enrichment',
  ...
  // Only an answer the background model service produced is used. By that
  // service's contract it came from an isolated call on an eligible model: the
  // ladder never reaches a CLI that cannot isolate, and the request carries no
  // tier, so the gateway never hops it to another provider. No eligible model,
  // an error, an empty answer or a refusal leaves the note exactly as the
  // deterministic pass made it.
  if (!answer.ok || answer.stopReason === 'refusal') return unenriched

  const parsed = extractJson<MemoryEnrichment>(answer.text)
  if (!parsed || typeof parsed !== 'object') return unenriched
  ...
  // A link is an edge in the memory graph, and one the model made up would
  // point recall at a note that does not exist. Only a link that resolves to a
  // real note is kept, written as that note's slug. The source's own links are
  // the source's words and stay exactly as they were.
  const modelLinks: string[] = []
  let dropped = 0
  for (const proposed of stringsFrom(parsed.links)) {
    const slug = opts.resolveLink?.(proposed) ?? null
    if (slug) modelLinks.push(slug)
    else dropped++
  }
  if (dropped > 0) {
    ctx.logger?.debug(
      { path: opts.path, dropped },
      'data-port: dropped model-proposed links that name no note in the vault or this import',
    )
  }
  ...
  return {
    ...
    result: {
      ...
      links: [...new Set([...base.links, ...modelLinks])],
      salience: typeof parsed.salience === 'number' ? Math.min(1, Math.max(0, parsed.salience)) : base.salience,
      enrichedBy: { provider: answer.provider, model: answer.model, route: answer.route },
    },
  }
}

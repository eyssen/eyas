...
export const dataPortModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    const service = createDataPortService({
      db: ctx.db,
      // Read through a getter, never captured by value: the background model
      // service is published by the model module, and a job asks for it when
      // it runs. The service itself resolves its gateway per call, so the
      // privacy and tracing wrappers installed later are always in the path.
      modelCtx: {
        get aux() {

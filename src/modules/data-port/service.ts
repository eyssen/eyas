...
import { isMemoryIndexBasename, parseMemoryIndex } from './memory-index-hooks.js'
...
/** The targets that become vault notes — the only notes a link can name. */
const VAULT_TARGETS = new Set<CandidateTarget>(['vault.semantic', 'vault.procedural'])

/**
 * Every note a model-proposed link may name (MCI-7): the notes the vault index
 * already holds, then every note this job is about to write to the vault, under
 * the slug the runner files it as. Built once per enriched job, from rows alone
 * — no source file is read — over the whole selection, so a link to a note
 * later in the same import resolves too, and a resumed job sees the same set.
 */
function buildEnrichmentLinkIndex(
  db: EyasDb,
  scanId: string,
  walk: {
    filter: CandidateFilter
    isSelected: (c: StoredCandidate) => boolean
    targetOf: (c: StoredCandidate) => CandidateTarget
  },
): LinkIndex {
  const index = createLinkIndex()
  try {
    const rows = db.all(sql`SELECT path, title FROM vault_index`) as Array<{ path: string; title: string | null }>
    for (const row of rows) {
      index.add(basename(row.path).replace(/\.md$/i, ''), [row.title])
    }
  } catch {
    // No vault index on this install: only notes of this import can be linked.
  }
  for (const batch of iterateCandidates(db, scanId, walk.filter, 500)) {
    for (const c of batch) {
      if (c.kind === 'noise' || !walk.isSelected(c) || !VAULT_TARGETS.has(walk.targetOf(c))) continue
      // A memory index is filed under a slug of its own, and no note links to one.
      if (isMemoryIndexBasename(c.relativePath)) continue
      const stem = basename(c.relativePath.replace(/\\/g, '/')).replace(/\.[^.]+$/, '')
      index.add(slugFromSource(c.relativePath, c.title, c.unit ?? null), [c.title, c.unit ? null : stem])
    }
  }
  return index
}
...
export interface DataPortServiceDeps {
  db: EyasDb
  /** The background model service for opt-in enrichment; absent = model-free imports only. */
  modelCtx: EnrichmentContext
...
export function createDataPortService(deps: DataPortServiceDeps) {
  ...
  async function runJob(jobId: string): Promise<void> {
    ...
    try {
       ...
       * even then only to the background model (an API provider or a CLI that can
       * run isolated). With none eligible the service answers without a model
       * call, and each note it would have enriched counts as a fallback. There is
       * no item cap — enrichment is metadata-only and its cost is time (A-17).
       * An enriched note names what answered (`enriched_by`), and keeps a
       * model-proposed link only when it resolves to a real note (MCI-7).
       */
      const useAi = enrichRequested && Boolean(deps.modelCtx.aux)
      // The notes an enriched note may link to. Built only when a model will be
      // asked: a deterministic import keeps the source's own links and nothing else.
      const linkIndex = useAi
        ? buildEnrichmentLinkIndex(deps.db, job.scanId, {
            filter: idSet ? {} : { importable: true },
            isSelected,
            targetOf: (c) => sel.target(c),
          })
        : null
            ...
            if (WORKSPACE_TARGETS.has(target)) {
              ...
              if (useAi && bodyFlagged) {
                ...
                const enriched = await enrichMemory(deps.modelCtx, note, transformed, {
                  ...
                  sourceProfile: profile,
                  ...(linkIndex ? { resolveLink: (target: string) => linkIndex.resolve(target) } : {}),
                })

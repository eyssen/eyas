...
export interface RollbackDepsHost {
  ...
  memory?: {
    ...
    /**
     * Where the vault lives, as the memory module publishes it
     * (`memoryService.vaultPath`, i.e. InstancePaths.vaultDir). The only
     * source: without it no raw note reader is offered.
     */
    vaultPath?: string
...
export function buildRollbackDeps(input: BuildRollbackDepsInput): RollbackDeps {
  ...
  return {
    ...
    get vault() {
      ...
      const vaultPath = host.memory?.vaultPath
      return vault
        ? {
            ...
            //
            // Read from the folder the memory module published — never a
            // guessed one. Without it, or for a path that leaves the vault or
            // is not there, there is no raw read and the parsed reader —
            // verbatim since round 2 — decides on the same four digest shapes.
            ...(vaultPath
              ? {
                  readRaw: (path: string) => {

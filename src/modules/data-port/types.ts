...
export interface MemoryTransformResult {
  ...
  /**
   * The background model that suggested this note's metadata, written as
   * `enriched_by` frontmatter. Set only by `enrichMemory`, and only from an
   * answer of the background model service (an isolated call); absent on a
   * deterministic import.
   */
  enrichedBy?: EnrichmentProvenance
}

/** Who enriched an imported note: what answered, and which rung of the background-model ladder chose it. */
export interface EnrichmentProvenance {
  provider: string
  /** The model that answered; null when the backend named none (a CLI pinned by provider only). */
  model: string | null
  /** 'tier' | 'default' | 'api' | 'isolated-cli' — the auxiliary resolver's route. */
  route: string
}

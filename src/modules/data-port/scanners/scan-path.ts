...
const PRIORITY_NAME_RE =
  /^(ai-memory|skills|memory|agents|semantic|procedural|99_Meta|SKILL\.md|MEMORY\.md|CLAUDE\.md|AGENTS\.md)$/i

const PRIORITY_PATH_RE =
  /(ai-memory|\/skills\/|\/memory\/|99_Meta|SKILL\.md|MEMORY\.md|CLAUDE\.md)/i
...
/**
 * Direct children of a home-sized root we *will* enter. GitHub/develop/tmp
 * would otherwise consume MAX_DIRS_TO_VISIT before Documents/Obsidian.
 */
const WIDE_ROOT_KEEP = new Set([
  ...ALLOWED_DOT_DIRS,
  'Documents',
  'Desktop',
])
...
export function priorityScore(relPath: string): number {
  ...
  let score = 0
  if (p.includes('ai-memory')) score += 30
  if (p.includes('claude-sessions')) score -= 20
...
/** Home (or a folder that looks like one): assistant dots + Documents, not source trees. */
export function isWideScanRoot(childNames: string[]): boolean {
  const names = new Set(childNames)
  const hasAssistant = [...ALLOWED_DOT_DIRS].some((d) => names.has(d))
  const hasHuge =
    names.has('GitHub') ||
    names.has('develop') ||
    names.has('Library') ||
...
export function walkFiles(root: string, maxFiles: number): string[] {
  ...
  while ((priorityQueue.length > 0 || normalQueue.length > 0) && collected.length < maxFiles) {
    ...
    const wideRoot = dir === rootResolved && isWideScanRoot(entries.map((e) => e.name))
    ...
    for (const e of entries) {
      ...
      if (wideRoot && !WIDE_ROOT_KEEP.has(e.name)) continue
...
export function scanDirectory(opts: {
  ...
  let wideRoot = false
  try {
    wideRoot = isWideScanRoot(readdirSync(root).map((n) => n))
  } catch {
    /* ignore */
  }
  ...
  for (const full of files) {
    ...
    const hint = classifyPath(rel, content, detected)
    totalBytes += size
  ...
  return {
    ...
    warnings: [
      ...(wideRoot || resolve(root) === resolve(homedir())
        ? [
            'This path looks like a home directory. The scan stays in assistant folders and Documents (Obsidian ai-memory, ~/.grok/memory, skills) — not GitHub or other source trees. Point at a folder if you want that tree.',
          ]
        : []),
      ...(hitFileCap
        ? [

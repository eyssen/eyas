...
import { listProfiles } from '../adapters/registry.js'
...
export interface InstructionHints {
  ...
  mentionsGemini: boolean
  mentionsWindsurf: boolean
  mentionsCopilot: boolean
  pathHints: string[]
}
...
export function parseInstructionHints(instructions: string): InstructionHints {
  ...
  return {
    ...
    mentionsCodex: /\bcodex\b/i.test(instructions),
    mentionsGemini: /\bgemini\b/i.test(instructions),
    mentionsWindsurf: /\bwindsurf\b/i.test(instructions),
    mentionsCopilot: /\bcopilot\b/i.test(instructions),
    pathHints,
  }
}
...
export function applyInstructionHints(
  ...
  return candidates.map((c) => {
    ...
    if (hints.wantsMemory && c.kind === 'session') boost += 0.12
    ...
    // Soft demotion when the owner named kinds and this row is data or code: it
    // stays importable and listed, only its rank drops. Never an unselection —
    // the selection line below can raise a row, never lower one (D-8).
    if (
      (hints.wantsMemory || hints.wantsSkills) &&
      (c.kind === 'knowledge' || c.kind === 'code') &&
      boost < 0.08
    ) {
      boost -= 0.1
    }
    ...
    // A noise row returned above, so the target is the only gate left here:
    // instructions may promote a listed candidate, never an unimportable one.
    const selectedByDefault = c.target !== 'none' && (c.selectedByDefault || boost >= 0.12)
...
export function inferProfileFromInstructions(
  ...
  // Most specific first: a vault is a place, an assistant is a format, so a
  // sentence naming both ("Claude memory under Obsidian") means the vault.
  const known = new Set<string>(listProfiles())
  const guess: Array<[boolean, SourceProfile]> = [
    [h.mentionsObsidian, 'obsidian'],
    [h.mentionsClaude, 'claude-code'],
    [h.mentionsGrok, 'grok-cli'],
    [h.mentionsCursor, 'cursor'],
    [h.mentionsCodex, 'codex'],
    [h.mentionsGemini, 'gemini-cli'],
    [h.mentionsWindsurf, 'windsurf'],
    [h.mentionsCopilot, 'copilot'],
  ]
  for (const [hit, profile] of guess) {
    if (hit && known.has(profile)) return profile
  }
  return current
}

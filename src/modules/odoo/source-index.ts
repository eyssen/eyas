...
export interface OdooModelHit {
  ...
  kind: 'model' | 'inherit'
  name?: string
  /** Root label (e.g. 18c) for multi-version cites. */
  rootLabel?: string
}
...
/** A checkout root with optional label for multi-version pin. */
export interface OdooRoot {
  path: string
  label?: string
}
...
function normalizeRoots(roots: string[] | OdooRoot[]): OdooRoot[] {
  return roots.map((r) =>
    typeof r === 'string'
      ? { path: r, label: r.replace(/\/+$/, '').split('/').pop() }
      : r,
  )
}

export async function searchOdooModels(
  roots: string[] | OdooRoot[],
  query: string,
  limit = 30,
  ...
  for (const root of normalizeRoots(roots)) {
    const files: string[] = []
    await walkPyXml(root.path, files, 15_000)
    for (const file of files) {
      ...
      while ((m = modelRe.exec(text)) !== null) {
        ...
        hits.push({
          model: m[1],
          file: relative(root.path, file).split(sep).join('/'),
...
export function citeOdooSrc(hit: { rootLabel?: string; file: string; line: number }): {
  citationId: string
  cite: string
} {
  const label = hit.rootLabel ?? 'src'
  const citationId = `odoo-src:${label}:${hit.file}:${hit.line}`
  return { citationId, cite: `[source:${citationId}]` }
}

 ...
 * checkout roots (EYAS_ODOO_SOURCE_PATHS or service.sourceRoots).
 * Model-agnostic coding chain foundation (P3).
 */
export function createOdooSourceTools(
  ...
  const rootsOf = async () => {
    const svc = getService()
    return resolveConfiguredRoots(undefined, svc?.sourceRoots)
  }
  ...
  return [
    {
      ...
      description:
        'Search local Odoo source checkouts for _name / _inherit model definitions. Requires EYAS_ODOO_SOURCE_PATHS (colon-separated) or odoo.sourceRoots. Cite as [source:odoo-src:file:line].',
      category: 'search',
      riskTier: 'green',
      inputSchema: {
        type: 'object',
      ...
      execute: async (input) => {
        ...
        if (!roots.length) {
          return {
            error:
              'No Odoo source roots configured. Set EYAS_ODOO_SOURCE_PATHS to local checkout paths (e.g. /path/to/odoo-odoo-18:/path/to/enterprise-18).',
          }
        }
        ...
        return {
          ...
          models: hits.map((h) => ({
            ...h,
            citationId: `odoo-src:${h.file}:${h.line}`,
            cite: `[source:odoo-src:${h.file}:${h.line}]`,
          })),
        }
      },
    },
    {
      ...
      description:
        'Search local Odoo Python sources for fields.X assignments. Optional model filter. Cite as [source:odoo-src:file:line].',
      category: 'search',
      riskTier: 'green',
      inputSchema: {
        type: 'object',
      ...
      execute: async (input) => {
        const roots = await rootsOf()
        if (!roots.length) {
          return { error: 'No Odoo source roots configured (EYAS_ODOO_SOURCE_PATHS).' }
        }
    ...
    },
    {
      ...
      description:
        'Search local Odoo XML files for record ids / xml ids. Cite as [source:odoo-src:file:line].',
      category: 'search',
      riskTier: 'green',
      inputSchema: {
        type: 'object',

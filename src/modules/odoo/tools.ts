...
import {
  ...
  type ProjectOdooConnections,
} from './connection-resolve.js'
import {
  citeOdooSrc,
...
export interface OdooService {
  ...
  sourceRoots?: string[]
  getProjectConnections?: (projectId: string) => ProjectOdooConnections | null
  getClientForConnection?: (connectionId: string) => Promise<OdooClient | null>
}
...
const connectionIdProp = {
  connectionId: {
    type: 'string',
    description:
      'Connection id from connections_list. When omitted, ticket tools use the conversation project ticket connection; other Odoo tools use the project default.',
  },
}
...
export function createOdooTools(getService: () => OdooService | null | undefined): ToolImplementation[] {
  async function resolveLiveClient(
    ...
    ctx?: ToolContext,
  ): Promise<{ client: OdooClient | null; connectionId: string | null }> {
    const svc = getService()
    const explicit = typeof input.connectionId === 'string' ? input.connectionId : null
    const project =
      ctx?.projectId && svc?.getProjectConnections
        ? svc.getProjectConnections(ctx.projectId)
        : null
    const connectionId = resolveOdooConnectionId({
      toolName,
      explicitConnectionId: explicit,
      project,
    })
    if (connectionId) {
      const fromConn = svc?.getClientForConnection
        ? await svc.getClientForConnection(connectionId)
        : null
      if (fromConn?.configured) return { client: fromConn, connectionId }
      // A project/explicit connection was chosen — do not silently use global secrets.
      return { client: null, connectionId }
    }
    if (svc?.client?.configured) return { client: svc.client, connectionId: null }
    return { client: null, connectionId: null }
  }

  function notConfigured(connectionId: string | null) {
    if (connectionId) {
      return { error: `Odoo connection ${connectionId} is not configured`, connectionId }
    }
    return { error: 'Odoo is not configured — set odoo-* secrets and enable the odoo module' }
  }
  ...
  return [
    {
      ...
      execute: async (input, ctx) => {
        const { client: c, connectionId } = await resolveLiveClient('odoo_search_tasks', input, ctx)
        if (!c) return notConfigured(connectionId)
    ...
    },
    {
      ...
      execute: async (input, ctx) => {
        const { client: c, connectionId } = await resolveLiveClient('odoo_get_task', input, ctx)
        if (!c) return notConfigured(connectionId)
        const model = (input.model as string) || 'project.task'
        const rows = await c.read(model, [input.id as number])
        if (!rows.length) return { error: `Record not found: ${model}#${input.id}`, connectionId }
        const r = rows[0]
        return {
          task: r,
          connectionId,
    ...
    },
    {
      ...
      execute: async (input, ctx) => {
        const { client: c, connectionId } = await resolveLiveClient('odoo_message_post', input, ctx)
        if (!c) return notConfigured(connectionId)
        const model = (input.model as string) || 'project.task'
        const messageId = await c.messagePost(model, input.id as number, input.body as string)
        return { ok: true, messageId, model, id: input.id, connectionId }
      },
    },
    {
      ...
      execute: async (input, ctx) => {
        const { client: c, connectionId } = await resolveLiveClient('odoo_write_task', input, ctx)
        if (!c) return notConfigured(connectionId)
        const model = (input.model as string) || 'project.task'
        const ok = await c.write(model, [input.id as number], input.values as Record<string, unknown>)
        return { ok, model, id: input.id, connectionId }
      },
    },
  ]
}

...
export const odooModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    try {
      ...
      if (tools?.registry) {
        const { createOdooTools } = await import('./tools.js')
        for (const tool of createOdooTools(() => (ctx as any).odoo)) {
          if (!tools.registry.has(tool.name)) tools.registry.register(tool)
        }
        ctx.logger.info('Odoo tools registered on tool registry')
      }
    } catch (err) {
...
export { createOdooTools } from './tools.js'

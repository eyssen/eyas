...
export interface PermissionBridgeDeps {
  ...
  validateToolCall(
    ...
    ctx?: { conversationId?: string; agentId?: string; workingDirectories?: readonly string[]; homeDir?: string; requireHuman?: boolean },
   ...
   * by run lineage.
   *
   * `workingDirectories` are the folders the provider's turn works in (for a
   * CLI: its cwd and every conversation folder it may touch). Passed to the
   * gate unchanged, so the memory-path policy can also refuse another
   * conversation's workspace; absent = only that refinement is off.
   *
   * `homeDir` is the HOME the CLI's tools run under when EYAS gave it its own
   * (<dataDir>/cli-homes/<provider>): the gate then expands `~`/`$HOME` under
   * it as well as under the operator's home.
   */
  ctx: { conversationId?: string; agentId?: string; runId?: string; workingDirectories?: readonly string[]; homeDir?: string }
  /**
  ...
  /**
   * The resumed run's do-not-repeat ledger (toolLedgerKey entries of the
   * destructive calls its predecessors already executed — resumeRun builds it,
   * the agent runner forwards it as metadata.idempotencyLedger). A CLI runs
   * its tools itself, so this bridge is the only EYAS code that sees such a
   * call before it runs: a gate-allowed call whose key is in the ledger is
   * refused instead of firing its side effect a second time. A call whose key
   * does not match (another input, another tool) is decided as usual.
   */
  ledger?: ReadonlySet<string>
  /**
   * Observer for every non-allow verdict, called once per refused call before
   * the verdict is returned — how a provider settles the tool row it opened
   * with the real outcome and raises an approval_required event. Never called
   * for an allow. A throwing observer never changes the verdict.
   */
  onDecision?: (decision: BridgeDecision) => void
  /**
   * The kernel file sandbox this query's shell runs in (B5), set only when
   * one is applied. In 'auto' the CLI may still run a command outside it
   * when the model asks (Claude Code: Bash with dangerouslyDisableSandbox):
   * such a call goes to the gate with requireHuman — its deterministic
   * checkpoint still denies, the judge is never asked, and only a human
   * approval or grant lets it run. In 'required' the CLI ignores the request
...
/** A shell call that asks to run outside the kernel sandbox (Claude Code's Bash flag). */
function asksToLeaveSandbox(name: string, input: Record<string, unknown>): boolean {
  return name === 'Bash' && input?.dangerouslyDisableSandbox === true
}

/** How the bridge refused one call (see PermissionBridgeDeps.onDecision). */
export type BridgeDecisionOutcome = Extract<ToolOutcome, 'denied' | 'approval_required' | 'skipped'>

/**
 * One refused tool call:
 * - 'denied': the gate (or a fail-closed rule) refused it;
 * - 'approval_required': it waits on a human — `approvalId` is the queued row
 *   when one was created;
 * - 'skipped': the ledger recognised it as already executed on the original run.
 */
export interface BridgeDecision {
  /** The runtime's id for the call (the SDK toolUseID; the ACP toolCallId). */
  toolUseId: string
  /** The tool name as the gate saw it (the mcp__eyas__ prefix stripped). */
  toolName: string
...
/** The refusal a ledger hit gets: what the CLI's model reads back. */
export const LEDGER_DUPLICATE_MESSAGE = 'already executed on the original run — duplicate side effect prevented'
 ...
 *   - Every allow is finally checked against the resumed run's do-not-repeat
 *     ledger: a call already executed on the original run is refused.
 *
...
export function createPermissionBridge(deps: PermissionBridgeDeps): CanUseToolFn {
  ...
  const notify = (decision: BridgeDecision): void => {
    if (!deps.onDecision) return
    try {
      deps.onDecision(decision)
    } catch (err) {
      logger?.warn?.({ toolName: decision.toolName, err: String(err) }, 'permission bridge: onDecision observer threw (ignored)')
    }
  }
  ...
  /** Enqueue an approval, report its id to the runner's park sink and return it. */
  const enqueueApproval = (rec: {
    ...
    try {
      ...
      if (typeof approvalId === 'number') {
        deps.onEscalatedApproval?.(approvalId, rec.toolName)
        return approvalId
      }
    } catch {
  ...
  return async (toolName, input, opts) => {
    ...
    /** A refusal by the gate or a fail-closed rule. */
    const refuse = (message: string): BridgePermissionResult => {
      const result = logDeny(name, message)
      notify({ toolUseId, toolName: name, outcome: 'denied', reason: message })
      return result
    }
    /**
     * A refusal that waits on a human (approvalId: the queued row, when one
     * exists). `parkable` false: no row can ever exist for it (no
     * conversation scope), so it never interrupts the provider's loop — there
     * is nothing for the run to park on.
     */
    const awaitApproval = (message: string, approvalId: number | undefined, parkable = true): BridgePermissionResult => {
      const result = parkable ? denyForApproval(name, message) : logDeny(name, message)
      notify({ toolUseId, toolName: name, outcome: 'approval_required', reason: message, ...(approvalId !== undefined ? { approvalId } : {}) })
      return result
    }
    /**
     * Every allow ends here: the ledger has the last word. It never widens a
     * verdict — it only turns an allow into a refusal for a repeat.
     */
    const allowUnlessRepeat = (): BridgePermissionResult => {
      if (deps.ledger?.size && deps.ledger.has(toolLedgerKey(name, input))) {
        const message = `${name}: ${LEDGER_DUPLICATE_MESSAGE}`
        logger?.info?.({ toolName: name, conversationId: ctx.conversationId, runId: ctx.runId }, 'permission bridge: ledger hit — duplicate refused')
        notify({ toolUseId, toolName: name, outcome: 'skipped', reason: message })
        return deny(message)
      }
      return { behavior: 'allow' }
    }

    // A command that asks to leave the kernel sandbox ('auto' only) is a
    // human's call: the gate runs only its deterministic checkpoint for it.
    const requireHuman = deps.cliSandboxMode === 'auto' && asksToLeaveSandbox(name, input)

    let det: GateDecision
    try {
      det = await validateToolCall(name, input, requireHuman ? { ...ctx, requireHuman: true } : ctx)
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      return refuse(`security gate threw while validating ${name} — denied (fail-closed): ${msg}`)
    }
    ...
    if (!det || typeof (det as { decision?: unknown }).decision !== 'string') {
      return refuse(`security gate returned a malformed verdict for ${name} — denied (fail-closed)`)
    }

    // Only a human may let such a command run: an 'allow' (a gate switched
    // off, a stub) becomes an escalation — grant check, then an approval —
    // and neither the judge nor the autonomy ladder is ever reached.
    if (requireHuman && det.decision === 'allow') {
      det = { decision: 'escalate', reason: UNSANDBOXED_SHELL_REASON, riskTier: 'red' }
    }
    ...
    switch (det.decision) {
      case 'allow':
        break
      case 'deny':
        return refuse(`gate denied ${name}: ${det.reason}`)
      case 'escalate': {
        ...
        if (autonomy) {
          ...
          if (!ctx.conversationId) {
            return awaitApproval(`approval required for ${name} but this execution path cannot receive grants (no conversation scope): ${det.reason}`, undefined, false)
          }
          approvalId = enqueueApproval({
        ...
        return awaitApproval(`approval required for ${name} (gate escalated): ${det.reason}`, approvalId)
      }
      case 'judge_error':
        return refuse(`security judge unavailable for ${name} — denied (fail-closed): ${det.reason}`)
      default:
        return refuse(`unknown gate verdict '${(det as { decision: string }).decision}' for ${name} — denied (fail-closed)`)
    }
    ...
    if (!autonomous) return allowUnlessRepeat()

    // Autonomous: the autonomy ladder governs (strictest-wins with the gate).
    if (!autonomy) return refuse(`autonomous run without an autonomy policy — refusing ${name} (fail-closed)`)
    ...
    if (!category) return allowUnlessRepeat()

    const { level, locked } = autonomy.resolve(category)
    if (level >= 3 && !locked) return allowUnlessRepeat()
    ...
    if (!ctx.conversationId) {
      return awaitApproval(`approval required (${category}) but this execution path cannot receive grants (no conversation scope): ${det.reason}`, undefined, false)
    }

    const approvalId = enqueueApproval({
      category,
      toolName: name,
      conversationId: ctx.conversationId,
      inputJson: JSON.stringify(input),
    ...
    return awaitApproval(`approval required (${category}): ${det.reason}`, approvalId)
  }
}

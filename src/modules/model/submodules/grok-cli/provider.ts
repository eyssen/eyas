  ...
  return {
    ...
    async *stream(request: ModelRequest): AsyncIterable<StreamEvent> {
      ...
      try {
        ...
        // NOTE (F1.3): `ModelRequest.isolated` is deliberately NOT honoured
        // here, and this provider does not advertise supportsIsolatedCompletion.
        // ACP `session/new` accepts `cwd`, `mcpServers` and a `_meta` carrying
        // systemPromptOverride/maxTurns (acp-client.ts) — and nothing that stops
        // the CLI loading its OWN memory and config (~/.grok) into the session.
        // Honouring the flag halfway would claim an isolation this cannot
        // deliver, so an isolation-needing caller must select a provider that
        // advertises the capability instead. Revisit if grok gains such a
        // switch; do not fabricate one.
        //

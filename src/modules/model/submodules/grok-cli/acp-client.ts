...
export async function* runGrokAcpPrompt(opts: GrokAcpRunOptions): AsyncGenerator<StreamEvent, GrokAcpRunResult> {
  ...
  const handleSessionUpdate = (params: { sessionId?: string; update?: Record<string, unknown> }) => {
    ...
    if (kind === 'tool_call') {
      ...
      if ((update.status as string) === 'completed' || (update.status as string) === 'failed') {
        push({ kind: 'event', event: { type: 'tool_use_end' } })
      }
      return
    }
  ...
  const runPromise = (async () => {
    try {
      ...
      if (!sessionId) {
        ...
        // is valid; EYAS can inject real MCP servers later if needed.

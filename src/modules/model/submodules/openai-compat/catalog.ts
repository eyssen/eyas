...
export interface CompatProviderDef {
  ...
  baseURL: string
  /**
   * Static seed models when list/fetch is empty. Identity only: whether a
   * model takes a reasoning parameter on this gateway is never declared here
   * (nor guessed from its id) — only a capability overlay row for this
   * provider id can grant it (reasoning/overlay.json).
   */

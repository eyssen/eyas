...
export function createClaudeCodeProvider(options: ClaudeCodeProviderOptions = {}): AIProvider {
  const { loadClaudeMd = false, toolExecutor, toolRegistry, logger: providerLogger, maxTurns = DEFAULT_MAX_TURNS, getGovernance } = options
  return {
    id: 'claude-code',
    name: 'Claude Code CLI',
    // The SDK can be told to load no user/project/local filesystem settings,
    // which is what an isolated request needs — see the settingSources block
    // in stream(). (The CLI's enterprise-managed policy tier always applies.)
    ...
    async *stream(request: ModelRequest): AsyncIterable<StreamEvent> {
      ...
      // asking the CLI to be an agent: one turn, no tools, no loaded settings.
      // See ModelRequest.isolated. Every branch below is gated on it so an
      // ordinary request keeps byte-identical options.
      ...
      const queryOptions: Record<string, unknown> = {
        ...
        cwd: request.metadata?.workingDirectory || process.cwd(),
        abortController,
      }
      ...
      // An isolated call gets no tools at all — not the EYAS bridge, and not
      // the SDK's own builtins, which would otherwise remain available simply
      // because nothing set `tools`. A one-shot extraction has no business
      // reading a filesystem.
      if (isolated) queryOptions['tools'] = []
      ...
      // settingSources: [] — no user/project/local filesystem settings: no
      // settings.json (hooks, permission rules), no CLAUDE.md at any tier, no
      // host skills, no project .mcp.json. EYAS's own memory is the only
      // memory. (The CLI's enterprise-managed policy tier is NOT source-gated
      // and still applies where deployed — no client option can suppress it.)
      ...
      if (isolated || !loadClaudeMd) {
        // settingSources [] is NOT the whole isolation story (proved live,
        // 2026-08-28): the CLI's auto-memory feature keys the machine-level
        // ~/.claude/projects/<cwd-slug>/memory/MEMORY.md on the WORKING
        // DIRECTORY, not on setting sources — an extractor running with the
        // server's cwd read the owner's own memory index and judged a fact
        // "already recorded" that EYAS's vault had never seen. Filesystem-
        // configured MCP servers leak the same way. Both have explicit kill
        // switches; the env spread keeps PATH/auth intact for the spawned CLI.
        queryOptions['env'] = { ...process.env, CLAUDE_CODE_DISABLE_AUTO_MEMORY: '1' }
      ...
      // An isolated request never resumes: a resumed session restores the whole
      // SDK context of the run it belongs to — including everything the empty
      // settingSources just kept out — which would void the guarantee silently.
      // Isolation wins over a sessionId rather than co-existing with it.
      let sessionValid = false
      if (!isolated && request.sessionId) {
        try {

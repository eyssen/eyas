...
import type {
  TriageResult, TaskCategory, TaskComplexity, RoutingTier, TierConfig,
} from './types.js'
...
${message.slice(0, 500)}`
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      const tier: RoutingTier = (CATEGORY_TIER_OVERRIDE as any)[category] ?? (COMPLEXITY_TO_TIER as any)[complexity] ?? 'standard'

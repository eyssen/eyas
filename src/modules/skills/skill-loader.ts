...
/**
 * Host skill files often leave a scalar unquoted even when it contains `: `
 * (`CRITICAL: always validate`). Strict YAML then throws nested-mapping.
 * Quote those scalars; already-quoted / block / flow values stay untouched.
 */
export function sanitizeFrontmatterYaml(raw: string): string {
  return raw.split('\n').map((line) => {
    ...
    return `${key}: "${rest.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`
  }).join('\n')
}

/** Listed `trigger_patterns` / `triggers`, else a hyphenated id becomes two match keys. */
export function resolveTriggerPatterns(frontmatter: Record<string, unknown>, id: string): string[] {
  const listed = frontmatter.trigger_patterns ?? frontmatter.triggers
  if (Array.isArray(listed)) {
    const cleaned = listed.filter((x): x is string => typeof x === 'string' && x.trim().length > 0)
...
export function createSkillLoader(db: any, logger: any) {
  return {
    async loadFromDirectory(dir: string, rootId = 'config/skills'): Promise<ScanResult> {
      ...
      for (const filePath of files) {
        ...
        try {
          ...
          if (!match) continue
          const frontmatter = parseYaml(sanitizeFrontmatterYaml(match[1]))
          ...
          const triggerPatterns = resolveTriggerPatterns(frontmatter, String(id))
          ...
          if (existing.length === 0) {
            ...
            if (sameFile) {
              ...
              db.run(sql`UPDATE skills SET name = ${frontmatter.name ?? id}, description = ${frontmatter.description ?? ''},
                        ...
                        trigger_patterns = ${JSON.stringify(triggerPatterns)},
    ...
    /**
     * The newest user-owned skill with that name. Bundled rows are deliberately
     * excluded: an import must never mistake a shipped skill for its own earlier
     * work and report "unchanged", and it must never propose replacing one.
     * Names are not unique, so the most recent row wins.
     */
    getByName(name: string): Skill | null {
      const rows = db.all(
        sql`SELECT * FROM skills WHERE name = ${name} AND source = 'user' ORDER BY created_at DESC LIMIT 1`,
      ) as any[]
      return rows.length > 0 ? toSkill(rows[0]) : null
    },

...
/** Classification a skill falls into for the dead-skill inventory (see classify-skill.ts). */
export type SkillCategory = 'healthy' | 'new' | 'shadowed' | 'orphan' | 'never-used' | 'dormant'
...
export interface Skill {
  ...
  enabled: boolean
  /** Why the skill was disabled (e.g. 'user', 'orphan'). Cleared on re-enable. */
  disabledReason?: string
  /** When the skill was disabled. Cleared on re-enable. */
  disabledAt?: string
  /** Who/what disabled the skill (e.g. 'user', 'detector'). Cleared on re-enable. */
  disabledBy?: string
  /** How many times the skill has been injected into context. */
  useCount: number

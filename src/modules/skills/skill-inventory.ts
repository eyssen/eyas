...
/**
 * Extra markdown skill directories from instance config (`skills.importRoots`).
 * Same isolation-by-prefix as extension roots: a core-root orphan sweep can
 * never match an imported row, even if the directory string equals CORE_ROOT.
 */
const IMPORT_ROOT_PREFIX = 'import:'
export function importRootId(dir: string): string {
  return `${IMPORT_ROOT_PREFIX}${dir}`
}

export const SOURCE_RANK: Record<'user' | 'generated' | 'imported' | 'extension-bundled' | 'core-bundled', number> = {
  user: 40,
  generated: 30,
  imported: 25,
...
export function originRank(o: SkillOrigin): number {
  ...
  if (o.source === 'bundled' && o.root.startsWith(IMPORT_ROOT_PREFIX)) return SOURCE_RANK.imported
...
/** Instance overlay paths. Empty / missing → no extra roots (product default). */
export function resolveSkillImportRoots(config: unknown): string[] {
  const raw = (config as { skills?: { importRoots?: unknown } } | undefined)?.skills?.importRoots
...
/**
 * Scan extra markdown skill directories. Each directory is its own
 * `import:` root so provenance and shadowing stay distinct from core and
 * extension packs. A missing directory is a complete empty scan (the loader
 * already treats ENOENT that way).
 */
export async function scanSkillImportRoots(
  loader: { loadFromDirectory(dir: string, rootId?: string): Promise<ImportScanResult> },
  dirs: string[],
  ...
  for (const dir of dirs) {
    const scan = await loader.loadFromDirectory(dir, importRootId(dir))
  ...
  return { inserted, updated, shadowed, complete, ...(error ? { error } : {}) }
}

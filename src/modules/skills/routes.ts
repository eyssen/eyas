...
import { recallableSkills } from './recallable.js'
...
export interface SkillsServices {
  ...
  /**
   * Live read of `memory.recall.includeSecrets` (D-7 / P-19), wired from the
   * module context the same way memory-service gets it. Absent means false: a
   * build with no config reader fails closed.
   */
...
export function createSkillsRoutes(app: Hono, services: SkillsServices) {
  ...
  api.post('/skills/match', requirePermission('read', 'Skill'), async (c) => {
    ...
    // A skill whose imported asset held a credential is never proposed unless
    // the owner opened memory.recall.includeSecrets — see recallable.ts.
    const allSkills = recallableSkills(services.loader.list(true), services.recall?.().includeSecrets ?? false)

...
import { runSkillScan, scanSkillImportRoots, resolveSkillImportRoots, importRootId } from './skill-inventory.js'
import { recallIncludesSecrets } from '@modules/memory/memory-index.js'
...
export const skillsModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    const services: SkillsServices = {
      loader, matcher, db: ctx.db, classifyConfig: resolveClassifyConfig(ctx.config),
      // D-7 / P-19 — the single recall accessor, read per call rather than
      // captured. All three skill consumers read it: this module's
      // /skills/match, the conversations turn gate, and the prompt wizard's
      // skills section.
  ...
  async onStart(ctx: ModuleContext) {
    ...
    // Extra instance roots (local.yaml), re-read on every start. Imported
    // files win on id collision. A root inside another tool's own folders
    // (~/.claude, ~/.grok, an Obsidian vault, …) is never a live source:
    // it is skipped, and the operator imports those files once with Data port.
    const { scan: importRoots, skipped } = selectImportRoots(resolveSkillImportRoots(ctx.config))
    for (const s of skipped) {
      ctx.logger.warn(
        { root: s.root, relation: s.relation, protected: s.label },
        `skills.importRoots: ${describeSkippedImportRoot(s)} — not scanned; ${IMPORT_ROOT_REMEDY}`,
      )
    }
    if (importRoots.length > 0) {
      const extra = await scanSkillImportRoots(services.loader, importRoots)
      ctx.logger.info(
        {
          ...
          roots: importRoots.map(importRootId),
        },
        'Scanned imported skills',
      )
    }

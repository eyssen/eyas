...
export const searchModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    // Initialize embedding provider (best-effort)
    ...
    if (embeddingProvider) {
      const bridge = createEmbeddingBridge(embeddingProvider)
      searchCtx.embeddingBridge = bridge
    } else {
      ctx.logger.info('No embedding provider available — vector search disabled')
    }

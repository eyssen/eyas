...
export function createSearchRoutes(
  ...
  app.get('/api/v1/search', requirePermission('read', 'SearchSource'), async (c) => {
    ...
    const sourceIdsRaw = c.req.query('sourceIds')
...
async function indexAsync(
  ...
  try {
    const chunks = await indexer.index(source) as Array<{
      id: string
      sourceId: string
      collection: string
      content: string
    ...
    // Group new chunks by file path for mtime / hash reuse
    ...
    // Load existing hashes + embeddings for reuse
    const existingRows = (db.all(
      sql`SELECT id, content_hash, embedding, embedding_model, metadata FROM search_chunks WHERE source_id = ${sourceId}`,
    ) as any[])
    const hashToEmbedding = new Map<string, { embedding: Buffer; model: string | null }>()
    for (const row of existingRows) {
      if (row.content_hash && row.embedding) {
        hashToEmbedding.set(row.content_hash, {
    ...
    // Skip unchanged files via search_file_state mtime
    const filesToIndex = new Set<string>()
    let skippedFiles = 0
    for (const [fp, fileChunks] of byFile) {
      const mtime = String(fileChunks[0]?.metadata?.mtime ?? '')
      if (mtime && fp) {
        const st = search.sources.getFileState(sourceId, fp)
    ...
    // Full rebuild of Orama/vector for this source (simpler + correct);
    // embedding reuse via content_hash still saves the expensive embed calls.
    ...
    const { embeddingToBuffer, bufferToEmbedding } = await import('./vector-index.js')
    const now = new Date().toISOString()
    const model = bridge?.model ?? null

    // Decide which chunks need fresh embeddings
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      ;(chunks[i] as any)._contentHash = contentHash
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const contentHash = (chunk as any)._contentHash as string
      const emb = embeddingByIndex.get(i) ?? null
      if (emb && search.vectorIndex) {
        search.vectorIndex.upsert(chunk as any, emb)
    ...
    if (chunks.length > 0) {
      await provider.addDocuments(source.type, chunks as any)
    }

    // Refresh file state for all current paths
    search.sources.removeFileStates(sourceId)
    for (const [fp, fileChunks] of byFile) {
      const mtime = String(fileChunks[0]?.metadata?.mtime ?? now)
      if (fp) search.sources.setFileState(sourceId, fp, mtime, fileChunks.length)
    }

    search.sources.setIndexed(sourceId, chunks.length)
    logger.info(
      'Indexed source "%s": %d chunks (%d new embeds, %d files mtime-skipped for state)',
      source.name,
      chunks.length,
      needEmbed.length,
      skippedFiles,

...
import { enqueueIndexJob, isIndexInFlight, runIndexJob } from './index-run.js'
...
export function createSearchRoutes(
  ...
  app.post('/api/v1/search/sources/:id/index', requirePermission('update', 'SearchSource'), async (c) => {
    ...
    if (isIndexInFlight(id) || source.status === 'indexing') {
      return c.json({ status: 'indexing', error: 'Index already running' }, 409)
    }

    search.sources.setStatus(id, 'indexing')
    enqueueIndexJob(id, () => runIndexJob({
      sourceId: id,
      source,
      indexer,
      search,

...
// Lazy-init state for web-tree-sitter. Languages and parsers are cached —
// loading the WASM grammar on every file is what blew RAM on Odoo trees.
...
async function getParser(wasmPath: string): Promise<any | null> {
  const cached = parserCache.get(wasmPath)
  if (cached) return cached
  let tsLanguage = languageCache.get(wasmPath)
  if (!tsLanguage) {
    try {
    ...
    languageCache.set(wasmPath, tsLanguage)
  }
  try {
...
export async function chunkCodeAST(
  ...
  function walk(node: any) {
    ...
    if (isFunction || isClass) {
      ...
      if (isClass && lineCount > LARGE_CLASS_THRESHOLD) {
        // Do not keep the full class body (Odoo models are huge). Outline + methods.
  ...
  try {
    ...
    try { tree.delete?.() } catch { /* wasm tree already gone */ }
  }

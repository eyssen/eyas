...
export interface SearchSourceConfig {
  paths?: string[]
  urls?: string[]
  /** Glob or path-segment patterns to include (e.g. addons, models). */
  include?: string[]
...
export interface SourceService {
  ...
  /** Update chunk_count while status stays `indexing` (live progress). */
...
/** One filesystem file queued for incremental code indexing. */
...
export interface ContentIndexer {
  ...
  /** Optional: walk files without chunking (enables incremental persist). */
  collectFiles?(source: SearchSource): Promise<FileToIndex[]>
  /** Optional: chunk a single collected file. */
  indexFile?(source: SearchSource, file: FileToIndex): Promise<Chunk[]>
}

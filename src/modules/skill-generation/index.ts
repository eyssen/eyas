...
export interface BuildOptions {
  ...
  /** The background model service, threaded through to the generator's model-authoring pass (see skill-generator.ts's authorSkillMd). Undefined gracefully falls back to the deterministic renderSkillMd() — never forced. */
...
export function buildSkillGenerationServices(
  ...
  const generator = createSkillGenerator({ rootDir }, { aux: opts.aux })
...
export const skillGenerationModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    const services = buildSkillGenerationServices({
      ...
      // By value is safe here: onStart runs after every module's onRegister,
      // and the service resolves its gateway per call.
      aux: ctx.auxiliaryModel,
    })

 ...
 *
 * Category 'skill.adopt' is dispatched by category, not by kind (see
 * autonomy-policy.ts's decide()) — so another feature can enqueue a DIFFERENT
 * payload shape under this same category (dead-skill-detector.ts's
 * `skill_disable` proposals do exactly this). Anything that isn't ours
 * (`skill?.slug` absent, or inputJson isn't even valid JSON) is silently
 * ignored rather than thrown on — a thrown handler surfaces as a guaranteed
 * spurious error on every one of that other feature's approvals.
 */

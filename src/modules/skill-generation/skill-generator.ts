...
export interface SkillGeneratorDeps {
  /**
   * The background model service (purpose 'skill_generation') — authors
   * name/description/whenToInvoke + a genuine procedural body from the
   * observed pattern (see authorSkillMd below). `tools`, `license` and
   * `version` stay deterministic regardless, so a hallucinated tool name or
   * license can't slip through. No eligible model, a failed call, or output
   * that fails SkillFrontmatterSchema falls back to renderSkillMd(); never
   * throws.
   */
...
export function renderFrontmatter(fm: SkillFrontmatter): string {
  ...
  // Backslash before quote: escaping the quote first would leave a value ending
  // in a lone backslash escaping its own closing quote, merging the next line in.
  const escape = (s: string) =>
    s.replace(/[\x00-\x1f\x7f]/g, ' ').replace(/\\/g, '\\\\').replace(/"/g, '\\"')
  const lines: string[] = ['---']
 ...
 * observation stats and mined trigger phrases to the background model and
 * asks for a genuine name/description/whenToInvoke + procedural body. Returns
 * null on any failure (no eligible model, a failed call, empty output, bad
 * JSON, or a frontmatter that fails SkillFrontmatterSchema) so the caller
 * fails open to renderSkillMd(). Never throws.
 */
async function authorSkillMd(
  ...
  const raw = await aux.completeText({ purpose: 'skill_generation', system, user, maxTokens: 800, temperature: 0.4, fallback: '' })
  if (!raw) return null

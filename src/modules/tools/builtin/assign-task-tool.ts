...
export interface AssignTaskToolDeps {
  ...
  bus?: EyasBus
  /** First assignment in a conversation without a team session. */
      ...
      execute: async (rawInput: Record<string, unknown>, ctx?: ToolContext) => {
        ...
        try {
          const child = conversations.createSubConversation({
            ...
            // The assigning turn's model: the card's colleague runs on its own
            // model, else on this one (H4).
            binding: ctx?.modelBinding ?? null,
          })
          ...
          const members = [ctx?.agentId, input.agentId].filter((id): id is string => Boolean(id))
          deps.ensureWorkSession?.(parentConversationId, members)

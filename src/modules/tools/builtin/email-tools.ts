...
export interface EmailToolsWiring {
  ...
  /**
   * Composes the branded HTML body at SEND time. Optional: absent, a draft
   * goes out as plain text exactly as before.
   *
   * Send time, not draft time, is deliberate — this is the one place an
   * artifact exists server-side, persisted, before it leaves, so a brand edited
   * between drafting and sending still applies.
   */
  renderBranded?: (input: { body: string; title?: string; conversationId?: string | null })
...
export function ensureEmailDraftTables(db: any): void {
  ...
  // F3 — the branded HTML body, composed at send time and stored alongside the
  // plain-text source so the sent message is auditable.
  try { db.run(sql`ALTER TABLE email_drafts ADD COLUMN body_html TEXT`) } catch { /* already exists */ }
    ...
    },
    {
      ...
      execute: async (input) => {
        ...
        try {
          ...
          // Compose the branded body NOW, not when the draft was written: a
          // brand edited in between still applies, and the model never gets to
          // author the wrapper.
          let bodyHtml: string | null = null
          let bodyText = draft.body
          if (wiring.renderBranded) {
            try {
              const rendered = wiring.renderBranded({
                body: draft.body,
                title: draft.subject,
              ...
              // A branding failure must not block an approved send.
            }
          }
          ...
          const now = new Date().toISOString()
          d.run(sql`UPDATE email_drafts SET status = 'sent', sent_at = ${now}, updated_at = ${now}, body_html = ${bodyHtml} WHERE id = ${draft.id}`)
          wiring.bus?.emit('email.draft_sent', { draftId: draft.id, to: draft.to_addr, branded: bodyHtml !== null })
          return { ok: true, draftId: draft.id, status: 'sent', branded: bodyHtml !== null }
        } catch (err) {

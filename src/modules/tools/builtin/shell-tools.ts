...
import { isPathInsideRoots, workspaceFromContext } from '../working-directories.js'
  ...
  return [
    {
      ...
      execute: (async (rawInput: Record<string, unknown>, ctx?: import('../types.js').ToolContext) => {
        ...
        // Deny-list first: injection is refused even without a workspace.
        ...
        if (!ws.ok) {
          return { exitCode: -1, stdout: '', stderr: ws.error, refused: true }
        }
        const cwd = input.workingDir ?? ws.primary
        if (!isPathInsideRoots(cwd, ws.roots)) {
          return {
            exitCode: -1,
            stdout: '',
            stderr: `Refused: workingDir escapes the conversation working directories.`,
            refused: true,
          }
        }

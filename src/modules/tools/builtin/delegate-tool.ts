...
export function createDelegateTool(
  ...
  const execute = async (input: Record<string, unknown>, ctx?: ToolContext) => {
    ...
    try {
      ...
      // The child stores the delegating turn's model: the specialist runs on
      // its own model, else on this one.
      const result = await delegationService.delegate(conversationId, agentId, fullTask, { binding: ctx?.modelBinding ?? null })
      return {
        delegatedTo: agentId,

...
import { displayRelative, workspaceFromContext } from '../working-directories.js'
...
function resolveWorkspace(ctx?: ToolContext) {
  return workspaceFromContext(ctx)
}

function resolveInWorkspace(userPath: string, ctx?: ToolContext) {
  const ws = resolveWorkspace(ctx)
  if (!ws.ok) return ws
  return resolveToolPath(userPath, ws.primary, ws.roots)
}
...
/**
 * Collect the files under `dir` for grep/glob. Never descends into a folder
 * the memory-sovereignty policy protects — EYAS's data dir, a nested
 * Obsidian vault, another tool's memory, a CLI home, another conversation's
 * workspace (judged against `roots`, the caller's folders) — and never lists
 * a protected file (the database, a file in security.foreignMemoryPaths), so
 * a walk over a repository that holds `data/` or a vault neither reads nor
 * lists them. This is why EYAS's grep and glob are never refused for a folder
 * that CONTAINS a protected place, while a CLI's own search is (path-policy
 * search scopes). A protected `dir` itself (a folder saved before a rule
 * existed) yields nothing. Symlinks are not followed.
 */
async function walkFiles(
  ...
  const ctx = { workingDirectories: roots }
  if (policy.isProtectedDir(dir, ctx)) return
  const walk = async (current: string): Promise<void> => {
    ...
    for (const ent of entries) {
      ...
      if (ent.isDirectory()) {
        if (policy.isProtectedDir(full, ctx)) continue
        await walk(full)
      } else if (ent.isFile()) {
        if (policy.isProtectedDir(full, ctx)) continue
        out.push(full)
  ...
  return [
    {
      ...
      execute: async (raw, ctx) => {
        ...
        const resolved = resolveInWorkspace(input.path, ctx)
...

    {
      ...
      execute: async (raw, ctx) => {
        ...
        if (input.path) {
          const r = resolveInWorkspace(input.path, ctx)
          if (!r.ok) return { error: r.error }
          searchRoots.push(r.absolute)
        } else {
          searchRoots.push(...ws.roots)
        }
        ...
        for (const searchRoot of searchRoots) {
          ...
          if (!st) continue
          if (st.isFile()) {
            if (!getPathPolicy().isProtectedDir(searchRoot, { workingDirectories: ws.roots })) files.push(searchRoot)
          } else {
            await walkFiles(searchRoot, files, MAX_WALK_FILES, ws.roots)
          }
        }
        if (files.length === 0 && input.path) return { error: 'path not found' }
        ...
        for (const file of files) {
          ...
          const rel = displayRelative(file, ws.roots)
...

    {
      ...
      execute: async (raw, ctx) => {
        ...
        for (const file of files) {
          ...
          const relFromRoot = displayRelative(file, ws.roots)
          const base = relFromRoot.split('/').pop() ?? ''
          if (re.test(relFromRoot) || re.test(base)) {

...
export async function registerBuiltinTools(
  ...
  for (const tool of createBrowserTools({
    getDocuments: () => (wiring.hasModule('documents') ? wiring.getService('documents') : null),
    getVaultBasePath: () => {
      try {
        return wiring.getService('memory')?.vault?.getBasePath?.() ?? null
      } catch {
        return null
      }
    },
    getSecrets: () => {
      try {
        return wiring.getService('secrets') ?? null
      } catch {
        return null
      }
    },

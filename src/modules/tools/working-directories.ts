...
export function parseWorkingDirectories(raw: unknown): string[] {
  ...
  if (typeof raw === 'string') {
    ...
    if (!trimmed) return []
    try {
      return parseWorkingDirectories(JSON.parse(trimmed))
    } catch {
      return []
    }
  }
  if (!Array.isArray(raw)) return []
  return raw.filter((p): p is string => typeof p === 'string').map((p) => p.trim()).filter(Boolean)
}
...
export function validateWorkingDirectories(
  ...
  const incoming = parseWorkingDirectories(raw)
...
export function workspaceFromContext(ctx?: ToolContext): WorkspaceResolve {
  const fromList = (ctx?.workingDirectories ?? []).map((p) => p.trim()).filter(Boolean)
...
export function toolWorkspaceFields(paths: string[] | null | undefined): {
  workingDirectory?: string
  workingDirectories?: string[]
} {
  const dirs = (paths ?? []).map((p) => p.trim()).filter(Boolean)

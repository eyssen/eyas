...
export const FOLDER_ERROR_CODES = [
  ...
  'sensitive',
  /**
   * Is, or lies inside, the workspace of a conversation the saver does not
   * own (ValidateWorkingDirectoriesOpts.mayUseWorkspace), or a run's
   * scratch folder — as written or through a symlink.
   */
  'otherWorkspace',
  /**
...
export interface ValidateWorkingDirectoriesOpts {
  ...
  eyasHome?: string
  /**
   * The conversation workspaces these folders may be in (see
   * conversationWorkspaceAccess): called with the id of each workspace a
   * folder is, or lies inside, as written and after realpath. A folder in a
   * workspace it refuses — or in a run's scratch folder, which belongs to no
   * conversation — is refused as `otherWorkspace`. Absent: no ownership
   * check (a project's or project type's folders, a CLI cwd check).
   */
...
const FOLDER_ERROR_TEXT: Record<FolderErrorCode, string> = {
  ...
  otherWorkspace: "is, or lies inside, the workspace of a conversation that is not yours, or a run's scratch folder",
...
export function validateWorkingDirectories(
  ...
  for (const item of parseNamedWorkingDirectories(input)) {
    ...
    // One workspace is a folder only for a conversation that may use it
    // (its own, or one of the same owner's) — judged as written and after
    // realpath, so a symlink into another user's workspace is refused too.
    if (opts.mayUseWorkspace && foreignWorkspaceOf(path, opts.mayUseWorkspace, policy)) return refuse('otherWorkspace', path)
    let real: string
    try {
...
//
// A conversation's workspace (<workspacesRoot>/<id>) is a folder like any
// other once it is stored on a conversation: every CLI run, the OpenCode
// terminal and opencode_run then work in it, and the memory-path policy
// counts it as that run's own. So who may name one is decided by ownership,
// the conversations module's own rule, at save time and again at run time.

/**
 * The workspaces one conversation's folders may be in: its own, and the
 * workspace of every conversation its human owner may open (the
 * conversations module's ownsConversation — a team/delegation child's
 * workspace included). Never another user's workspace. `conversationId`
 * null: a conversation not created yet. `ownerUserId` null (the owner is
 * unknown): only its own workspace.
 */
export function conversationWorkspaceAccess(
  ...
  return (workspaceId) =>
    (conversationId !== null && workspaceId === conversationId) ||
    (ownerUserId !== null && ownerUserId.length > 0 && ownsConversation(workspaceId, ownerUserId))
}

/** The conversations module's ownership answers the run-time rule needs (ConversationService). */
...
/**
 * The rule for one conversation's stored folders at run time: its own
 * workspace, and its human owner's other conversations' (ownerOf, so a
 * team/delegation child keeps its parent's). Without the conversations
 * module: its own workspace only (fail closed). Undefined for a run without
 * a conversation: its folders are no conversation's.
 */
export function storedFolderWorkspaceAccess(
  conversations: ConversationOwnership | null | undefined,
  conversationId: string | null | undefined,
): ((workspaceId: string) => boolean) | undefined {
  if (!conversationId) return undefined
  const owner = typeof conversations?.ownerOf === 'function' ? conversations.ownerOf(conversationId) : null
  return conversationWorkspaceAccess(conversationId, owner, (cid, uid) =>
    typeof conversations?.ownsConversation === 'function' ? conversations.ownsConversation(cid, uid) : false)
}

/**
 * The first workspace a path is, or lies inside (as written and after
 * realpath: PathPolicy.workspacesOf), that `mayUse` refuses — or any run
 * scratch folder, which belongs to no conversation. Null when there is none.
 */
export function foreignWorkspaceOf(
  path: string,
  mayUse: (conversationId: string) => boolean,
  policy: PathPolicy = getPathPolicy(),
): WorkspaceRef | null {
  for (const ref of policy.workspacesOf(path)) {
    if (ref.scratch || !mayUse(ref.id)) return ref
  }
  return null
}
...
const PROTECTED_FOLDER_CODES: ReadonlySet<FolderErrorCode> = new Set<FolderErrorCode>([
  'home', 'providerHome', 'vault', 'eyasData', 'sensitive', 'otherWorkspace',

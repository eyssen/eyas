...
- **LLM diagnoser.** Uses a narrow `LlmClient` interface and is not wired
  (`ops/index.ts` passes no `llm`). When it is, the client must be backed by
  the background model service (`ctx.auxiliaryModel.completeText`, with its
  own purpose), never by the model gateway directly.

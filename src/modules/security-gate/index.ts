...
import { expandHome } from '@shared/memory-sovereignty/shell-paths.js'
...
import type { SecurityCallContext, SecurityCheckResult, SecurityEvent, SecurityGateConfig } from './types.js'
...
/**
 * Build the configured memory-sovereignty path policy and make it the
 * process-wide one: the instance layout (data dir, workspaces, Studio,
 * browser downloads, CLI homes), the configured database path and the
 * owner's security.foreignMemoryPaths. Every channel — this gate, the Claude
 * Code hook, ACP file servicing, folder validation — asks it through
 * getPathPolicy(). Should this fail, the lazy default (the same layout
 * without the configured extras) keeps answering.
 */
function installConfiguredPathPolicy(ctx: ModuleContext): void {
  const foreignMemoryPaths: string[] = ctx.config?.security?.foreignMemoryPaths ?? []
  try {
    ...
    ctx.logger.error(
      ...
      'memory-path policy: configured policy could not be built — the built-in default applies (security.foreignMemoryPaths is not enforced)',
    )
    return
  }
  const home = currentHomeDir()
  const ignored = foreignMemoryPaths.filter((p) => !isAbsolute(expandHome(p.trim(), home)))
  if (ignored.length > 0) {
    ctx.logger.warn({ ignored }, 'security.foreignMemoryPaths: entries that are not absolute paths are ignored')
  }
}
...
export const securityGateModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    // The memory-sovereignty path policy every channel asks (the database
    // path is one of its EYAS-private locations).
    installConfiguredPathPolicy(ctx)
    ...
    // The judge runs on the background model service, read per call: module
    // registration order is not guaranteed, and the service itself resolves
    // the traced, privacy-masked gateway and the routing tiers on every call.
    ...
    function logEvent(result: SecurityCheckResult, toolName: string, input: Record<string, unknown>, extra?: SecurityCallContext & { sessionRiskScore?: number }): void {
    ...
    /**
     * The memory-path policy alone, for channels that check a path before
     * (or instead of) a full gate decision: the Claude Code PreToolUse hook,
     * ACP file requests. A violation is logged as one security_events row
     * (checkpoint 'deterministic') and returned as the deny; a clean path
     * returns null and logs nothing — the channel's own gate decision, if
     * any, logs that call.
     */
    function checkMemoryPath(toolName: string, input: Record<string, unknown>, callCtx?: SecurityCallContext): SecurityCheckResult | null {
      const name = toolName.replace(/^mcp__eyas__/, '')
      const denied = deterministicGate.checkMemoryPath(name, input, callCtx)
      if (denied) logEvent(denied, name, input, callCtx)
      return denied
    }
    ...
    const gate = {
      async validateToolCall(
        ...
        callCtx?: SecurityCallContext,
      ): Promise<SecurityCheckResult> {
        // A command that asked to leave the kernel sandbox: a deterministic
        // deny still wins; otherwise a human decides — never the judge, and
        // the streak is not touched. Also with the gate switched off.
        if (callCtx?.requireHuman) {
          if (config.enabled) {
            const det = deterministicGate.check(toolName, input, callCtx)
            if (det.decision === 'deny') {
              logEvent(det, toolName, input, callCtx)
              return det
            }
          } else {
            const memory = checkMemoryPath(toolName, input, callCtx)
            if (memory) return memory
          }
          const human: SecurityCheckResult = {
            decision: 'escalate',
            checkpoint: 'deterministic',
            reason: UNSANDBOXED_SHELL_REASON,
            riskTier: 'red',
            timestamp: new Date().toISOString(),
          }
          logEvent(human, toolName, input, callCtx)
          return human
        }

        if (!config.enabled) {
          // Memory sovereignty is not a setting of this gate: its hard deny
          // stands with the gate switched off.
          const memory = checkMemoryPath(toolName, input, callCtx)
          if (memory) return memory
        ...
        // Checkpoint 1: Deterministic gate (the memory-path hard deny included)
        const det = deterministicGate.check(toolName, input, callCtx)
  ...
  async onStart(ctx: ModuleContext) {
    ...
    createSecurityGateRoutes(
      ...
      // The memory-policy card lists the kernel sandbox of the CLI providers
      // that are switched on (every CLI when the model catalog is absent).
      {
        enabledProviders: () =>
          ctx.providerConfig?.listProviders().filter((p) => p.enabled).map((p) => p.id) ?? null,
      },
    )

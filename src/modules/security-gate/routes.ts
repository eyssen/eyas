...
import { buildMemoryPolicyReport, type MemoryPolicyReportDeps } from './memory-policy-report.js'
...
export function createSecurityGateRoutes(
  ...
  memoryPolicy: MemoryPolicyReportDeps = {},
): void {
  ...
  // 'Memory outside EYAS': what the memory-path policy protects on this host
  // (absolute paths — hence read SecurityEvent, i.e. owner/admin), the
  // kernel file sandbox of each switched-on CLI provider, and the last 24
  // hours of memory-path refusals and unsandboxed-shell escalations.
  router.get('/memory-policy', requirePermission('read', 'SecurityEvent'), async (c) => {
    return c.json(await buildMemoryPolicyReport(db, memoryPolicy))
  })

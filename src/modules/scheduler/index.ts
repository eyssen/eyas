...
export const schedulerModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    // Built-in handlers
    // agent_run jobs run the agent through the shared runner entry the agent
    // module publishes on ctx.agents (plural). Both services are read per
    // run: module start order is not guaranteed.
    scheduler.registerHandler(
      'scheduler.agent_run',
      createAgentRunHandler({
        db: ctx.db,
        logger: ctx.logger,
        getAgents: () => (ctx as any).agents,
        getConversations: () => (ctx as any).conversations,
      }),
    )

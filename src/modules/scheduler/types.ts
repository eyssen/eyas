...
/** Which job is executing, handed to its handler with the parsed config. */
export interface JobHandlerContext {
  jobId: string
  /** Who created the job: a user id, 'agent', or undefined for a system job. */
  createdBy?: string
}

export type JobHandler = (config?: Record<string, unknown>, job?: JobHandlerContext) => Promise<unknown>

/**
 * A handler failure that carries structured context (e.g. the conversation a
 * failed agent_run created). The scheduler records `result` on the failed
 * execution row next to the error message, so Recent executions can link it.
 */

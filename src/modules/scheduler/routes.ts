...
import { parseAgentRunConfig } from './agent-run-handler.js'

const AGENT_RUN_HANDLER = 'scheduler.agent_run'
...
export function createSchedulerRoutes(app: Hono, scheduler: SchedulerService) {
  ...
  api.post('/scheduler/jobs', requirePermission('create', 'Scheduler'), async (c) => {
    ...
    // An agent_run job with a config its handler would refuse is a dead job:
    // refuse it here, where the caller can still fix it.
    if (handler === AGENT_RUN_HANDLER) {
      const cfg = parseAgentRunConfig(parsed.data.handlerConfig)
      if (!cfg.ok) return c.json({ error: `Invalid agent_run handlerConfig: ${cfg.error}` }, 400)
    }
    ...
    const job = scheduler.create({
      ...
      // The authenticated user, never a client-supplied value: an agent_run
      // job's conversation is owned by its creator (agent-run-handler.ts).
      createdBy: userId,
  ...
  api.patch('/scheduler/jobs/:id', requirePermission('update', 'Scheduler'), async (c) => {
    ...
    // Same rule as create, checked only when the patch touches the handler or
    // its config (a stored config is never re-judged on an unrelated edit).
    const effectiveHandler = data.handler ?? existingJob.handler
    if (effectiveHandler === AGENT_RUN_HANDLER && (data.handler !== undefined || data.handlerConfig !== undefined)) {
      const cfg = parseAgentRunConfig(data.handlerConfig !== undefined ? data.handlerConfig : existingJob.handlerConfig)
      if (!cfg.ok) return c.json({ error: `Invalid agent_run handlerConfig: ${cfg.error}` }, 400)
    }

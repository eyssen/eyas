...
export function createSchedulerService(
  ...
  async function executeJob(job: ScheduledJob, opts?: { actor?: string; scheduledFor?: string }): Promise<void> {
    ...
    try {
      const result = await handler(config, { jobId: job.id, createdBy: job.createdBy })
      ...
      if (execId != null) {
        ...
        db.run(sql`INSERT INTO job_executions (job_id, status, started_at, completed_at, duration_ms, result, actor, scheduled_for)
          VALUES (${job.id}, 'completed', ${startedAt}, ${completedAt}, ${durationMs}, ${resultJson}, ${opts?.actor ?? 'system'}, ${opts?.scheduledFor ?? null})`)
      }
      ...
      // A JobFailure carries what the handler got done before it failed (the
      // conversation a failed agent_run created, say): kept on the row.
      const failResult = err instanceof JobFailure ? JSON.stringify(err.result) : null

      if (execId != null) {
        db.run(sql`UPDATE job_executions SET status = 'failed', completed_at = ${completedAt}, duration_ms = ${durationMs}, error = ${errMsg}, result = ${failResult} WHERE id = ${execId}`)
      } else {
        db.run(sql`INSERT INTO job_executions (job_id, status, started_at, completed_at, duration_ms, error, result, actor, scheduled_for)
          VALUES (${job.id}, 'failed', ${startedAt}, ${completedAt}, ${durationMs}, ${errMsg}, ${failResult}, ${opts?.actor ?? 'system'}, ${opts?.scheduledFor ?? null})`)
      }
  ...
  return {
    ...
    async run(id: string, actor?: string): Promise<void> {
      ...
      // run() is manual by definition, so an unknown caller is 'user', never
      // 'system'. executeJob defaults to 'system' — reserved for "a timer fired
      // it" (spec §5.1) — and forwarding an undefined actor would record a
      // hand-pressed Run Now as a timer, which is the exact confusion the
      // column exists to remove. Matches logAdmin's default one line above.
      await executeJob(job, { actor: actor ?? 'user', scheduledFor: new Date().toISOString() })
    },

  ...
  async function executeJob(job: ScheduledJob, opts?: { actor?: string; scheduledFor?: string }): Promise<void> {
    ...
    try {
      ...
      if (execId != null) {
        ...
        db.run(sql`INSERT INTO job_executions (job_id, status, started_at, completed_at, duration_ms, error, actor, scheduled_for)
          VALUES (${job.id}, 'failed', ${startedAt}, ${completedAt}, ${durationMs}, ${errMsg}, ${opts?.actor ?? 'system'}, ${opts?.scheduledFor ?? null})`)
      }

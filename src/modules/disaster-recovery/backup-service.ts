...
export const DEFAULT_BACKUP_EXCLUDES = [
  ...
  'data/eyas.pid',
  'data/eyas.log',
  // CLI session stores inside the EYAS-owned CLI homes (e.g.
  // cli-homes/grok-cli/.grok/sessions): transcripts EYAS never resumes and
  // purges after every run. `*` crosses `/` in tar exclude patterns (GNU tar
  // and bsdtar alike), so one pattern covers every nesting. The rest of a
  // home — the EYAS sign-in credentials — stays in the backup.
  'data/cli-homes/*/sessions',
] as const

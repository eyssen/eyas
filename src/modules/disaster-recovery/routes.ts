...
export function createDisasterRecoveryRoutes(
  ...
  app.post('/api/v1/backup/destinations', requirePermission('manage', 'Backup'), async (c) => {
    ...
    dest.secretRefs = await persistInlineSecrets(dest.id, dest.secretRefs, (body as any).secrets, setSecret)
  ...
  app.put('/api/v1/backup/destinations/:id', requirePermission('manage', 'Backup'), async (c) => {
    ...
    dest.secretRefs = await persistInlineSecrets(id, dest.secretRefs, (body as any).secrets, setSecret)
...
async function persistInlineSecrets(
  destId: string,
  ...
  for (const [field, value] of Object.entries(secrets ?? {})) {
    if (!value) continue
    if (isEnvOrVaultRef(value)) {
      refs[field] = value
      continue
    }
    const vaultName = destinationSecretName(destId, field)
    if (setSecret) {
      await setSecret(vaultName, value)
      refs[field] = vaultName
    } else {
      // No vault available — keep the value so resolveSecrets can use it.
      refs[field] = value
    }
  }
  return refs
...
function publicDest(d: DestinationConfig) {
  return {
    ...
    enabled: d.enabled,
    settings: d.settings,
    // Never return credential material — only env/vault ref names.
    secretRefs: Object.fromEntries(
      Object.entries(d.secretRefs ?? {}).map(([field, ref]) => [
        field,
        isEnvOrVaultRef(ref) ? ref : '********',
      ]),
    ),
    secretsConfigured: Object.fromEntries(
      Object.entries(d.secretRefs ?? {}).map(([field, ref]) => [
        field,
        !!(ref && (process.env[ref] || !isEnvOrVaultRef(ref) || ref.startsWith('backup-dest-'))),
      ]),
    ),
  }
}

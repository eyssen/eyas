...
/**
 * The model surface research uses: the auxiliary model service's isolated
 * one-shot call (purpose 'research'), never the gateway directly.
 */
export type ResearchAux = Pick<AuxiliaryModelService, 'complete'>
...
export interface ResearchReport {
  ...
  /**
   * The body was assembled without model synthesis: no background model could
   * write it (none eligible, budget stopped, or the call failed), so the
   * sections list the top sources with their snippets.
   */
  degraded: boolean
  error?: string
  createdAt: string

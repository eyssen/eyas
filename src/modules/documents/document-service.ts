  ...
  return {
    async upload(input: UploadInput): Promise<DocumentRecord> {
      ...
      if (file.length > maxBytes) {
        throw new Error(
          `File size ${file.length} bytes exceeds the size limit of ${effectiveLimits.maxFileSizeMb}MB (${maxBytes} bytes).`,
        )
      }
      ...
      if (!mimeMatchesAllowedTypes(mimeType, effectiveLimits.allowedTypes)) {
        throw new Error(
          `MIME type "${mimeType}" is not allowed. Allowed types: ${effectiveLimits.allowedTypes.join(', ')}`,
        )
      }

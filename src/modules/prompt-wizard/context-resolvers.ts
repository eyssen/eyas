...
import { screenStoredWorkingDirectories, storedFolderWorkspaceAccess } from '@modules/tools/working-directories.js'
...
export async function resolveWorkingDirectoriesContextImpl(
  ...
  try {
    ...
    // Only the folders the run will really use: one a protection rule now
    // refuses — or one in the workspace of another user's conversation — is
    // left out of the run (agent runner, K2), so it is not named to the
    // model either.
    const mayUseWorkspace = storedFolderWorkspaceAccess((ctx as any).conversations, conversationId)
    const dirs = screenStoredWorkingDirectories(conv?.workingDirectories ?? conv?.working_directories, mayUseWorkspace ? { mayUseWorkspace } : {}).entries.map((e) => e.path)

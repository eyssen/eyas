...
import { screenStoredWorkingDirectories } from '@modules/tools/working-directories.js'
...
export async function resolveWorkingDirectoriesContextImpl(
  ...
  try {
    ...
    // refuses is left out of the run (agent runner, K2), so it is not named
    // to the model either.
    const dirs = screenStoredWorkingDirectories(conv?.workingDirectories ?? conv?.working_directories).entries.map((e) => e.path)

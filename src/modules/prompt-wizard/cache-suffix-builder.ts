...
import type { VoiceProfile, VoiceScope, ContextSection } from './types.js'
import type { SectionBudget } from './token-budget.js'
import { createSectionCollector } from './section-collector.js'

/**
 * The stable runtime facts of the suffix. The clock is not here: date and time
 * change every turn, so they travel in the per-message turn block
 * (assembler.ts) and the system prompt stays byte-stable between turns.
 */
...
/** One conversation tag as it appears on the suffix `tags:` line. */
export interface ConversationTagLine {
...
export interface CacheSuffixInput {
  ...
  conversationTags?: ConversationTagLine[] | null
  runtime: RuntimeContext
  activeVoice: { scope: VoiceScope; reason: string; profile: VoiceProfile }
  budget: SectionBudget
}

/** One suffix line, e.g. `tags: area:alpha, module:bravo`. Null when there are none. */
export function formatConversationTagsLine(tags: ConversationTagLine[]): string | null {
...
export function buildCacheSuffix(input: CacheSuffixInput): { content: string; sections: ContextSection[] } {
  ...
  if (input.memory && (input.memory.workingMemory.length > 0 || input.memory.goalAncestry)) {
    ...
    parts.push(c.push('memory-context', lines.join('\n'), input.budget.memoryContext))
  }
  ...
  if (input.codeSearch) {
    ...
    parts.push(c.push('code-search-context', lines.join('\n'), budget))
  }
  ...
  if (input.workingDirectories) {
    ...
    parts.push(c.push('working-directories', lines.join('\n'), budget))
  }

  const tagsLine = input.conversationTags?.length ? formatConversationTagsLine(input.conversationTags) : null
  if (tagsLine) {
    parts.push(c.push('conversation-tags', tagsLine))
  }
  ...
  parts.push(c.push('runtime', runtimeLines.join('\n'), input.budget.runtime))

  parts.push(
    c.push(
      'active-voice',
      renderActiveVoice(input.activeVoice.scope, input.activeVoice.reason, input.activeVoice.profile),
      input.budget.activeVoice,
    ),
  )

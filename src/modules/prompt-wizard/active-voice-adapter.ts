...
/**
 * The channel context a prompt is resolved with when its caller supplies none
 * (every entry path today): an owner DM. Channel replies decide their
 * audience from the same context, so the voice and the audience agree.
 */
export const FALLBACK_CHANNEL_CONTEXT = {
  channelType: 'web',

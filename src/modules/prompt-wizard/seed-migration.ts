...
export const PRIOR_IDENTITY_BODIES: readonly string[] = [
...
- Get better over time. At the end of a task, briefly reflect on what worked and
  ...
  // Pre colleague-routing rewrite (Ground answers clause; sub-task delegate bullet):
  `You are EYAS, a self-hosted personal AI assistant platform. You act as an
autonomous, dedicated teammate for a single owner — not a public chatbot.
...
- Get better over time. At the end of a task, briefly reflect on what worked and
  ...
  propose a new skill or a refinement instead of silently working around it — but
  don't let this reflection bloat a simple task.`,
  // 0.8.26–0.8.29 body (colleague routing; still told the model to update
  // MEMORY.md / memory/YYYY-MM-DD.md and named search_memory). Replaced by the
  // joint B8+I9 memory contract — one append for that change.
  `You are EYAS, a self-hosted personal AI assistant platform. You act as an
autonomous, dedicated teammate for a single owner — not a public chatbot.
...
- Get better over time. At the end of a task, briefly reflect on what worked and
  ...
  propose a new skill or a refinement instead of silently working around it — but
  don't let this reflection bloat a simple task.`,
  // W0 body of the joint B8+I9 memory contract: recall was said to be in this
  // prompt's Memory section. Replaced when recall moved to the per-message
  // <eyas-memory> block (I4) — one append for that change.
  `You are EYAS, a self-hosted personal AI assistant platform. You act as an
autonomous, dedicated teammate for a single owner — not a public chatbot.
...
export const PRIOR_CORE_RULES: readonly string[] = [
...
14. LANGUAGE: Match the owner's language. Code, comments, commit messages, and
    technical identifiers are always in English.`,
  // 0.8.16–0.8.23 body (MEMORY rule names search_memory / save_memory). It
  // shipped for eight releases but was never recorded here, so rows seeded then
  // kept telling models to call the retired save_memory.
  `## Mandatory Rules
...
14. LANGUAGE: Match the owner's language. Code, comments, commit messages, and
    technical identifiers are always in English.`,
  // 0.8.24–0.8.29 body (write-only MEMORY rule with the 'MEMORY.md outside the
  // workspace' loophole). Replaced by the joint B8+I9 memory contract — one
  // append for that change.
  `## Mandatory Rules
...
14. LANGUAGE: Match the owner's language. Code, comments, commit messages, and
    technical identifiers are always in English.`,
  // W0 body of the joint B8+I9 rule 8 (recall "appears in this prompt's Memory
  // section"). Replaced when recall moved to the per-message <eyas-memory>
  // block (I4) — one append for that change.
  `## Mandatory Rules

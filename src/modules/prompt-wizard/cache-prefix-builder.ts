...
import { toolAddressingNote } from '@modules/model/tool-addressing.js'
...
export interface CachePrefixInput {
  ...
  agentsList?: { name: string; oneLine: string }[]
  budget: SectionBudget
  /**
   * How the target provider's model names EYAS tools. Non-native addressing
   * adds one line to the tool inventory's footer (the listed names are
   * canonical); absent or native adds nothing.
   */
...
export function buildCachePrefix(input: CachePrefixInput): { content: string; sections: ContextSection[] } {
  ...
  if (input.toolsList.length > 0) {
    // The addressing note rides in the footer, which renderInventory keeps at
    // every size: dropping it would leave a CLI model calling bare names that
    // its host resolves to its own tools.
    const footer = ['Full tool schemas are delivered via the provider native tool API.', toolAddressingNote(input.toolAddressing)]
      .filter(Boolean)
      .join('\n')
  ...
  if ((input.agentsList?.length ?? 0) > 0) {
    const agents = renderInventory({
      heading: 'The following agents are on the roster. Copy the id field into tool calls — never the name or a slug.',
      ...
      budgetTokens: input.budget.availableAgents,
    })
    parts.push(c.push('available-agents', agents.content, input.budget.availableAgents))
  }

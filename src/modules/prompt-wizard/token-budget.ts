...
export const DEFAULT_BUDGET_FULL: SectionBudget = {
  ...
  // sections are exempt from the shrink ratio, so this is a flat +300 on the
  // declared sum (8400 → 8700), still inside the 8800 reserve below.
  coreRules: 800,
  personality: 200,
  ...
  availableAgents: 400,
  teamContext: 400,
  // Spec §16-9: 1 200-token reserve for the single injected memory section.
  memoryContext: 1_200,
  runtime: 200,
  activeVoice: 100,
}
...
export function shrinkForContextWindow(effectiveCtx: number, override?: Partial<SectionBudget>): SectionBudget {
  ...
  const target = Math.min(9000, Math.floor(effectiveCtx * 0.4))
  ...
  const scaled: SectionBudget = {
    ...
    availableAgents: Math.max(100, Math.floor(DEFAULT_BUDGET_FULL.availableAgents * ratio)),

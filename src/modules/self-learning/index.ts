...
export const selfLearningModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    // The background model service is read through a getter, per call: module
    // registration order is not guaranteed, so a by-value read here could stay
    // undefined. agentRegistry is not threaded in yet — the agent module's
    // onStart populates ctx.agents; onStart below fills it in on the same deps
    // object.
  ...
  async onStart(ctx: ModuleContext) {
    ...
    // Thread the agent registry into the learner now — the agent module's
    // onStart already ran; onRegister above ran too early for it to exist.

...
export interface ExecutionLearnerDeps {
  /**
   * The background model service (purpose 'self_learning') — turns each
   * metric anomaly below into a CONCRETE patch (a rewritten prompt line /
   * constraint / routing rule) instead of the constant generic sentence, fed
   * with the agent's ACTUAL current systemPrompt + constraints. Read at call
   * time (self-learning/index.ts wires it as a getter). No eligible model, a
   * failed call, or a missing agentRegistry/agent falls back to the
   * pre-existing generic sentence; never throws.
   */
...
/** One fail-open background pass: the answer, or `fallback` without an eligible model. */
async function authorPatch(deps: ExecutionLearnerDeps, opts: { system: string; user: string; fallback: string }): Promise<string> {
  const aux = deps.aux
  if (!aux) return opts.fallback
  return aux.completeText({ purpose: 'self_learning', maxTokens: 200, temperature: 0.4, ...opts })
}
    ...
    async learn(daysBack: number = 30, modelPassEnabled: boolean = true): Promise<ExecutionInsight[]> {
      ...
      for (const row of toolStats) {
        ...
        if (errorRate > 0.3) {
          ...
          // context to feed, but authorPatch itself fails open without an
          // eligible background model, so this is safe to call unconditionally

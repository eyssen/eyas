...
import type { RunConversationEntry } from '@modules/agent/run-deps.js'
...
export const proactiveAssistantModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    // Bot executor requires the agent module. Its runs go through the ONE
    // runner entry the agent module publishes (ctx.agents.runConversation),
    // resolved per run: module registration order is not guaranteed, and a
    // partial dependency bundle assembled here is exactly how board runs used
    // to lose designs, documents and durable-memory capture. Safe by default:
    // it only acts on conversations the operator placed in a bot-capable stage.
    let botExecutor: ReturnType<typeof createBotExecutor> | null = null
    if (ctx.hasModule('agent')) {
      botExecutor = createBotExecutor({
        db: ctx.db,
        logger: ctx.logger,
        runConversation: (conversationId, overrides) => {
          const run = (ctx as any).agents?.runConversation as RunConversationEntry | undefined
          if (!run) return Promise.reject(new Error('Agent runner is not available — the agent module did not publish ctx.agents.runConversation'))
          return run(conversationId, overrides)
        },
      })
    }

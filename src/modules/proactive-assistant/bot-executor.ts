...
export function createBotExecutor(deps: {
  ...
  logger: any
  /**
   * I10 — the shared runner entry (ctx.agents.runConversation). A board run
   * gets exactly what a retry, a scheduled run or a hand-off of the same card
   * gets: recall via the prompt assembler's turn block, attached designs,
   * documents, durable-memory capture, the critic and the context recorder.
   * The caller resolves it lazily; the agent module publishes it.
   */
  runConversation: RunConversationEntry
}) {
  ...
  async function pass(): Promise<number> {
    ...
    for (const stage of stages) {
      ...
      for (const conv of convs) {
        ...
        const result = await deps.runConversation(conv.id)
        if (result.ran) {
          processed++

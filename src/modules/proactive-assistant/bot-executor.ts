...
export function createBotExecutor(deps: {
  ...
  critic?: any
  /** P1 — deterministic verify commands */
  verify?: any
  eventStore?: any
  getCheckpoint?: any
  ...
  /** Task 11 — records what actually reached the model on each background run. */
  contextRecorder?: any
}) {
    ...
    for (const stage of stages) {
      ...
      for (const conv of convs) {
        ...
        const result = await runConversation(conv.id, {
          ...
          critic: deps.critic, verify: deps.verify, eventStore: deps.eventStore, getCheckpoint: deps.getCheckpoint,
          budgetEngine: deps.budgetEngine, pricingOverrides: deps.pricingOverrides,
          contextRecorder: deps.contextRecorder,
        })
        if (result.ran) {
          processed++

...
export const forgeModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    // registration order is not guaranteed, so a by-value read here could
    // stay undefined.
    const proposalEngineDeps: ProposalEngineDeps = {
      toolRegistry,
      skillRegistry,
      get aux() {
    ...
    ;(ctx as any).forge = { config, collector, analyzer, proposalStore, proposalEngine, experimentRunner, applier }
  ...
  async onStart(ctx: ModuleContext) {
    ...
    const { config, collector, analyzer, proposalStore, proposalEngine, experimentRunner, applier } = (ctx as any).forge as {
      config: ForgeConfig

...
export function createConversationRoutes(
  ...
  router.post('/api/v1/conversations/:id/messages', requirePermission('create', 'ConversationMessage'), async (c: any) => {
    ...
    try {
      memoryIndex = getMemoryIndex?.({ projectId: effectiveProjectId((conv as any).projectId ?? null) })
      if (memoryIndex) {
    ...
    try {
      // Resume / skill-proposal retry carries no body.content — the user
      // message was stored on the attempt that stopped. Same fallback as
      // capture below: the transcript is the source of truth.

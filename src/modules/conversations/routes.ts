...
export function createConversationRoutes(
  ...
  getSkills?: () => { loader: { list(enabled?: boolean): any[] }; matcher: { match(query: string, skills: any[], maxResults?: number): { skill: { id: string; content: string; name: string }; matchScore: number }[] } } | undefined,
  ...
  /**
   * Answer a skill proposal. Two buttons, no typing: the body is a boolean.
   * The client re-sends the turn with `resume: true` afterwards — the user's
   * message is already stored, so the re-run must not add it again.
   */
  router.post('/api/v1/conversations/:id/skill-decision', requirePermission('update', 'Conversation'), async (c: any) => {
    const id = c.req.param('id')
    const body = await c.req.json().catch(() => ({}))
    const skillId = typeof body.skillId === 'string' ? body.skillId : ''
    if (!skillId || typeof body.accept !== 'boolean') {
    ...
    skillDecisions.set(id, skillId, body.accept ? 'accepted' : 'declined')
    return c.json({ skillId, decision: body.accept ? 'accepted' : 'declined' })
  })

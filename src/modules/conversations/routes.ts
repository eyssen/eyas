...
export function createConversationRoutes(
  ...
  getBoard?: () => { projects: { getWithStages(id: string): { defaultAgentId: string | null; stages: { id: string; isClosed: boolean; sortOrder: number }[] } | null } } | undefined,
  ...
  router.post('/api/v1/conversations', requirePermission('create', 'Conversation'), async (c: any) => {
    ...
    // Resolve stage and agent from project
    ...
    if (board) {
      ...
      if (project) {
        ...
        if (body.stageId) {
          ...
          const firstOpen = project.stages.find(s => !s.isClosed) ?? project.stages[0]

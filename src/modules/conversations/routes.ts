...
import { generatePlan } from '@modules/agent/planning.js'
import {
...
export function createConversationRoutes(
  ...
  /**
   * Lazy related-prior-work block. Appended after getStudio so existing
   * positional call sites (including those that stop at getMemoryIndex) keep
   * compiling. Same fail-soft contract as getMemoryIndex.
   */
  ...
  getMemoryAssemble?: (opts: {
    query: string
    conversationId: string
    projectId?: string | null
  }) => Promise<{ content: string; ids: string[]; tokens: number } | null>,
): void {
    ...
    if (board) {
      ...
      if (project) {
        ...
        if (body.workingDirectories === undefined) {
          ...
          if (pinned?.length) {
            updateFields.workingDirectories = pinned
          } else {
            try {
          ...
          updateFields.workingDirectories = parsed.entries.length
            ? serializeWorkingDirectories(parsed.entries)
            : null
        }
  ...
  router.patch('/api/v1/conversations/:id', requirePermission('update', 'Conversation'), async (c: any) => {
    ...
    if (
      ...
      const parsed = validateWorkingDirectories(body.workingDirectories)
      if (!parsed.ok) throw new HTTPException(400, { message: parsed.error })
      body.workingDirectories = parsed.entries.length
        ? serializeWorkingDirectories(parsed.entries)
        : null
    }
  ...
  router.post('/api/v1/conversations/:id/messages', requirePermission('create', 'ConversationMessage'), async (c: any) => {
    ...
    let memoryIndex: import('@modules/memory/memory-index.js').MemoryIndexResult | null | undefined = null
    try {
      let query = typeof body.content === 'string' ? body.content : ''
      if (!query.trim()) {
        const stored = (typeof fullConv !== 'undefined' ? fullConv.messages : null)
          ?? chatService.get(id)?.messages
          ?? []
        for (let i = stored.length - 1; i >= 0; i--) {
          if (stored[i].role === 'user') { query = stored[i].content ?? ''; break }
        }
      }
      const projectId = effectiveProjectId((conv as any).projectId ?? null)
      if (getMemoryAssemble) {
        const assembled = await getMemoryAssemble({ query, conversationId: id, projectId })
        if (assembled) {
          const { MEMORY_ASSEMBLE_SECTION_KEY } = await import('@modules/memory/v2/assemble.js')
          system = system ? `${system}\n\n${assembled.content}` : assembled.content
          appendSection(MEMORY_ASSEMBLE_SECTION_KEY, assembled.content, assembled.ids.join(','))
        }
      } else {
        memoryIndex = getMemoryIndex?.({ projectId })
        if (memoryIndex) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        if (body.plan === true && !body.resume) {
          try {
            const result = await generatePlan(
              ...
              { gateway, provider: providerId, model: modelId },
            )
        ...
        try {
          ...
          if (agentRunner) {
            ...
            for await (const event of agentRunner.run(runOptions)) {
              switch (event.type) {
                ...
                case 'cancelled':
                  ...
                  send(JSON.stringify({ type: 'cancelled', reason: (event as any).reason }))
                  break
                case 'tool_use_end':
            ...
            for await (const event of gateway.stream(streamRequest)) {
              if (event.type === 'text') {
                ...
                send(JSON.stringify({ type: 'tool_use', name: event.name, id: event.id, input: event.input }))
          ...
          if (runSignal.aborted) {
            try { send(JSON.stringify({ type: 'cancelled' })) } catch { /* stream already closed */ }
          }
        }
        controller.close()
      },

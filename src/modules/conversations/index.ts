...
export const conversationsModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    createConversationRoutes(
      ...
      () => (ctx as any).designs,
      // Lazy for the same reason: the memory module may start later, and the
      // accessor it publishes owns the fail-soft logging.
      // `opts` must travel: the route resolves the conversation's effective
      // project and passes it here. Swallowing it made every interactive turn
      // projectless, so no `project` note could ever rank into a prompt.
      (opts) => (ctx as any).memoryIndex?.(opts) ?? null,
      ...
      skillDecisions,
      // Lazy, same reason as the index: the memory module may start later, and
      // it owns the fail-soft handling of its own extraction.
      ...
      () => (ctx as any).studio,
      // Lazy, same reason as the index: the memory module may start later, and
      // it owns the fail-soft logging. `opts` must travel so the query, project,
      // and excludeVaultPaths the route resolved actually reach the renderer.
      (opts) => (ctx as any).relatedWork?.(opts) ?? null,
      (opts) => (ctx as any).memoryAssemble?.(opts) ?? Promise.resolve(null),
    )

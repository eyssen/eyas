      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
          ...
          const parentSearchCtx = parent.searchContext
            ...
            : null
          db.run(sql`INSERT INTO conversations (id, task_id, title, status, provider_id, model_id, user_id, tokens_used, project_id, stage_id, mode, agent_id, parent_conversation_id, goal_description, team_session_id, search_context, created_at, updated_at)
            VALUES (${id}, ${taskId}, ${input.title}, ${status}, ${parent.providerId}, ${parent.modelId}, ${parent.userId}, 0, ${parent.projectId}, ${parent.stageId}, 'managed', ${input.agentId ?? null}, ${input.parentConversationId}, ${input.goalDescription}, ${parent.teamSessionId}, ${parentSearchCtx}, ${now}, ${now})`)
          break
        } catch (err: any) {
          if (attempt === 2 || !err.message?.includes('UNIQUE')) throw err
        }

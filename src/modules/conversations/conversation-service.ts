...
export interface ConversationUpdate {
  ...
  searchContext?: import('@modules/search/types').SearchContextSpec | null
  /** Bare paths or named entries, as a folder save stores them (serializeWorkingDirectories). */
  workingDirectories?: Array<string | { name: string; path: string }> | null
  godMode?: boolean
...
export interface ConversationService {
  ...
  /**
   * The human owner ownsConversation answers true for: the nearest
   * non-'system' owner up the parent chain. Null when there is none (no such
   * conversation, or every ancestor is 'system').
   */
...
export function createConversationService(
  ...
  return {
    ...
    ownsConversation(conversationId: string, userId: string): boolean {
      const owner = this.ownerOf(conversationId)
    ...
    ownerOf(conversationId: string): string | null {
      ...
      for (let i = chain.length - 1; i >= 0; i--) {
        ...
        if (owner && owner !== SYSTEM_OWNER) return owner
      }
      return null
    },
  }

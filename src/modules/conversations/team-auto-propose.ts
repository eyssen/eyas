...
export function decideTeamAutoPropose(input: TeamAutoDecideInput): TeamAutoDecision {
  ...
  // Cards are for missing roles / epic work / explicit ask — not every
  // multi-step task. `expert` maps to propose_team's `epic`.
  if (complexity === 'expert') {
    return { action: 'propose', complexity, reason: 'first message looks epic — team card' }
  }

  if (orchestration === 'deep' || orchestration === 'auto') {
    if (complexity === 'trivial' || complexity === 'simple') {
      return orchestration === 'deep'
        ? { action: 'nudge', complexity, reason: 'deep mode + light task — soft nudge only' }
        : { action: 'none', complexity, reason: 'simple task — stay solo' }
    }
    return {
      action: 'nudge',
      complexity,
      reason: `${orchestration} mode + ${complexity} — spawn specialists, no team card`,
    }
  }
...
export function buildTeamNudgeDirective(complexity: string, reason: string): string {
  return [
    ...
    'If slices of this work belong to specialists, call `run_specialist` (in parallel when independent) with their roster id. If another colleague owns it, call `handoff_to_colleague`. Call `propose_team` only if a required specialist is missing from the roster.',
    'If the task is a single short answer you own, continue solo.',
  ].join(' ')
}

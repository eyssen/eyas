...
export interface SkillMatchSummary {
  ...
  /**
   * The skill's imported content holds a credential (capability
   * `contains-secrets`, D-7). Only ever true when the owner opened
   * `memory.recall.includeSecrets` — the matcher never sees a flagged skill
   * otherwise — and it travels on the summary so the proposal card can say
   * what accepting would paste into the system prompt.
   */

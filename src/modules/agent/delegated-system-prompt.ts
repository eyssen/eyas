...
export async function buildDelegatedSystemPrompt(args: DelegatedPromptArgs): Promise<AssembledSystem> {
  ...
  return {
    ...
    ...(assembled.delivery ? { delivery: assembled.delivery } : {}),
    // The per-message turn block (clock + recall) is not part of the system:
    // the caller hands it to the runner, which attaches it to the message.
    ...(assembled.turn ? { turn: assembled.turn } : {}),
  }
}

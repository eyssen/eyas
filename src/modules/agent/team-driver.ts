...
import { createSharedRunSeq, orchestratorEventToOrchestration } from '@shared/orchestration-events.js'
...
async function driveTeamLoop(
  ...
  // earlier process already persisted, and the member runs' own tool events
  // (the agent runner emits them on this run) interleave in the order they
  // happened — so every next seq continues above the persisted high-water mark.
  const nextSeq = createSharedRunSeq(id, broadcaster?.latestSeq ? (runId) => broadcaster.latestSeq!(runId) : undefined)
  ...
  // phase_started); falls back to the run root when the phase is unknown. A
  // member's tool activity is not reported here: the agent runner emits it
  // on the member's node (runId = this team session) for every provider.

...
export interface DelegationDeps {
  ...
  /**
   * Create the specialist's sub-conversation. `binding` is the delegating
   * turn's effective provider+model (ToolContext.modelBinding): the child
   * stores it, so a specialist without a model of its own runs on it.
   */
  createChildConversation: (parentId: string, agentId: string, task: string, binding?: DelegationBinding | null) => string
  ...
  /** After the child row exists, before execute — attach a worktree etc. */
  prepareChild?: (parentConversationId: string, targetAgentId: string, childConversationId: string) => void
}

/** The delegating turn's effective provider+model. */
...
export function createDelegationService(deps: DelegationDeps) {
  ...
  return {
    ...
    async delegate(
      ...
      task: string,
      opts?: { binding?: DelegationBinding | null },
      ...
      const childId = deps.runTransaction(() => {
        ...
        return deps.createChildConversation(fromConversationId, targetAgentId, task, opts?.binding ?? null)
      })
      ...
      deps.prepareChild?.(fromConversationId, targetAgentId, childId)

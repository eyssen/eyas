...
function toAgentDefinition(raw: any): AgentDefinition {
  return {
    ...
    // A provider without a model is meaningless: read only alongside one.
    provider: raw.model ? (raw.provider ?? undefined) : undefined,
    model: raw.model ?? undefined,
    maxTurns: raw.max_turns ?? undefined,
    // A value off the ladder (only a row the boot repair has not seen) reads as Auto.
    effort: normalizeEffortSetting(raw.effort) ?? undefined,
...
/**
 * One-time upgrade of legacy colleagues (H4): a model id that exactly one
 * provider's catalog (model_config) lists gets that provider, so the
 * colleague's binding is a provider+model pair. An id several providers list,
 * an id no catalog lists and a tier alias stay provider-less — the resolver
 * binds those to their owner at run time and never guesses here. Counts every
 * catalog row, enabled or not: a model switched off under one provider still
 * makes the id ambiguous. Returns the number of rows updated.
 */
export function backfillAgentProviders(db: any): number {
  const pending = db.all(sql`SELECT id FROM agent_definitions
    WHERE provider IS NULL AND model IS NOT NULL AND model != ''
      AND (SELECT COUNT(DISTINCT mc.provider_id) FROM model_config mc WHERE mc.model_id = agent_definitions.model) = 1`) as Array<{ id: string }>
  if (pending.length === 0) return 0
  db.run(sql`UPDATE agent_definitions
    SET provider = (SELECT MIN(mc.provider_id) FROM model_config mc WHERE mc.model_id = agent_definitions.model)
    WHERE provider IS NULL AND model IS NOT NULL AND model != ''
      AND (SELECT COUNT(DISTINCT mc.provider_id) FROM model_config mc WHERE mc.model_id = agent_definitions.model) = 1`)
  ...
  return {
    create(input: CreateAgentInput): AgentDefinition {
      ...
      db.run(sql`INSERT INTO agent_definitions
        (id, name, role, description, goal, backstory, tier, agent_type, system_prompt, capabilities, tools, constraints, provider, model, max_turns, effort, enabled, source, avatar, tags, monthly_token_budget, tokens_used_month, config, created_at, updated_at)
        VALUES (
          ${input.id},
          ${input.name},
          ${input.role},
          ...
          ${input.model ? (input.provider ?? null) : null},
          ${input.model ?? null},
          ${input.maxTurns ?? null},
          ${input.effort ?? null},
          ${enabled ? 1 : 0},
      ...
      return {
        ...
        provider: input.model ? (input.provider ?? undefined) : undefined,
    ...
    update(id: string, patch: Partial<CreateAgentInput>): void {
      ...
      // The provider belongs to the model: a patch that names a model without
      // a provider leaves none (a stale provider must never pair with a new
      // model), and clearing the model clears it.
      const provider = !model
        ? null
        : patch.provider !== undefined
          ? patch.provider
          : patch.model !== undefined ? null : (existing.provider ?? null)
      ...
      for (const file of files) {
        try {
          ...
          db.run(sql`INSERT OR IGNORE INTO agent_definitions
            (id, name, role, description, goal, backstory, tier, agent_type, system_prompt, capabilities, tools, constraints, provider, model, max_turns, enabled, source, avatar, tags, monthly_token_budget, tokens_used_month, created_at, updated_at)
            VALUES (
              ${data.id},
              ${data.name},
              ${data.role ?? ''},
              ...
              ${data.model ? (data.provider ?? null) : null},
              ${data.model ?? null},
              ${data.maxTurns ?? null},

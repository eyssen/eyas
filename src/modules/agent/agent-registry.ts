  ...
  return {
    create(input: CreateAgentInput): AgentDefinition {
      ...
      db.run(sql`INSERT INTO agent_definitions
        (id, name, role, description, goal, backstory, tier, agent_type, system_prompt, capabilities, tools, constraints, model, max_turns, effort, enabled, source, avatar, tags, monthly_token_budget, tokens_used_month, config, created_at, updated_at)
        VALUES (
          ${input.id},
          ${input.name},
          ${input.role},

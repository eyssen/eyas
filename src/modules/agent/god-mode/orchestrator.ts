...
export function createGodModeOrchestrator(deps: {
  ...
  async function reviewSurvivors(
    ...
    await Promise.all(survivors.map(async (reviewer) => {
      ...
      try {
        // Task 11 — this review call has no assembler and no system-prompt
        // zone (the whole prompt rides as a user message), so it is recorded
        // as a single raw-append section, same shape as executeAgent's
        // unassembled path.
        const compositionId = runConversationDeps.contextRecorder?.record({
          sections: prompt ? [{
            zone: 'append', key: 'raw-system', content: prompt,
            chars: prompt.length, estimatedTokens: estimateTokens(prompt),

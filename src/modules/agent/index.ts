...
export const agentModule: EyasModule = {
  ...
  async onStart(ctx: ModuleContext) {
    ...
    const runDeps = {
      ...
      // Related-prior-work on the background path. Resolved per call so a
      // `config reload` of memory.relatedWork.enabled takes effect, and so
      // the memory module's own onStart (which may run after this one) is
      // the source of the accessor.
      relatedWork: (opts: import('@modules/memory/related-work.js').RelatedWorkOptions) =>
        (ctx as any).relatedWork?.(opts) ?? null,
      // Same lazy resolution: the memory module's accessor applies live
      // `memory.index.budgetChars` and may register after this module.
      memoryIndex: (opts: import('@modules/memory/memory-index.js').MemoryIndexOptions) =>
        (ctx as any).memoryIndex?.(opts) ?? null,
      memoryAssemble: (opts) => (ctx as any).memoryAssemble?.(opts) ?? Promise.resolve(null),
    }

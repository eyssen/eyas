...
import { resumeRun, DESTRUCTIVE_TOOLS } from './conversation-runner.js'
import {
  resumeParked,
...
export const agentModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    // When the memory module is available we wire an onComplete hook that
    // promotes findings/decisions to a durable vault note (P3.14).
    const teamSessionService = createTeamSessionService(ctx.db, {
      onComplete: async (session, entries) => {
        try {
          ...
          if (!memoryService?.vault || !memoryService?.indexer) return
          const { createTeamSessionPromoter } = await import('../memory/vault/team-session-promoter.js')
          const promoter = createTeamSessionPromoter({
            vault: memoryService.vault,
            indexer: memoryService.indexer,
            logger: ctx.logger,
          })
          promoter.promote(session, entries)
        } catch (err) {
          ctx.logger.debug({ err: String(err) }, 'team session vault promotion skipped')
        }
      },
    ...
    async function executeAgent(
      ...
      try {
        const result = runner.run({
          ...
          toolContext: { conversationId, userId: 'system', agentId, logger: ctx.logger, teamSessionId, sessionId: teamSessionId, agentRole: agentDef.role, ...toolWorkspaceFields(conv?.workingDirectories) },
          metadata: { conversationId, agentId, origin, autonomous: true, teamSessionId },
    ...
    ;(ctx as any).agents = { registry, runner, messaging, orchestrator, teamSessions: teamSessionService, delegation, supervisor, getCheckpoint: getCheckpointServices, executeAgent, critic: criticDeps, verify: verifyDeps, budgetEngine, orchestrationOwnership }

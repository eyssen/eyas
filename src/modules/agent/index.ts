...
import { backfillAgentProviders, createAgentRegistry } from './agent-registry.js'
...
import { supervisorOutcomeOf, type RunOutcome } from './run-outcome.js'
...
import { createOrchestrator, gcOrphanedWorktrees, createWorktree } from './orchestrator.js'
import { agentHasWriterTools } from './parallel-routing.js'
import { parseWorkingDirectories, isGitRepo } from '@modules/tools/working-directories.js'
...
import { findModelOwner, resolveModelRef, resolveRunBinding, runConversationRow, turnBindingOf, type ModelPair } from '@modules/model/binding.js'
...
import { resumeRun, runConversation, DESTRUCTIVE_TOOLS, type ResumeRunDeps } from './conversation-runner.js'
import { createRunConversationEntry, createRunDeps } from './run-deps.js'
import { wireHandoffRuns } from './handoff-run.js'
import {
  resumeParked,
...
import { observeRunEvents } from '@modules/observability/context-recorder.js'
import { IMPORT_ROOT_REMEDY, describeSkippedImportRoot, selectImportRoots } from '@shared/memory-sovereignty/import-roots.js'
...
import { captureRunEnd } from '@modules/memory/capture/run-end.js'
import { createRunCapture } from '@modules/memory/v2/run-capture.js'
import type { ExternalToolExecutionEntry } from '@modules/tools/tool-executor.js'
...
export const agentModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    // Effort values that are not a ladder rung (written before validation
    // existed) become Auto; idempotent.
    clearOffLadderEffort(ctx.db, 'agent_definitions', ctx.logger)
    // A colleague's model binding is provider+model (H4). Runs once, in the
    // boot that adds the column: a model id exactly one provider's catalog
    // lists gets that provider. An id several providers list, or a tier alias
    // ('sonnet'), stays provider-less and is bound to its owner at run time.
    try {
      ctx.db.run(sql`ALTER TABLE agent_definitions ADD COLUMN provider TEXT`)
      try {
        backfillAgentProviders(ctx.db)
      } catch (err) {
        ctx.logger.warn({ err: String(err) }, 'Agent model providers not backfilled (no model catalog) — provider-less models resolve at run time')
      }
    } catch { /* exists */ }
    ...
    // call site through the one run-deps bundle (createRunDeps below). Config is
    // read per access so a `config reload` takes effect without a restart, and
    // the background model service is read per access too: module
    ...
    const criticDeps: ConversationCriticDeps = {
      ...
      get aux() { return ctx.auxiliaryModel },
    }
    ...
    const runner = createAgentRunner({
      ...
      logger: ctx.logger,
      // A tool the provider's runtime ran itself (a CLI-native tool) gets its
      // tool_executions row here; the runner reports only those, so a bridged
      // EYAS tool (logged by the executor) is never recorded twice. Lazy: the
      // executor is read per call.
      recordExternalToolExecution: (entry) => {
        const executor = (ctx as any).tools?.executor as
          | { recordExternal?: (e: ExternalToolExecutionEntry) => boolean }
          | undefined
        executor?.recordExternal?.({
      ...
      // G6 — every run with a conversation emits its run tree, whatever the
      // provider. Lazy: the persisting event service is created further down
      // in this module's setup.
      getOrchestrationSink: () => (ctx as any).orchestration ?? undefined,
      get pricingOverrides() { return (ctx.config as any)?.model?.pricing },
      // Tool I/O and reasoning of every run, whatever the provider, offered to
      // L0 — kept only when memory.l0.captureToolResults / captureThinking say
      // so (the memory module installs those switches; off without it).
      startRunCapture: (run) => createRunCapture({ db: ctx.db, ...run }),
      // Whose conversation workspaces a run's stored folders may be in (the
      // conversations module's ownership rule). Lazy: read per run.
      getConversations: () => (ctx as any).conversations ?? undefined,
    })
    ...
    const orchestrator = createOrchestrator({
      ...
      agentRunner: runner,
      // Team members resolve their model through the binding resolver, read
      // per run (the model module publishes it; order is not guaranteed).
      get modelBinding() { return ctx.modelBinding },
      // The team proposal and the re-planner run through the background
      // model service, read per access (module registration order).
      get aux() { return ctx.auxiliaryModel },
      get conversations() { return (ctx as any).conversations },
      // A member without its own effort inherits the lead's (effort-intent.ts).
      db: ctx.db,
      ...
      // The durable-fact pass on each member's brief and answer (F1), read per
      // run: the memory module publishes it, in whatever order modules start.
      get memoryCapture() { return (ctx as any).memoryCapture },
    })
    ...
    async function executeAgent(
      ...
      task: string,
      opts?: {
        origin?: 'pipeline' | 'delegation'
        /**
         * Who reads the answer. 'external' (an A2A peer task) gets no
         * recalled owner memory: the turn block carries the time only.
         * Default 'owner' — a specialist, delegate or pipeline stage works
         * for the owner.
         */
      ...
      // The run's model (H4, model/binding.ts): the conversation's binding.
      // For a delegation's sub-conversation that is the specialist's own
      // pair, else the delegating turn's (stored on it at creation), else the
      // install default, fixed on the conversation on first use. Provider and
      // model always come from one binding. A binding nothing can serve fails
      // the run below (a coded error), never a guessed provider.
      let provider: string | undefined
      let model: string | undefined
      // Recorded with the reply, so a fallback (the specialist's own model
      // unavailable) is never silent.
      let turnBinding: TurnBinding | undefined
      let bindingError: unknown
      try {
        const run = await resolveRunBinding({
          resolver: ctx.modelBinding,
          conversation: runConversationRow(conv, agentId, conversationId),
          agent: agent ?? null,
          text: task,
          materialize: (id, pair) => convService?.materializeBinding?.(id, pair.providerId, pair.modelId) ?? null,
        })
        provider = run.providerId
        model = run.modelId
        turnBinding = run.resolved ? turnBindingOf(run.resolved) : undefined
        if (run.resolved?.note) {
          ctx.logger.info({ agentId, conversationId, note: run.resolved.note, providerId: provider, modelId: model }, 'executeAgent: running on a fallback model')
        }
      } catch (err) {
        bindingError = err
      }

      const toolDefs = scopedToolDefinitions(
        (ctx as any).tools?.registry,
        resolveToolScope({ agentTools: agentDef.tools, orchestration: conv?.orchestration }),
        { agentId, logger: ctx.logger },
      )

      // The run's effort (effort-intent.ts): the conversation's own level,
      // else Max when Deep, else the specialist's own effort, else the
      // delegating conversation's (inherited up the parent chain). Delegation,
      // specialist and pipeline runs resolve it identically; the gateway
      // clamps it per model.
      const effort = loadEffortIntent(
        { db: ctx.db, getAgent: (id) => registry.get(id) },
        conversationId,
        {
          agentId,
          ...(conv ? {
      ...
      // Memory provenance of this run's rows. The task was written by an
      // agent (delegation, pipeline) or, for an A2A task (external
      // audience), by a peer — never by the owner.
      const entryPath: CaptureEntryPath = opts?.audience === 'external' ? 'a2a' : origin
      const taskAuthor: CaptureAuthor = opts?.audience === 'external' ? 'peer' : 'agent'

      // Save user message (the delegated task) to child conversation
      if (convService) {
        convService.addMessage(conversationId, {
          role: 'user',
          content: task,
      ...
      // The durable-fact pass (F1) on the task and whatever the run answered,
      // through the entry every run path shares (memory/capture/run-end.ts):
      // the same gate, cap and run row as an interactive turn. Read per call —
      // the memory module publishes it, in whatever order modules start.
      const captureExchange = (answer: string) => captureRunEnd((ctx as any).memoryCapture, {
        conversationId,
      ...
      let tokensUsed = 0
      // Every model call's usage: prices the rollup and the reply's TurnMeta.
      const tally = createUsageTally()
      const pricing = () => (ctx.config as any)?.model?.pricing
      let stopReason: string | undefined
      // The final call's requested vs effective effort, recorded with the reply.
      let turnEffort: TurnEffort | undefined
      const toolNames: string[] = []
      // D6 — how the loop ended (the runner's 'done' outcome), mirrored from
      // runConversation/orchestrator.
      ...
      try {
        // Nothing can serve this run: fail it like any other run failure.
        ...
        const delegated = await buildDelegatedSystemPrompt({
          ...
          // Recall's query starts from the task; an external audience (A2A)
          // gets none.
          turnText: task,
        ...
        const compositionId = (ctx as any).contextRecorder?.record({
          ...
          provider,
          model,
          ...deliveryRecordFields(delegated.delivery),
          historyEstimatedTokens: estimateMessagesTokens([{ content: task }]),
        }) ?? null
        ...
        const result = runner.run({
          ...
          delivery: delegated.delivery?.profile,
          // The clock and the task's recall, attached to the task message.
          turn: delegated.turn,
          ...
          // An A2A task is not a delegation, whatever its request origin says.
          entryPath,
        })
        // What the provider measures on the run's last model call goes to its
        // composition (context occupancy), however the run ends.
        for await (const event of observeRunEvents(result, (ctx as any).contextRecorder, compositionId)) {
          ...
          if (event.type === 'turn_complete') {
            ...
            tally.add((event as any).usage)
          }
          // D6 — how the loop ended: the runner's single 'done' terminal.
          if (event.type === 'done') {
            runOutcome = event.outcome
        ...
        const outcome = supervisorOutcomeOf(runOutcome)

        // Save whatever the run produced before it stopped, either way, with
        // how the run ended (G7 TurnMeta).
        const persistText = (turnOutcome: TurnOutcome) => {
          if (convService && text) {
            const turnMeta = buildTurnMeta({
              outcome: turnOutcome,
              stopReason,
              summary: tally.summarize(provider, model, pricing()),
              ...(turnBinding ? { binding: turnBinding } : {}),
              ...(turnEffort ? { effort: turnEffort } : {}),
            })
            convService.addMessage(conversationId, { role: 'assistant', content: text, provider: provider ?? null, model: model ?? null, entryPath, turnMeta })
          }
          // On every outcome that left text behind, as the interactive turn sink does.
          captureExchange(text)
        }
        ...
        if (parkedApprovalId !== undefined) {
          ...
          const costUsd = tally.summarize(provider, model, pricing()).costUsd ?? 0
        ...
        persistText(cancelled ? 'cancelled' : (runOutcome ?? 'completed'))
        ...
        if (convService && partial) {
          ...
          convService.addMessage(conversationId, { role: 'assistant', content: partial, provider: provider ?? null, model: model ?? null, entryPath, turnMeta })
        }
        captureExchange(partial)
    ...
    const delegation = createDelegationService({
      ...
      createChildConversation(parentId: string, agentId: string, task: string, binding?: ModelPair | null) {
        const convService = (ctx as any).conversations
        ...
        const child = convService.createSubConversation({
          ...
          agentId,
          // The delegating turn's model, stored as the child's pair (H4).
      ...
      prepareChild(parentId: string, agentId: string, childId: string) {
        try {
          const convService = (ctx as any).conversations
          if (!convService) return
          const parent = convService.get(parentId)
          const sessionId = parent?.teamSessionId
          if (!sessionId) return
          const session = teamSessionService.get(sessionId)
          if (!session) return
          let cfg: { source?: string; members?: string[] } = {}
          try { cfg = JSON.parse(session.config || '{}') } catch { return }
          if (cfg.source !== 'implicit') return
          const memberIds = Array.isArray(cfg.members) ? cfg.members : []
          const writers = memberIds.filter((id) => agentHasWriterTools(registry.get(id)?.tools))
          if (writers.length < 2 || !agentHasWriterTools(registry.get(agentId)?.tools)) return
          const parentDirs = parseWorkingDirectories(parent.workingDirectories)
          const basePath = parentDirs[0]
          if (!basePath || !isGitRepo(basePath)) return
          const wt = createWorktree(basePath, agentId)
          convService.update(childId, { workingDirectories: [wt.path, ...parentDirs.slice(1)] })
        } catch (err) {
          ctx.logger.warn({ err: String(err), parentId, agentId, childId }, 'specialist worktree skipped')
        }
      },
    ...
    // I10 — the one dependency bundle and runner entry every background
    // conversation run shares: board bot runs, scheduled agent_run jobs,
    // colleague hand-offs, retry/refresh, the approval resume, the auto-retry
    // sweep, boot recovery and God Mode. Other modules call
    // ctx.agents.runConversation(id) rather than assembling their own deps.
    const runDeps = createRunDeps({
      ...
      verify: verifyDeps,
      budgetEngine,
      getCheckpoint: getCheckpointServices,
    })
    const runConversationEntry = createRunConversationEntry(runDeps)

    ;(ctx as any).agents = { registry, runner, messaging, orchestrator, teamSessions: teamSessionService, delegation, supervisor, getCheckpoint: getCheckpointServices, executeAgent, critic: criticDeps, verify: verifyDeps, budgetEngine, orchestrationOwnership, godMode: godModeStore, runDeps, runConversation: runConversationEntry }
    ...
    if (toolsCtx?.registry) {
      const ensureWorkSession = (conversationId: string, memberIds: string[]) => {
        try {
          teamSessionService.ensureImplicitWorkSession(conversationId, memberIds)
        } catch (err) {
          ctx.logger.warn({ err: String(err), conversationId }, 'implicit work session skipped')
        }
      }

      const { createDelegateTool } = await import('@modules/tools/builtin/delegate-tool.js')
      for (const t of createDelegateTool(delegation, registry, { ensureWorkSession })) {
        toolsCtx.registry.register(t)
      }

      // assign_task — the ASYNC counterpart of run_specialist. Board and
      ...
      const { createHandoffTool } = await import('@modules/tools/builtin/handoff-tool.js')
      for (const t of createHandoffTool({
        getConversations: () => (ctx as any).conversations,
        registry,
        bus: ctx.bus,
      })) toolsCtx.registry.register(t)
  ...
  async onStart(ctx: ModuleContext) {
    ...
    createAgentRoutes(ctx.http, (ctx as any).agents.registry, {
      db: ctx.db,
      dataDir: (ctx.config as any)?.dataDir ?? 'data',
      getConversations: () => (ctx as any).conversations,
      // A colleague's model preference (an id or tier alias) → the model it
      // runs on and its reasoning capability, read per call so a models
      // refresh is seen at once.
      effortTargetFor: (model, providerId) => {
        const registry = ctx.reasoningRegistry
        if (!registry) return null
        if (providerId) return { providerId, modelId: model, capability: registry.get(providerId, model) }
        if (!ctx.providerConfig) return null
        const ref = resolveModelRef({
          providerConfig: ctx.providerConfig,
          isRegistered: (id) => !!ctx.model?.getProvider(id),
        }, model)
        return ref ? { ...ref, capability: registry.get(ref.providerId, ref.modelId) } : null
      },
      // A written provider+model must be an enabled model of an active
      // provider; a bare model id gets the provider exactly one catalog
      // lists it under. Read per call: a models refresh is seen at once.
      modelCatalog: {
        isSelectable: (pair) => ctx.modelBinding ? ctx.modelBinding.isSelectable(pair) : true,
        ownerOf: (modelId) => ctx.providerConfig
          ? findModelOwner({ providerConfig: ctx.providerConfig, isRegistered: (id) => !!ctx.model?.getProvider(id) }, modelId, { exact: true })
          : null,
      },
    })

    // Markdown personas from instance directories (agent.importRoots), read on
    // every start. A root inside another tool's own folders (~/.claude/agents,
    // an Obsidian vault, …) is skipped — those come in once through Data port.
    // A file creates its agent and keeps it in step only until the agent is
    // edited (or deleted) in EYAS; an agent the import did not create is
    // never overwritten.
    const { importPersonasFromDirectory, resolvePersonaImportRoots, createPersonaImportLedger } = await import('./persona-import.js')
    const { scan: personaRoots, skipped: skippedPersonaRoots } = selectImportRoots(resolvePersonaImportRoots(ctx.config))
    for (const s of skippedPersonaRoots) {
      ctx.logger.warn(
        { root: s.root, relation: s.relation, protected: s.label },
        `agent.importRoots: ${describeSkippedImportRoot(s)} — not scanned; ${IMPORT_ROOT_REMEDY}`,
      )
    }
    if (personaRoots.length > 0) {
      const personaLedger = createPersonaImportLedger(ctx.db)
      for (const dir of personaRoots) {
        try {
          const r = await importPersonasFromDirectory((ctx as any).agents.registry, dir, { ledger: personaLedger })
          if (r.created + r.updated > 0 || r.kept.length + r.deleted.length + r.shadowed.length + r.failed > 0) {
            ctx.logger.info(
              { dir, created: r.created, updated: r.updated, unchanged: r.unchanged, keptEdited: r.kept, keptDeleted: r.deleted, shadowed: r.shadowed, failed: r.failed },
              'Imported personas from markdown (agents edited or deleted in EYAS are left as they are)',
            )
          }
        } catch (err) {
    ...
    // I10 — the one bundle every background run shares (run-deps.ts),
    // created with the agent services in onRegister.
    const runDeps: ResumeRunDeps = agents.runDeps
    ...
    const approvalResumeDeps: ApprovalResumeDeps = {
      ...
      supervisor,
      resumeRun: (runId: string, opts) => resumeRun(runId, runDeps, opts),
      logger: ctx.logger,
    }
    ...
    createRunSupervisionRoutes(ctx.http, supervisor, ctx.db, {
      ...
      resumeRun: (runId: string, opts?: { seedFromCheckpoint?: boolean }) => resumeRun(runId, runDeps, opts),
    ...
    // I10 — handoff_to_colleague starts the colleague: its home thread (goal =
    // the brief) runs through the shared runner entry. The entry is read per
    // event, like every other lazily published service.
    wireHandoffRuns({
      bus: ctx.bus,
      db: ctx.db,
      runConversation: () => (ctx as any).agents?.runConversation,
      logger: ctx.logger,
    })

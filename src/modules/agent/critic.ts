...
6. GROUNDING: factual claims about code, docs, tickets, or company knowledge must show evidence from retrieval tools (search_indexed, memory_search, search_knowledge, get_page) or explicit [source:…] citations in the transcript. If the goal asks to research/find/cite/look up OR to implement/fix/refactor against a real codebase and there is zero retrieval evidence, verdict is 'incomplete'.`
...
export function deterministicGroundingCheck(input: CriticInput): CriticResult | null {
  ...
  const hasCites =
    ...
    /\b(search_indexed|memory_search|search_memory|search_knowledge|get_page)\b/.test(input.transcript)

...
export interface CriticInput {
  ...
  /** Whether any retrieval tool (search_*, *_memory, knowledge) was invoked. */
...
6. GROUNDING: factual claims about code, docs, tickets, or company knowledge must show evidence from retrieval tools (search_indexed, search_memory, search_knowledge, get_page) or explicit [source:…] citations in the transcript. If the goal asks to research/find/cite/look up OR to implement/fix/refactor against a real codebase and there is zero retrieval evidence, verdict is 'incomplete'.`
 ...
 * when grounding is required but the transcript shows no retrieval evidence.
...
export function deterministicGroundingCheck(input: CriticInput): CriticResult | null {
  ...
  const hasCites =
    ...
    /\b(search_indexed|search_memory|search_knowledge|get_page)\b/.test(input.transcript)

  if (hasCites) return null

  return {
    verdict: 'incomplete',
    reason: 'Grounding required but no retrieval tools or [source:…] citations appear in the transcript.',
    missing: ['Retrieve sources via search_indexed/search_memory/search_knowledge and cite them as [source:<id>]'],
  }
}

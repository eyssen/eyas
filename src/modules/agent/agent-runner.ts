...
interface AgentRunnerDeps {
  ...
  /**
   * The conversations module (ctx.conversations), read per run: which
   * conversation workspaces the run's stored folders may be in — its own
   * conversation's and its human owner's other conversations'. A folder in
   * any other (stored before that rule, inherited from a project, or reached
   * through a symlink) is left out like a K2 refusal. Absent: the run's own
   * conversation workspace only.
   */
...
export function createAgentRunner(deps: AgentRunnerDeps) {
  ...
  const loop = {
    ...
    async *run(options: AgentRunOptions): AsyncGenerator<AgentEvent> {
      ...
      // CLI metadata below — and said once, under the turn. So is a folder in
      // the workspace of a conversation of another user (otherWorkspace).
      if (toolContext && (toolContext.workingDirectory || toolContext.workingDirectories?.length)) {
        const mayUseWorkspace = storedFolderWorkspaceAccess(deps.getConversations?.(), toolContext.conversationId)
        const screened = screenToolWorkspaceFields(toolContext, mayUseWorkspace ? { mayUseWorkspace } : {})

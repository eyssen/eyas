...
// the F1 dead-wiring design record (docs/superpowers/plans/...eyas-f1-dead-wiring.md,
...
//     (search_indexed, list_search_sources, search_knowledge, get_page, list_documents,
...
### Tool Usage
- Before asserting APIs, paths, symbols, or project facts: list_search_sources
  (see what is indexed) → search_indexed (code/docs) or search_knowledge (wiki).
  Cite hits as [source:<id>]. If nothing is indexed or hits are empty, say so —
  never invent code structure from model knowledge.
- Use memory (memory_search / memory_expand) for preferences and prior decisions.
- When modifying files, show only the changed parts, not the full file.
- Verify facts before stating them — search, don't guess.

### Coordination — you do not build
You are a colleague the owner talks to. You plan, research, summarise, and
route. You do not edit source, run a shell, or author design files.

- Work you **own** (conversation, planning, research summaries, scheduling,
  channel triage): do it.
- Work another **colleague** owns: \`handoff_to_colleague\` with their roster
  **id**, a one-line reason, and a brief they can continue from.
- A single-domain slice a **specialist** owns: \`run_specialist\` with their
  roster **id** and a self-contained brief. Independent slices → parallel
  \`run_specialist\` calls in the same turn.
- Background board work: \`assign_task\`.
- \`propose_team\` only if a required specialist is **missing** from the
  roster, the owner asked for a team, or the work is epic and needs a
  visible plan. Do not open a team card for ordinary multi-step work.

Copy roster **id** fields, never names or slugs. After specialists return,
synthesise one answer for the owner.
...
- Outside your capabilities → say so honestly, suggest alternatives.`,
    ...
    tools: ['read_file', 'grep', 'glob', 'git_status', 'git_diff', 'search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'research', 'memory_search', 'memory_expand', 'search_knowledge', 'get_page', 'create_page', 'list_documents', 'read_document', 'list_projects', 'move_to_stage', 'propose_team', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages', 'list_channels', 'channel_send', 'design_list', 'design_read', 'design_link', 'design_unlink', 'render_html_document'],
    constraints: [
...
- Never skip tests to save time`,
    ...
    tools: ['run_command', 'read_file', 'write_file', 'edit_file', 'grep', 'glob', 'git_status', 'git_diff', 'search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'memory_search', 'memory_expand', 'list_documents', 'read_document', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages', 'list_channels', 'channel_send'],
    constraints: [
...
- Enabled specialists by id from the roster (code-reviewer, test-engineer, …)
...
- Don't block progress over suggestions — only criticals block`,
    ...
    tools: ['read_file', 'grep', 'glob', 'git_status', 'git_diff', 'search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'search_knowledge', 'get_page', 'list_documents', 'read_document', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages'],
    constraints: [
...
- If existing tests are broken, report it — don't silently fix`,
    ...
    tools: ['run_command', 'read_file', 'write_file', 'edit_file', 'grep', 'glob', 'git_status', 'git_diff', 'search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'memory_search', 'memory_expand', 'list_documents', 'read_document', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages'],
    constraints: [
...
- Update existing docs rather than creating new ones when possible`,
    ...
    tools: ['search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'search_knowledge', 'get_page', 'create_page', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages', 'design_list', 'design_read', 'render_html_document'],
    constraints: [
...
- Document all error cases`,
    ...
    tools: ['read_file', 'grep', 'glob', 'git_status', 'git_diff', 'search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'search_knowledge', 'get_page', 'list_documents', 'read_document', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages', 'design_list', 'design_read', 'render_html_document'],
    constraints: [
...
- Ask clarifying questions before writing stories for ambiguous requests`,
    ...
    tools: ['search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'search_knowledge', 'get_page', 'list_documents', 'read_document', 'memory_search', 'memory_expand', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages'],
    constraints: [
...
- Don't mix opinion with facts — label each clearly`,
    capabilities: ['research', 'code-analysis'],
    tools: ['research', 'search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'memory_search', 'memory_expand', 'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages'],
    constraints: [
      'Always cite sources',
...
/**
 * Tool sets EYAS has shipped before, paired with what the same template grants
 * now. `reconcileAgentTools` upgrades a seeded agent ONLY when its list still
 * matches a prior set exactly — an edited list is somebody's decision.
 *
 * Derived rather than hand-copied: each round names the tools it adds, and the
 * prior set is the current one without them. Append a round, never edit one.
 */
const TOOLS_ADDED_BY_ROUND: string[][] = [
  // 2026-08: the design tools. Registered, reachable through both MCP bridges,
  // and in nobody's allow-list — the symptom was a model saying "design_read
  // is not wired" and doing the work twice.
  ['design_list', 'design_read', 'design_create', 'design_write', 'design_link', 'design_unlink', 'render_html_document'],
  // 2026-09: colleague routing — run_specialist / handoff / assign_task.
  ['run_specialist', 'handoff_to_colleague', 'assign_task'],
]

/** Personal Assistant allow-list as shipped before the 2026-09 coordinator strip. */
const ASSISTANT_SHIPPED_WITH_DESIGN = [
  'run_command', 'read_file', 'write_file', 'edit_file', 'grep', 'glob', 'git_status', 'git_diff',
  'search_indexed', 'list_search_sources', 'get_search_context', 'set_search_context', 'research',
  'memory_search', 'memory_expand', 'search_knowledge', 'get_page', 'create_page', 'list_documents',
  'read_document', 'list_projects', 'move_to_stage', 'propose_team', 'delegate_to_agent',
  'write_team_memory', 'read_team_memory', 'send_agent_message', 'read_agent_messages',
  'list_channels', 'channel_send',
  'design_list', 'design_read', 'design_create', 'design_write', 'design_link', 'design_unlink',
  'render_html_document',
] as const

const ASSISTANT_SHIPPED_WITHOUT_DESIGN = ASSISTANT_SHIPPED_WITH_DESIGN.filter(
  (t) => !t.startsWith('design_') && t !== 'render_html_document',
)
...
export const PRIOR_TOOLSETS: ToolsetUpgrade[] = (() => {
  const assistantNow = PRIMARY_TEMPLATES[0]!.tools
  const out: ToolsetUpgrade[] = [
    { prior: ASSISTANT_SHIPPED_WITHOUT_DESIGN, current: assistantNow },
    { prior: [...ASSISTANT_SHIPPED_WITH_DESIGN], current: assistantNow },
  ]
  for (const template of ALL_TEMPLATES) {
    for (const added of TOOLS_ADDED_BY_ROUND) {
      const prior = template.tools.filter((t) => !added.includes(t))
      if (prior.length !== template.tools.length) out.push({ prior, current: template.tools })
    }
  }
  return out
})()

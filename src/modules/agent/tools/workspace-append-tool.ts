...
// MEMORY.md and memory/YYYY-MM-DD.md are deliberately NOT targets: models never
// write memory (core rule 8). EYAS records memory itself; the workspace MEMORY.md
// is the owner's own note file and is not sent to the model.
...
export function createWorkspaceAppendTool(deps: WorkspaceAppendDeps) {
  return {
    ...
    description:
      'Append content to your own workspace file (AGENTS.md or TOOLS.md). Not for memory — EYAS records memory automatically.',
    inputSchema: workspaceAppendInputSchema,
    async invoke(
      agentId: string,
      input: WorkspaceAppendInput,
    ): Promise<{ ok: true; bytesAdded: number }> {
      // Re-validate here too: a caller that bypasses the executor must not
      // reach a memory file.
      input = workspaceAppendInputSchema.parse(input)

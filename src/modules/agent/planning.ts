...
/**
 * The temperature every plan call uses: reproducible JSON, not creativity.
 * The completion's owner applies it (the background service for the rubric
 * planner, the conversation's own model for plan-first).
 */
export const PLAN_TEMPERATURE = 0.2

/**
 * One plan-generation call, injected by the caller: the instruction (sent as
 * the system prompt) and the task (the single user message) in; the answer
 * text, or why there is none, out. The caller owns which model answers and
 * how (background: the auxiliary model service; interactive: the
 * conversation's own binding), so one generator serves both.
 */
export type PlanCompletion = (prompt: { system: string; user: string }) => Promise<{ text: string } | { error: string }>
...
export async function generatePlan(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const user = input.additionalContext
      ? `Task:\n${input.originalRequest}\n\nContext:\n${input.additionalContext}`
      : `Task:\n${input.originalRequest}`

    let answer: { text: string } | { error: string }
    try {
      answer = await opts.complete({ system: PLAN_JSON_INSTRUCTIONS, user })
    } catch (err) {
      answer = { error: err instanceof Error ? err.message : String(err) }
    }
    // No answer (no eligible model, a failed call): another attempt would ask
    // the same completion the same question. Only a malformed answer retries.
    if ('error' in answer) return { error: { attempts: attempt, lastMessage: answer.error } }
    ...
    const planCandidate = {
      ...
      steps: (Array.isArray(parsed.value.steps) ? parsed.value.steps : []).map((s: any, idx: number) => ({
        id: `step-${idx + 1}`,
        title: s.title,
      ...
      risks: (Array.isArray(parsed.value.risks) ? parsed.value.risks : []).map((r: any, idx: number) => ({
        id: `risk-${idx + 1}`,

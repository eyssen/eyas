...
import { stripThinkingBlocks } from '@modules/model/helpers.js'
...
import { runCritic, isRetrievalTool, type CriticPlanStep } from './critic.js'
import { maybePlanTask } from './planning-runner.js'
import { PLAN_TEMPERATURE, type PlanCompletion } from './planning.js'
import type { AuxiliaryModelService, AuxRequest } from '@modules/model/auxiliary.js'
import {
...
// resolves its settings exactly as the foreground one does.
...
import { captureInstruction } from '@modules/conversations/l0-capture.js'
import { captureRunEnd } from '@modules/memory/capture/run-end.js'
import type { CaptureAuthor } from '@modules/memory/v2/ingest-bridge.js'
import { unwrapUntrusted } from '@shared/untrusted.js'
...
import { toolNamesOfRuns } from '@modules/tools/execution-log.js'
...
export interface ConversationCriticDeps {
  ...
  maxRounds?: number
  /**
   * The background model service (ctx.auxiliaryModel) — read per access, so
   * wire it as a getter. It answers both the rubric planner (purpose
   * 'planner') and the critic (purpose 'critic') with isolated one-shots on an
   * eligible model; absent or without one, the run gets no rubric and stays
   * Unverified.
   */
  aux: Pick<AuxiliaryModelService, 'complete'> | undefined
}

/** Longest plan answer the rubric planner accepts: a 12-step plan fits comfortably. */
const RUBRIC_PLAN_MAX_TOKENS = 3000

/**
 * The rubric planner's completion: one isolated background call (purpose
 * 'planner'), attributed to the run. No eligible model — or no service at
 * all — is an error answer, so the run proceeds without a rubric.
 */
function rubricPlanCompletion(
  aux: Pick<AuxiliaryModelService, 'complete'> | undefined,
  attribution: Pick<AuxRequest, 'origin' | 'conversationId' | 'runId' | 'agentId'>,
): PlanCompletion {
  return async ({ system, user }) => {
    if (!aux) return { error: 'no background model service' }
    const result = await aux.complete({
      purpose: 'planner',
      system,
      user,
      temperature: PLAN_TEMPERATURE,
      maxTokens: RUBRIC_PLAN_MAX_TOKENS,
      ...attribution,
    })
    if (result.ok) return { text: result.text }
    if (result.reason === 'error') return { error: result.error?.message ?? 'background model call failed' }
    if (result.reason === 'empty') return { error: 'empty response' }
    return { error: `no eligible background model (${result.reason})` }
  }
}
 ...
 * agent runner" unit. Other modules reach it only through the shared entry
 * ctx.agents.runConversation (run-deps.ts: one dependency bundle), so board
 * bot runs, scheduled agent_run jobs, colleague hand-offs and the
 * POST /agent/runs/:id/retry route all run the same code with the same deps.
 *
...
export interface ConversationRunnerDeps {
   ...
   */
  memoryCapture?: import('@modules/memory/capture/run-end.js').MemoryCaptureFn
  /**
   * The model binding resolver (ctx.modelBinding; pass a getter — the model
   * module publishes it). The run's provider and model come from one binding
   * of the conversation (H4). Absent: the same precedence without catalog
   * checks (model/binding.ts resolveRunBinding).
   */
  modelBinding?: Pick<BindingResolver, 'resolve'>
  /** Fixes the install default on a conversation that has no pair (conversation-service materializeBinding). */
  materializeBinding?: (conversationId: string, pair: ModelPair) => ModelPair | null
}
...
/**
 * Who wrote the message a background run's capture reads. No message row
 * stores its author, so it is read from where the text came from:
 *  - a stored channel/A2A message (an <untrusted-input> block) is a peer's;
 *  - the goal itself, or a message in a sub-conversation (a delegated task,
 *    a team brief, a God Mode worker), was written by an agent — the same
 *    author the goal's L0 row carries (captureInstruction in runConversation);
 *  - any other stored message is the owner's own, typed in the chat.
 */
function backgroundAuthor(
  storedUser: string,
  conv: { goal_description?: string | null; parent_conversation_id?: string | null },
): CaptureAuthor {
  if (storedUser && unwrapUntrusted(storedUser)) return 'peer'
  if (!storedUser || storedUser === conv.goal_description || conv.parent_conversation_id) return 'agent'
  return 'owner'
}
...
export async function runConversation(
  ...
  // feature that reads it (which is exactly how effort/orchestration stayed
  // dead — they were read below but never selected).
  ...
  try {
    ...
    // The goal is this run's instruction, yet it only travels in the model
    // call — no message row holds it. Remember it once (a retry or resume of
    // the same goal adds nothing), as agent-authored text: a card's goal may
    // come from another agent, so it is never owner trust.
    captureInstruction(db, {
      conversationId: conv.id,
      text: conv.goal_description,
      author: 'agent',
    ...
    // its fixed pair, or for a colleague's card the agent's pair, else the
    // stored one, else the install default (fixed on the card). Provider and
    // model come from ONE binding, never the card's provider with the agent's
    // model. Nothing that can serve it fails the run (the catch below).
    const { providerId: provider, modelId: model } = await resolveRunBinding({
      resolver: deps.modelBinding,
      conversation: {
        id: conv.id,
        modelBinding: isModelBindingMode(conv.model_binding)
          ? conv.model_binding
          : defaultBindingMode({ agentId: conv.agent_id, parentConversationId: conv.parent_conversation_id ?? null }),
        providerId: conv.provider_id ?? null,
        modelId: conv.model_id ?? null,
        agentId: conv.agent_id,
        parentConversationId: conv.parent_conversation_id ?? null,
        // A fixed model the user chose in the picker is never swapped for
        // another: unavailable, the run fails (the catch below).
        ...(conv.model_user_chosen === 1 ? { modelUserChosen: true } : {}),
      },
      agent,
      text: conv.goal_description,
      materialize: deps.materializeBinding,
    })
      ...
      if (existing) {
        ...
        try {
          const decision = await maybePlanTask(conv.goal_description, {
            complete: rubricPlanCompletion(critic.aux, {
              origin: 'scheduled',
              conversationId: conv.id,
    ...
    const orchestrationDirective = buildOrchestrationDirective(conv.orchestration)
    ...
    // Recalled memory is not here: it is the assembler's turn block, which
    // the runner attaches to the run's message (AssembledPrompt.turn).
    const extraSystem = [orchestrationDirective, designBlock?.content]
      .filter((s) => s && s.trim())
    ...
    // The turn block when the prompt could not be assembled: the system
    // prompt falls back to the string, the clock and recall still go out.
    ...
    if (deps.promptAssembler) {
      try {
        const base = await deps.promptAssembler.buildForPrimary({
          ...
          channelContext: null,
          // I7: sized for the model this run calls; the runner reads
          // base.delivery (tool support, window) and base.turn (clock +
          // recall) from systemPrompt.
          target: { providerId: provider ?? null, modelId: model ?? null },
          // The run's message is its goal; recall composes its query from it
          // plus the conversation's own title and user turns.
          turnText: conv.goal_description,
        })
    ...
    const runMessages = overrides.messages ?? [{ role: 'user' as const, content: conv.goal_description }]
    ...
    const compositionId = deps.contextRecorder?.record({
      ...
      // An unassembled run still records what its turn block delivered.
      ...deliveryRecordFields(assembledPrompt?.delivery ?? turnOnly?.delivery),
      historyEstimatedTokens: estimateMessagesTokens(runMessages),
    }) ?? null

    // What the provider measures on the run's last model call goes to the
    // composition (context occupancy), however the run ends.
    for await (const event of observeRunEvents<any>(agentRunner.run({
      // Warm-resume seeds the lossless checkpoint history; a fresh run/retry
      // starts from the goal.
      ...
      // The assembled prompt carries its own turn block; this is the one
      // built without it.
      ...
      toolContext: {
        ...
        projectId: effectiveProjectId(conv.projectId ?? conv.project_id ?? null),
        logger,
      ...
      // Effort (effort-intent.ts): the conversation's own level, else Max
      // when Deep, else the agent's, else the nearest delegating parent's —
      // read from the stored rows, so a first run, a retry, a resume after an
      // approval and a boot recovery all send the same intent. The gateway
      // resolves it per model.
      effort: loadEffortIntent(
        { db, getAgent: (id) => agentRegistry.get(id) },
        conv.id,
        {
          agentId: conv.agent_id,
          self: {
      ...
      // status: the runner's single 'done' terminal carries it.
      if (event.type === 'done') outcome = supervisorOutcomeOf(event.outcome)
    ...
    if (critic && !outcome && !cancelled && handle?.sessionId && !feedback) {
      try {
        ...
        if (!transcript) {
          ...
          // Tool evidence from the runner's own events AND from the executor
          // log of every judged run: a CLI reports a bridged EYAS tool under
          // its own name or a display title (ACP), while the executor logged
          // the canonical one — so every provider path is credited alike.
          const retrievalUsed = toolNames.some((n) => isRetrievalTool(n))
            || toolNamesOfRuns(db, judged).some((n) => isRetrievalTool(n))
          ...
          const verdict = await runCritic(
            {
              ...
              // The memory EYAS delivered with this run's message counts as
              // evidence on every provider (the critic's rule 6).
              injectedMemoryIds: (assembledPrompt?.delivery ?? turnOnly?.delivery)?.recall?.ids,
            },
            {
              aux: critic.aux,
    ...
    const costUsd = costAcc.finalize(provider, model, deps.pricingOverrides)
    ...
    // Nothing the extractor could read (no answer) is no call and no run row
    // (captureRunEnd): a skip row per autonomous run would only inflate the
    // diagnostics it exists to keep honest.
    if (deps.memoryCapture) {
      try {
        ...
        const storedUser = lastOf('user')
        const userMessage = storedUser || (conv.goal_description ?? '')
        ...
        captureRunEnd(deps.memoryCapture, {
          ...
          userMessage,
          assistantMessage,
          author: backgroundAuthor(storedUser, conv),
          entryPath: 'background',
        })
...
export const DESTRUCTIVE_TOOLS = [
  ...
  'browser_select', 'browser_dialog', 'browser_upload', 'browser_evaluate', 'browser_download',
  'browser_tabs', 'browser_storage', 'browser_replay',
  'browser_use_exec', 'agent_browser_run',
  'create_page', 'move_to_stage', 'create_sub_conversation',
  'send_agent_message', 'write_team_memory', 'save_memory',
  'delegate_to_agent', 'run_specialist', 'handoff_to_colleague', 'assign_task', 'propose_agent_creation', 'propose_team',
  ...
  // CLI-native tools under their canonical names (shared/canonical-tool-name.ts):
  // the runner records what Claude Code / Grok / Kimi ran themselves under
  // these, and their permission bridge checks a resumed run against them.
  // run_command / write_file / delete_file are listed elsewhere in this set.
  'edit_file', 'move_file',
...
export async function resumeRun(
  ...
  if (seedFromCheckpoint) {
    try {
      ...
      // Reasoning blocks are bound to the exact prefix they were produced
      // under; the resumed run's prefix differs (recap, new system), so a
      // replayed block would be rejected. The reasoning restarts once. The
      // checkpoint holds no turn block (agent-runner removes the one it
      // attached before saving), so the resumed run attaches a fresh one.
      if (Array.isArray(mm) && mm.length > 0) messages = stripThinkingBlocks(mm as ModelMessage[])
  ...
  // (carrying the canonical toolName + normalized argHash — toolLedgerKey). The
  // runner records them for its own executor AND for tools a CLI provider ran
  // itself, so the same recap and ledger arm a resumed Claude Code / Grok /
  // Kimi run (the ledger reaches their permission bridge via the runner's
  // request metadata). Using ToolResult(success) — not ToolCall — means a
  // failed destructive op is NOT blocked (it can be retried). A query FAILURE
  // here must FAIL SAFE (refuse) rather than degrade to an empty ledger.

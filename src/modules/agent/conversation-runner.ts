...
import { runCritic, type CriticPlanStep, type CriticTier } from './critic.js'
...
export async function runConversation(
  ...
  try {
    ...
    const systemString = orchestrationDirective
      ? [agent.systemPrompt, orchestrationDirective].filter(Boolean).join('\n\n')
      : agent.systemPrompt
    ...
    if (deps.promptAssembler) {
      try {
        ...
        assembledPrompt = orchestrationDirective
          ? { ...base, reminders: [...base.reminders, orchestrationDirective] }
          : base
      } catch {
    ...
    for await (const event of agentRunner.run({
      ...
      metadata: { conversationId: conv.id, userId: 'bot', agentId: conv.agent_id, teamSessionId: conv.team_session_id ?? undefined, origin: 'scheduled' as const, autonomous: true },
    ...
    let round = 0
    if (critic && !outcome && !cancelled && handle?.sessionId) {
      try {
        ...
        if (!transcript) {
          ...
          const verdict = await runCritic(
            { goal: conv.goal_description, planSteps, transcript },
            {
              gateway: critic.gateway,
    ...
    handle?.fail(String(err?.message ?? err), kind)
    logger.error(`Conversation runner: failed on conversation ${conv.id}: ${err?.message ?? err}`)
    db.run(sql`UPDATE conversations SET status = 'idle' WHERE id = ${conv.id}`)

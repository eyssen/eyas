...
export interface ConversationRunnerDeps {
  ...
  /**
   * Related-prior-work block on the background path. When present, the
   * accessor applies live `memory.relatedWork` config (same freshness as
   * capture). Absent: fall back to a direct `buildRelatedWork` so existing
   * tests that do not pass the accessor keep working.
   */
  relatedWork?: (opts: import('@modules/memory/related-work.js').RelatedWorkOptions) => import('@modules/memory/related-work.js').RelatedWorkResult | null
...
export async function runConversation(
  ...
  try {
    ...
    // fail-soft contract as the design block above. The accessor is preferred
    // because it applies live `memory.index` config; without it (a build with
    // no memory module, or an older test) the runner builds the index itself.
    let memoryBlock: { content: string; paths?: string[]; ids?: string[] } | null = null
    ...
    try {
      ...
      if (deps.memoryAssemble) {
        const assembled = await deps.memoryAssemble({
          query,
          conversationId: conv.id,
          projectId,
        })
        if (assembled) memoryBlock = assembled
      } else {
        const opts = { projectId }
        if (deps.memoryIndex) {
          memoryBlock = deps.memoryIndex(opts)
        } else {
        ...
        try {
          ...
          if (deps.relatedWork) {
            relatedBlock = deps.relatedWork(relatedOpts)
          } else {
            const { buildRelatedWork } = await import('@modules/memory/related-work.js')
            relatedBlock = buildRelatedWork(db, relatedOpts)
          }
        } catch (err: any) {
      ...
      logger.warn(`Conversation runner: memory assemble failed for conversation ${conv.id}: ${err?.message ?? err}`)
    }

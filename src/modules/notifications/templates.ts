...
function defaultHtml(payload: NotificationPayload): string {
  const color = SEVERITY_COLORS[payload.severity] ?? '#3b82f6'
  return `
<!DOCTYPE html>
<html>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="border-left: 4px solid ${color}; padding: 16px; background: #f9fafb; border-radius: 4px;">
    <h2 style="margin: 0 0 8px 0; color: #111827; font-size: 16px;">${escapeHtml(payload.title)}</h2>
    ${payload.body ? `<p style="margin: 0; color: #4b5563; font-size: 14px;">${escapeHtml(payload.body)}</p>` : ''}
  </div>
  <p style="margin: 16px 0 0 0; color: #9ca3af; font-size: 12px;">
    ${payload.event} · ${new Date(payload.createdAt).toLocaleString()}
  </p>
  <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 16px 0;">
  <p style="color: #9ca3af; font-size: 11px;">Sent by EYAS Notifications</p>
</body>
</html>`.trim()
}
...
function digestHtml(notifications: NotificationRecord[]): string {
  const rows = notifications.map(n => {
    const color = SEVERITY_COLORS[n.severity] ?? '#3b82f6'
    return `
    <tr>
      <td style="padding: 8px 12px; border-bottom: 1px solid #e5e7eb;">
        <span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: ${color}; margin-right: 8px;"></span>
        <strong>${escapeHtml(n.title)}</strong>
        ${n.body ? `<br><span style="color: #6b7280; font-size: 13px;">${escapeHtml(n.body)}</span>` : ''}
      </td>
      <td style="padding: 8px 12px; border-bottom: 1px solid #e5e7eb; color: #9ca3af; font-size: 12px; white-space: nowrap;">
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 16px 0;">
  <p style="color: #9ca3af; font-size: 11px;">EYAS Notification Digest</p>
</body>
</html>`.trim()
}

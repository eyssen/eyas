...
/** The CASL check a plain shell needs on top of `create OpenCode`: the owner's and an admin's. */
const SHELL_ACTION = 'manage'
 ...
 * `workingDirectories` list included) are dropped. `kind: 'shell'` (a plain
 * login shell) is for a caller who may manage OpenCode only.
 */
...
export function createOpencodeRoutes(
  ...
  app.post('/api/v1/opencode/sessions', requirePermission('create', 'OpenCode'), async (c) => {
    ...
    // A plain shell runs as the EYAS server's OS user with no folder jail and
    // no approval step: the owner's and an admin's tool (manage OpenCode),
    // never a `user`'s — `create OpenCode` alone opens the OpenCode TUI only.
    if (parsed.data.kind === 'shell') {
      const ability = (c.get as (key: string) => unknown)('ability') as { can?: (action: string, subject: string) => boolean } | undefined
      if (!ability?.can?.(SHELL_ACTION, 'OpenCode')) {
        return c.json({ error: `Forbidden: cannot ${SHELL_ACTION} OpenCode` }, 403)
      }
    }

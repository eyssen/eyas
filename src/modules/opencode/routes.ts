...
import { folderRefusedNotice } from '@modules/tools/working-directories.js'
...
import { resolveSessionCwd, resolveTuiFolders } from './path-guard.js'
...
export function createOpencodeRoutes(
  ...
  app.get('/api/v1/opencode/models', requirePermission('read', 'OpenCode'), async (c) => {
  ...
  app.post('/api/v1/opencode/sessions', requirePermission('create', 'OpenCode'), async (c) => {
    ...
    try {
      ...
      // an id that is not a safe folder name is refused here.
      const fallback = ensureConversationWorkspace(parsed.data.conversationId)
      if (parsed.data.kind === 'tui') {
        // OpenCode's model works in the TUI's folders: the same screening as
        // every other CLI run (K2), never a refused folder.
        const folders = resolveTuiFolders({ requested: parsed.data.cwd, workingDirectories: parsed.data.workingDirectories, fallback })
        cwd = folders.cwd
        ...
        if (folders.refused.length > 0) {
          deps.logger?.warn({ conversationId: parsed.data.conversationId, refused: folders.refused.map((r) => r.code) }, 'OpenCode terminal: stored folder refused — left out')
        }
      } else {
        cwd = resolveSessionCwd({ requested: parsed.data.cwd, workingDirectories: parsed.data.workingDirectories, fallback })
    ...
    let cmd: TuiCommand
    try {
      cmd = parsed.data.kind === 'tui'
        ? await deps.resolveTuiCommand()
  ...
  // before, for exactly one session — else a signed-in user allowed to create
  // OpenCode sessions. The auth module lets exactly these two paths through
  // its deny-by-default check for a valid proof only (delegated-bearer.ts);
  // here the proof is used up. A forged, stale, replayed or dead-key proof is
  // a 401, a CASL denial a 403.

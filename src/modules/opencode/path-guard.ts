...
/**
 * The folders of an OpenCode terminal (the TUI, and the shell). OpenCode's
 * model works there with the tools the human approves in the terminal, so
 * the folders pass the screening every other CLI run's folders pass (K2): a
 * folder that is, sits in or contains a protected place (EYAS's data,
 * another tool's storage, a notes vault, the home folder) is dropped — the
 * requested cwd too — and the terminal opens in the first allowed folder,
 * else in `fallback` (the conversation's own workspace). `screen` may add
 * rules (the route adds the workspace-ownership rule). A requested cwd must
 * still lie inside the request's folders when it names any.
 */

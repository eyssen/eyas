      ...
      execute: async (input, ctx) => {
        ...
        try {
          const result = await agent.run({
            ...
            // The run's folders, server-side (a context with only a primary
            // folder has that one): the model's cwd must lie inside them.
            workingDirectories: ctx.workingDirectories?.length
              ? ctx.workingDirectories
              : ctx.workingDirectory ? [ctx.workingDirectory] : undefined,

...
import { ensureConversationWorkspace, resolveCliCwd } from '@modules/model/cli-runtime/workspaces.js'
import { foreignWorkspaceOf, storedFolderWorkspaceAccess, type ConversationOwnership } from '@modules/tools/working-directories.js'
...
export interface DeveloperAgentDeps {
  ...
  /**
   * The conversations module (ctx.conversations), read per task: which
   * conversation workspaces the task's folders may be in — its own, and
   * those of its human owner's other conversations (ownerOf +
   * ownsConversation). Absent: only the task conversation's own workspace.
   */
  getConversations?: () => ConversationOwnership | undefined
  /** The task folder (default: the shared CLI cwd resolver). */
  resolveCwd?: (input: DeveloperTaskInput, rules?: TaskFolderRules) => string
  /**
...
/** Which conversation workspaces a task's folders may be in (see conversationWorkspaceAccess). */
export interface TaskFolderRules {
  /** Default: only the task conversation's own workspace. */
  mayUseWorkspace?: (conversationId: string) => boolean
}

/**
 * The conversation's folders a task may use: a folder that is, or lies
 * inside, a workspace `mayUse` refuses (or a run's scratch folder) — as
 * written or through a symlink — is dropped. Such a folder can only be one
 * stored before the save-time rule, or inherited from a project.
 */
export function screenTaskFolders(
  workingDirectories: readonly string[] | undefined,
  mayUse: (conversationId: string) => boolean,
): { kept: string[]; dropped: string[] } {
  const kept: string[] = []
  const dropped: string[] = []
  for (const raw of workingDirectories ?? []) {
    if (typeof raw !== 'string' || raw.trim().length === 0) continue
    if (foreignWorkspaceOf(raw.trim(), mayUse)) dropped.push(raw)
    else kept.push(raw)
  }
  return { kept, dropped }
}
 ...
 * folders, or inside its own workspace when it has none), else the first
 * folder, else the conversation's own workspace — through the one CLI cwd
 * resolver, never the EYAS install root. The cwd is the model's argument: it
 * can never name a folder the conversation does not have, nor a workspace
 * of a conversation the task may not use (`rules`), as written or after
 * realpath; a stored folder in such a workspace is skipped.
 */
export function resolveTaskCwd(input: DeveloperTaskInput, rules: TaskFolderRules = {}): string {
  const mayUse = rules.mayUseWorkspace ?? ((id: string) => id === input.conversationId)
  const stored = screenTaskFolders(input.workingDirectories, mayUse).kept
  const requested = input.cwd?.trim()
  if (requested) {
    const bounds = stored.length > 0 ? stored : [ensureConversationWorkspace(input.conversationId)]
    if (!isUnderRoot(requested, bounds)) {
      throw new Error('cwd is outside the conversation working directories')
    }
    if (foreignWorkspaceOf(requested, mayUse)) {
      throw new Error("cwd is another conversation's workspace")
    }
  }
  const cwd = resolveCliCwd({
    metadata: {
      ...
      runId: input.runId,
      workingDirectories: requested ? [requested] : stored,
    },
  })
  // Once more on the folder the resolver chose (realpathed): a conversation
  // workspace the task may not use is never its folder. The resolver's own
  // fallbacks are the conversation's workspace or the run's scratch folder.
  const foreign = foreignWorkspaceOf(cwd, mayUse)
  if (foreign && !foreign.scratch) throw new Error("cwd is another conversation's workspace")
  return cwd
}
...
export function createDeveloperAgent(deps: DeveloperAgentDeps) {
  ...
  return {
    async run(input: DeveloperTaskInput): Promise<DeveloperTaskResult> {
      ...
      try {
        // The workspaces this task may work in: its conversation's own and
        // its human owner's other conversations' — never another user's.
        // Folders naming any other are dropped before the cwd is chosen and
        // before the gate learns the task's folders.
        const mayUseWorkspace = storedFolderWorkspaceAccess(deps.getConversations?.(), input.conversationId)
          ?? ((id: string) => id === input.conversationId)
        const screened = screenTaskFolders(input.workingDirectories, mayUseWorkspace)
        if (screened.dropped.length > 0) {
          deps.logger.warn({ conversationId: input.conversationId, dropped: screened.dropped.length }, "OpenCode task: a folder in another conversation's workspace was left out")
        }
        const folders = screened.kept
        const directory = resolveCwd({ ...input, workingDirectories: folders }, { mayUseWorkspace })
        ...
        const bridge = gate
          ? createPermissionBridge({
              ...
              ctx: {
                ...
                workingDirectories: taskRoots(directory, folders),

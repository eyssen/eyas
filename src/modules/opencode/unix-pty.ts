...
import { deliverPluginKey } from './plugin-tokens.js'
  ...
  return (opts: PtySpawnOptions): PtyHandle => {
    ...
    try {
      child = spawn(opts.file, opts.args, {
        ...
        env: opts.env,
        detached: true,
        // An OpenCode TUI's memory key rides on fd 3 (plugin-tokens.ts).
        stdio: opts.pluginKey ? [slaveFd, slaveFd, slaveFd, 'pipe'] : [slaveFd, slaveFd, slaveFd],
      })
    } finally {
      try { closeSync(slaveFd) } catch { /* parent copy */ }
    }
    if (opts.pluginKey) deliverPluginKey(child, opts.pluginKey)

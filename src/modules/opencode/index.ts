...
// (developer-agent.ts); the TUI asks the human in the terminal, so the
// terminals are for callers who may manage OpenCode (permissions.ts). An attach
...
import { createTerminalWsHandler, endTerminalsOnAccessChange, type TerminalWsHandler } from './terminal-ws.js'
...
import type { OpencodeConversationsAccess, OpencodeToolsAccess } from './routes.js'
import { buildShellCommand, buildTuiCommand } from './isolation.js'
import { buildAbilityForRole } from '@modules/permissions/roles.js'
import type { RoleId } from '@modules/permissions/types.js'
import { activeRoleOf, createTerminalAccess, OPENCODE_PERMISSIONS, OPENCODE_SUBJECT } from './permissions.js'
...
export interface OpencodeModuleApi {
  ...
  /**
   * Whether a user may open or attach to a terminal now (manage OpenCode,
   * active account, role read fresh). The terminal socket asks it at the
   * upgrade (ws-server.ts), when the socket opens, before every frame, on a
   * periodic sweep and when auth reports the user's role or status changed.
   */
  ...
  /** Stops listening for role and status changes (module stop). */
  unsubscribeAccessChanges: () => void
}
...
export const opencodeModule: EyasModule = {
  ...
  async onRegister(ctx: ModuleContext) {
    ...
    try {
      // read = the page; create = headless use; manage = settings and the
      // interactive terminals (owner and admin by default) — permissions.ts.
      ctx.permissions.registerSubject(OPENCODE_SUBJECT, OPENCODE_PERMISSIONS)
    } catch {
      /* already registered */
    }
  ...
  async onStart(ctx: ModuleContext) {
    ...
    const agent = createDeveloperAgent({
      ...
      home,
      // Resolved per task: which conversation workspaces the task's folders
      // may be in (the conversations module's ownership rule).
      getConversations: () => (ctx as { conversations?: import('@modules/tools/working-directories.js').ConversationOwnership }).conversations,
      sessions,
    ...
    // The terminal right for the socket, which has no request ability: the
    // user's current role and status (as the WS topic ACL reads them) through
    // the same CASL registry every route uses.
    const mayUseTerminal = createTerminalAccess({
      roleOf: activeRoleOf(ctx.db),
      abilityFor: (role) => buildAbilityForRole(role as RoleId, ctx.permissions),
    })

    const wsHandler = createTerminalWsHandler({ pty, logger: ctx.logger, mayUseTerminal })

    // A demoted, suspended or archived user's open terminals end at once
    // (auth routes emit this); the handler's own per-frame check and sweep
    // cover a change made any other way.
    const accessChanges = endTerminalsOnAccessChange({ bus: ctx.bus, handler: wsHandler, logger: ctx.logger })

    const api: OpencodeModuleApi = {
      pty,
      runner: ocRunner,
      agent,
      wsHandler,
      mayUseTerminal,
      unregisterPluginBearer,
      unsubscribeAccessChanges: () => accessChanges.unsubscribe(),
    }
    ...
    createOpencodeRoutes(ctx.http, {
      ...
      pluginTokens,
      sessions,
      // A terminal opens only for a conversation the caller may open (the
      // conversations module's ownsConversation) and in its stored folders.
      getConversations: () => (ctx as { conversations?: OpencodeConversationsAccess }).conversations,
      logger: ctx.logger,
      ...
      // A plain shell: the same filtered environment and
      // EYAS-owned HOME as the TUI, never the server's process.env.
      resolveShellCommand: () => buildShellCommand({ home, eyasBaseUrl: eyasBase, workspacesRoot }),
    })
  ...
  async onStop(ctx: ModuleContext) {
    ...
    api?.unsubscribeAccessChanges()
    api?.wsHandler.dispose()
    api?.pty.destroyAll()

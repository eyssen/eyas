...
import type { ContentBlock, ModelGateway } from '@modules/model/types.js'
...
/** Output cap for one design completion: a whole canvas is large. */
const DESIGN_MAX_TOKENS = 8000

/**
 * The design editor's completion over the model gateway: one isolated call
 * (no tools, one turn) that names no provider, so the gateway binds it to the
 * install default (model/binding.ts resolveDefault: the Standard tier, else
 * the default provider) — never to registration order. The gateway is read
 * per call so the privacy and tracing wrappers installed later are used.
 * Throws when no model can answer; the AI route turns that into a failed run.
 */
export function createDesignCompletion(getGateway: () => Pick<ModelGateway, 'complete'>): CompleteFn {
  return async ({ system, user }) => {
    const res = await getGateway().complete({
      system,
    ...
    // Defensive: a provider that answers with a bare string still counts.
    const content = res?.content as ContentBlock[] | string | undefined

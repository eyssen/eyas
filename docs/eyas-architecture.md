...
(no user/project/local settings.json, CLAUDE.md, skills, `.mcp.json`, cwd-keyed
auto-memory or filesystem MCP configs; the CLI's enterprise-managed policy tier
cannot be suppressed client-side), so EYAS's own
...
- **Fordulonkent, nem cache-prefix szekciokent.** A `DEFAULT_BUDGET_FULL` 8400,
  a `shrinkForContextWindow` celja 8800: barmilyen >400 nettó bovites MINDEN
  ugynok MINDEN szekciojat aranyosan lezsugoritja. A `design-context` ugyanezen
  az uton ment.
...
- **Ket kulonbozo bekotes, mert a ket ut nem ugyanazt latja.** A
  `ConversationRunnerDeps`-nek van `db`-je es loggere, tehat a hatter-ut
  kozvetlenul epiti az indexet. A `createConversationRoutes`-nak **egyik sincs**
  (pozicionalis service-lista), ezert lusta accessort kap utolso parameterkent,
  a `getDesigns` mintajara — es a fail-soft logolas a memoria-modulban van, ahol
  van logger.
...
dedup — kulon fazis volt, es a kovetkezo szekcio az; a `save_memory` tool es a
kezzel irt fajl a manualis ut, valtozatlanul.
...
egy elbukott beszelgetes: a `capture()` sajat try/catch-e nyeli el, mindket
belepesi pont (interaktiv SSE-route es `conversation-runner`) `void ... .catch()`
modon inditja. **Ismert hiany:** a God Mode fordulok nem rogzitenek — a
God Mode ag sajat streammel visszater, mielott a fordulo utani blokk lefutna,
igy sem jegyzet, sem `memory_capture_runs` sor nem keletkezik.
...
modell-koltest** szamolja — sikeres extrakcio, `unparsable` valasz es `error`
futas fogyaszt belole, `too-short` es `cap-reached` nem. Husz rovid nyugtazas
...
utra megy. A privacy modul a szoveget **lemezre iras elott** sanitizalja, nem
olvasaskor: egy olvasas-ideju redakcio sem a fajlt, sem a belole epulo FTS
indexet nem erne el. A modul hianyaban a fuggveny identitas.
...
okukkal (`too-short`, `cap-reached`, `unparsable`, `error`), az extrakciok a
kiirt jegyzetek szamaval es a `kinds` JSON-tombbel. Ket hallgatas szandekos: a
...
szomszedos sorokban mar ott volt — igy a PreCompact osszefoglalok vegre eljutnak
az epizodikus memoriaba.

### Tartos memoria: kapcsolodo korabbi munka

A tartos index a *teny* kort zarja: egy allando jegyzet ott van minden
promptban, toolhivas nelkul. A *feladat* kor nyitva maradt. Egy uj
beszelgetes, ami azt mondja, „hivd kozvetlenul az MNB SOAP API-t az IAP
helyett", nem latja a korabbi dontest, hogy a Cloudflare 1010 blokkolta
az IAP-t a podokrol — hacsak a modell meg nem hivja a `search_memory`-t,
amit 24 beszelgetesbol nullaszor tett. A kapcsolodo korabbi munka ezt a
lyukat zarja: egy masodik, lekerdezes-felteteles blokk a promptban, kod
injektalja, nem toolhivas.

**A blokk lekerdezes-felteteles, strukturalis, FTS-only, modellhivas
nelkul.** Minden fordulon, mielott a modell meghivodna, az aktualis
felhasznaloi uzenet (interaktiv) vagy a `goal_description` (hatter) a
lekerdezes. Kihagyas (nincs szekcio, nincs hiba), ha:
- `memory.relatedWork.enabled` hamis (alap **be**, mert az opt-in
  ujra 0/24 lenne);
- a lekerdezes rovidebb, mint `minQueryChars` (alap 40, Unicode
  kodpontokban, ugyanaz a szamolas, mint a capture — kulcsszolista
  nincs);
- az FTS a szurok utan ures.

Fail-soft: egy throw figyelmeztetest logol, a fordulo a szekcio nelkul
megy tovabb. Hianyzo related-work blokk egy plusz felidezes nelkuli
fordulo, soha nem egy valasz nelkuli fordulo.

Nincs modellhivas. Csak lexikalis FTS. Az embedding ezen az uton
szandekosan ki van kapcsolva: egy hianyzo vagy beragadt embedding
provider nem kesleltetheti es nem uritheti a blokkot, es egy
provider-csere nem valtoztathatja meg, mi jon vissza.

**Mit keres.** Egy lexikalis menet, felso korlattal:
1. Vault jegyzetek (szemantikus + proceduralis), ugyanaz a
   `vaultNoteInScope` ablak, mint a `search_memory` `scope=current`.
2. Ervenyes epizodikus sorok (`valid_until IS NULL`), `project_id`
   szerint szurve, ha van effektiv projekt; projekt nelkuli
   beszelgetes csak null `project_id`-ju epizodikus sort lat.
3. L0 uzenetek (`role IN ('user','assistant')`), JOIN a
   `conversations` tablara, hogy `status != 'deleted'`.

Kemeny szurok minden forrason:
- **A jelenlegi beszelgetes kizarva.** A fordulonak mar megvannak a
  sajat uzenetei.
- **D1/D2.** Mas projektek vault jegyzetei, epizodikus sorai es
  beszelgetesei kint vannak, ha van effektiv projekt. A projekt
  nelkuli beszelgetes mas projektek L0-jat sem latja.
- **Dedup a fordulo memory-index utjaival.** Egy vault ut, ami mar
  index-sorkent ki van irva, itt nem ismetlodik. Az index a *teny*;
  a related-work a *feladat*-talalat.

Sorrend: vault, aztan epizodikus, aztan L0, belul BM25 (elojel-forditott,
ugyanaz, mint a `memory-service`). Nem RRF: ezen az uton nincs vektor
csatorna. `maxHits` alap 5. `budgetChars` alap 1 200 (csak egesz sorok;
csonka sor kiesik, es ha barmi kiesett, az utolso sor `- … N more hits
not shown — use search_memory`). `maxSnippetChars` alap 140.

**L0 FTS a `conversation_messages` felett.** Semmi uj nem irodik; a
tabla mar a ground truth. Virtualis tabla `conversation_fts`, FTS5
external-content, oszlop `body`. Tokenizer: `unicode61 remove_diacritics
2` (a magyar `űrlap` talal az `urlap`-ra; ha a CREATE regi SQLite-on
elbukik, diacritic nelkul ujraepul, egyszer logol). Csak
`role IN ('user','assistant')` indexelodik (D3). Az indexelt torzs
`substr(content, 1, 4000)` — a beillesztett dokumentumok korlatlanok,
a capture mar ugyanerre a 4 000-re vag.

DELETE/UPDATE triggerek, hogy a hard-delete ne hagyjon FTS-szellemet.
A soft-delete `conversations.status = 'deleted'`; a kereses mindig
JOIN-ol es szuri, tehat egy torolt statuszu beszelgetes akkor sem jon
vissza, ha az FTS meg tartja a szoveget.

A backfill **darabolt** (`rowid > :after LIMIT 500`), soha nem
`INSERT ... VALUES('rebuild')` a boot uton (az uzenettorzsek
beragaszthatjak az inditast). A `memory.onStart` fire-and-forget
ciklust indit; a boot nem var. A trigger-utan elo uzenetek azonnal
kereshetok, meg ha a tortenelem backfillje meg fut.

**A `search_memory` alapbol latja az L0-t.** Nincs uj tool. A
`MemorySearchResult.source` kap egy `'conversation'` erteket. Ugyanaz
a D1/D2 / `status != 'deleted'` / jelenlegi-beszelgetes-kizaras, mint
a related-work. A `scope=all` az egyetlen ut mas projektek L0-jahoz.
A HTTP `GET /api/v1/memory/search` szuretlelen a vault jegyzetekre
es **tartalmazza** az L0-t, cimkevel; az operator oldal diagnostika,
a tool a scoped ut.

**Ket kulonbozo bekotes, ugyanaz a fail-soft.** Interaktiv: lusta
accessor a `createConversationRoutes` **utolso** pozicionalis
parameterekent (kozbeszuras neman eltolna a `skillDecisions` /
`getMemoryCapture` / media / studio slotokat). A hatter-ut
(`conversation-runner`) kozvetlenul epiti, a `buildMemoryIndex`
melle. Prompt-sorrend: memory-index, aztan related-work. Az allando
utasitasok megelozik a feladat-talalatokat.

**Amit ez a hullam szandekosan nem nyit ki.** Packing (working →
epizodikus → vault, decay, orphan-GC) — F4. F3 UI (projekt- /
beszelgetes-memoria listak). Trigram, vektor-hidratacio, tool-kimenet
indexeles (D3). God Mode capture: a God Mode ag tovabbra is a prompt-
osszeallitas elott ter vissza; a workerek a `runConversation`-on
at latjak a blokkot a `goal_description`-bol, de a God Mode *rogzites*
marad az ismert hiany. Provider-memoria olvasas/iras nincs.

...
## [0.8.32-beta] - 2026-09-25 — The terminal is the owner's

### Security / OpenCode

- **The OpenCode terminal is the owner's and an admin's, like the plain shell.** The `user` role could open the OpenCode TUI with `create OpenCode` (the default until now). In the TUI the person at the keyboard approves OpenCode's tool requests themselves — the EYAS security gate answers only the headless tasks EYAS starts (`permission-gate.ts`) — and OpenCode has no kernel sandbox, so a TUI is effectively a shell as the EYAS server's OS user. `POST /api/v1/opencode/sessions` now needs `manage OpenCode` for both kinds, the OpenCode TUI and the plain shell, checked by the route's `requirePermission` before the body is read or anything is looked up (`403 Forbidden: cannot manage OpenCode`; nothing starts, no folder is made); the shell's separate in-handler check is folded into it. The terminal socket `/api/v1/opencode/terminal/:id` checks the same right twice, each time with the user's role and status read fresh from `users` through the one CASL registry (`opencode/permissions.ts` `createTerminalAccess` + `activeRoleOf`, exposed as `ctx.opencode.mayUseTerminal`): at the upgrade (`ws-server.ts`: `403` after the JWT and before the session is looked up, so a refusal reveals no session; a module that wires no check gets `403`, fail closed) and again when the socket opens (`terminal-ws.ts`: an `error` frame with `code: "forbidden"`, close `1008`, nothing attached). A suspended or archived admin, and a user whose role was lowered, is refused at the next connect, and a terminal they already have open ends as well (next entry).
- **An open terminal ends when its user loses the right.** The socket asked the right only when it opened; after that every `input` and `resize` frame went to the PTY unchecked. The D14 `closeUserSockets` on suspend, archive and logout closes only the sockets in the WS registry, which terminal sockets never join, and a role change closed nothing — so a demoted, suspended or archived admin (a compromised account the owner is locking out, say) kept an interactive shell as the server's OS user, and typing kept its idle timer from ever firing. The terminal socket (`terminal-ws.ts`) now asks `mayUseTerminal` again, with role and status read fresh: before every client frame (`input`, `resize`, `ping`, raw text), uncached; on a sweep every 15 seconds while any terminal is attached (`TERMINAL_REVALIDATE_MS`; unref'd, stopped when the last socket leaves and at module stop), which covers a change made by any writer and a terminal that only streams output; and at once on the new bus event `eyas.auth.user.access_changed` (`auth/events.ts` `USER_ACCESS_CHANGED`, `{userId: actor, targetId, role?, status?}`), which the auth routes emit after a role or status change in `PATCH /api/v1/users/:id` and after an archive (`DELETE /api/v1/users/:id`) — never for a refused change or another field, and never failing the request. The OpenCode module listens for it (`endTerminalsOnAccessChange`, Zod-checked payload); the audit module logs it with the actor as the user and the changed user as the target; `ws-bridge.ts` does not forward it to clients. A terminal that lost the right ends: every socket attached to it (two tabs included) gets the `error` frame with `code: "forbidden"` and close `1008`, the PTY is destroyed, no frame still in flight reaches it, and its last output and exit frame go to no ended socket. A check that throws ends the terminal (fail closed). The web already shows the `forbidden` frame in the user's language. Tests: `tests/modules/opencode/terminal-ws.test.ts` (demoted, suspended and archived while attached, each frame kind, two tabs, the sweep with fake timers, the bus event), `tests/modules/auth/routes-users.test.ts` and `routes-users-archive.test.ts` (what is reported, and when not).
- **One meaning per OpenCode right, written down in `opencode/permissions.ts`.** `read` — the OpenCode page (status, models, saved settings) and listing or closing your own terminals; `create` — headless use: the EYAS memory API OpenCode's plugin calls (`/memory/search`, `/memory/expand`) for a signed-in caller; `manage` — the settings, and opening or attaching to a terminal. Defaults: owner and admin `manage`, user `read` (was `read, create`), agent `create`, guest nothing. Closing a terminal (`DELETE /api/v1/opencode/sessions/:id`) now needs `read` instead of `create`: it only ends the caller's own terminal (another user's id stays the same `404` as a missing one), so someone who lost the right can still close a leftover but never reattach to it. Listing stays `read` and own-only. `opencode_run` is, as before, authorized on the tool path — the caller's `execute Tool`, the colleague's toolset, its red-tier approval and the security gate on every OpenCode tool call — and never by the OpenCode subject: gating it on `create OpenCode` would have taken headless tasks away from every `user` conversation (an interactive turn's tool actor is the signed-in user), so colleagues keep delegating to OpenCode in anyone's conversation. The memory routes keep `create` for a signed-in caller, so the `user` role no longer calls them directly; the plugin's per-session proofs are unchanged. The subject's registration is one exported constant (`OPENCODE_PERMISSIONS`) that the route and permission tests import instead of retyping it.
- **`GET /api/v1/opencode/access`** (read OpenCode) returns `{terminal, settings}` from the caller's own CASL ability, so the web shows an OpenCode control only to someone the server would let use it.

### Web

- **The terminal icon is shown only to someone who may open terminals.** The conversation top bar asks `GET /opencode/access` (`pages/opencode/access.ts` `useOpencodeAccess`) and shows the OpenCode terminal toggle only when the server answers `terminal: true` — never while the answer loads, never on an error (no read right, the module not loaded); the web decides nothing by role name. There was no ability hook in `src/web` to reuse, so the server's own CASL answer is the one check. If the server still refuses, the terminal panel says why in the user's language (`opencode.terminal.forbidden`, from the `403` of `POST /opencode/sessions` or the socket's `forbidden` frame), and the socket error that was hard-coded English is translated too (`opencode.terminal.socketError`). The OpenCode page's sign-in hint now says the owner or an admin signs OpenCode in. All six locales.

### Handbook

- **OpenCode:** a new *Who can open a terminal* section — why the terminal is effectively a shell on the server, the three rights and their default roles, the `403` and socket refusal, the fresh role and status check at every connect and for as long as the terminal stays open (every frame, every 15 seconds, at once on a role change, suspension or archive — the terminal then ends), that headless `opencode_run` tasks are unchanged, `GET /api/v1/opencode/access`, and how to give someone the terminal (the admin role). The workflow, the web-terminal row, the sign-in note, the *Not a sandbox* paragraph, the memory API paragraph (who holds `create`) and the upgrade list follow it (en/hu/de/es/fr/tlh).
- **Conversations — Top bar:** the terminal icon is shown only to the owner and admins. **Users & permissions:** an OpenCode terminal entry, which says that lowering someone's role, suspending or archiving them ends their open terminals at once. **Security & privacy:** the OpenCode bullet says who can open a terminal or a plain shell, and why (en/hu/de/es/fr/tlh).

### Upgrade notes

- **Users lose the OpenCode terminal.** After the update a `user` no longer sees the terminal icon, and `POST /api/v1/opencode/sessions` and the terminal socket answer `403`; owners and admins keep the terminal and the plain shell. The agent role's API keys cannot open a terminal either. Colleagues' `opencode_run` tasks in users' conversations run as before, and the OpenCode page still shows to users.
- **Open terminals end with the right.** Lowering someone's role, suspending or archiving them now ends their open OpenCode terminals and plain shells at once — whatever runs in them is stopped — and a role or status changed some other way (the database edited by hand) ends them within 15 seconds or at their next keystroke. Before, an open terminal ran on until it was closed.
- **Nothing to migrate.** The rights of the built-in roles are not stored — the `roles` table holds names only, and every request rebuilds its ability from the registered defaults — so every install takes the new default at its next start, and there is no customised grant to keep. For the same reason there is no setting that gives the `user` role `manage OpenCode`: to let someone use the terminal, give them the admin role (`PATCH /api/v1/users/<id>` with `{"role": "admin"}`, by the owner or an admin), which also gives them every other admin right. A test (`tests/modules/opencode/permissions.test.ts`) fails if role grants ever start being stored, since this change would then need a migration.

## [0.8.31-beta] - 2026-09-24 — Your own folders only

### Security / OpenCode

- **An OpenCode terminal opens only for a conversation you may open.** `POST /api/v1/opencode/sessions` took the conversation id, the folder and the folder list from the request and never checked who owned the conversation, so another user's conversation id opened a terminal in that conversation's workspace, and a folder list in the body let a request name any workspace as its folder. The route now applies the conversations module's own rule (`ownsConversation`: the owner, or for a team/delegation child the nearest human owner up its chain; no admin override, as on every conversation route). Any other id — another user's, or one that does not exist — is the same `404` for the OpenCode terminal and the shell, nothing starts and no workspace folder is created for it; without the conversations module it is a `503` (fail closed). The terminal's folders are now the conversation's stored Folders, read on the server, else its own workspace; a `workingDirectories` list in the body is ignored, and a requested `cwd` outside those folders is a `400`. An id that cannot be a workspace folder name is a `400` before anything is looked up. The web terminal no longer sends folders, which also fixes a terminal that failed with `invalid session payload` for a conversation whose first folder has a display name. This closes the direct request only; the stored Folders themselves are closed by the next entry.
- **A conversation's Folders can name only its owner's workspaces — on save and at run time.** The folder check accepted any single conversation workspace ("a single workspace is fine"), and the workspaces root, with every user's conversation ids, can be listed with `GET /api/v1/filesystem/browse`. So a `user` could `PATCH /api/v1/conversations/<own>` with `workingDirectories: ["<workspaces>/<another user's conversation>"]` (`200`) and then open the OpenCode terminal of that own conversation (`200`), which worked in the other user's workspace — as did `opencode_run`, every CLI run (Claude Code, Grok, Kimi) and EYAS's own file tools, because a stored folder counts as the run's own workspace for the memory-path policy. `POST /api/v1/projects/:id/conversations` (a board card) stored its `workingDirectories` without any check at all. Now a conversation workspace — the folder `<workspaces>/<id>`, anything below it, or a link into it, judged as written and after realpath (the path policy's new `workspacesOf`) — is a folder only of that conversation and of the conversations the same human owner may open (the conversations module's `ownsConversation`, so a team/delegation child keeps its parent's workspace); a run's scratch folder under `_runs` is no conversation's. Saving anything else on `POST` / `PATCH /api/v1/conversations` or on a board card is a `400 {error, code: "otherWorkspace", path}`, before any row is written; the board card now passes the whole folder check. At run time the same rule screens a conversation's stored folders — one saved before this release, inherited from a project, or reached through a symlink: the agent runner (EYAS tools, the gate, CLI metadata), the chat route's direct-gateway branch, the prompt's folder list, the OpenCode terminal (both kinds) and `opencode_run` leave it out with the `folderRefused` notice (reason `otherWorkspace`), and it never reaches the gate as the run's own folder. `opencode_run` also refuses a model-named task folder there (after realpath), and a task folder the model names in a conversation without folders must lie in its own workspace. Projects and project types keep their folder check without the ownership rule (only owners and admins edit them); a conversation using such a folder is still screened at run time. `ConversationService.ownerOf` names the human owner `ownsConversation` answers true for. `GET /api/v1/filesystem/browse` still lists the workspaces root's folder names; a listed id no longer opens anything.
- **A plain shell is the owner's and an admin's, and never gets the server's environment.** `kind: "shell"` on `POST /api/v1/opencode/sessions` gave any caller with `create OpenCode` — the `user` role by default — a login shell as the EYAS server's OS user, started with the server's own `process.env` (EYAS_MASTER_KEY, provider API keys). From there `cd` reaches every workspace and `env` / `data/sqlite` everything else, so the ownership checks above were no boundary for its user. It now needs `manage OpenCode` (owner and admin by default); anyone else gets `403` before the conversation is looked up, and no folder is made. The shell starts with the OpenCode terminal's filtered environment (`isolation.ts` `buildShellCommand`: the `buildOpencodeEnv` allowlist, HOME the EYAS-owned OpenCode home, no server password or plugin key) and its folders pass the same screening as the TUI's. The web page never opened a shell and does not now. **Not a sandbox:** the terminal is still no file-system sandbox for its own user — a command approved in the OpenCode TUI (`bash`, `external_directory`) runs as the server's OS user and can reach what that user can. The `user` role keeps `create OpenCode` (the self-approved TUI) by default; whether it should is the owner's decision, and the handbook says to grant it only to people trusted with that.
- **Audited, unchanged:** listing and closing terminals (`GET` / `DELETE /api/v1/opencode/sessions[/:id]`) and the terminal socket (`/api/v1/opencode/terminal/:id`) already answer only the terminal's own user (404 otherwise); `POST /api/v1/opencode/memory/search|expand` bind a plugin call to the session its proof names and give a signed-in caller a session's conversation only as its bound user (else global memory only), and the bindings are made on the server from the run, never from a request; terminal output capture already records only into the terminal user's own conversation.

### Security / Dependencies

- **Vitest 4.1.11 closes Dependabot #5 (GHSA-82fw-gwwq-j7x9).** Vitest before 4.1.11 let a redirected module mock in `@vitest/mocker` read a file outside the project (path traversal, medium). The dev dependency moves from `^3.1.0` (3.2.4 installed) to `^4.1.11`, and every `@vitest/*` package it brings is 4.1.11 (none is listed directly, in the root or in `src/web` / `packages/*`). It is test tooling only: nothing EYAS ships or runs in production loads it. Vite stays at 7.3.1 and jsdom at 29.0.1, both inside Vitest 4's peer ranges; every package the upgrade adds is MIT. `vitest.config.ts` and `vitest.e2e.config.ts` need no change: they already set `exclude` and `coverage.include` themselves, use none of the options Vitest 4 removed, and `server.deps.inline` is still the supported spelling.
- **Tests follow Vitest 4, none removed, skipped or loosened.** `vi.spyOn` on a method that is already a spy now returns that same spy with its call history (Vitest 3 returned a fresh one). The home page and the running-agents, attention and schedule widget tests spy on the shared `api` client in every test, so a later test's call count or `not.toHaveBeenCalled()` also saw the earlier tests' calls (6 failures); they now restore their spies after each test. `ReturnType<typeof vi.fn>` now names a mock that may also be a constructor, which no longer fits a plain function type, so 14 test files name the mock with Vitest's `Mock` type instead (30 type errors); one OpenCode privacy test spies on the service's own `redactToolOutput` instead of wrapping that generic method in `vi.fn`. The suite passes as before (the same two known failures), and `bun run lint` is back to its 9 known errors.

### Handbook

- **OpenCode:** the task-folder paragraph says that the terminal opens only for your own conversation (another user's is *not found*, for an admin too), in the folders EYAS reads from the conversation, that a named task folder of a conversation without folders must lie in its own workspace, and that a saved folder in another user's conversation workspace is left out of the terminal and of `opencode_run`. A new *Not a sandbox for its own user* paragraph says that what the terminal runs runs as the server's OS user, that the plain shell is for the owner and admins only and starts with the terminal's filtered environment, and that the `user` role can open the OpenCode terminal by default. The sign-in note now warns against `opencode auth login` in a terminal of your own outside EYAS (en/hu/de/es/fr/tlh).
- **Conversations — Folders:** a new refusal, the workspace of another user's conversation (a folder in it, a link into it, or a run's `_runs` folder), and the `otherWorkspace` code in the API's code list (en/hu/de/es/fr/tlh).

### Web

- **A refused folder in another user's workspace is explained in your language.** `projects.folders.error.otherWorkspace` in all six locales, used by every folder editor (a conversation's Folders, projects, project types).
